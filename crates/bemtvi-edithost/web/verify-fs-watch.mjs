// Playwright verifier for the browser edit-host's streaming `btv.fs.watch` over a REAL
// `bemtvi --daemon --listen` (Phase 3b of the off-tick plan). A browser `btv.fs.watch` has no local
// filesystem watcher — the arm crosses the wire to the daemon's recursive `notify` watcher (the
// same coalescing watcher the native `btv.fs.watch` rides), and each coalesced change batch returns
// as a `luafs_change` push the Worker lands into the watch stream's `:next()`. The browser twin of
// the native btv.fs watch leg; daemon-only (serverless OPFS has no change source).
//
...
//
// Prereqs: ./build.sh (dist/eh.mjs + eh.wasm + vendor/msgpack), `cargo build -p bemtvi`, Chromium.
...
const root = mkdtempSync(join(tmpdir(), "bemtvi-fswatch-"));
...
try {
  ...
  await luaResult(page, `_G.__ev = nil
     _G.__watch = btv.fs.watch("${watchDir}", { recursive = true })
  ...
  check("btv.fs.watch surfaces a daemon-side change (kind + path) over WebTransport",
  ...
  await luaResult(page, `_G.__werr = nil
     _G.__badwatch = btv.fs.watch("${join(root, "does-not-exist")}", {})
  ...
  check("btv.fs.watch on a missing path rejects the stream (error envelope round-trips)",
  ...
  await luaResult(page, `_G.__stopped = nil
     local w = btv.fs.watch("${watchDir}", { recursive = true })
     w:stop()
  ...
  check("btv.fs.watch :stop() ends the iteration (next() resolves nil)",
...
console.log(failures === 0
  ? "\nALL PASS — browser btv.fs.watch streams daemon-side changes over WebTransport (change, error, stop)"
  : `\n${failures} FAILED`);
