...
//! verbatim).
//!
//! The `%`-format mini-language splits cleanly along bemtvi's purity boundary, so
...
pub struct StatuslineCtx {
    ...
    /// charset its bytes are in on disk. bemtvi's built-in default status line shows
...
pub struct StatusSegment {
    ...
    /// A click handler for an `btv.statusline` segment cell (a `v:lua.…` reference),
...
// ---------------------------------------------------------------------------
// Segment composition — the `btv.statusline` (lualine-shaped) surface.
//
// The `%`-format engine above is one way to drive the status line. The other is
// a declarative list of named *segments* (`btv.statusline.setup{ left=…, right=…
...
/// The declarative segment layout from `btv.statusline.setup{}`: ordered segment
...
pub struct SegmentLayout {
    ...
    /// statusline (e.g. bemtvi-line) manages its own padding and section arrows and
...
/// Compose a `btv.statusline` [`SegmentLayout`] into the final [`StatusSegment`]s
...
/// carry an [`on_click`](StatusSegment::on_click) handler (an `btv.statusline`
...
mod tests {
    ...
    fn colored_cells(name: &str) -> Option<Vec<StatusSegment>> {
        match name {
            "btvline_a" => Some(vec![StatusSegment {
                text: " NORMAL ".into(),
                group: Some("lualine_a_normal".into()),
                on_click: None,
            }]),
            "btvline_b" => Some(vec![StatusSegment {
                text: " main ".into(),
    ...
    fn default_separator_inserts_unstyled_connector_spaces() {
        ...
        let spec = SegmentLayout {
            left: vec!["btvline_a".into(), "btvline_b".into()],
            right: vec![],
            separator: " ".into(),
        };
