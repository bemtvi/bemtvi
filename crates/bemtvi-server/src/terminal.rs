...
use bemtvi_core::{BufferId, Rgb, Style, TerminalOp};
use rmpv::Value;
...
impl EditHost {
    ...
    pub(crate) fn sync_terminal_styles(&mut self) {
        ...
        let browsing = self.editor.mode != bemtvi_core::Mode::Terminal;
        let Some(emu) = self.terminals.get_mut(&buf) else {
            return;
        };
        if !browsing {
...
pub(crate) mod native {
    ...
    fn open_pty(
        ...
        // Advertise a terminal type describing bemtvi's own vt100 emulator (which projects
        // indexed AND 24-bit `Rgb` color), overriding whatever `TERM` bemtvi inherited. This
        ...
        // with bemtvi's truecolor-first rendering and works whether the PTY is local or on the
        ...
        // depth so a low-color terminal degrades the child too — for now bemtvi assumes
