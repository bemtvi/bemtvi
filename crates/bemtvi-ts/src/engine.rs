...
use bemtvi_core::syntax::{GrammarInstall, GrammarRequest};
use bemtvi_core::{BufferEdit, BufferId, FoldRange, IndentParams, OpenOutcome, Span, SyntaxEngine};
...
struct BufferState {
    ...
    /// [`line_background_lines`](bemtvi_core::syntax::SyntaxEngine::line_background_lines)
...
pub struct Engine {
    /// bemtvi's own data dir — the writable root (where `:TSInstall` lands) and the
    ...
    /// (`btv.treesitter.fragment_context`), which also ships the defaults.
...
struct FragmentContext {
    ...
    /// be framed as `class __btv:` + one indented line + a dedent, which is a syntax
...
impl Engine {
    ...
    pub fn load_language_now(&mut self, language: &str) -> bool {
        let language = bemtvi_core::resolve_language(language);
    ...
    /// `btv.treesitter.highlight`) ask this after painting nothing: it separates "this
    ...
    pub fn language_pending(&self, lang: &str) -> bool {
        let lang = bemtvi_core::resolve_language(lang);
    ...
    /// preview, an LSP doc float, `btv.treesitter.highlight`) inject exactly as an
    ...
    /// in turn — `field: Vec<String>` inside `struct __btv { … }`, a bare statement
    /// inside `fn __btv() { … }`. The *first framing that parses cleanly* wins, and
    ...
    fn resolve_fragment(
        ...
        for context in self
            .fragment_contexts
            .get(bemtvi_core::resolve_language(lang))
            .cloned()
            .unwrap_or_default()
        {
    ...
    pub fn set_fragment_context(&mut self, lang: &str, templates: Vec<String>) {
        ...
        self.fragment_contexts
            .insert(bemtvi_core::resolve_language(lang).to_string(), contexts);
    }
...
/// The synchronous backend the editor owns (`bemtvi-core`'s [`SyntaxEngine`]).
...
fn normalize_lang(raw: &str) -> Option<String> {
    ...
    Some(bemtvi_core::resolve_language(&norm).to_string())
}
...
fn is_engine_query(name: &str) -> bool {
    bemtvi_core::ENGINE_QUERY_NAMES.contains(&name)
}
