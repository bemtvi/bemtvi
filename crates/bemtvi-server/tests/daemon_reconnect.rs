...
//! [`RemoteHostFs`](bemtvi_server::RemoteHostFs) built by
//! [`connect_daemon_reconnecting_on`](bemtvi_server::connect_daemon_reconnecting_on), talking
...
//!   retry budget is exhausted the link parks [`Disconnected`](bemtvi_server::DaemonStatus)
//!   until a manual [`reconnect`](bemtvi_server::ReconnectHandle::reconnect); and
//!   [`disconnect`](bemtvi_server::ReconnectHandle::disconnect) drops a live link on demand.
...
use bemtvi_test_harness::{attach, await_lines, buf_lines, exec_lua, feed, spawn, DaemonFs};
        ...
        move || {
            ...
            Box::pin(async move {
                ...
                let h = tokio::spawn(async move {
                    let _ = bemtvi_server::serve_fs_daemon(dr, dw, Box::new(fake)).await;
                });
...
async fn modified(rpc: &bemtvi_rpc::Rpc) -> bool {
...
/// modified.
fn set_line(rpc: &bemtvi_rpc::Rpc, text: &str) {
...
async fn save_until(rpc: &bemtvi_rpc::Rpc, fake: &DaemonFs, path: &str, want: &str) -> bool {
    for _ in 0..200 {
...
async fn start(
    ...
    bemtvi_rpc::Rpc,
    tokio::sync::mpsc::UnboundedReceiver<bemtvi_rpc::Incoming>,
    ReconnectHandle,
) {
    let (client, handle) =
...
/// The current `btv.daemon.status()` phase, or `None`.
...
/// Editor integration (Phase 3): the link status is mirrored to `btv.daemon.status()`, a
...
async fn status_mirror_event_and_commands_drive_the_link() {
    ...
    exec_lua(
        &rpc,
        "_G.dsc = 0; btv.autocmd.create('User', { pattern = 'DaemonStatusChanged', \
...
async fn dedicated_thread_wrapper_auto_recovers() {
    ...
    let (client, _handle) =
        bemtvi_server::connect_daemon_reconnecting(dialer.make(), fast_policy())
            .expect("the initial daemon dial succeeds");
    let DaemonClient { host_fs, .. } = client;
    let init = ServerInit {
        file: Some(PATH.to_string()),
...
/// Like [`start`], but also wires the daemon-backed **proc** seam (`btv.run` /
...
/// Kick a daemon-backed `btv.run` and stash its result in `_G.res`.
async fn spawn_probe(rpc: &bemtvi_rpc::Rpc) {
    exec_lua(
        rpc,
        "_G.res = nil\n\
         btv.run({ cmd = 'sh', args = { '-c', 'echo hi' } }):next(function(r) _G.res = r end)",
    )
    .await;
}
...
async fn probe_code(rpc: &bemtvi_rpc::Rpc) -> Option<i64> {
    for _ in 0..100 {
...
/// A `btv.run` issued while the link is DOWN must fail loud (`code = -1`, the cause
