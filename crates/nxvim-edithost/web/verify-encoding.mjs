...
try {
  ...
  const lua = async (src) =>
    String((await page.evaluate((s) => window.__nxvim.execLua(s).then((r) => r.result), src)) ?? "")
      .replace(/^ok:/, "");
  ...
  await page.evaluate(() => window.__nxvim.feed(":e /enc/latin1.txt<CR>"));
  await sleep(150); // the off-tick OPFS read lands a tick later
  const latin1Line = await page.evaluate(() => window.__nxvim.lines());
  ...
  await page.evaluate(() => window.__nxvim.feed(":e /enc/invalid.txt<CR>"));
  await sleep(150);
  const invalidLine = await page.evaluate(() => window.__nxvim.lines());
  ...
  await page.evaluate(() => window.__nxvim.feed(":w<CR>"));
  await sleep(150);
