//! Black-box tests for `nx.win.set_cursor` as the snippet-engine **jump** primitive
//! (P5): a pure-Lua engine that owns its tabstop session moves the caret between
//! tabstops by calling `nx.win.set_cursor` from its own (insert-mode) jump key. These
...
//!
//! `nx.win.set_cursor(win, line, col)` is 1-based line / 0-based byte col (the
...
async fn cursor(rpc: &Rpc) -> String {
    match exec_lua(
        rpc,
        "local c = nx._cur_cursor or {}; return string.format('%d,%d', (c.row or 1) - 1, c.col or 0)",
    )
    .await
    {
...
async fn an_insert_mode_map_jumps_the_caret_and_typing_lands_there() {
    ...
    let (rpc, _inc) = start(
        &dir,
        "nx.keymap.set('i', '<Tab>', function() nx.win.set_cursor(0, 2, 2) end)",
    )
    .await;
...
async fn the_jump_moves_the_reported_cursor() {
    ...
    let (rpc, _inc) = start(
        &dir,
        "nx.keymap.set('i', '<Tab>', function() nx.win.set_cursor(0, 2, 3) end)",
    )
    .await;
...
async fn a_jump_to_end_of_line_allows_insert_past_the_last_char() {
    ...
    let (rpc, _inc) = start(
        &dir,
        "nx.keymap.set('i', '<Tab>', function() nx.win.set_cursor(0, 2, 5) end)",
    )
    .await;
    feed(&rpc, "ihello<CR>world<Esc>");
    feed(&rpc, "gg0i");
