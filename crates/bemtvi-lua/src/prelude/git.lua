-- btv.git — a promise-always git API (docs/plans/2026-07-24-native-git-gix.md).
-- Part of the pure-Lua `btv.*` prelude (see runtime.rs); the Lua half over the
-- `btv._git_op` Rust bridge in install.rs, backed natively by the `gix` engine in the
-- `bemtvi-git` crate. Loaded AFTER promise.lua — every op builds on `btv.promise`.
--
-- Shape mirrors `btv.fs`: one-shot ops return a PROMISE of the result; there are NO
-- callbacks. A failed op REJECTS with a `{ code, message }` table — `code` a short
-- stable hint (`ENOREPO` / `ENOENT` / `EGIT`):
--
--   btv.async(function()
--     local h = btv.await(btv.git.head(btv.buf.name(0)))         -- { branch, detached, sha }
--     local ok, err = pcall(btv.await, btv.git.discover(dir))   -- err.code == "ENOREPO" / …
--   end)()
--
-- The bridge runs each op OFF the editor tick (`btv._git_op` queues a LoopOp::Git and
...
--
-- `btv.git_local.*` is the LOCAL-always twin (the git sibling of `btv.fs_local` /
-- `btv.run_local`): its ops act on the client machine's disk even in a daemon session.
-- The plugin manager (`btv.plugins`) uses it — its repos live on the local disk that
-- plugins load from.

btv.git = btv.git or {}
btv.git_local = btv.git_local or {}

-- Queue an off-tick git op described by `job` ({ op = "<name>", … }) via `bridge`
-- (`btv._git_op` or `btv._local_git_op`) and return a pending promise. A callback id is
-- registered in `btv._cb_fns`; the server fires `btv._run_cb(id, false, err, value)` when
...
-- Build the two parallel surfaces (`btv.git` over `btv._git_op`, `btv.git_local` over
-- `btv._local_git_op`) from one verb table, so the twins never drift.
local function define(surface, bridge)
  ...
  --
  -- ```lua
  -- btv.git.checkout(dir, sha, { detach = true })  -- git checkout --detach <sha>
  -- btv.git.checkout(dir, "main")                  -- git checkout main
  -- ```
  --
...
define(btv.git, btv._git_op)
define(btv.git_local, btv._local_git_op)
