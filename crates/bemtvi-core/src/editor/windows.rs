...
/// focused window's cursor cell (`cursor`). `relative` values bemtvi does not
...
impl FloatAnchor {
    ...
    /// RPC/Lua parse, used to format `nvim_win_get_config` and the `btv._wins`
    /// mirror.
...
/// shared by every surface (floats, `btv.view`, pickers, the panel). A 9-grid:
...
impl BorderStyle {
    ...
    /// and the single source of truth shared by the RPC and Lua-effect parsers.
    /// `None` for a style bemtvi cannot render yet; each caller reports its own
    /// context-specific error (per the no-silent-fallback rule).
    pub fn from_keyword(s: &str) -> Option<Self> {
        Some(match s {
...
pub(crate) struct Window {
    ...
    /// bemtvi's loclists are strictly per-window: a split inherits a *clone* of its
...
pub(crate) struct WindowLayout {
    ...
    /// (the `btv.statusline` custom-segment cache) by window.
...
impl Editor {
    ...
    /// The per-workspace option overlay, for the server to mirror to Lua (`btv.wso`) and to
    ...
    /// an unknown id. Backs `vim.fn.winrestview`'s `leftcol` / `btv.win.set_leftcol`.
    ...
    /// closing a window in bemtvi never unloads its buffer (the buffer stays in the
    ...
    pub fn open_float_window(
        ...
        // options bemtvi models: `'number'`/`'relativenumber'`/`'cursorline'` off,
        ...
        // (`nvim_win_set_option(win, "number")` / `btv.wo[win].signcolumn`). The
    ...
    pub(crate) fn split(&mut self, dir: SplitDir) {
        ...
        // The split inherits a *clone* of the parent's location list (bemtvi's
    ...
    pub(crate) fn close_window(&mut self) {
        ...
        let cur = self.windows.current;
        // Closing a plugin-view window (`:q`/`:close`/`<C-w>c` on an `btv.view` buffer) is
        ...
        // bemtvi-diff closing all three diff panes when one is `:q`'d). The window itself
    ...
    pub(crate) fn focus_window(&mut self, id: WindowId) {
        ...
        // grabbing `btv.view` float — focus is pinned to its window. Every focus change
    ...
    pub fn resize(&mut self, width: usize, height: usize) {
        ...
        // re-detect viewports for the `btv.decor` signal (`editor/decor.rs`).
    ...
    pub(crate) fn window_statusline_visible(&self, region: WindowRegion, floating: bool) -> bool {
        if floating {
            ...
            // per-window opt-in could grow here later; bemtvi has none yet.)
            return false;
        }
