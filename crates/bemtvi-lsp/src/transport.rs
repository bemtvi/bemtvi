...
//! transport (`bemtvi-server`'s `RemoteLspTransport`) that tunnels the server's
...
//! to.
//!
//! [`HostProc`]: bemtvi_server (the server-side process seam)
...
/// `bemtvi-server`'s remote one) name one type rather than repeating the boxed shape.
