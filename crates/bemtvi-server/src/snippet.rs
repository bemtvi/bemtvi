...
//! in `bemtvi-core`; this module is the thin server glue: it stores the snippets
//! `btv.snippet.add` registers, offers them as completion candidates for the
...
/// only in this phase; function bodies are rejected loud at `btv.snippet.add`).
#[derive(Clone, Debug)]
...
/// `MenuItem.key` offset for a **plugin `on_accept`** row (a `btv.complete.source`
...
impl EditHost {
    ...
    pub(crate) fn complete_snippet_dispatch(&mut self, gen: u64) {
        ...
        let items: Vec<bemtvi_core::MenuItem> = self
            .snippet_complete
            .iter()
            .enumerate()
            .map(|(i, e)| bemtvi_core::MenuItem {
                ...
                ..bemtvi_core::MenuItem::new(e.trigger.clone(), SNIPPET_COMPLETE_KEY_BASE + i)
            })
            .collect();
        if !items.is_empty() {
    ...
    pub(crate) fn complete_snippet_accept(&mut self, idx: usize) {
        ...
        let parsed = match bemtvi_core::parse_snippet(&entry.body) {
            Ok(p) => p,
            Err(e) => {
                self.editor
    ...
    /// Apply a `btv.snippet.add` registration to the per-filetype store.
