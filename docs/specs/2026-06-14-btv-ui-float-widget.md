...
component that completion, the fuzzy picker, and `btv.ui.select` all render
...
names the *engines* (`btv.complete`, `btv.picker`, `btv.ui.select`); this document
...
work (build-order steps 2–3) and of [`btv.lsp`](2026-06-14-btv-lsp-design.md)
...
- **The engines** — `btv.complete` / `btv.picker` / `btv.ui.select` — stay separate
...
| --- | --- | --- | --- | --- |
| **Completion** | docs sidebar | — | cursor | `btv.complete` |
| **Choice / code-action** | — | — | cursor | `btv.ui.select` |
| **Command palette** | — | ✅ | editor | picker (no preview) |
| **Fuzzy finder** | ✅ | ✅ | editor | `btv.picker` |
...
3. **Carries a generation token** on every query edit, so a source response for a
   ...
   Decision 3) — superseded `btv.spawn` jobs (live grep) are cancelled via
   `ctx.on_cancel`.
...
them as `btv.ui.float` (rendered content), a **sibling** of the list-widget on the
...
`btv.tree` docks are also separate (a persistent edge-anchored window, not a
float).
...
- **`btv.complete`** opens the widget `{ list + preview="markdown", no prompt,
  cursor }` on trigger; the buffer is the query; accept inserts text / expands a
  snippet. The `"lsp"` source is the native client ([`btv.lsp`](2026-06-14-btv-lsp-design.md) Phase C).
- **`btv.picker`** opens `{ list + prompt + preview, editor }`; static sources
  ...
  (definition/references/symbols) route here in `btv.lsp` Phase C.
- **`btv.ui.select`** opens `{ list, no preview, no prompt (optional filter),
  ...
  callback). Code actions and any `btv.ui.select` caller use it.

The public surface stays exactly the three task-shaped engines the plugin-API
spec names. Optionally, a low-level `btv.ui.menu` exposes the widget directly for
...
1. **The list-float widget core** — list rendering in a float (generalize the
   ...
   Immediately backs **`btv.ui.select`** (code actions) — the simplest consumer.
2. **The prompt input-grab** + dynamic-source forwarding + generation tokens →
   unlocks **`btv.picker`** (static + dynamic sources, build-order step 2).
3. **The preview pane** (by kind, both layouts) → completes the picker and backs
   the completion docs sidebar.
4. **`btv.complete`** drives the widget `{ cursor + preview }` from the input path
...
- **Reusing the bottom `panel`** — is the loclist panel folded into this widget
  ...
  (Lean: keep separate for now; revisit when `btv.tree` generalizes the dock.)
