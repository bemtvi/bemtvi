...
//
// `vim.regex` is backed by the real vim regexp engine (`bemtvi-regex`), so `\zs`
...
/// `BtvSubstituteDelete` group — the "removed" side of the live `:s` diff preview.
...
/// Whether every `BtvSubstituteDelete` span in the frame resolved to a concrete
...
fn subst_deleted_all_styled(view: &[(Value, Value)]) -> bool {
    view_get(view, "highlights")
        ...
        .flatten()
        .filter_map(Value::as_array)
        .filter(|s| s.get(2).and_then(Value::as_str) == Some("BtvSubstituteDelete"))
...
async fn substitute_replacement_preview_marks_deleted_and_added() {
    ...
    assert!(
        ...
        "BtvSubstituteDelete resolves to a concrete style via the built-in fallback"
    );
