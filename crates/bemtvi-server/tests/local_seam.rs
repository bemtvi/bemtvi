//! The public **local-always** seam — `btv.run_local` / `btv.fs_local` (prelude/localseam.lua),
//! the twins of `btv.run` / `btv.fs` that always act on the client machine even in a daemon
//! session (`btv.http.fetch_local` is the HTTP twin). Backs the plugin manager and remote
...
//! harness the local seam behaves exactly like `btv.run` / `btv.fs`; these prove the surface
...
/// OFF the editor tick (like `btv.run` / `btv.fs`), so the global its chain sets is nil until
...
async fn run_local_runs_a_child_and_resolves_the_exit_result() {
    ...
    exec_lua(
        ...
        "_G.r = nil\n\
         btv.async(function()\n\
         \x20 _G.r = btv.await(btv.run_local({ cmd = 'printf', args = { 'hi-local' } }))\n\
         end)()",
    )
    .await;
...
async fn run_local_reports_a_spawn_failure_as_code_minus_one() {
    ...
    exec_lua(
        ...
        "_G.r = nil\n\
         btv.async(function()\n\
         \x20 _G.r = btv.await(btv.run_local({ cmd = 'definitely-not-a-real-binary-xyz' }))\n\
         end)()",
    )
    .await;
    ...
    assert_eq!(
        code.as_i64(),
        Some(-1),
        "a spawn failure resolves code = -1 (never rejects), like btv.run",
    );
}
...
async fn fs_local_round_trips_the_client_disk() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.done = nil\n\
             btv.async(function()\n\
             \x20 btv.await(btv.fs_local.write('{file_lua}', 'from-fs-local'))\n\
             \x20 _G.seen = btv.await(btv.fs_local.exists('{file_lua}'))\n\
             \x20 _G.text = btv.await(btv.fs_local.read_text('{file_lua}'))\n\
             \x20 _G.done = true\n\
             end)()",
        ),
    )
...
async fn fs_local_rename_moves_on_the_client_disk() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.done = nil\n\
             btv.async(function()\n\
             \x20 btv.await(btv.fs_local.write('{tmp_lua}', 'atomic-payload'))\n\
             \x20 btv.await(btv.fs_local.rename('{tmp_lua}', '{final_lua}'))\n\
             \x20 _G.done = true\n\
             end)()",
        ),
    )
...
async fn fs_local_read_text_honors_the_encoding_option() {
    // The local twin took the same `{ encoding = … }` opts table as `btv.fs.read_text`
    ...
    exec_lua(
        ...
        &format!(
            "_G.enc = nil\n\
             _G.bare = nil\n\
             btv.async(function()\n\
             \x20 _G.enc = btv.await(btv.fs_local.read_text('{p}', {{ encoding = 'latin1' }}))\n\
             end)()\n\
             btv.async(function()\n\
             ...
             \x20 local ok = pcall(btv.await, btv.fs_local.read_text('{p}'))\n\
    ...
    assert_eq!(
        ...
        "café\n",
        "the encoding reached the decoder, exactly as btv.fs.read_text would"
    );
