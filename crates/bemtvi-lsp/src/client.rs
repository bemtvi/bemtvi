...
pub(crate) fn new_client(
    ...
    MainLoop::new_client(|_server| {
        ...
        // what decides whether bemtvi asks the user before applying.
        ...
        // `btv.lsp._register_capability`) and acked — an ack is required, and until this
...
/// bemtvi's base [`client_capabilities`] with the config's `extra` capabilities
/// (a raw JSON value) deep-merged over them — the config wins on any conflict, so
/// it can both extend (add a capability bemtvi doesn't advertise) and override
    ...
    ClientCapabilities {
        ...
        workspace: Some(WorkspaceClientCapabilities {
            workspace_edit: Some(WorkspaceEditClientCapabilities {
                ...
                // the edits that fill it); bemtvi applies them in the order sent.
                ...
                // protocol's `abort`. Declared because it is exactly what bemtvi
                ...
                // and mark some `needsConfirmation`, which bemtvi asks about before
            ...
            // nothing. bemtvi has exactly one folder per client (the key's root; a
            ...
            // capabilities bemtvi genuinely honors (`client/registerCapability` is
            // answered in both clients; the registration is forwarded to
            // `btv.lsp._register_capability`, which arms the watches):
            //
...
/// behavior), and bemtvi's base capabilities with the config's `capabilities`
...
/// The standard LSP semantic token *types* bemtvi understands, advertised in the
...
/// The standard LSP semantic token *modifiers* bemtvi understands. As with the
