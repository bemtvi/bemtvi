...
async fn substitute_confirm_ctrl_e_scrolls_without_consuming_the_match() {
    ...
    // no edit — `^E` is not an answer. (bemtvi keeps the cursor on screen every
...
// the embedded `bemtvi-regex` engine. `pcre` keeps the historical behavior.
...
async fn bare_bar_is_alternation_inside_a_substitute_pattern() {
    // bemtvi's regex flavor is PCRE, so a *bare* `|` is alternation (vim's `\|`) and a
