-- ~~~ nxvim-lspconfig — 407 language servers, configured, with nothing to write ~~~
--
-- Run it (from the repo root) — needs `lua-language-server` on your PATH:
--
--     NXVIM_CONFIG=examples/lspconfig \
--       cargo run -p nxvim -- examples/lspconfig/sample.lua
--
-- The FIRST run clones the plugin: nxvim will report it as missing, so run
...
--
-- `nx.lsp` is the control surface: it spawns servers, syncs documents, routes
...
--
-- nxvim-lspconfig is that data: a native port of nvim-lspconfig, one curated
-- table per server, rewritten against `nx.*` so nothing blocks the editor. Put it
-- on the runtimepath and `nx.lsp.enable("lua_ls")` is the whole configuration —
...
--   3. Put the cursor on `nx.buf` (line 8 of sample.lua) and press `K`.  →  a
--      hover float. Then `gd` on `helper` (line 20)  →  jumps to its definition.
--      Both are nxvim's built-in maps, installed buffer-local on attach.
...
--      taught the server that `nx` is real, and the *other* undefined name is not.
...
--      Those are this plugin's keymaps (section 4), on top of nxvim's core set.
...
--      `nx.lsp.config` — most servers read their settings only at `initialize`.
--   8. `:lua print(#require("nxvim-lspconfig").available())`.  →  `407`. And
--      `:lua print(table.concat(require("nxvim-lspconfig").for_filetype("lua"), ", "))`
--      →  every bundled server that serves Lua, this config's override included.
--   9. `:help nxvim-lspconfig`.  →  the full reference. (Needs the nxvim-help
--      plugin, declared below, and `:NxHelptags ALL` once.)

------------------------------------------------------------------------------
-- 1. Install the plugin, and enable ONE server.
--
--    This is the whole native path. `nx.lsp` reads `lsp/<name>.lua` straight off
--    the runtimepath, so there is no `require`, no registration, and no `cmd` to
--    write. `nx.lsp.enable` may be called before or after `nx.lsp.config`, and a
--    late enable serves buffers that are already open.
------------------------------------------------------------------------------
nx.plugins({
  { "nxvim/nxvim-lspconfig" },
  { "nxvim/nxvim-help" }, -- so step 9's `:help nxvim-lspconfig` works
})
...
nx.lsp.config("lua_ls", {
  settings = {
    Lua = {
      -- nxvim's Lua is PUC 5.4 — NOT LuaJIT. Telling the server otherwise makes
      -- it offer completions for a runtime you are not running.
      runtime = { version = "Lua 5.4", path = { "lua/?.lua", "lua/?/init.lua" } },
      -- `nx` is the plugin API and `vim` is the bounded compat surface; without
      -- this every `nx.*` call in your own config reads as an undefined global.
...
nx.lsp.config("*", {
  on_attach = function(client, bufnr)
    ...
    nx.notify(("LSP: %s attached to buffer %d"):format(client.name, bufnr))
  end,
})
...
--
--    The keymaps it installs (on top of nxvim's own gd/gD/gr/K/<C-k>):
--
...
--    `nx.lsp` awaits `cmd` / `root_dir` / `before_init` before it spawns.
--
--    This one is not enabled (there is no such server to run); it is here as the
--    shape to copy. Uncomment the `nx.lsp.config` call to see it rejected loudly
...
--     local pkg = nx.await(util.find_upward(util.bufname(bufnr), "package.json"))
--     if not pkg then
...
nx.keymap.set("n", "<leader>li", function()
  ...
  nx.notify(#names .. " client(s): " .. (table.concat(names, ", ") ~= "" and table.concat(
    names,
    ", "
  ) or "none"))
...
nx.keymap.set("n", "<leader>lr", function()
  nx.async(function()
    local cfg = nx.lsp.get_config("lua_ls")
    local root = nx.await(util.root(0, cfg.root_markers))
    nx.notify("lua_ls root markers resolved to: " .. tostring(root))
  end)()
