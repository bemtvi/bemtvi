...
//!
//! Proven by spawning the real `bemtvi-gui` binary with a throwaway config whose
...
//! `cargo test -p bemtvi-gui --test signal_exit -- --ignored`.
...
fn scratch_config() -> (PathBuf, PathBuf, PathBuf) {
    ...
    let dir = std::env::temp_dir().join(format!("bemtvi_gui_signal_{}_{n}", std::process::id()));
...
fn wait_for_exit(child: &mut Child, timeout: Duration) -> Option<std::process::ExitStatus> {
    let deadline = Instant::now() + timeout;
    while Instant::now() < deadline {
        match child.try_wait().expect("wait for bemtvi-gui") {
...
fn killing_the_gui_runs_the_exit_sequence() {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_bemtvi-gui"))
        .env("BEMTVI_CONFIG", &dir)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn bemtvi-gui");

    // Up and running (a GPU window can take a while to come up on a cold cache).
    assert!(
        wait_for(Duration::from_secs(90), || entered.exists()),
        "bemtvi-gui never reached VimEnter"
    );
    ...
    let status = wait_for_exit(&mut child, Duration::from_secs(30)).unwrap_or_else(|| {
        let _ = child.kill();
        panic!("bemtvi-gui did not exit after SIGTERM");
    });
