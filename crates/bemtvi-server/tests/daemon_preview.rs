...
async fn picker_preview_fetches_an_unloaded_file_over_the_wire() {
    let dir = bemtvi_test_harness::temp_dir("daemon_preview");
...
"#;
    ...
    exec_lua(&rpc, "btv.picker.open('preview_test')").await;
...
/// A RELATIVE preview target (what `rg`/`grep`/btv.fs emit) is resolved against the
...
async fn relative_preview_target_resolves_against_the_remote_cwd() {
    let dir = bemtvi_test_harness::temp_dir("daemon_preview_rel");
    let fake = DaemonFs::default();
    fake.set("/virtual/start.txt", "start\n")
...
"#;
    ...
    exec_lua(&rpc, "btv.picker.open('rel_test')").await;
