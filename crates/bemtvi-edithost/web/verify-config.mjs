...
try {
  ...
  const INIT_LUA = [
    ...
    // A UIEnter autocmd: fired when the UI attaches, with btv.ui.caps() already
    ...
    "btv.on('UIEnter', {}, function() vim.g.__uienter = tostring(btv.ui.caps().keyboard_protocol) end)",
  ].join("\n");
  ...
  check("config: a UIEnter autocmd fired with btv.ui.caps() populated",
  ...
  await page.evaluate(() => window.__bemtvi.feed("ggdGQ"));
  const afterMap = await page.evaluate(() => window.__bemtvi.lines());
  ...
  const afterBroken = await page.evaluate(() => window.__bemtvi.lines());
