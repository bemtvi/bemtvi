...
//! bemtvi's "the core owns *which* cells" split — see `docs/architecture.md`): a
...
pub(crate) struct MouseSelect {
    ...
    /// blockwise selection awaits a blockwise Visual mode (not yet in bemtvi).
    count: u8,
...
impl Editor {
    ...
    pub fn mouse(&mut self, ev: MouseEvent) {
        // The command-line wildmenu is bemtvi's own interactive overlay (neovim doesn't
        ...
        match (ev.button, ev.action) {
            ...
            // The command-line wildmenu (`btv.cmdline_complete`) is non-grabbing like the
    ...
    /// Lua before every callback (`btv._mouse_pos`).
    ...
    ///
    /// The geometry mirrors the client `DockLayout` (`bemtvi-tui` `render.rs`): the
    ...
    /// client `DockLayout` (`bemtvi-tui` `render.rs`): where each region's window
