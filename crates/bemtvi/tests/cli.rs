//! Black-box CLI tests: spawn the real `bemtvi` binary in its headless roles
...
//! `BEMTVI_CONFIG` and its own temp working dir, and the `--lua` role persists no
...
fn fresh_dir(tag: &str) -> PathBuf {
    ...
    let dir = bemtvi_test_harness::temp_root()
        .join(format!("bemtvi_cli_{tag}_{}_{n}", std::process::id()));
...
fn bemtvi(cfg: &Path) -> Command {
    let mut c = Command::new(env!("CARGO_BIN_EXE_bemtvi"));
    c.env("BEMTVI_CONFIG", cfg);
    c.env_remove("BEMTVI_RUNTIMEPATH");
    c.stdin(Stdio::null());
...
fn run(mut cmd: Command, timeout: Duration) -> (ExitStatus, String, Duration) {
    ...
    let mut child = cmd.spawn().expect("spawn bemtvi");
    let status = loop {
        ...
        if start.elapsed() > timeout {
            let _ = child.kill();
            let _ = child.wait();
            panic!("bemtvi did not exit within {timeout:?}");
        }
...
/// A shell/CI running `bemtvi --lua '...print(x)...'` must be able to read it back.
#[test]
fn lua_oneshot_print_goes_to_stdout() {
    let cfg = fresh_dir("cfg");
    let mut cmd = bemtvi(&cfg);
...
/// A `--lua` script's error-writer output (`btv.err_writeln`) must reach **stderr**,
...
fn lua_oneshot_err_write_goes_to_stderr() {
    ...
    let mut cmd = bemtvi(&cfg);
    cmd.arg("--lua")
        .arg("btv.err_writeln('err_stderr_marker'); print('out_stdout_marker')");
    let (status, stdout, stderr, _) = run_io(cmd, Duration::from_secs(45));
    assert!(
        status.success(),
    ...
    assert!(
        ...
        "btv.err_writeln output must reach stderr, got stderr: {stderr:?}"
    );
    assert!(
...
/// success to the shell (the old path only `btv.notify`d into a UI nobody attached).
#[test]
...
fn lua_oneshot_statement_code_works() {
    ...
    assert!(
        marker.is_file(),
        "the awaited btv.fs.write must have landed before exit"
    );
    let _ = std::fs::remove_dir_all(&cfg);
...
fn workspace_lua_oneshot_runs_in_the_workspace_dir() {
    ...
    cmd.current_dir(&launch)
        ...
        .arg(&ws)
        .arg("--lua")
        .arg("btv.fs.write('ws_marker.txt', 'hi')");
    let (status, stderr, _) = run(cmd, Duration::from_secs(45));
    assert!(
        status.success(),
...
/// `--extract-lua-runtime DIR` dumps the embedded `btv.*`/`vim.*` prelude to DIR as real
...
/// source — here we assert the `btv` namespace file landed and defines the global.
#[test]
fn extract_lua_runtime_writes_prelude_to_explicit_dir() {
    let cfg = fresh_dir("cfg");
    let out = fresh_dir("rt");
    let mut cmd = bemtvi(&cfg);
    cmd.env_remove("BEMTVI_RUNTIME");
    ...
    let btv = out.join("prelude").join("btv.lua");
    assert!(
        btv.exists(),
        "the btv prelude file must be written, missing {}",
        btv.display()
    );
    let src = std::fs::read_to_string(&btv).unwrap();
    assert!(
        src.contains("btv = btv or {}"),
        "the extracted file must be the real prelude source that defines `btv`"
    );
    ...
    let stub = out.join("bemtvi-globals.lua");
    assert!(
        stub.exists(),
...
/// With no DIR argument, extraction targets `$BEMTVI_RUNTIME` when it is set — the
/// directory a user's `.luarc.json` `["$BEMTVI_RUNTIME"]` resolves to. Since the var is
/// already exported, the tool must NOT re-print the `export BEMTVI_RUNTIME=…` hint.
#[test]
fn extract_lua_runtime_honors_bemtvi_runtime_env() {
    let cfg = fresh_dir("cfg");
    let out = fresh_dir("rt_env");
    let mut cmd = bemtvi(&cfg);
    cmd.env("BEMTVI_RUNTIME", &out);
    ...
    assert!(
        out.join("prelude").join("btv.lua").exists(),
        "extraction must target $BEMTVI_RUNTIME when no DIR is given"
    );
    assert!(
        !stdout.contains("export BEMTVI_RUNTIME"),
...
/// With neither a DIR argument nor `$BEMTVI_RUNTIME`, extraction falls back to the
/// standard data dir (`stdpath("data")/runtime`, driven here by `XDG_DATA_HOME`) and
/// prints the `export BEMTVI_RUNTIME=…` line the user must add to their shell.
#[test]
fn extract_lua_runtime_defaults_to_data_dir_and_prints_export() {
    ...
    assert!(
        btv.exists(),
        "the default must be <data-dir>/bemtvi/runtime, missing {}",
        btv.display()
    );
    assert!(
        stdout.contains("export BEMTVI_RUNTIME"),
...
/// bemtvi exposes `$BEMTVI_RUNTIME` in its OWN process environment (mirroring `$BEMTVI_ARGV`),
/// so the editor's Lua — and every `btv.run` child, which inherits this process's env — sees
...
#[test]
fn bemtvi_runtime_env_defaults_and_is_visible_to_lua() {
    let cfg = fresh_dir("cfg");
    let data = fresh_dir("xdg_data");
    let mut cmd = bemtvi(&cfg);
    cmd.env_remove("BEMTVI_RUNTIME");
    cmd.env("XDG_DATA_HOME", &data);
    cmd.arg("--lua")
        .arg("print(os.getenv('BEMTVI_RUNTIME') or 'BEMTVI_RUNTIME_IS_NIL')");
    let (status, stdout, stderr, _) = run_io(cmd, Duration::from_secs(45));
    assert!(
        status.success(),
        "--lua reading $BEMTVI_RUNTIME must exit 0: {status:?} (stderr: {stderr})"
    );
    let want = data.join("bemtvi").join("runtime");
    assert!(
        stdout.contains(want.to_str().unwrap()),
        "the editor must default $BEMTVI_RUNTIME to <data-dir>/bemtvi/runtime, got stdout: {stdout:?}"
    );
    let _ = std::fs::remove_dir_all(&cfg);
    let _ = std::fs::remove_dir_all(&data);
}

/// The editor exposes `$BEMTVI_RUNTIME` set-if-absent: an already-exported value (a user's
/// shell export, or a custom `--extract-lua-runtime DIR` location) is respected verbatim,
/// never clobbered by the default.
#[test]
fn bemtvi_runtime_env_respects_existing_export() {
    let cfg = fresh_dir("cfg");
    let custom = fresh_dir("custom_rt");
    let mut cmd = bemtvi(&cfg);
    cmd.env("BEMTVI_RUNTIME", &custom);
    cmd.arg("--lua")
        .arg("print(os.getenv('BEMTVI_RUNTIME') or 'BEMTVI_RUNTIME_IS_NIL')");
    let (status, stdout, stderr, _) = run_io(cmd, Duration::from_secs(45));
    assert!(
    ...
    assert!(
        ...
        "an exported $BEMTVI_RUNTIME must be respected, got stdout: {stdout:?}"
    );
    let _ = std::fs::remove_dir_all(&cfg);
