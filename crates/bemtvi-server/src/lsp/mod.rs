...
//! servers.
//!
//! Where `bemtvi-lsp` owns the client machinery (spawning/supervising servers and
...
use bemtvi_lua::{DiagnosticData, LspServerCapabilities};
...
pub(crate) struct AppliedEdit {
    pub(crate) outcome: bemtvi_lsp::ApplyEditOutcome,
...
/// daemon and in the browser — so the filesystem half rides the same seam `btv.fs`
...
pub(crate) struct WorkspaceFsJob {
    ...
    pub(crate) job: Option<bemtvi_lua::FsJob>,
}
...
pub(crate) struct PendingConfirmEdit {
    pub(crate) edit: bemtvi_lsp::WorkspaceEditData,
...
pub(crate) struct PendingApplyEdit {
    ...
    /// (meaningful because bemtvi declares the `abort` failure-handling strategy).
...
impl LspDocState {
    ...
    /// this buffer. Used when a server is stopped (`btv.lsp.stop`) — a buffer left
...
pub(crate) struct InlayHintSpan {
    ...
    pub(crate) resolve_data: Option<bemtvi_lsp::serde_json::Value>,
}
...
impl LspReqKind {
    ...
    /// server that produced the item), or bemtvi doesn't model one. Those fall back
...
pub(crate) struct PendingLspReq {
    ...
    pub(crate) tick: u64,
    /// The `btv._cb_fns` id that settles the issuing verb's promise, or `0` for a
...
impl LspFanout {
    ...
    /// does. `btv.lsp.hover{ name = … }` still narrows the round to one server.
    ///
...
/// The options a `btv.lsp.code_action(opts)` call carries from Lua to the reply:
...
pub(crate) fn lsp_range_to_bytes_in(
    ...
    // [`Editor::apply_edits_to`](bemtvi_core::Editor) (`e_row - s_row`) and the
...
/// read without a final newline it is bemtvi bookkeeping the server has never been told
...
/// [`Editor::apply_edits_to`](bemtvi_core::Editor::apply_edits_to) takes, in **document**
...
/// reaches is one bemtvi really passed through, which is what makes this correct
...
/// Map an LSP [`DiagnosticSeverity`] to bemtvi's severity code (`1`=error,
...
/// bemtvi's native byte columns — there is no server to negotiate an encoding with,
...
/// [`LspServerCapabilities`], the boundary that keeps `bemtvi-lua` free of the LSP
...
/// Lua mirror (`btv._diagnostics`) holds for `vim.diagnostic.get`. Positions are
/// the raw 0-based LSP coordinates; `col`/`end_col` are byte offsets under the
/// UTF-8 encoding bemtvi advertises first (the negotiated default), matching
...
/// The subset of `vim.diagnostic.config` keys bemtvi has a backing surface for,
/// threaded from Lua via [`LspOp::DiagnosticConfig`](bemtvi_lua::LspOp). `underline`
...
/// `$BEMTVI_LSP_CMD` overrides the whole command (the mock hook, the LSP analogue
/// of `BEMTVI_TS_WORKER`), else the config's `cmd` is used verbatim. `None` when no
/// program can be determined (an empty `cmd` and no override).
///
/// `$BEMTVI_LSP_CMD_<name>` overrides just the server named `name`, and wins over the
/// blanket `$BEMTVI_LSP_CMD`. Without it a test cannot stand up **two** distinct mock
...
/// named `ts_ls` reads `$BEMTVI_LSP_CMD_TS_LS`.)
pub(crate) fn lsp_spawn(name: &str, cmd: &[String]) -> Option<ServerSpawn> {
    let per_server: String = name
        ...
        .collect();
    let override_cmd = std::env::var(format!("BEMTVI_LSP_CMD_{per_server}"))
        .or_else(|_| std::env::var("BEMTVI_LSP_CMD"));
...
/// `$BEMTVI_LSP_ROOT`, absolutized, if set — an explicit workspace-root override
...
pub(crate) fn lsp_root_override() -> Option<PathBuf> {
    std::env::var_os("BEMTVI_LSP_ROOT").map(|root| absolutize(Path::new(&root)))
}
