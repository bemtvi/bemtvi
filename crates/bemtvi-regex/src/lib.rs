//! Vim regular expressions for bemtvi — the real thing.
//!
...
// ---------------------------------------------------------------------------
// FFI surface (see csrc/nvim/regexp.h and csrc/shim/btvre_shim.h)
...
unsafe extern "C" {
    ...
    fn btvre_buf_free(buf: *mut c_void);
    fn btvre_buf_set_iskeyword(buf: *mut c_void, iskeyword: *const c_char) -> bool;
    fn btvre_buf_set_tabstop(buf: *mut c_void, tabstop: i64);
    fn btvre_win_new(buf: *mut c_void) -> *mut c_void;
    fn btvre_win_free(win: *mut c_void);
    fn btvre_set_current(buf: *mut c_void, win: *mut c_void);
    fn btvre_set_mark_provider(
        lookup: Option<extern "C" fn(*mut c_void, c_int, *mut LinenrT, *mut ColnrT) -> bool>,
        userdata: *mut c_void,
    );
    fn btvre_set_interrupt(value: bool);
    fn btvre_take_last_error() -> *const c_char;
    fn btvre_set_regexpengine(engine: i64);
    fn btvre_profile_setlimit(ms: i64) -> u64;
    fn btvre_win_set_cursor(win: *mut c_void, lnum: LinenrT, col: ColnrT);
    fn btvre_buf_set_visual(
        buf: *mut c_void,
        start_lnum: LinenrT,
        start_col: ColnrT,
        end_lnum: LinenrT,
...
/// The `buf` pointer most recently made current via `btvre_set_current`, or 0
...
/// `buf` is already current. `btvre_set_current` reparses the option strings
...
fn make_current(_guard: &MutexGuard<'_, ()>, buf: *mut c_void, win: *mut c_void) {
    if CURRENT_BUF.load(Ordering::Relaxed) != buf as usize {
        unsafe { btvre_set_current(buf, win) };
...
fn set_default_context(guard: &MutexGuard<'_, ()>) {
    ...
    let &(buf, win) = CTX.get_or_init(|| {
        ...
        unsafe {
            let buf = btvre_buf_new(empty_line, std::ptr::null_mut(), 1);
            let win = btvre_win_new(buf);
...
impl VimRegex {
    ...
    pub fn compile_with(
        ...
        let prog = unsafe {
            btvre_set_regexpengine(match engine_sel {
                Engine::Auto => 0,
                Engine::Backtracking => 1,
                Engine::Nfa => 2,
            });
            let prog = vim_regcomp(cpat.as_ptr(), flags);
            btvre_set_regexpengine(0);
            prog
        };
        if prog.is_null() {
...
impl VimBuffer {
    ...
    pub fn from_lines<S: AsRef<str>>(lines: &[S]) -> Result<Self, VimRegexError> {
        ...
        let (buf, win) = unsafe {
            let buf = btvre_buf_new(buffer_get_line, ud, line_count);
            let win = btvre_win_new(buf);
            (buf, win)
        };
    ...
    pub fn set_iskeyword(&mut self, iskeyword: &str) -> Result<(), VimRegexError> {
        ...
        // `btvre_set_current` and rebuilds the character tables against the
        // new value (`btvre_buf_set_iskeyword` rebuilds the buffer-local
        // table itself, but the cache must not assume anything stayed put).
        CURRENT_BUF.store(0, Ordering::Relaxed);
        if unsafe { btvre_buf_set_iskeyword(self.buf, c.as_ptr()) } {
            Ok(())
        } else {
    ...
    pub fn set_tabstop(&mut self, tabstop: u16) {
        let _guard = engine();
        unsafe { btvre_buf_set_tabstop(self.buf, i64::from(tabstop)) };
    }
    ...
    pub fn exec(
        ...
        let nlines = unsafe {
            btvre_set_mark_provider(Some(buffer_mark_lookup), ud);
            ...
            btvre_set_mark_provider(None, std::ptr::null_mut());
            r
        };
...
impl Drop for VimBuffer {
    fn drop(&mut self) {
        ...
        unsafe {
            btvre_win_free(self.win);
            btvre_buf_free(self.buf);
        }
    }
}
