-- ~~~ nxvim quickfix & location-list playground ~~~
--
-- The quickfix list is vim's "run a tool, get a navigable list of file:line hits,
-- step through them" workflow. nxvim has the whole thing: the `errorformat`
...
-- the canonical `nx.qf` surface (with `vim.fn.setqflist`/`getqflist` aliases).
--
-- Launch it (from the repo root, so the relative paths the fake compiler prints
-- resolve against the CWD):
--
--     NXVIM_CONFIG=examples/quickfix \
--       cargo run -p nxvim -- examples/quickfix/sample.c
--
-- WHAT TO TRY:
--   :make            run the fake compiler ('makeprg' below). It prints three
--                    gcc-style diagnostics against sample.c; nxvim parses them
...
--                    walks forward again. nxvim keeps the last 10.
...
--                    nx.qf (see the keymap below) — the dogfooded API.
...
-- (This is also nxvim's default efm, set explicitly here so the tour is legible.)
nx.o.errorformat = "%f:%l:%c: %t%*[^:]: %m,%f:%l:%c: %m,%f:%l: %m"
...
nx.o.makeprg = "sh examples/quickfix/fakecc.sh"
...
nx.o.grepprg = "grep -n $* examples/quickfix/sample.c /dev/null"

-- <leader>q: build a quickfix list straight from Lua and open it. This is the
-- canonical nx.qf surface — the same one :make feeds under the hood.
vim.g.mapleader = "\\"
