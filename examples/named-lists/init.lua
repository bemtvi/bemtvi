-- ~~~ nxvim: named lists (window-independent quickfix-flavored lists) ~~~
--
...
--
--   nx.qf.list(name, items[, opts])   create / replace the list `name` (opts.title,
--                                      opts.action); repaints its tab if open
--   nx.qf.show(name)                   open or focus the list's dock tab
--   nx.qf.drop(name)                   close its tab and forget the list
--
...
local function todo_items()
  ...
  for i, line in ipairs(vim.api.nvim_buf_get_lines(0, 0, -1, false)) do
    if line:find("TODO") or line:find("FIXME") then
      ...
      items[#items + 1] = { filename = nx.buf.name(), lnum = i, col = 1, text = line, type = typ }
    end
  end
  return items
end
...
-- it again after editing the file: nx.qf.list replaces the contents in place, so the
...
nx.keymap.set("n", "<leader>ll", function()
  nx.qf.list("long-lines", long_line_items(), { title = "Lines over 40 cols" })
  nx.qf.show("long-lines")
...
nx.keymap.set("n", "<leader>td", function()
  nx.qf.drop("todos")
