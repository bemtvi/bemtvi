//! Behavior tests for `btv.http.mount` — the plugin HTTP *server* surface (one
...
use bemtvi_test_harness::{exec_lua, lua_bool, start_attached};
use rmpv::Value;
...
/// `btv.http.origin()` is nil, so there is no origin to connect to.
#[tokio::test]
async fn no_listener_until_something_mounts() {
    let (rpc, _incoming) = start().await;
    assert_eq!(
        exec_lua(&rpc, "return btv.http.origin() == nil").await,
        Value::Boolean(true),
    ...
    exec_lua(&rpc, "btv.o.httpport = 0; btv.o.httphost = '127.0.0.1'").await;
    assert_eq!(
        exec_lua(&rpc, "return btv.http.origin() == nil").await,
        Value::Boolean(true),
...
async fn mount_serves_a_real_get() {
    ...
    let origin = lua_string(&rpc, "return btv.http.origin()").await.unwrap();
...
async fn async_handler_responds_later() {
    ...
    let url = mount(
        ...
        "slow",
        r#"function(req, respond)
             btv.timer(function() respond({ body = "answered later" }) end, 30)
           end"#,
    )
    .await;
    let response = tokio::task::spawn_blocking(move || get(&url))
...
async fn status_and_headers_reach_the_client() {
    ...
    let url = mount(
        ...
        r#"function(req, respond)
             respond({
               status = 418,
               headers = { ["content-type"] = "text/plain", ["x-btv"] = "mounted" },
...
async fn bad_name_errors() {
    ...
    let err = lua_string(
        ...
        local ok, err = pcall(function()
          btv.http.mount({ name = "has/slash", on_request = function() end })
        end)
        return tostring(err)
        "#,
    )
...
async fn httpport_option_is_honored() {
    ...
    let port = free_port();
    exec_lua(&rpc, &format!("btv.o.httpport = {port}")).await;
    let url = mount(
        &rpc,
        "fixed",
...
/// `btv.o.httpport` can never disagree with reality.
///
...
async fn failed_rebind_reverts_the_option() {
    let (rpc, _incoming) = start().await;
    let good = free_port();
    exec_lua(&rpc, &format!("btv.o.httpport = {good}")).await;
    let url = mount(
        &rpc,
        "stayput",
    ...
    exec_lua(&rpc, &format!("btv.o.httpport = {taken}")).await;
    // The failure is off-tick; wait for the option to snap back to the port being served.
    assert!(
        await_truthy(&rpc, &format!("btv.o.httpport == {good}")).await,
