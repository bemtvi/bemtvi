# `btv.lsp` — the native LSP control surface

**Status:** **Phases A–C landed (A 2026-06-16; B + C 2026-06-17).** Re-introduces
bemtvi's LSP *control surface* in the `btv.*` namespace, after the btv break
([`e9bb90c`](#background)) deleted the old `vim.lsp` Lua clone. The LSP
**engine** is untouched and carries forward whole: the
[`bemtvi-lsp` crate + the `lsp/` server subtree](2026-06-02-lsp-support-design.md)
...
engine half of that document (Decisions 1–7, the `bemtvi-lsp` crate, the
per-buffer server state, document sync over the edit journal) is **unchanged
and authoritative**; this spec does not restate it.

## Background

Commit `e9bb90c` ("canonical btv.* namespace") completed the ADR 0002 break by
...
- **`bemtvi-lsp`** — `LspManager`, the per-`(language, root)` child servers, the
  `LspCommand`/`LspEvent` channel pair, `ProviderCaps`, the protocol layer.
- **`bemtvi-server/src/lsp/`** — `sync` / `completion` / `diagnostics` / `edit` /
  `inlay` / `semantic` / `request`, still declared (`mod lsp`) and driven in the
  event loop (`on_lsp_events`).
- **The queued-effect seam** — `LspOp` in `bemtvi-lua/src/ops.rs`
  ...
  surface drives. *(The comments get re-pointed at `btv.lsp` as part of Phase A.)*
- **`btv.diagnostic`** — already migrated and btv-native (`get` / `goto_next` /
...
So "bring back LSP" is **re-introducing the control surface, btv-shaped, against
...
4. **Dogfood the shared engines** (ADR 0002 §5). LSP does **not** own UI. Its
   results flow into the server-owned surfaces: hover → `btv.ui.float`, locations
   → `btv.picker`, code actions → `btv.ui.select`, completion → `btv.complete`,
   diagnostics → `btv.diagnostic` / the decoration layer.
...
### Server configuration — `btv.lsp.config` / `btv.lsp.enable`
...
`btv.lsp.config[name] = {…}` table-assignment sugar.
...
btv.lsp.config("rust_analyzer", {                       -- merges into the above
  settings = { ["rust-analyzer"] = { check = { command = "clippy" } } },
})

btv.lsp.enable({ "rust_analyzer", "lua_ls" })           -- activate
```
...
bemtvi keeps it declarative engine state instead, so it behaves identically under
the wasm edit-host). On the first buffer whose `filetype` matches an enabled
server, the engine resolves the root and fires `LspOp::Start`. `btv.lsp.disable`
is the inverse.
...
**Bundled presets ship as pre-registered `config()` data** — an btv-native
replacement for lspconfig, reusing the existing `lsp/<server>.lua` config-file
pattern (`bemtvi-lua/src/luafs.rs` already sources these). A user only
`enable("rust_analyzer")`s; `cmd` / `filetypes` / `root_markers` come from the
preset and can be overridden by their own `config()` call.

`btv.lsp.start(cfg, { bufnr })` remains available as the low-level, un-merged
direct start (the raw `LspOp::Start`) for advanced/manual use.

### Language features — flattened verbs, server-owned UI

Flattened under `btv.lsp.*` (the user's choice — no `.buf` sub-namespace; bemtvi
has no competing `btv.lsp.win`/etc. grouping to disambiguate). Each is a thin
verb over the surviving op; the **server decides where the answer lands**:

| Verb | Op | Result surface |
| --- | --- | --- |
| `btv.lsp.hover()` | `BufRequest{Hover}` | `btv.ui.float` |
| `btv.lsp.signature_help()` | `BufRequest{SignatureHelp}` | `btv.ui.float` |
| `btv.lsp.definition()` | `BufRequest{Definition}` | 1 hit → jump; N → `btv.picker` |
| `btv.lsp.declaration()` | `BufRequest{Declaration}` | ″ |
| `btv.lsp.type_definition()` | `BufRequest{TypeDefinition}` | ″ |
| `btv.lsp.implementation()` | `BufRequest{Implementation}` | ″ |
| `btv.lsp.references()` | `BufRequest{References}` | `btv.picker` |
| `btv.lsp.rename(name?)` | `Rename{new_name}` | `name` nil → `btv.ui.input` prompt, then apply `WorkspaceEdit` |
| `btv.lsp.code_action()` | `CodeAction` | `btv.ui.select` of actions; apply on choose |
| `btv.lsp.format()` | `Format` | apply edits |
| `btv.lsp.document_symbol()` | *new* `BufRequest{DocumentSymbol}` | `btv.picker` |
| `btv.lsp.workspace_symbol(q)` | *new* `BufRequest{WorkspaceSymbol}` | `btv.picker` |
...
owns the surface**. Until `btv.picker` lands (build-order step 2), multi-location
...
btv.lsp.config("*", {
  on_attach = function(client, bufnr)            -- runs at settle, never at frame time
    local function map(k, fn) btv.keymap.set("n", k, fn, { buffer = bufnr }) end
...
### Completion — a built-in `btv.complete` source

LSP completion is **not** a verb here; it is the built-in `"lsp"` source of the
`btv.complete` engine (native plugin API, worked example 1), fanned out with
generation tokens. There is no `btv.lsp.completion` / omnifunc. Lands with the
...
| --- | --- | --- |
| `btv.bo.lsp_semantic_tokens` (per-buffer) | `vim.lsp.semantic_tokens.start` / `stop` | `SemanticTokensEnable` |
| `btv.o.lsp_semantic_tokens` (editor-wide gate, default on) | `…semantic_tokens.enable(bool)` | `SemanticTokensConfig` |
| `btv.bo.lsp_inlay_hints` (per-buffer, default off) | `vim.lsp.inlay_hint.enable(bool, {bufnr})` | `InlayHintEnable` |
...
manual refresh stays a verb (`btv.lsp.semantic_tokens_refresh(bufnr)` →
`SemanticTokensRefresh`) since "drop the cache and re-request" has no readable
state to model as a noun.

### Diagnostics

Already `btv.diagnostic.*` and btv-native. LSP `publishDiagnostics` feeds it; this
spec adds nothing. `btv.lsp.*` and `btv.diagnostic.*` stay distinct namespaces (as
in neovim), since diagnostics outlive any LSP client (a linter or
`btv.diagnostic.set` can publish without a server).

### Introspection & escape hatch

```lua
local clients = btv.lsp.clients({ bufnr = 0 })          -- snapshot tables (reads the mirror)
local c = clients[1]
...
receives the same handle. `btv.lsp.request`/`btv.lsp.notify` are sugar resolving
the buffer's primary client. These are the generic path for protocol features
bemtvi has no first-class verb for; an unimplemented or uncapable request fails
loud (principle 5).

## Mapping to the engine seam (nothing new in `bemtvi-lsp`)
...
(`btv.lsp` is step 1; picker is step 2; completion is step 3):

- **Phase A — parity, on machinery that all exists. _(done 2026-06-16.)_** New
  `prelude/lsp.lua`: `btv.lsp.config` (merge registry, `"*"` base, neovim
  deep-merge), `btv.lsp.enable` / `disable` with engine-side FileType dispatch, the
  flattened language verbs routing to **today's** surfaces (jump / loclist / float /
  `btv.ui.select` / `btv.ui.input`), `btv.lsp.request` / `notify` / `clients`,
  `on_attach` / `on_init` / `on_exit`. Restores the deleted feature set, btv-shaped.
  ...
  `_remove_client`) that `bemtvi-server` hard-calls are now defined (they were
  dangling). Covered by `crates/bemtvi/tests/lsp_config.rs` (merge precedence,
...
- **Phase B — the nouns. _(done 2026-06-17.)_** The semantic-token / inlay-hint
  surface landed as the neovim-shaped function tables `btv.lsp.semantic_tokens`
  (`start`/`stop`/`enable`/`force_refresh`/`get_at_pos`) and `btv.lsp.inlay_hint`
  (`enable`/`is_enabled`/`get`), with `vim.lsp.*` aliases — driving the existing
  toggle ops and the now-defined read mirrors (`btv._set_semantic_tokens` /
  `btv._set_inlay_hints`, which `bemtvi-server` hard-calls each reply; they were
  dangling before, so the mirror push silently errored). The buffer-option *nouns*
  (`btv.bo.lsp_semantic_tokens`, …) remain available as optional sugar over the same
  ops. Covered by `crates/bemtvi/tests/lsp_features.rs` (enable→request→reply→mirror
  round-trips; off-by-default inlay hints; `get_at_pos`).
- **Phase C — engine integration. _(done 2026-06-17.)_** Goto-with-many-hits,
  `references`, and document/workspace symbols re-routed into `btv.picker` (the
  built-in `"location"` preview), confirm jumps via `btv.picker.edit`; a single goto
  ...
  name/kind/location). The `"lsp"` `btv.complete` source landed earlier (Phase 4-C
...
`nvim_buf_get_lines` / cursor / the `redraw` view. `bemtvi-lsp::mock` stands in
