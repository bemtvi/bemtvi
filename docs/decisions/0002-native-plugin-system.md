# 0002 — A native plugin system over `btv.*`

**Status:** accepted (2026-06-11). Records the boundary of bemtvi's `vim.*`
muscle-memory aliases and the shape of its extensibility. Companion design:
[the native plugin API (`btv.*`)](../specs/2026-06-11-native-plugin-api.md).
Bounds the scope of [ADR 0001](0001-native-engines-vendored-lua-apis.md) (see
*Relationship to ADR 0001* below).

## Context

bemtvi's architecture rests on three commitments (architecture.md):
`bemtvi-core` is **pure and synchronous**; Lua influences the editor only
...
**bemtvi has its own plugin system. The only `vim.*` surface is a closed
whitelist of muscle-memory aliases over `btv.*`.**

1. **Extensibility is the native provider API (`btv.*`).** The server owns
...
2. **Colorschemes are bemtvi's own, loaded as Lua data.** A colorscheme is pure
   data — a table of highlight-group definitions registered through the `btv`
...
3. **Every editor API lives in the `btv.*` namespace — a clean break.** Config
   files are `btv` scripts: `init.lua` is written against the same `btv.*`
   surface plugins use (options, keymaps, events, user commands, LSP setup
   via `btv.lsp`, tree scripting via `btv.treesitter`). There is no `vim.*`
   API and no vendored neovim Lua surface: of the existing `vim.treesitter` /
   `vim.lsp` machinery, what serves bemtvi's objectives is refactored into the
   `btv` API, and the rest is deleted. Per the no-silent-stubs rule, anything
...
4. **A closed whitelist of muscle-memory aliases.** An enumerated set of
   `vim.*` names is kept as thin aliases of their `btv.*` equivalents. The
   admission test: high frequency in real configs, declarative or
   callback-shaped (never blocking, never frame-time), and 1:1 onto an `btv`
   ...
   - **Declarative registrations:** a **partial `vim.api` table** containing
     ...
     `nvim_del_autocmd` / `nvim_clear_autocmds` (→ `btv.on`),
     `nvim_create_user_command` (→ `btv.command`), and `nvim_set_hl`
     (→ `btv.hl.define`) — any other `vim.api` access fails loud — plus
     `vim.filetype.add` (→ `btv.filetype`).
   - **Callback-shaped async** (the neovim APIs already designed this way):
     `vim.notify` (→ `btv.notify`), `vim.schedule` (run at settle),
     `vim.defer_fn` (→ `btv.timer`), `vim.ui.input` / `vim.ui.select`
     (→ `btv.ui.*`), and `vim.system` in its **callback form only**
     (→ `btv.run` / `btv.run_stream`; promise-shaped, never blocking).
   - **Treesitter highlight toggle:** `vim.treesitter.start(buf, lang?)` /
     ...
     state, not because the namespace is: `start` sets `btv.bo.filetype` / lang
     and `btv.bo.ts_highlight = true`, `stop` sets `ts_highlight = false` and
     leaves `filetype`. bemtvi models *which* language (`filetype`) and
   ...
   the same objects as `btv`, with `btv` semantics (snapshot reads, queued
...
5. **First-party features are `btv` plugins — dogfood the API.** Everything that
   can reasonably be built as an `btv.*` Lua plugin is built as one. The
   UI-orchestration surfaces (picker, completion, statusline, snippets, tree
   docks) and the behavior composed on top of them ship as bundled `btv` plugins,
   not as bespoke Rust. bemtvi is the plugin API's first and most demanding
   consumer; a feature that can't be expressed against `btv.*` is a gap to close
...
priority — the bridge pattern this decision generalizes into the `btv`
...
paths — is refactored into the `btv.*` API (`btv.treesitter`, `btv.lsp`) in
bemtvi's own shape, free of the bug-for-bug-with-upstream constraint that
...
unchanged, but it is now fed by **declarative buffer state** (`btv.bo.filetype`
/ `btv.bo.ts_highlight`, point 4 above) instead of an imperative `TsOp` from
vendored Lua — the engine seam is kept, the command skin is replaced by a noun.

## Consequences

- **`btv.view` widens the plugin-owned *content* surface — not the entity-mutation
  one.** The buffer-text mutation API is a single, bounded entry point —
  `btv.buf.set_lines` (alias `nvim_buf_set_lines`), an **async** whole-line write
  ...
  `<CR>` handler" is generalized off its bottom-edge assumption into `btv.view`: a
...
- **The UI-orchestration surfaces split native seam / Lua feature**: the server
  ...
  orchestration and UX that drive each registry ship as **bundled `btv` plugins**
...
- **The lasting `vim.*` is exactly one thing: the muscle-memory alias
  ...
  donor code for the `btv` build-out: refactored under `btv.*` where it serves
  bemtvi's objectives, deleted where it doesn't.
- **Plugin asynchrony lives in `btv`** (`btv.run` / `btv.timer` / `btv.fs` /
  `btv.ui.input`, callback-based). Nothing in the plugin API can block the
