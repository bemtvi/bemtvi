...
// `clearDaemonLiveState()` silently forgot the live ids, so an in-flight `nx.run` promise,
// `nx.process` on_exit, and `nx.socket` on_close all hung forever across a link drop — a
// silent stub, violating CLAUDE.md "fail loud".
//
// Flow: spawn a real `nxvim --daemon --listen` and a local TCP echo peer, open the page
// with `?daemon=<uri>`, start one in-flight leg of each kind (a long-running `nx.run`
// child, a duplex `nx.process` child on `cat`, an `nx.socket` connection), force the link
// to drop (window.__nxvim.debugDropDaemon), and assert each callback fires promptly with
...
// (target/debug/nxvim), and a Chromium for Playwright. Run:  node verify-drop-failures.mjs
...
const TCP_PORT = 8155; // the nx.socket peer
...
async function lua(page, code) {
  return page.evaluate(
    (c) => window.__nxvim.execLua(c).then((r) => String(r.result)),
    code,
  );
}
...
async function status(page) {
  const r = await lua(page, "return nx.daemon.status()");
...
// ── A local TCP peer for nx.socket (the daemon dials it; never sends, never closes) ──────
...
try {
  ...
  const setup = await lua(
    ...
    -- one-shot proc leg: a child that outlives the link (nx.run resolves on exit only)
    nx.run({ cmd = "sleep", args = { "60" } }):next(function(r) _G.__proc = r.code end)
    -- duplex proc leg: cat blocks on stdin forever
    nx.process.open({ cmd = "cat", on_exit = function(code) _G.__dproc = code end })
  ...
  const sockUp = await until(page, () => window.__nxvim.execLua("return _G.__sock_up").then((r) => String(r.result)), (v) => v.includes("true"));
  ...
  await page.evaluate(() => window.__nxvim.debugDropDaemon());
  ...
  check("proc: the in-flight nx.run on_exit fired with a -1 exit (never leaked)", proc === "-1", `state=${state}`);
  check("dproc: the duplex nx.process on_exit fired with a -1 exit", dproc === "-1", `state=${state}`);
  check(
    "sock: the nx.socket on_close fired with the loud connection error",
