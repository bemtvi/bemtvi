#!/usr/bin/env bash
# Netlify build entry for the **python-demo** site (crates/bemtvi-edithost) — the standard wasm
...
# standard site (bemtvi); the demo site (bemtvi-demo) is configured by THIS crate's netlify.toml
# (../netlify-edithost is the repo root; the file is crates/bemtvi-edithost/netlify.toml). In the
# Netlify dashboard, the only setting the demo site needs is:
#
#   Base directory:    crates/bemtvi-edithost   (makes Netlify read this crate's netlify.toml,
