//! The daemon wire protocol, `btv.http` half (the `http_op` leg).
//!
//! Proves async `btv.http.fetch` runs **on the daemon over a real wire** in a
//! native-daemon session: the editor is given a [`RemoteHttp`](bemtvi_server::RemoteHttp)
...
use bemtvi_server::{RemoteHttp, ServerInit};
use bemtvi_test_harness::{attach, exec_lua, spawn};
...
/// Start a server whose `btv.http` seam is a [`RemoteHttp`] talking to a
...
async fn spawn_with_daemon_http() -> (Rpc, UnboundedReceiver<Incoming>) {
    ...
    tokio::spawn(async move {
        let _ = bemtvi_server::serve_http_daemon(daemon_reader, daemon_writer).await;
    });
...
/// `btv.http.fetch` runs on the daemon over the wire: the actor has no local `ureq`, so a
/// resolved 200 with the server's body can only have come from the daemon's round-trip.
#[tokio::test]
async fn btv_http_fetch_runs_on_the_daemon_over_the_wire() {
    ...
    exec_lua(
        ...
        &format!(
            r#"_G.__status = nil
               _G.__body = nil
               btv.http.fetch("{base}/thing"):next(
...
{
    let (rpc, mut incoming) = bemtvi_rpc::connect(reader, writer);
    while let Some(msg) = incoming.recv().await {
        if let Incoming::Request { id, method, .. } = msg {
            if method == "http_op" {
                let envelope = bemtvi_lua::http_result_to_value(&Err(bemtvi_lua::HttpError {
...
/// `btv.http.fetch_local` bypasses the daemon: in a daemon session where the daemon rejects
...
#[tokio::test]
async fn btv_http_fetch_local_bypasses_the_daemon() {
    ...
    exec_lua(
        ...
        &format!(
            r#"_G.routed = nil
               ...
               btv.http.fetch_local("{base}/x"):next(
