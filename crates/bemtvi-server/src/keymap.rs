...
//!
//! **Matching without a timer.** bemtvi processes keys synchronously, in
//! `btv_input` batches, with no idle timer — so vim's `timeoutlen` ambiguity
...
//! ([`Keymaps::flush`], the `bemtvi_input_flush` RPC) after `timeoutlen` of no
...
//! read-only oracle — [`bemtvi_core::command_status`], a pure fold over the same
...
use bemtvi_lua::{RawKeymap, RawRhs};
...
#[derive(Clone, Debug)]
pub enum MappingRhs {
    /// A Lua function RHS, keyed by id in `btv._keymap_fns`; the server runs it
...
pub struct Mapping {
    ...
    /// RHS is affected — a string RHS ignores it (bemtvi has no expression evaluator).
    pub expr: bool,
...
#[derive(Default)]
pub struct Keymaps {
    /// `btv._keymaps_version` the cached [`snapshot`](Self::snapshot) was last
    ...
    /// at `btv_ui_attach`). When set, an LHS is parsed with [`parse_keys_raw`] so
...
impl Keymaps {
    /// Cache a fresh registry snapshot (read when `btv._keymaps_version`
    ...
    /// `btv_ui_attach`; a no-op when the value is unchanged.
    ...
    /// behavior bemtvi encodes as `default` keymaps (the `cmdline` control keys `<CR>`
...
/// bemtvi-specific multi-cursor *placement* bucket; `''` (vim's `:map`) covers
/// normal + visual + placement (every normal-ish mode). Operator-pending (`'o'`)
/// is **deferred**: bemtvi has no
...
fn mode_buckets(code: &str) -> &'static [char] {
    match code {
        ...
        // in Select mode and never leaks into the `btv.ui.select` widget.
        "s" => &['s'],
        "m" => &['m'],
        // The Helix selection-first modes share one bucket (`mode_key` maps both
        // `HelixNormal`/`HelixSelect` to `'h'`). A `btv.keymap.set('helix', …)` lands
        ...
        // `btv.view` / quickfix buffers, and the read-only scratch / loclist listings,
