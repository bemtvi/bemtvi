...
// `btv.run` (and `vim.system`) promise. `btv.run_stream` exercises the streaming sibling.
//
...
// Run a one-shot `btv.run{…}` (the Lua expression is spliced in) and poll for its resolved result,
...
try {
  ...
  // ── 1) one-shot btv.run: compute on stdout, a distinct line on stderr, clean exit 0. The sum is
  ...
  const r1 = await runOneShot(page, "P1", `btv.run({ cmd = "python", args = { "-c", ${JSON.stringify(code1)} } })`);
  check("btv.run: the in-browser python computed sum(0..100)=5050 on stdout",
    !!r1 && /SUM 5050/.test(r1.stdout), `r1=${JSON.stringify(r1)}`);
  check("btv.run: stderr is captured SEPARATELY from stdout",
    !!r1 && /ERRLINE/.test(r1.stderr) && !/ERRLINE/.test(r1.stdout), `r1=${JSON.stringify(r1)}`);
  check("btv.run: a clean run exits 0", !!r1 && r1.code === 0, `r1=${JSON.stringify(r1)}`);

  // ── 2) a non-zero exit code propagates (sys.exit(3)). ────────────────────────────────────────
  const r2 = await runOneShot(page, "P2", `btv.run({ cmd = "python", args = { "-c", "import sys; sys.exit(3)" } })`);
  check("btv.run: sys.exit(3) surfaces as code 3", !!r2 && r2.code === 3, `r2=${JSON.stringify(r2)}`);

  // ── 3) a python traceback → exit 1 with the error on stderr. ─────────────────────────────────
  const r3 = await runOneShot(page, "P3", `btv.run({ cmd = "python", args = { "-c", "raise ValueError('boom')" } })`);
  ...
  const r5 = await runOneShot(
    page, "P5",
    `btv.run({ cmd = "python", args = { "-c", ${JSON.stringify(code5)} }, stdin = "hello stdin" })`,
  );
  check("btv.run: the child's stdin was piped through (read + uppercased)",
    !!r5 && /HELLO STDIN/.test(r5.stdout) && r5.code === 0, `r5=${JSON.stringify(r5)}`);

  // ── 6) btv.run_stream: stdout arrives as line batches, then end-of-stream. ────────────────────
  ...
  await page.evaluate(
    (code) =>
      window.__bemtvi.execLua(`
        ...
        _G.SDONE = false
        btv.async(function()
          local st = btv.run_stream({ cmd = "python", args = { "-c", ${JSON.stringify(code)} } })
          for batch in btv.await_each(st) do
  ...
  check("btv.run_stream: all five streamed stdout lines arrived (LINE 0..4)",
...
console.log(failures === 0
  ? "\nALL PASS — serverless `vim.system`/`btv.run` runs CPython (Pyodide) in-browser: stdout/stderr/exit captured, streaming works"
  : `\n${failures} FAILED`);
