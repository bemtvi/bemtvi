//! End-to-end coverage of the native `btv.git.*` API (docs/plans/2026-07-24-native-git-gix.md).
//!
...
//! `bemtvi-git`); only the test's fixture setup shells out to `git` to build the repo,
...
/// `btv.git.head` reports the branch, a full sha, and detached=false.
#[tokio::test]
async fn head_reports_branch_and_sha() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.h = nil\n\
             btv.git.head({file:?}):next(function(r) _G.h = r end, function(e) _G.err = e end)",
        ),
    )
    .await;
...
/// `btv.git.diff_file` folds working-tree-vs-HEAD line changes into counts. Editing
...
async fn diff_file_counts_a_modified_line() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.d = nil\n\
             btv.git.diff_file({repo:?}, {file:?}):next(function(r) _G.d = r end)",
        ),
    )
    .await;
...
/// `btv.git.show` returns the HEAD blob bytes (the committed content), NOT the edited
...
async fn show_returns_head_blob_not_worktree() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.s = nil\n\
             btv.git.show({file:?}, 'HEAD'):next(function(r) _G.s = r end, function(e) _G.serr = e end)",
        ),
    )
    .await;
...
/// `btv.git.status` reports a modified tracked file with a worktree-column `M`.
#[tokio::test]
async fn status_reports_a_modified_file() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.st = nil\n\
             btv.git.status({repo:?}):next(function(r) _G.st = r end, function(e) _G.sterr = e end)",
        ),
    )
    .await;
...
/// behavior bemtvi-diff relies on: `open()` discovers from the parent dir when the path
...
async fn show_uncommitted_file_rejects_enoent_not_enorepo() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.code = nil\n\
             btv.git.show({ghost:?}, 'HEAD'):next(function() _G.code = 'RESOLVED' end, function(e) _G.code = e.code end)",
        ),
    )
    .await;
...
/// `btv.git_local.clone` clones a (local) source repo into a fresh dir and checks out its
...
async fn clone_creates_worktree() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.dir = nil\n\
             btv.git_local.clone({src:?}, {dest:?}, {{ depth = 1 }})\
...
/// `btv.git_local.checkout(dir, sha, {detach=true})` detaches HEAD onto an older commit
...
async fn checkout_detach_resets_worktree() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.done = nil\n\
             btv.async(function()\n\
               btv.await(btv.git_local.clone({src:?}, {dest:?}))\n\
               btv.await(btv.git_local.checkout({dest:?}, {first:?}, {{ detach = true }}))\n\
               _G.done = true\n\
    ...
    exec_lua(
        ...
        &format!(
            "_G.h = nil\n\
             btv.git.head({dest:?}):next(function(r) _G.h = r end)",
        ),
    )
    .await;
...
/// `btv.git_local.pull` fast-forwards the branch to a new upstream commit (updating the
...
async fn pull_fast_forwards_then_reports_noop() {
    ...
    exec_lua(
        &rpc,
        &format!("btv.git_local.clone({src:?}, {dest:?}):next(function() _G.cloned = true end, function(e) _G.err = e end)"),
    )
    .await;
    ...
    exec_lua(
        &rpc,
        &format!("_G.p = nil\nbtv.git_local.pull({dest:?}):next(function(r) _G.p = r end, function(e) _G.perr = e end)"),
    )
    .await;
    ...
    exec_lua(
        &rpc,
        &format!("_G.p2 = nil\nbtv.git_local.pull({dest:?}):next(function(r) _G.p2 = r end)"),
    )
    .await;
...
/// `btv.git_local.pull` refuses a non-fast-forward (the branches diverged) with `ENOTFF`,
...
async fn pull_rejects_non_fast_forward() {
    ...
    exec_lua(
        &rpc,
        &format!("btv.git_local.clone({src:?}, {dest:?}):next(function() _G.cloned = true end)"),
    )
    .await;
    ...
    exec_lua(
        ...
        &format!(
            "_G.code = nil\n\
             btv.git_local.pull({dest:?}):next(function() _G.code = 'RESOLVED' end, function(e) _G.code = e.code end)",
        ),
    )
    .await;
    settle(&rpc, &["_G.code"]).await;
...
/// `btv.git_local.submodule_update{init, recursive}` clones a missing submodule and checks
...
async fn submodule_update_inits_and_checks_out() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.done = nil\n\
             btv.async(function()\n\
               btv.await(btv.git_local.clone({super_repo:?}, {dest:?}))\n\
               btv.await(btv.git_local.submodule_update({dest:?}, {{ init = true, recursive = true }}))\n\
               _G.done = true\n\
             end)():catch(function(e) _G.err = e end)",
        ),
    )
...
// one clone, `btv.git_local.*` in a parallel clone of the same source — and assert the
...
/// Oracle: `btv.git_local.checkout(dir, sha, {detach})` produces the same worktree as
...
async fn checkout_matches_git_oracle() {
    ...
    // My side: clone + btv.git_local.checkout.
    ...
    exec_lua(
        ...
        &format!(
            "btv.async(function()\n\
               btv.await(btv.git_local.clone({src:?}, {mine:?}))\n\
               btv.await(btv.git_local.checkout({mine:?}, {first:?}, {{ detach = true }}))\n\
               _G.done = true\n\
             end)():catch(function(e) _G.err = e end)",
        ),
    )
...
/// Oracle: `btv.git_local.pull` fast-forwards to exactly the same worktree as `git pull
/// --ff-only`.
#[tokio::test]
async fn pull_matches_git_oracle() {
    ...
    exec_lua(
        &rpc,
        &format!("btv.git_local.clone({src:?}, {mine:?}):next(function() _G.cloned = true end)"),
    )
    .await;
    settle(&rpc, &["_G.cloned"]).await;
    assert_eq!(
    ...
    exec_lua(
        &rpc,
        &format!("btv.git_local.pull({mine:?}):next(function() _G.pulled = true end, function(e) _G.err = e end)"),
    )
    .await;
...
/// Oracle: `btv.git_local.submodule_update{init,recursive}` checks out the submodule to
...
async fn submodule_update_matches_git_oracle() {
    ...
    // Mine: btv.git_local.clone + submodule_update.
    ...
    exec_lua(
        ...
        &format!(
            "btv.async(function()\n\
               btv.await(btv.git_local.clone({super_repo:?}, {mine:?}))\n\
               btv.await(btv.git_local.submodule_update({mine:?}, {{ init = true, recursive = true }}))\n\
               _G.done = true\n\
             end)():catch(function(e) _G.err = e end)",
        ),
    )
...
/// remote. `btv.git_local.submodule_update` must check it out to the same content as `git
...
async fn submodule_relative_url_matches_git_oracle() {
    ...
    // Mine: btv.git_local.clone + submodule_update (my resolver resolves `../dep`).
...
async fn discover_rejects_outside_a_repo() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.code = nil\n\
             btv.git.discover({dir:?}):next(function() _G.code = 'RESOLVED' end, function(e) _G.code = e.code end)",
        ),
    )
    .await;
    settle(&rpc, &["_G.code"]).await;
        ...
        &format!(
            "_G.done, _G.err = nil, nil\n\
             btv.async(function()\n\
               btv.await(btv.git_local.clone({src:?}, {dest:?}))\n\
               btv.await(btv.git_local.checkout({dest:?}, {first:?}, {{ detach = true }}))\n\
               btv.await(btv.git_local.checkout({dest:?}, \"main\"))\n\
               _G.done = true\n\
             end)():catch(function(e) _G.err = e end)",
        ),
    )
    ...
    exec_lua(
        ...
        &format!(
            "_G.done, _G.err = nil, nil\n\
             btv.async(function()\n\
               btv.await(btv.git_local.clone({src:?}, {dest:?}))\n\
               btv.await(btv.git_local.checkout({dest:?}, \"side\"))\n\
               _G.done = true\n\
             end)():catch(function(e) _G.err = e end)",
        ),
    )
    ...
    exec_lua(
        ...
        &format!(
            "_G.done, _G.err = nil, nil\n\
             btv.async(function()\n\
               btv.await(btv.git_local.clone({src:?}, {dest:?}))\n\
               btv.await(btv.git_local.checkout({dest:?}, \"no-such-branch\"))\n\
               _G.done = true\n\
             end)():catch(function(e) _G.err = e end)",
        ),
    )
    ...
    exec_lua(
        ...
        &format!(
            "_G.err1, _G.ok2 = nil, nil\n\
             btv.async(function()\n\
               btv.await(btv.git_local.clone({src:?}, {dest:?}, {{ depth = 1 }}))\n\
               local ok, e = pcall(btv.await, btv.git_local.checkout({dest:?}, {first:?}, {{ detach = true }}))\n\
               if not ok then _G.err1 = tostring(e and e.message or e) end\n\
               -- ...until the clone is unshallowed, after which it succeeds.\n\
               btv.await(btv.git_local.fetch({dest:?}, {{ unshallow = true }}))\n\
               btv.await(btv.git_local.checkout({dest:?}, {first:?}, {{ detach = true }}))\n\
               _G.ok2 = true\n\
...
async fn fetch_updates_tracking_refs_without_touching_the_worktree() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.done, _G.err = nil, nil\n\
             btv.git_local.fetch({dest:?}):next(function() _G.done = true end)\n\
    ...
    exec_lua(
        ...
        &format!(
            "_G.pulled, _G.perr = nil, nil\n\
             btv.git_local.pull({dest:?}):next(function(r) _G.pulled = r.updated end)\n\
...
/// `btv.git.status(path, { ignored = true })` reports git-ignored paths as porcelain's
...
async fn status_reports_ignored_paths_only_when_asked() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.st, _G.sterr = nil, nil\n\
             btv.git.status({repo:?}, {{ ignored = true }})\n\
