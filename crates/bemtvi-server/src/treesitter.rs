...
//! ([`bemtvi_core::Editor`] owns a `SyntaxEngine`); there is no worker process and
...
//! engine reports [`ts_parse_pending`](bemtvi_core::Editor::ts_parse_pending), and the
...
use crate::EditHost;
use bemtvi_core::{unicode, view::WindowView, BufferId, WinHl};
use rmpv::Value;
...
pub(crate) struct SyntaxState {
    ...
    /// filetype=…`, `btv.bo.filetype` / `btv.bo.ts_highlight`) still invalidates the
...
impl EditHost {
    ...
    /// Answer one `btv.treesitter.highlight`: highlight `text` as `lang` and settle the
    ...
    /// Re-run the `btv.treesitter.highlight` asks parked on a grammar that has now
    ...
    fn install_grammar(&mut self, outcome: crate::GrammarOutcome) -> bool {
        ...
        // its lines and re-highlights them; a parked `btv.treesitter.highlight` runs
    ...
    /// concatenation via
    /// [`set_resolved_ts_query`](bemtvi_core::Editor::set_resolved_ts_query) (which
    ...
    fn resolve_runtimepath_queries(&mut self, lang: &str) -> bool {
        ...
        for name in bemtvi_core::ENGINE_QUERY_NAMES {
    ...
    /// an LSP doc float and `btv.treesitter.highlight` paint through.
    ///
    ...
    /// [`preview_highlights_bg`](bemtvi_core::Editor::preview_highlights_bg) does.
    ...
    /// ([`ts_base_query_raw`](bemtvi_core::Editor::ts_base_query_raw)) rather than the
        ...
        while i < pending.len() {
            ...
            for sub in ["queries", "after/queries"] {
                for dir in rtp {
                    ...
                    for extra in bemtvi_core::parse_query_inherits(&text) {
                        if seen.insert(extra.clone()) {
                            pending.push(extra);
                        }
                    }
                    if bemtvi_core::query_extends(&text) {
            ...
            .map(|seg| {
                ...
                if let Some(spans) = ts_spans {
                    for (i, s) in spans.iter().enumerate() {
                        intervals.push(crate::extmarks::HlInterval {
                            ...
                            priority: bemtvi_core::TS_HL_PRIORITY,
                            order: i as u32,
                            capture: true,
                        });
                    }
                ...
                for &(sb, eb) in &control {
                    intervals.push(crate::extmarks::HlInterval {
                        ...
                        group: "SpecialKey",
                        priority: bemtvi_core::SPECIAL_KEY_PRIORITY,
                        order: 0,
                        capture: false,
                    });
                }
