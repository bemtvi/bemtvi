//! Black-box tests for the native `btv.buf.set_text` API (alias `nvim_buf_set_text`):
...
use bemtvi_rpc::Rpc;
use bemtvi_test_harness::{exec_lua, start_with_file as open};
use rmpv::Value;

/// The current buffer's lines joined with `|`.
async fn lines(rpc: &Rpc) -> String {
    match exec_lua(rpc, "return table.concat(btv.buf.lines(0, 0, -1), '|')").await {
...
async fn set_and_read(rpc: &Rpc, args: &str) -> String {
    exec_lua(rpc, &format!("btv.buf.set_text(0, {args})")).await;
    lines(rpc).await
}
...
async fn the_edit_marks_the_buffer_modified() {
    let (rpc, _inc) = open("ab\n").await;
    exec_lua(&rpc, r#"btv.buf.set_text(0, 0, 0, 0, 0, { "z" })"#).await;
    assert_eq!(
...
async fn the_promise_resolves_after_the_edit_is_visible() {
    ...
    exec_lua(
        ...
        r#"btv.async(function()
             btv.await(btv.buf.set_text(0, 0, 0, 0, 5, { "HELLO" }))
             _G.__seen = table.concat(btv.buf.lines(0, 0, -1), "|")
           end)()"#,
    )
    .await;
...
async fn a_nomodifiable_buffer_fails_loud() {
    ...
    let ok = exec_lua(
        &rpc,
        r#"return (pcall(btv.buf.set_text, 0, 0, 0, 0, 1, { "X" }))"#,
    )
    .await;
...
async fn a_non_string_replacement_line_fails_loud() {
    ...
    let ok = exec_lua(
        &rpc,
        "return (pcall(btv.buf.set_text, 0, 0, 0, 0, 0, { 42 }))",
    )
    .await;
    assert_eq!(ok.as_bool(), Some(false));
}
...
async fn a_newline_bearing_line_fails_loud() {
    ...
    let ok = exec_lua(
        &rpc,
        r#"return (pcall(btv.buf.set_text, 0, 0, 0, 0, 0, { "x\ny" }))"#,
    )
    .await;
    assert_eq!(ok.as_bool(), Some(false));
}
