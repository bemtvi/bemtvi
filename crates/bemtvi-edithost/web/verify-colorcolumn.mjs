...
try {
  ...
  await page.evaluate(() => window.__bemtvi.feed("ione two three four five six<CR>seven eight nine ten<Esc>"));
  await sleep(200);
  ...
  // Set two rulers.
  await page.evaluate(() => window.__bemtvi.execLua("vim.opt.colorcolumn = '3,6'"));
  await sleep(200);
  ...
  await page.evaluate(() => window.__bemtvi.execLua("vim.opt.colorcolumn = ''"));
  await sleep(200);
