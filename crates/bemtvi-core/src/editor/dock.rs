...
//! screen edge (bemtvi's VSCode-style side/bottom panels). A dock holds an
...
impl Editor {
    ...
    /// open. Backs `btv.dock.close` / `:DockClose`.
    ...
    /// Focus the dock on `side` (`btv.dock.focus` / `:DockFocus`, and the
    ...
    /// Backs `btv.dock.toggle` / `:DockToggle`.
    pub(crate) fn toggle_dock(&mut self, side: DockSide) {
        if self.dock_is_open(side) {
            ...
            self.echo(format!("btv.dock: no dock on {} to toggle", side.keyword()));
        }
    }

    /// Whether `side`'s dock is open (`btv.dock.is_open`). Public mirror of
    ...
    // the `btv.dock.*` Lua API carries, validated loudly here.
    ...
    /// core of the string-keyed `btv.dock.*` verbs.
    ...
    /// `btv.dock.open{ side, size?, buf? }` — open/focus a dock by side keyword.
    ...
    /// `btv.dock.toggle(side)` — toggle a dock's visibility by side keyword.
    pub fn toggle_dock_named(&mut self, side: &str) {
        self.with_dock_side(side, |e, s| e.toggle_dock(s));
    }

    /// `btv.dock.hide(side)` — hide a dock (keep its content) by side keyword.
    pub fn hide_dock_named(&mut self, side: &str) {
        self.with_dock_side(side, |e, s| e.hide_dock(s));
    }

    /// `btv.dock.show(side)` — show (un-hide) and focus a dock by side keyword.
    ...
    /// [`Editor::dock_is_open`] for the server's `btv._docks` mirror.
    ...
    /// `btv.dock.opt(side).<name> = <number>` — set a numeric dock option by side
    ...
    /// `btv.dock.opt(side).<name> = <string>` — set a string dock option by side
    ...
    pub fn set_dock_option_str(&mut self, side: &str, name: &str, value: String) {
        ...
        match name {
            ...
            "winhighlight" => {
                ...
                if !bad.is_empty() {
                    self.echo(format!(
                        "btv.dock: ignoring malformed winhighlight: {bad:?}"
                    ));
                }
    ...
    /// The dock-option values for `side` (keyword), for the `btv._dock_opts` read
    ...
    /// A dock's title (the `btv.dock` strip label), for the view projection. Empty
