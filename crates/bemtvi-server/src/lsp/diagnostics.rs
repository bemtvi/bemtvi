...
use bemtvi_core::extmark::DIAGNOSTIC_NS;
use bemtvi_core::unicode;
use bemtvi_core::Buffer;
use bemtvi_core::WinHl;
use bemtvi_lsp::lsp_types::Diagnostic;
use bemtvi_lsp::PositionEncoding;
use rmpv::Value;
...
impl EditHost {
    ...
    /// leaving `vim.diagnostic.get` fresh mid-insert. bemtvi holds the update one
    ...
    /// (`btv._diagnostics[bufnr]`, what the synchronous `vim.diagnostic.get` reads)
    ...
    pub(crate) fn push_diagnostics_mirror(&mut self, buffer: bemtvi_core::BufferId) {
    ...
    pub(crate) fn commit_pending_diagnostics(&mut self) {
        ...
        let mut refreshed: Vec<bemtvi_core::BufferId> = Vec::new();
    ...
    fn merged_sources(
        ...
        // bemtvi's native bytes — `Utf8` makes the shared conversion the identity.
        let client = self
            .client_diagnostics
            .get(&buffer)
            .into_iter()
    ...
    fn merged_len(&self, buffer: bemtvi_core::BufferId) -> usize {
        let lsp: usize = self
            .lsp_states
            .get(&buffer)
            .into_iter()
    ...
    pub(crate) fn refresh_diagnostic_marks(&mut self, buffer: bemtvi_core::BufferId) {
    ...
    pub(crate) fn diag_counts_for(&self, buffer: bemtvi_core::BufferId) -> [usize; 4] {
    ...
    /// [`bemtvi_core::Editor::open_location_list`] by `:LspDiagnostics` /
    /// `vim.diagnostic.setloclist`. `None` when there are no diagnostics, or the
    /// buffer has no file path to navigate to.
    pub(crate) fn diagnostics_location_list(&self) -> Option<Vec<bemtvi_core::LocListEntry>> {
...
fn diagnostic_code(d: &Diagnostic) -> Option<String> {
    use bemtvi_lsp::lsp_types::NumberOrString;
    match d.code.as_ref()? {
