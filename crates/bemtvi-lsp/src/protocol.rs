...
/// `name` is the user-chosen LSP config name (the `btv.lsp` control surface in the
/// lua crate), arbitrary rather than a fixed filetype. bemtvi runs at most one child
...
/// from the resolved Lua config's `cmd` (or the `BEMTVI_LSP_CMD` test override). The
...
pub struct ServerSpawn {
    ...
    /// Deep-merged OVER bemtvi's base `client_capabilities` at `initialize`.
...
/// The position encoding negotiated at `initialize` (Decision 4). bemtvi columns
...
/// per feature bemtvi implements. Surfaced to Lua as `client.server_capabilities`
...
pub enum LspRequest {
    ...
    /// editor opens in `btv.picker`. Position-less (whole document); the `uri` is the
    /// requesting buffer.
    DocumentSymbol {
        uri: Url,
    },
    ...
    CodeAction {
        ...
        /// The caller's kind filter (`btv.lsp.code_action{ context = { only = … } }`),
...
pub struct ReqToken {
    pub kind: u16,
    pub generation: u64,
    /// The Lua deferred-callback id ([`btv._cb_fns`]) a generic
...
pub enum LspReply {
    ...
    /// character columns and the `kind` are dropped, bemtvi folds whole lines). An
...
/// editor, decoded editor-side ([`crate::lsp::semantic`](../../bemtvi-server))
...
pub struct ApplyEditOutcome {
    ...
    /// to a server precisely because bemtvi declares a `failureHandling` strategy
...
/// 0-based buffer-line span bemtvi folds. The protocol's `startCharacter` /
/// `endCharacter` and the semantic `kind` (comment/imports/region) are dropped —
/// bemtvi's fold model is whole-line, and the line span is all its containment →
/// level builder needs.
#[derive(Clone, Copy, Debug)]
...
pub enum LspEvent {
    ...
    /// capability *after* the handshake, the dynamic half of the protocol. bemtvi
    ...
    /// `btv.lsp._register_capability`'s job in Lua (one implementation over
    /// `btv.fs.watch`, so it serves the local, daemon and browser sessions alike).
    RegisterCapability {
        key: ServerKey,
