...
//! `+N`/`-N` entries, which nxvim has no `'textwidth'` to anchor). The *painting*
//! itself is covered by the `nxvim-tui` paint tests; this pins the core→wire step.
...
async fn colorcolumn_skips_relative_and_junk_entries() {
    ...
    // `+1`/`-2` are `'textwidth'`-relative (unsupported — nxvim has no textwidth) and
