//! Behavior tests for the `btv.on_key_pending` event — the engine-computed
...
use bemtvi_test_harness::{barrier, exec_lua, feed, start_attached};
use rmpv::Value;
...
const RECORDER: &str = "_G.kp = {}\n\
     vim.g.mapleader = ' '\n\
     btv.on_key_pending(function(ctx)\n\
       local parts = {}\n\
...
const G_MAPS: &str = "btv.keymap.set('n', 'gd', function() end, { desc = 'Go to definition' })\n\
     btv.keymap.set('n', 'gD', function() end, { desc = 'Go to declaration' })\n\
     btv.keymap.set('n', 'gr', function() end, { desc = 'Find references' })\n";

/// `;;`-joined record of every `btv.on_key_pending` event so far.
...
/// A BUFFER-LOCAL mapped prefix on a focused `btv.view` buffer surfaces through the
/// pending oracle — the keys-helper / which-key path for a plugin view (e.g.
/// bemtvi-diff's panes, which bind `co`/`ct`/`cb`/`cp`/… buffer-local on each read-only
...
async fn buffer_local_prefix_on_focused_view_fires_pending() {
    ...
    exec_lua(
        &rpc,
        "local buf = vw:bufnr()\n\
         btv.keymap.set('n', 'co', function() end, { buffer = buf, desc = 'choose ours' })\n\
         btv.keymap.set('n', 'ct', function() end, { buffer = buf, desc = 'choose theirs' })\n\
         vw:focus()",
    )
    .await;
    barrier(&rpc).await;
...
/// (separate `btv_input` batches), the way a TUI sends interactive keystrokes: a
...
async fn group_continuation_then_descend() {
    ...
    exec_lua(
        ...
        &format!(
            "{RECORDER}\
             btv.keymap.set('n', '<leader>gs', function() end, {{ desc = 'stage' }})\n\
             btv.keymap.set('n', '<leader>gc', function() end, {{ desc = 'commit' }})\n\
             btv.keymap.set('n', '<leader>w', function() end, {{ desc = 'write' }})"
        ),
    )
    .await;
...
async fn map_without_desc_has_empty_desc() {
    ...
    exec_lua(
        ...
        &format!(
            "{RECORDER}\
             btv.keymap.set('n', '<leader>w', function() end)"
        ),
    )
    .await;
    feed(&rpc, "<Space>");
...
async fn user_map_on_a_builtin_key_wins_over_the_merge() {
    ...
    exec_lua(
        &rpc,
        &format!("{RECORDER}btv.keymap.set('n', 'gg', function() end, {{ desc = 'top!' }})"),
    )
    .await;
    feed(&rpc, "g");
...
/// active widget bucket. With a `btv.ui.select` menu open, its built-in `gg` (a two-key
...
async fn widget_prefix_lists_the_active_widgets_keys() {
    ...
    exec_lua(&rpc, "btv.ui.select({ 'aaa', 'bbb', 'ccc' }, {})").await;
    barrier(&rpc).await;
    feed(&rpc, "g"); // withholds the select widget's `gg` prefix
    assert_eq!(events(&rpc).await, "select|g|g/First item/map");
}

/// With no `btv.on_key_pending` listener registered the editor still maps + fires
...
async fn no_listener_input_unaffected() {
    ...
    exec_lua(
        ...
        "_G.fired = false\n\
         vim.g.mapleader = ' '\n\
         btv.keymap.set('n', '<leader>w', function() _G.fired = true end, { desc = 'write' })",
    )
    .await;
    feed(&rpc, "<Space>w");
    ...
    exec_lua(
        ...
        &format!(
            "{RECORDER_B}\
             ...
             btv.keymap.set('n', '<leader>fg', function() end, {{ desc = 'grep' }})"
        ),
    )
    .await;
...
/// With no `btv.on_key_pending` listener, the built-in grammar runs untouched — the
...
/// A user-registered object (`btv.textobject.map`) shows in the which-key object menu
...
async fn text_object_menu_lists_user_registered_objects() {
    ...
    exec_lua(
        &rpc,
        "btv.textobject.map('il', '@loop.inner')\n\
         btv.textobject.map('if', '@function.inside')", // override the built-in `if`
    )
    .await;
    feed(&rpc, "ihello<Esc>0");
    feed(&rpc, "di");
...
async fn pending_keys_render_modified_gt_as_named_escape() {
    ...
    exec_lua(
        ...
        &format!(
            "{RECORDER}\
             btv.keymap.set('n', '<C-gt>d', function() end, {{ desc = 'gt-d' }})\n\
             btv.keymap.set('n', '<C-gt>x', function() end, {{ desc = 'gt-x' }})"
        ),
    )
    .await;
    feed(&rpc, "<C-gt>");
