...
//   1. while the picker is open, the only on-screen cursor (`#btv-cursor` / `.cur-block`)
...
//
// Faithfulness (not a no-op): the picker is opened through the real `btv.picker` API and
...
try {
  ...
  await page.evaluate(() => window.__bemtvi.execLua(`btv.picker.open('files')`));
  await sleep(300);
  await page.evaluate(() => window.__bemtvi.feed("ab"));
  await sleep(300);
