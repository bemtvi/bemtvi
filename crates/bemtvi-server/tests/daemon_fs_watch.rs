//! The daemon wire protocol, `btv.fs.watch` half — the **streaming watch leg**
...
//! remote session's files are. Everything built on `btv.fs.watch` rides it — the LSP
...
//! [`RemoteFsWatch`](bemtvi_server::RemoteFsWatch) as its watch seam, so the event-loop
//! actor has **no** local watcher for `btv.fs.watch` and can only send `luafs_watch` over
...
use bemtvi_server::{RemoteFsWatch, ServerInit};
use bemtvi_test_harness::{exec_lua, q, start_attached, temp_dir};
...
/// Start a server whose `btv.fs.watch` seam is a [`RemoteFsWatch`] talking to a
...
async fn spawn_with_daemon_fs_watch() -> (Rpc, UnboundedReceiver<Incoming>, ArmLog) {
    ...
    tokio::spawn(async move {
        let (rpc, mut incoming) = bemtvi_rpc::connect(daemon_reader, daemon_writer);
        ...
        let _ = bemtvi_server::serve_luafs_watch_daemon_on(rpc, rx).await;
    });
...
async fn spawn_with_reconnecting_daemon() -> (
    ...
    let (client, handle) = bemtvi_server::connect_daemon_reconnecting_on(
        make,
...
/// about no watch, so the link re-arms every live one. Without that, a `btv.fs.watch`
...
/// A file created on the **daemon's** disk reaches `btv.fs.watch` over the wire, carrying
...
#[tokio::test]
async fn a_daemon_side_change_reaches_btv_fs_watch() {
    ...
    let err = exec_lua(&rpc, "return _G.err").await;
    panic!("no daemon watch batch reached btv.fs.watch (watch error: {err:?})");
}
...
async fn a_daemon_arm_failure_rejects_loud() {
    ...
    arm_watch(&rpc, std::path::Path::new("/bemtvi/definitely/not/here")).await;
