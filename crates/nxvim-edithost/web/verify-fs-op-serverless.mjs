// Playwright verifier for SERVERLESS `nx.fs` over OPFS (Phase 3 of the off-tick plan). With NO
// daemon connected, a browser `nx.fs.*` op runs against the Origin Private File System in the
// Worker — the JS twin of the daemon's `run_fs_job`. This proves the full op set round-trips
// against OPFS (the same sandbox `:e`/`:w` persist to), so a plugin's `nx.fs` works in the common
// no-daemon browser mode, not just with a daemon (verify-fs-op.mjs covers the daemon path).
//
// Faithfulness (not a no-op): every op is exercised end-to-end through the real `nx.fs` promise
...
// Run a `nx.fs` chain that stashes its outcome in `_G.<g>`, then poll until that global settles.
...
try {
  ...
  // No `?daemon=` — serverless. `nx.fs` must route to OPFS, not fail loud.
  ...
  const wrote = await settle(page, "__w", `nx.fs.mkdir("${ROOT}", { recursive = true }):next(function()
       nx.fs.write("${file}", "HELLO-FROM-OPFS\\nsecond"):next(function() _G.__w = "wrote" end,
         function(e) _G.__w = "werr:" .. e.code end)
     end, function(e) _G.__w = "mkerr:" .. e.code end)`);
  check("serverless nx.fs.mkdir + write resolve against OPFS", /wrote/.test(String(wrote)), `w=${JSON.stringify(wrote)}`);
  const text = await settle(page, "__text",
    `nx.fs.read_text("${file}"):next(function(t) _G.__text = t end, function(e) _G.__text = "ERR:"..e.code end)`);
  check("serverless nx.fs.write + read_text round-trip through OPFS",
  ...
  const mk = await settle(page, "__mk", `nx.fs.mkdir("${ROOT}/sub", { recursive = true }):next(
       function() _G.__mk = "made" end, function(e) _G.__mk = "err:"..e.code end)`);
  check("serverless nx.fs.mkdir creates a subdirectory", /made/.test(String(mk)), `mk=${JSON.stringify(mk)}`);
  const names = await settle(page, "__names", `nx.fs.readdir("${ROOT}"):next(function(entries)
       local out = {}
  ...
  check("serverless nx.fs.readdir lists OPFS entries with kinds",
  ...
  const stat = await settle(page, "__stat", `nx.fs.stat("${file}"):next(function(s)
       _G.__stat = "S[" .. s.type .. ":" .. tostring(s.size) .. "]"
     end, function(e) _G.__stat = "ERR:"..e.code end)`);
  // "HELLO-FROM-OPFS\nsecond" = 22 bytes.
  check("serverless nx.fs.stat reports type=file and the byte size",
    /S\[file:22\]/.test(String(stat)), `stat=${JSON.stringify(stat)}`);

  // ── 4. copy duplicates a file; the copy reads back identical ──────────────────────────────
  const copied = await settle(page, "__copied", `nx.fs.copy("${file}", "${ROOT}/copy.txt"):next(function()
       nx.fs.read_text("${ROOT}/copy.txt"):next(function(t) _G.__copied = t end, function(e) _G.__copied="rerr:"..e.code end)
     end, function(e) _G.__copied = "cerr:"..e.code end)`);
  check("serverless nx.fs.copy duplicates a file (copy reads back identical)",
  ...
  const renamed = await settle(page, "__ren", `nx.fs.rename("${ROOT}/copy.txt", "${ROOT}/renamed.txt"):next(function()
       nx.fs.exists("${ROOT}/copy.txt"):next(function(oldExists)
         nx.fs.read_text("${ROOT}/renamed.txt"):next(function(t)
  ...
  check("serverless nx.fs.rename moves the file (old gone, new content intact)",
  ...
  const removed = await settle(page, "__rm", `nx.fs.remove("${ROOT}/renamed.txt"):next(function()
       nx.fs.exists("${ROOT}/renamed.txt"):next(function(ex) _G.__rm = ex and "STILL-EXISTS" or "GONE-OK" end)
     end, function(e) _G.__rm = "rmerr:"..e.code end)`);
  check("serverless nx.fs.remove unlinks the file (exists() then false)",
  ...
  const code = await settle(page, "__code", `nx.fs.read_text("${ROOT}/nope.txt"):next(
       function(_) _G.__code = "RESOLVED?!" end, function(e) _G.__code = e.code end)`);
  check("serverless nx.fs read of a missing path rejects with err.code == ENOENT",
...
console.log(failures === 0
  ? "\nALL PASS — serverless browser nx.fs runs against OPFS (write/read_text/readdir/stat/copy/rename/remove/ENOENT)"
  : `\n${failures} FAILED`);
