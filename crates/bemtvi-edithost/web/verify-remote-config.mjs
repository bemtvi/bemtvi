...
// `bemtvi --daemon --listen` ships its *own* config + plugins over WebTransport, and the
...
// `BEMTVI_CONFIG` dir lives on Node's disk (the page origin can't read it), and the page is
...
// (target/debug/bemtvi), and a Chromium for Playwright. Run:  node verify-remote-config.mjs
...
async function evalInt(page, expr) {
  return page.evaluate((e) => window.__bemtvi.execLua("return " + e).then((r) => String(r.result)), expr);
}

// ── The daemon's config dir (on real disk; BEMTVI_CONFIG points the daemon at it) ───────────
const cfg = mkdtempSync(join(tmpdir(), "bemtvi-remote-cfg-"));
...
// ── Spawn the real daemon with BEMTVI_CONFIG → the dir above; parse its connect URI ─────────
const daemon = spawn(BEMTVI, ["--daemon", "--listen", "127.0.0.1:0"], {
  stdio: ["ignore", "pipe", "pipe"],
  env: { ...process.env, BEMTVI_CONFIG: cfg },
});
let uri = null;
let daemonOut = "";
...
try {
  ...
  const opt = await until(page, () => window.__bemtvi.execLua("return vim.g.remote_opt").then((r) => String(r.result)), (v) => /13$/.test(v));
