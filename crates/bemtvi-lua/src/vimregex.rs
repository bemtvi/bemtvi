...
//! the real vim regexp engine ([`bemtvi_regex`] — vim's own backtracking + NFA
...
//! dialect from bemtvi's default `/` search** (canonical regex — see
//! `bemtvi-core/src/search.rs`, whose `'regexsyntax'` option can also switch *it* to
...
use bemtvi_regex::{Engine, LineMatch, PatternKind, VimRegex};
