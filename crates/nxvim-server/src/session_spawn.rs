...
//! `nxvim` and `nxvim-gui`). Every native client builds a session the same way:
...
/// `NXVIM_DAEMON_CMD="ssh host nxvim --daemon"`. Unset = each client's default
/// (the TUI re-invokes itself with `--daemon`; the GUI spawns the sibling `nxvim`
/// binary).
pub const DAEMON_CMD_ENV: &str = "NXVIM_DAEMON_CMD";

/// The `$NXVIM_DAEMON_CMD` daemon command, if the env var is set: the command
...
/// shared `/tmp`:
///
/// - **No symlink clobber (CWE-377).** A fixed name (`nxvim-daemon.log`) opened
