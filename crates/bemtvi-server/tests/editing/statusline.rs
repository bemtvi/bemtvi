...
async fn statusline_bare_variable_expression_errors_loudly() {
    let (rpc, mut incoming) = start(None).await;
    // A bare variable in `%{}` is not evaluable in pure core (bemtvi has no
...
// ----- `btv.statusline` — the declarative segment registry (lualine shape) -----
//
// A `btv.statusline.setup{}` layout composes named segments into the same `status`
...
#[tokio::test]
async fn btv_statusline_builtins_compose() {
    let (rpc, mut incoming) = start(None).await;
    ...
    exec_lua(
        &rpc,
        r#"btv.statusline.setup{ left = { "mode", "filename" }, right = { "location" } }"#,
    )
    .await;
...
#[tokio::test]
async fn btv_statusline_layout_overrides_statusline_format() {
    let (rpc, mut incoming) = start(None).await;
    // While a segment layout is active it takes precedence over `'statusline'`.
    feed(&rpc, ":set statusline=ZZZ<CR>");
    exec_lua(
        &rpc,
        r#"btv.statusline.setup{ left = { "filename" }, right = {} }"#,
    )
    .await;
    let map = redraw_after(&rpc, &mut incoming, "<Esc>").await;
    let text = status_text(&map);
...
#[tokio::test]
async fn btv_statusline_custom_segment_renders_published_cells() {
    let (rpc, mut incoming) = start(None).await;
    ...
    exec_lua(
        ...
        btv.statusline.setup{ left = { 'git' }, right = {} }
        "#,
    )
    .await;
    let map = redraw_after(&rpc, &mut incoming, "<Esc>").await;
...
#[tokio::test]
async fn btv_statusline_invalidate_repaints_segment() {
    let (rpc, mut incoming) = start(None).await;
    ...
    exec_lua(
        ...
        r#"
        _G.branch = 'one'
        btv.statusline.segment{ name = 'git', render = function() return { { text = _G.branch } } end }
        btv.statusline.setup{ left = { 'git' }, right = {} }
        "#,
    )
    .await;
    ...
    exec_lua(
        &rpc,
        r#"_G.branch = 'two'; btv.statusline.invalidate('git')"#,
    )
    .await;
    let map = redraw_after(&rpc, &mut incoming, "<Esc>").await;
    let text = status_text(&map);
...
#[tokio::test]
async fn btv_statusline_event_rerenders_on_declared_event() {
    ...
    exec_lua(
        ...
        btv.statusline.setup{ left = { 'counter' }, right = {} }
        "#,
    )
    .await;
    let map = redraw_after(&rpc, &mut incoming, "<Esc>").await;
...
#[tokio::test]
async fn btv_statusline_unknown_segment_errors_loudly() {
    let (rpc, _incoming) = start(None).await;
    ...
    let res = exec_lua(
        &rpc,
        r#"
        local ok, err = pcall(function() btv.statusline.setup{ left = { 'nonesuch' } } end)
...
/// a `btv.statusline` custom segment renders a distinct cell per window.
...
#[tokio::test]
async fn btv_statusline_custom_segment_caches_per_window() {
    let (rpc, mut incoming) = start(None).await;
    ...
    exec_lua(
        ...
        btv.statusline.setup{ left = { 'wi' }, right = {} }
        "#,
    )
    .await;
    // Sole window: focused.
...
#[tokio::test]
async fn btv_statusline_custom_segment_follows_focus_change() {
    let (rpc, mut incoming) = start(None).await;
...
#[tokio::test]
async fn btv_statusline_custom_segment_tracks_per_window_buffer() {
    ...
    let dir = temp_dir("btv_stl_perwin_buf");
    let a = dir.join("a.txt");
    let b = dir.join("b.txt");
    std::fs::write(&a, "a\n").expect("write a");
    std::fs::write(&b, "b\n").expect("write b");
    ...
    exec_lua(
        ...
        btv.statusline.segment{
          name = 'buftail',
          render = function(ctx)
            local name = btv.buf.name(ctx.buf)
            return { { text = name:match('[^/]+$') or '?' } }
          end,
        }
        btv.statusline.setup{ left = { 'buftail' }, right = {} }
        "#,
    )
    .await;
...
#[tokio::test]
async fn btv_statusline_window_local_layout_overrides_global() {
    let (rpc, mut incoming) = start(None).await;
    ...
    exec_lua(
        &rpc,
        r#"btv.statusline.setup{ win = 0, left = { 'mode' }, right = {} }"#,
    )
    .await;
...
#[tokio::test]
async fn btv_statusline_window_can_opt_back_to_format() {
    let (rpc, mut incoming) = start(None).await;
    ...
    .await;
    feed(&rpc, ":split<CR>");
    exec_lua(&rpc, r#"btv.statusline.setup{ win = 0, format = true }"#).await;
    let st = all_window_status_texts(&redraw_after(&rpc, &mut incoming, "<Esc>").await);
    assert_eq!(st.len(), 2, "two windows: {st:?}");
    assert!(
    ...
    exec_lua(&rpc, r#"btv.statusline.reset(0)"#).await;
    let st = all_window_status_texts(&redraw_after(&rpc, &mut incoming, "<Esc>").await);
    assert!(
...
#[tokio::test]
async fn btv_statusline_layout_does_not_leak_into_custom_tabline() {
    let (rpc, mut incoming) = start(None).await;
