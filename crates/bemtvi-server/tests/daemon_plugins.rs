//! The `btv.plugins` manager is REMOTE-AWARE: in a daemon (edit-host) session it
...
//!
//! Faithful, not a no-op: the session's `btv.fs` is a real [`RemoteFsJobs`] seam
//! (`FsBackend::Remote`) answered over an in-process wire by a daemon-side `LuaFs`
//! that COUNTS every op it serves. A control `btv.fs.exists` proves the counter is
//! live (a session `btv.fs` op DOES cross to the daemon). Then a local-`dir` plugin is
...
use bemtvi_lua::{LuaDirEntry, LuaFs, LuaStat, StdLuaFs};
use bemtvi_rpc::{Incoming, Rpc};
use bemtvi_server::{RemoteFsJobs, ServerInit};
use bemtvi_test_harness::{attach, exec_lua, lua_bool, q, spawn, temp_dir};
...
/// Start a daemon-backed session whose `btv.fs` seam is a [`RemoteFsJobs`] answered by a
...
async fn plugin_management_stays_local_in_a_daemon_session() {
    ...
    // Control: a *session* `btv.fs` op DOES cross to the daemon — proves the counter is
    // live, so a zero delta below is meaningful (not a dead wire).
    exec_lua(
        &rpc,
        &format!(
            "_G.__ctl = nil
             btv.fs.exists(\"{p}\"):next(function(v) _G.__ctl = v end)
             return 1",
            p = q(&repo)
        ),
    )
    ...
    assert!(saw_control, "control btv.fs.exists should reach the daemon");
