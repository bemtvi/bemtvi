//! The daemon wire protocol, `nx.fs` working-directory half — **a relative `nx.fs` path
...
//! edit-host must absolutize a relative `nx.fs` path against `DirState` *before* it crosses
//! the wire — exactly as `drain_pending_opens` does for a relative `:edit`. Without that, a
//! `nx.fs.readdir(".")` lists the daemon's launch dir and silently ignores `:cd`.
//!
//! Faithful, not a no-op: a real `nxvim --daemon` (`run_daemon_io`) serves the real disk
...
/// Start a full edit-host session against a real `nxvim --daemon` over an in-process
/// duplex (every host seam over one link, as the binary does over stdio), with the
/// session cwd seeded to `cwd`. The daemon serves the real disk, so a relative `nx.fs`
...
/// Run `nx.fs.readdir(path)` and poll until it settles, returning its entry names sorted
...
async fn readdir_names(rpc: &Rpc, path: &str) -> String {
    exec_lua(
        ...
        &format!(
            "_G.__names = nil
             nx.async(function()
               local ok, es = pcall(nx.await, nx.fs.readdir(\"{path}\"))
...
/// Run `nx.run { cmd }` (no `cwd`) and poll until it exits, returning its stdout — the
...
async fn run_stdout(rpc: &Rpc, cmd: &str) -> String {
    exec_lua(
        ...
        &format!(
            "_G.__out = nil
             nx.run({{ cmd = {{ {cmd} }} }}):next(function(r) _G.__out = r.stdout end)
             return 1"
        ),
    )
    .await;
...
/// A `nx.fs.readdir(".")` in a daemon session lists the *session* cwd (the seeded remote
...
async fn readdir_dot_resolves_against_the_session_cwd() {
    ...
    assert!(
        ...
        "nx.fs.readdir(\".\") must resolve `.` against the session cwd (got: {names:?})"
    );
}

/// After a remote `:cd` into a subdirectory, `nx.fs.readdir(".")` follows it — the same
...
/// A `nx.run` with no `cwd` runs the child in the *session* cwd (like neovim's
...
async fn spawn_without_cwd_runs_in_the_session_cwd() {
    ...
    assert!(
        ...
        "a no-cwd `nx.run` must inherit the session cwd (got: {out:?})"
    );
    ...
    assert!(
        ...
        "after `:cd sub`, a no-cwd `nx.run` must follow the new cwd (got: {out:?})"
    );
}
