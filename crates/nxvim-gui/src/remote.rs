...
//!
//! `:connect [user@]host[:port][/file]` and `:connect nxvim://…` switch a running
...
//! seams are pointed at a remote `nxvim --daemon` (see
//! [`crate::session::spawn_session`]). This module parses the target into a
//! [`ConnectTarget`] and, for the SSH form, builds the `ssh … nxvim --daemon` command
...
/// Where a `:connect` switches the window to: an SSH-spawned `nxvim --daemon` over
...
pub enum ConnectTarget {
    /// `[user@]host[:port][/file]` — `ssh … nxvim --daemon` over stdio, with the GUI's
    /// `SSH_ASKPASS` pointed at this binary so prompts pop a native dialog.
    Ssh(RemoteSpec),
    /// `nxvim://HOST:PORT/TOKEN?cert=HASH` — dial a `--daemon --listen` QUIC listener
    /// ([`nxvim_server::connect_quic`]).
    Quic(String),
}
...
/// Parse a `:connect [user@]host[:port][/file]` / `:connect nxvim://…` command line —
...
pub fn is_connect_uri(arg: &str) -> bool {
    arg.starts_with("nxvim://")
}

/// The remote command run over ssh — `$NXVIM_REMOTE_CMD`, else `nxvim --daemon` (the
...
fn remote_cmd() -> String {
    std::env::var("NXVIM_REMOTE_CMD").unwrap_or_else(|_| "nxvim --daemon".to_string())
}
...
/// is `nxvim --daemon [file]` ([`remote_cmd`]), each piece shell-quoted so the remote
...
/// [`nxvim_server::connect_daemon`] drives it for the five host seams.
///
...
/// "Permission denied", a "nxvim: command not found" from the remote shell, the remote
...
/// program it execs, marking a re-invoked `nxvim-gui` as the askpass helper.
