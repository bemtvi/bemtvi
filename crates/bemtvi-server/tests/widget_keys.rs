...
//! `btv.keymap.set('picker', …)` rebinds it, an empty-function map disables it, and
...
use bemtvi_test_harness::{attach, barrier, cursor, exec_lua, feed, lines, spawn, temp_dir};
...
/// A user `btv.keymap.set('picker', …)` rebinds a picker action to a new key: `<C-j>`
...
async fn user_rebind_moves_selection() {
    ...
    let (rpc, _incoming) = start(
        &dir,
        &src("btv.keymap.set('picker', '<C-j>', btv.picker.actions.next)"),
    )
    .await;
    open(&rpc).await;
...
async fn empty_map_disables_a_default_key() {
    ...
    let (rpc, _incoming) = start(
        &dir,
        &src("btv.keymap.set('picker', '<C-n>', function() end)"),
    )
    .await;
    open(&rpc).await;
...
async fn editing_map_does_not_leak_into_picker() {
    ...
    let (rpc, _incoming) = start(
        &dir,
        &src("_G.leaked = false\n\
              btv.keymap.set('n', '<C-n>', function() _G.leaked = true end)"),
    )
    .await;
    open(&rpc).await;
...
async fn rebind_confirm_key() {
    ...
    let (rpc, _incoming) = start(
        &dir,
        &src("btv.keymap.set('picker', '<C-y>', btv.picker.actions.confirm)"),
    )
    .await;
    open(&rpc).await;
...
// ===== Phase 2: btv.ui.select (the promptless list, 'select' bucket) ==========

/// Open a three-item `btv.ui.select` whose `:next` records the chosen item in
...
/// A user `btv.keymap.set('select', …)` rebinds a select action to a new key.
#[tokio::test]
async fn select_user_rebind() {
    ...
    open_select(
        &rpc,
        "btv.keymap.set('select', '<C-j>', btv.ui.select_actions.last)",
    )
    .await;
...
async fn select_editing_map_does_not_leak() {
    ...
    open_select(
        &rpc,
        "_G.leaked = false\n\
         btv.keymap.set('n', 'j', function() _G.leaked = true end)",
    )
    .await;
...
// listing is `filetype=btvdir`, navigation is plain normal-mode motion, and `<CR>`
// open / `-` parent are buffer-local default maps from the `FileType btvdir` autocmd.
...
async fn open_explorer(init_lua: &str) -> (Rpc, UnboundedReceiver<Incoming>) {
    ...
    // The explorer (a Lua plugin) fills the listing asynchronously (`btv.fs.readdir`
...
/// `<CR>` default map (from the `FileType btvdir` autocmd) opens it.
#[tokio::test]
...
/// map in a `FileType btvdir` autocmd (vim's ftplugin model), not a widget bucket.
...
async fn explorer_user_rebind() {
    let (rpc, _incoming) = open_explorer(
        "btv.autocmd.create('FileType', { pattern = 'btvdir', callback = function(a)\n\
         \x20 btv.keymap.set('n', '<C-j>', btv.explorer.actions.open, { buffer = a.buf })\n\
         end })",
    )
    .await;
    feed(&rpc, "jj<C-j>");
...
async fn explorer_is_an_ordinary_buffer() {
    let (rpc, _incoming) = open_explorer(
        "_G.fired = false\n\
         btv.keymap.set('n', 'X', function() _G.fired = true end)",
    )
    .await;
...
/// A user `btv.keymap.set('cmdline', …)` rebinds a cmdline action to a new key:
...
async fn cmdline_user_rebind() {
    ...
    let (rpc, _incoming) = start(
        &dir,
        "btv.keymap.set('cmdline', '<C-j>', btv.cmdline.actions.submit)",
    )
    .await;
...
async fn cmdline_empty_map_disables_submit() {
    ...
    let (rpc, _incoming) = start(&dir, "btv.keymap.set('cmdline', '<CR>', function() end)").await;
...
async fn cmdline_editing_map_does_not_leak() {
    ...
    let (rpc, _incoming) = start(
        &dir,
        "_G.leaked = false\n\
         btv.keymap.set('n', '<C-p>', function() _G.leaked = true end)",
    )
    .await;
