...
impl Editor {
    ...
    /// (0-based, clamped). The single home every built-in listing and `btv.panel.open` flow
    /// through.
    ...
    /// Mount a **scripted panel** (`btv.panel.open{ name?, lines, filetype?, height? }`): a
    /// named panel (default name `[Panel]`) whose `filetype` (default `btvpanel`, whose
    ...
    pub fn open_script_panel(
        ...
        self.open_named_panel(
            ...
            lines,
            0,
            filetype.as_deref().unwrap_or("btvpanel"),
            height,
        );
    ...
    /// Whether a panel is currently open. Backs the `bemtvi_panel_is_open` RPC and the
    /// focus lock.
    ...
    /// `btv.view` modal float ([`Editor::view_float_lock`], a stack — focus pins to the
