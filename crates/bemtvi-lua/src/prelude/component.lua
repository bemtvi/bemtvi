-- btv.component — a Vue-shaped component model for plugin UIs, surface-agnostic.
--
...
-- async** — write `btv.await(...)` straight inside them (each runs in an `btv.async`
-- coroutine), so a `setup` that loads from `btv.fs`, or a `render` that fetches to display,
-- reads top-to-bottom.
--
...
--   * `"view"`  — a focus-taking, navigable `btv.view` buffer (dock / split / grabbing float).
--               `render` returns `{ lines, decor }`; `ctx` gains `keymap_set` / `line` /
--               `set_cursor` / `bufnr`. This is what a file tree / picker / modal dialog
--               wants. `btv.view.component(def)` is the sugar for it.
--   * `"float"` — a NON-focus `btv.ui.float` content float (the which-key surface). It never
...
-- The `"view"` backend: a focus-taking, navigable `btv.view` buffer. `render` -> { lines, decor }.
--
...
local function view_backend(opts)
  btv.view._component_ns = btv.view._component_ns or btv.ns.create("btv.view.component")
  local v = btv.view.create({
    name = opts.name or "btv-component",
  ...
  -- exists — its winid arrives a tick after the surface shows via the `btv._view_win`
  ...
  return {
    show = show,
    -- The backing buffer number arrives a tick after create/mount (the `btv._view_buf` mirror).
    ready = function()
      return v:bufnr() ~= nil
    end,
    apply = function(out)
      ...
      v:set_lines(lines)
      if out.decor then
        v:set_decor(btv.view._component_ns, out.decor)
      end
    end,
    close = function()
      v:close()
    ...
    ctx = function()
      return {
        ...
        -- A thin wrapper over `btv.keymap.set(mode, lhs, rhs, opts)` — same signature — that
        ...
        keymap_set = function(mode, lhs, rhs, user_opts)
          ...
          btv.keymap.set(mode, lhs, rhs, merged)
        end,
      }
    end,
  }
...
-- `btv.ui.float` is a SINGLE editor-wide content-float slot (the core holds one
...
-- The `"float"` backend: a non-focus `btv.ui.float` content float. `render` ->
  ...
  adapter.apply = function(out)
    ...
    if handle and handle:is_open() then
      ...
      if content_float_owner ~= nil and content_float_owner ~= adapter then
        return btv.notify(
          "btv.component: a float component is already displaying; only one content float can be open at a time",
          4
        )
      end
      content_float_owner = adapter
      handle = btv.ui.float(lines, {
        persist = true,
        title = fopts.title,
...
-- session, exactly like a raw `btv.view.create{ persist=}`. On a restore, core reserves the
-- view's slot and dispatches it to that namespace's `btv.view.on_restore` handler. Since
...
btv._component_restorers = btv._component_restorers or {} -- ns -> { id -> adopt_fn }
btv._component_router_ns = btv._component_router_ns or {} -- ns -> true (router registered)

local function register_component_restorer(ns, raw_namespace, id, adopt)
  local reg = btv._component_restorers[ns]
  if not reg then
    reg = {}
    btv._component_restorers[ns] = reg
  end
  reg[id] = adopt
  if not btv._component_router_ns[ns] then
    btv._component_router_ns[ns] = true
    ...
    btv.view.on_restore(function(rid, place)
      local fn = btv._component_restorers[ns] and btv._component_restorers[ns][rid]
      if fn then
        fn(place)
      end
    end, raw_namespace)
    ...
    btv._claim_pending_restore(ns, id)
  end
end

-- ----- btv.component: the generic core ---------------------------------------------------

-- `btv.component(def)` -> { mount(opts) } — build a reactive, Vue-shaped UI component for
...
--     May be async (call `btv.await(...)` straight inside it).
...
--   * `"view"`  — a focus-taking, navigable `btv.view` buffer (dock / split / grabbing
--     float): the file-tree / list / modal-dialog case. `render` returns
--     `{ lines, decor }` (or a bare line list). `btv.view.component(def)` is the sugar.
--   * `"float"` — a NON-focus `btv.ui.float` content float (the which-key surface): never
...
-- `btv.notify` rather than crashing the editor.
--
-- Persistence (view surface) — `mount{ persist = "<id>", … }` opts a view component into
-- cross-session restore, the high-level form of `btv.view.create{ persist=}` +
-- `btv.view.on_restore`. The framework resolves the owning namespace ONCE (from the
-- mount call site, or `opts.namespace` for a no-attribution context — the same escape-hatch
-- contract as `btv.shada.plugin()`), records only `(namespace, id)` + the view's slot in the
...
--   * `ctx.store` — `btv.shada.plugin(ns)` for the resolved owner namespace: an isolated,
...
-- ```
function btv.component(def)
  assert(type(def) == "table", "btv.component: pass a { setup, render } table")
  assert(type(def.render) == "function", "btv.component: a render function is required")
  local make_backend = def.backend
  if not make_backend then
    make_backend = BACKENDS[def.surface or "view"]
    if not make_backend then
      error("btv.component: unknown surface '" .. tostring(def.surface) .. "'", 2)
    end
  end
  ...
  local function instantiate(opts, show_mode)
    ...
    do_render = function()
      ...
      btv.promise
        ...
        :catch(function(e)
          btv.notify("btv.component: render error: " .. tostring(e), 4)
        end)
    end
    ...
    -- call site and threads it here explicitly, so this `btv.shada.plugin(ns)` never has to
    ...
    if opts._resolved_ns then
      ...
      ctx.store = btv.shada.plugin(opts._resolved_ns)
    end
    ...
    btv
      .async(function()
        btv.await(
          btv.wait_for(backend.ready, { message = "the component surface never became ready" })
        )
        -- `btv.promise.resolve` makes this uniform whether `setup` returns a plain value or a
        -- promise; and because we're inside an `btv.async` coroutine, a `setup` that calls
        -- `btv.await(...)` directly suspends here too. Either async style works.
        if def.setup then
          state = btv.await(btv.promise.resolve(def.setup(ctx, ctx.props)))
        end
        do_render()
      end)()
      :catch(function(e)
        btv.notify("btv.component: setup error: " .. tostring(e), 4)
      end)
  ...
  local function mount_persistent(opts)
    ...
    local ns = btv._resolve_namespace(opts.namespace, "btv.view.component: persist")
    opts._resolved_ns = ns -- ctx.store / ctx.namespace key
    opts._create_ns = ns -- explicit owner ns for btv.view.create in off-stack contexts
    ...
    -- Fresh fallback: `btv.on_next_tick` runs AFTER boot's restore dispatch, so when no
...
-- `btv.view.component(def)` — the view-backed component (a focus-taking `btv.view` buffer): the
-- common case (file tree, list, modal dialog). Sugar over `btv.component` with the view
...
-- restart — see the Persistence note on `btv.component` above.
function btv.view.component(def)
  assert(type(def) == "table", "btv.view.component: pass a { setup, render } table")
  return btv.component({
    setup = def.setup,
    render = def.render,
