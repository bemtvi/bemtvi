// Repro harness for the decor-todo bug: source the nx.decor TODO-keyword config from
...
try {
  ...
  await page.evaluate(() => window.__nxvim.feed("ggdGiTODO and FIXME here<Esc>"));
  ...
  const last = await luaResult("local l = nx._decor.last; return l and (l.name .. ':' .. #l.marks) or 'nil'");
  check("decor: provider ran + published marks (Lua nx._decor.last)",
  ...
  const exts = await luaResult(`
    local ns = nx.ns.create("nx.decor:todo-keywords")
...
console.log(failures === 0
  ? "\nALL PASS — nx.decor renders on the web"
  : `\n${failures} FAILED`);
