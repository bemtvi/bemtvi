...
// host) an async spawn (`nx.run` / `nx.run_stream`) used to be dropped with only an `echo`,
...
// real `nx.run` through the wasm tick (no proc host set) and asserts it RESOLVES with code -1.
//
...
// "no host" path. `nx.run` queues a spawn; the tick (exec_lua drains loop_ops + apply effects)
// must complete it with code -1 and run the `:next` continuation, all within this call.
readStr(eh_exec_lua(h,
  `_G.code = "pending"\n` +
  `nx.run({ cmd = "rg", args = { "--files" } }):next(function(r) _G.code = r.code end)`));

const code = readStr(eh_exec_lua(h, "return _G.code"));
check("hostless nx.run RESOLVES (no silent hang)", !/pending/.test(code), `code=${JSON.stringify(code)}`);
check("hostless nx.run resolves with spawn-failure code -1", /(^|\D)-1\b/.test(code) || / -1/.test(code), `code=${JSON.stringify(code)}`);
