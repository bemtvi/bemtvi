...
/// bemtvi shows them, so this is the authoritative width that cursor / span /
...
/// [`display_line`]`(line)` — the substituted `^X` / `<xx>` tokens are 2/4 chars
...
/// UTF-16 position encoding. bemtvi columns are byte offsets; this is the
