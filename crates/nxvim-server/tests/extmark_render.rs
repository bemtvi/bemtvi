...
use nxvim_test_harness::{exec_lua, start_with_file, wait_redraw, window0_field};
use rmpv::Value;
...
async fn virt_mark(rpc: &Rpc, ns_name: &str, row: usize) -> u64 {
    exec_lua(
        rpc,
        &format!(
            "_G.{ns_name} = nx.ns.create('{ns_name}')
             return nx.buf.set_extmark(0, _G.{ns_name}, {row}, 0, {{
...
async fn deleting_one_virtual_line_mark_leaves_the_other_rendering() {
    ...
    exec_lua(&rpc, &format!("nx.buf.del_extmark(0, _G.va, {a})")).await;
    assert_eq!(
...
async fn replacing_a_virtual_line_mark_in_place_keeps_it_rendering() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "nx.buf.set_extmark(0, _G.va, 2, 0, {{
...
async fn replacing_a_virtual_line_mark_with_a_plain_one_stops_it_rendering() {
    ...
    exec_lua(
        &rpc,
        &format!("nx.buf.set_extmark(0, _G.va, 1, 0, {{ id = {id}, hl_group = 'Comment', end_row = 1, end_col = 3 }})"),
    )
    .await;
...
async fn clearing_one_namespace_leaves_another_namespaces_virtual_line() {
    ...
    virt_mark(&rpc, "vb", 3).await;
    assert_eq!(virtual_rows(&rpc, &mut inc).await, 2);
    exec_lua(&rpc, "nx.buf.clear_namespace(0, _G.va, 0, -1)").await;
    assert_eq!(virtual_rows(&rpc, &mut inc).await, 1);
}
...
async fn clearing_a_range_that_spares_the_mark_keeps_it_rendering() {
    ...
    exec_lua(&rpc, "nx.buf.clear_namespace(0, _G.va, 0, 3)").await;
    assert_eq!(virtual_rows(&rpc, &mut inc).await, 1);
}
...
async fn an_undo_keeps_virtual_lines_consistent() {
    ...
    virt_mark(&rpc, "va", 1).await;
    assert_eq!(virtual_rows(&rpc, &mut inc).await, 1);
    exec_lua(&rpc, "nx.cmd('normal! x')").await;
...
async fn nested_and_overlapping_highlight_marks_render_identically() {
    ...
    exec_lua(
        &rpc,
        "local ns = nx.ns.create('hl')
         -- A long range spanning all three lines, plus a short one nested inside it
         -- on the last line, plus one overlapping the two.
         nx.buf.set_extmark(0, ns, 0, 0, { hl_group = 'ErrorMsg', end_row = 2, end_col = 10 })
         nx.buf.set_extmark(0, ns, 2, 2, { hl_group = 'WarningMsg', end_row = 2, end_col = 5 })
         nx.buf.set_extmark(0, ns, 1, 4, { hl_group = 'Search', end_row = 2, end_col = 3 })",
    )
    .await;
    let _ = exec_lua(&rpc, "return 1").await;
...
async fn deleting_a_plain_mark_does_not_switch_off_virtual_lines() {
    ...
    let plain = exec_lua(
        &rpc,
        "_G.p = nx.ns.create('plain')
         return nx.buf.set_extmark(0, _G.p, 4, 0, { hl_group = 'Comment', end_row = 4, end_col = 3 })",
    )
    .await
    .as_u64()
    .expect("an id");
    exec_lua(&rpc, &format!("nx.buf.del_extmark(0, _G.p, {plain})")).await;
    assert_eq!(
        virtual_rows(&rpc, &mut inc).await,
        1,
...
async fn a_long_mark_is_found_past_a_shorter_one() {
    ...
    exec_lua(
        ...
        "local ns = nx.ns.create('hl')
         ...
         nx.buf.set_extmark(0, ns, 0, 0, { hl_group = 'ErrorMsg', end_row = 5, end_col = 5 })
         -- B: starts after A but ends back on line 1, well before line 5.
         nx.buf.set_extmark(0, ns, 1, 0, { hl_group = 'WarningMsg', end_row = 1, end_col = 3 })",
    )
    .await;
    let _ = exec_lua(&rpc, "return 1").await;
    let map = wait_redraw(&mut inc, |m| window0_field(m, "highlights").is_some()).await;
...
async fn typing_cost(marks: usize, opts: &str) -> std::time::Duration {
    ...
    if marks > 0 {
        exec_lua(
            &rpc,
            &format!(
                "_G.b = nx.ns.create('bench')
                 for i = 0, {} do nx.buf.set_extmark(0, _G.b, i, 0, {{ {} }}) end",
                marks - 1,
                opts
            ),
        )
        .await;
        assert_eq!(
            exec_lua(&rpc, "return #nx.buf.extmarks(0, _G.b, 0, -1, {})")
                .await
                .as_u64(),
            Some(marks as u64),
    ...
    nxvim_test_harness::feed(&rpc, "2500GI");
    let _ = exec_lua(&rpc, "return 1").await;
    let started = std::time::Instant::now();
    for _ in 0..300 {
        nxvim_test_harness::feed(&rpc, "z");
    }
    let _ = exec_lua(&rpc, "return 1").await;
    started.elapsed()
}
...
async fn put_sign(rpc: &Rpc, row: usize) {
    exec_lua(
        rpc,
        &format!(
            "_G.s = _G.s or nx.ns.create('signs')
             nx.buf.set_extmark(0, _G.s, {row}, 0, {{ sign_text = '>>' }})"
        ),
    )
    .await;
}
    ...
    exec_lua(
        ...
        &format!(
            ...
            "vim.api.nvim_set_hl(0, 'MyBg', {{ bg = 0x332211 }})
             local ns = nx.ns.create('lbg')
             nx.buf.set_extmark(0, ns, {last}, 0, {{ line_hl_group = 'MyBg' }})"
        ),
    )
    .await;
    let _ = exec_lua(&rpc, "return 1").await;
...
async fn virtual_text_on_the_last_visible_row_renders() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "local ns = nx.ns.create('vt')
             nx.buf.set_extmark(0, ns, {last}, 0, {{ virt_text = {{ {{ ' hint', 'Comment' }} }} }})"
        ),
    )
    .await;
    let _ = exec_lua(&rpc, "return 1").await;
