...
//! is the server-level twin of the `nxvim-ts` engine test (`text_objects_at`),
...
//! `NXVIM_DATA_DIR` pins the engine's search path to the temp dir. It does need a
...
/// on drop, so parallel binaries don't see a stale `NXVIM_DATA_DIR`.
...
/// `NXVIM_DATA_DIR`, and open `SAMPLE` as a `.rs` buffer — binding
...
async fn user_registry_overrides_a_builtin_verbatim() {
    ...
    exec_lua(&rpc, "nx.textobject.map('if', '@function.outer')").await;
    feed(&rpc, "/1<CR>");
    feed(&rpc, "dif");
    let text = lines(&rpc).await.join("\n");
...
async fn unmap_reverts_to_the_builtin() {
    ...
    exec_lua(&rpc, "nx.textobject.map('if', '@function.outer')").await;
    exec_lua(&rpc, "nx.textobject.unmap('if')").await;
