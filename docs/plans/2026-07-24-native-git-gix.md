...
nxvim shells out to the `git` **binary** in three places, all Lua-side (`nx.run` /
`nx.run_local` — there is no Rust git shell-out):

- **Plugin manager** (`crates/nxvim-lua/src/prelude/plugins.lua`) — `clone
  --filter=blob:none`, `checkout --detach <sha>`, `pull --ff-only`, `submodule
  update --init --recursive`, run via the **local** twin (`nx.run_local`).
- **nxvim-line** (`~/work/nxvim-plugins/nxvim-line/lua/nxvim-line/git.lua`) —
  `rev-parse --abbrev-ref HEAD` (branch), `diff -U0 -- <file>` (hunk +/~/-
  counts), `rev-parse --absolute-git-dir` (for the `.git` watch).
- **nxvim-diff** (`~/work/nxvim-plugins/nxvim-diff/lua/nxvim-diff/git.lua`) —
  `rev-parse --show-prefix` (repo-relative path), `show HEAD:<rel>` (blob@HEAD).

Replace **all** of it with a native, first-party async Lua API `nx.git.*` (plus a
`nx.git_local.*` twin), backed by the **`gix`** crate. **No `git` binary** anywhere
when we're done — the chosen scope is a full gix-only replacement including
clone/fetch/checkout/pull/submodule.

This dogfoods the same principle as `nx.fs` / `nx.run` / `nx.http.fetch`: a
...
## Design — model exactly on `nx.fs`

The `nx.fs` seam is the template (traced end-to-end; see
`docs/plans/2026-06-16-nx-fs-api.md` and `-off-tick-daemon-leg.md`). One typed job,
...
`cb_id` threaded `nx._next_cb_id()` → `LoopOp.id` → `LoopEvent.id` →
`run_callback(id)` → `nx._run_cb(id)` → the promise closure.
...
- **Data types + wire codec are wasm-safe** and live in `nxvim-lua` (no gix): a new
  `GitJob` / `GitValue` / `GitError` in `ops.rs`, and a `gitwire.rs` codec over
  `rmpv::Value`. These compile everywhere.
- **The gix-backed executor lives in a new native-only crate `nxvim-git`**
  (`run_git_job(repo_ctx, &GitJob) -> Result<GitValue, GitError>`), a peer of the
  other native engines (`nxvim-ts`, `nxvim-lsp`, `nxvim-regex`). `nxvim-server`
...
- **Topologies.** Native-bare: run the executor on the blocking pool. Native-daemon
  ...
  **daemon-side** against a real fs (this is how "remote git" works — git runs where
  ...
  (`nx.fs.watch` and plugin-sync git already fail loud serverless). "gix-only" means
  *no `git` binary*, not *gix-in-the-browser*.

The **`nx.git_local` twin** reuses the fs `local: true` mechanism: a `local`-flagged
...
Lua VM — exactly why `nx.run_local` exists today).
...
```
nx.git.discover(path)        -> { root=<toplevel abs>, git_dir=<abs>, prefix=<repo-rel dir> } | rejects if not a repo
...
repo from it. `nx.git.discover` replaces `rev-parse --show-toplevel /
...
```
nx.git_local.clone(url, dir, { filter="blob:none", depth= })  -> resolves dir
nx.git_local.checkout(dir, rev, { detach=true })              -> resolves nil
nx.git_local.pull(dir, { ff_only=true })                      -> resolves { updated=<bool>, sha= }
nx.git_local.submodule_update(dir, { init=true, recursive=true }) -> resolves nil
```
...
1. **Typed job/value/error** — `crates/nxvim-lua/src/ops.rs`: `GitJob`, `GitValue`,
   `GitError`, and `LoopOp::Git { id, job, local }` (`FsJob`/`FsValue`/`FsError`,
   `LoopOp::Fs`). Plain data, no gix, wasm-safe.
2. **Executor** — new crate `crates/nxvim-git`: gix wrapper + `run_git_job(&GitJob)
   -> Result<GitValue, GitError>` (`luafs.rs::run_fs_job`). A `GitError` shaper maps
   gix errors to `{ code, message }`. Native-only.
3. **Wire codec** — `crates/nxvim-lua/src/gitwire.rs`: `git_job_to_value` /
   `_from_value`, `git_result_to_value` / `_from_value` over `rmpv::Value`; leg
   constant `"git_op"` (`fswire.rs`, `LUAFS_OP`).
4. **Lua prelude** — `crates/nxvim-lua/src/prelude/git.lua`: `run_git(job)` mirroring
   `run_fs` (fs.lua:29), one wrapper per verb, `nx.git` + `nx.git_local` tables;
   register in `PRELUDE_MODULES` after `promise` (`runtime.rs:486`).
5. **Rust bridges** — `install.rs`: `nx._git_op(job, cb_id)` + `nx._local_git_op`
...
10. **Wasm seam** — `edithost.rs` `HostEffects::git_op` (:196) + `has_remote_git`
    gate; `nxvim-edithost/src/lib.rs` `WasmEffects` impl → `Sink.git_ops`, FFI
...
`nxvim-git` with `gix.workspace = true`; `nxvim-git` itself is an **optional** dep of
`nxvim-server` gated by `native`.

## Phasing (commit + pause for review between phases)

- **Phase 1 — read API + plugin ports.** Touchpoints 1–10 wired for the read verbs
  (`discover`/`head`/`show`/`diff_file`/`status`), native + daemon + `nx.git_local`
  twin + serverless-reject. Port nxvim-line and nxvim-diff off `nx.run "git"` onto
  `nx.git.*`. Sliced: (1a) crate+types+executor+codec; (1b) Lua prelude+bridges+
...
- **Phase 2 — plugin-manager mutation ops.** Add the mutation `GitJob`s
  ...
  onto `nx.git_local.*`. **Risk:** gix's recursive submodule update is not a one-call
...
test (commit a file, branch, stage a change) and drive `nx.git.*` via `exec_lua`,
asserting on real results — mutation-test each (break the code, watch it fail).
`nxvim-line`/`nxvim-diff` keep their own plugin test suites (`--test-plugin`);
...
- Not reimplementing full `git` CLI surface; just the verbs the tree actually needs,
  extensible via `nx.git.*` (the API is the extension point, not command flags).
