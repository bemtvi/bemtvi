//! Behavior tests for `btv.plugins` — bemtvi's native package / plugin manager.
//!
...
//! via `btv.run`.)
//!
//! `btv.plugins` is async end to end: a sync clones over `btv.run`, a load sources
//! `plugin/` scripts off the tick over `btv.fs`. So each test kicks the operation,
//! then POLLS an observable (a `_G` flag the plugin's own scripts set, or the real
//! filesystem) until it settles — exactly like the `btv.fs` / `btv.run` tests.
...
fn git(cwd: &Path, args: &[&str]) {
    let status = Command::new("git")
        ...
        .env("GIT_AUTHOR_EMAIL", "test@bemtvi")
        .env("GIT_COMMITTER_NAME", "bemtvi test")
        .env("GIT_COMMITTER_EMAIL", "test@bemtvi")
        .status()
        .expect("spawn git");
...
/// is the `btv.ui.float` content-float slot — the restart notice — distinct from the
/// manager's own `btv.view` window. Take-latest, so the reader task settles.
...
/// user's `init.lua` (`<config>/bemtvi-lock.json`), so a `sync()` under the default
...
async fn setup_root(rpc: &Rpc, tag: &str) -> PathBuf {
    ...
    exec_lua(
        rpc,
        &format!(
            "btv.plugins.setup_manager({{ root = \"{}\", config = \"{}\" }})",
            q(&root),
            q(&base.join("config"))
        ),
    )
...
async fn plugins_namespace_present() {
    ...
    assert_eq!(
        lua_bool(
            &rpc,
            "return type(btv.plugins) == 'table' and type(btv.plugins.sync) == 'function'"
        )
        .await,
        Some(true)
    );
    ...
    assert_eq!(
        lua_bool(&rpc, "return getmetatable(btv.plugins).__call ~= nil").await,
        Some(true)
    );
    assert_eq!(
        lua_bool(
            &rpc,
            "return btv.user_command.get().PluginSync ~= nil and btv.user_command.get().PluginList ~= nil"
        )
        .await,
        Some(true)
    );
...
async fn sync_clones_and_loads_eager_plugin() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins {{ {{ \"file://{repo}\", name = \"alpha\",\n\
               config = function() require(\"alpha\").setup() end }} }}\n\
             btv.plugins.sync():catch(function(e) _G.sync_err = tostring(e and e.message or e) end)",
            repo = q(&repo)
        ),
    )
    .await;
    ...
    assert_eq!(
        lua_bool(&rpc, "return btv.plugins._loaded.alpha == true").await,
        Some(true)
    );
    assert_eq!(
        lua_bool(
            &rpc,
            "return #btv.runtime_file('colors/alpha.lua', false) == 1"
        )
        .await,
        Some(true)
    );
...
async fn lazy_cmd_defers_load_until_invoked() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins {{ {{ \"file://{repo}\", name = \"beta\", cmd = \"Beta\",\n\
               config = function() require(\"beta\").setup() end }} }}\n\
             btv.plugins.install():next(function() _G.done = true end)
    ...
    assert_eq!(
        lua_bool(&rpc, "return btv.user_command.get().Beta ~= nil").await,
        Some(true)
    );
    assert_ne!(
        lua_bool(&rpc, "return btv.plugins._loaded.beta == true").await,
        Some(true)
    );
    assert_ne!(
...
async fn local_dir_plugin_loads_without_clone() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins {{ {{ name = \"gamma\", dir = \"{dir}\",\n\
...
async fn local_dir_expands_leading_tilde() {
    ...
    let ok = lua_bool(
        ...
        btv.plugins.add({ name = "tildeplug", dir = "~/dev/tildeplug", enabled = false })
        local spec = btv.plugins._specs["tildeplug"]
...
async fn clean_removes_undeclared_dirs() {
    ...
    exec_lua(
        &rpc,
        "btv.plugins.clean():next(function(r) _G.cleaned = #r end)",
    )
    .await;
    assert!(
...
// A plugin's `config` may be a plain function OR an async one (it btv.awaits, e.g.
...
async fn config_accepts_an_async_function() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins {{ {{ name = \"delta\", dir = \"{dir}\", config = function()\n\
               local txt = btv.await(btv.fs.read_text(\"{f}\"))\n\
               _G.delta_cfg = #txt > 0\n\
             end }} }}",
            dir = q(&repo),
            f = q(&initlua)
    ...
    assert!(
        ...
        "an async config (one that btv.awaits) must run to completion"
    );
    assert_eq!(
        lua_bool(&rpc, "return btv.plugins._loaded.delta == true").await,
        Some(true)
    );
}
...
async fn init_accepts_an_async_function() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins {{ {{ name = \"epsilon\", dir = \"{dir}\", init = function()\n\
               local txt = btv.await(btv.fs.read_text(\"{f}\"))\n\
    ...
    assert!(
        ...
        "an async init (one that btv.awaits) must run to completion"
    );
}

// ----- a failed clone fails loud (never hangs) -------------------------------

// The manager runs git in-process via `btv.git_local` (gix — no `git` binary, so no
    ...
    exec_lua(
        ...
        &format!(
            "btv.plugins.setup_manager({{ root = \"{root}\", config = \"{cfg}\" }})\n\
             btv.plugins {{ {{ \"file:///no/such/repo\", name = \"zeta\" }} }}\n\
             btv.plugins.install():next(\n\
    ...
    exec_lua(
        ...
        &format!(
            "btv.plugins.setup_manager({{ root = \"{root}\", config = \"{cfg}\" }})\n\
             ...
             btv.plugins.sync():next(function() _G.synced = true end)\n\
               :catch(function(e) _G.err = tostring(e and e.message or e) end)",
            root = q(&root),
...
async fn first_run_offers_recommended_and_persists_on_yes() {
    ...
    assert!(
        poll_true(&rpc, "return btv.plugins._prompting == true").await,
        "the recommended-set welcome should appear on a fresh setup"
    );
    assert!(
        poll_true(&rpc, "return vim.bo.filetype == 'btvpluginswelcome'").await,
    ...
    assert!(
        feed_until(&rpc, "<CR>", "return btv.plugins._loaded.zeta == true").await,
    ...
    exec_lua(&rpc, "btv.plugins._order = {}; btv.plugins.bootstrap()").await;
    tokio::time::sleep(Duration::from_millis(120)).await;
    assert_ne!(
        lua_bool(&rpc, "return btv.plugins._prompting == true").await,
        Some(true),
...
async fn welcome_confirm_opens_the_manager_and_installs_there() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins.recommend({{ {{ \"file://{repo}\", name = \"iota\" }} }})\n\
             btv.plugins.bootstrap()",
            repo = q(&repo)
        ),
    )
    .await;
    assert!(poll_true(&rpc, "return vim.bo.filetype == 'btvpluginswelcome'").await);

    // Confirming the welcome hands off to the manager dashboard (not a silent sync)…
    assert!(
        feed_until(&rpc, "<CR>", "return vim.bo.filetype == 'btvplugins'").await,
    ...
    assert!(
        poll_true(&rpc, "return btv.plugins._loaded.iota == true").await,
...
async fn first_run_decline_writes_nothing_but_marks_asked() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins.recommend({{ {{ \"file://{repo}\", name = \"eta\" }} }})\n\
             btv.plugins.bootstrap()",
            repo = q(&repo)
        ),
    )
    .await;
    assert!(poll_true(&rpc, "return btv.plugins._prompting == true").await);
    ...
    assert!(
        feed_until(
            &rpc,
            "<Esc>",
            "return vim.bo.filetype ~= 'btvpluginswelcome'"
        )
        .await,
    ...
    assert_ne!(
        lua_bool(&rpc, "return btv.plugins._loaded.eta == true").await,
        Some(true),
...
async fn vim_enter_triggers_the_first_run_prompt() {
    ...
    std::fs::write(
        ...
        format!(
            "btv.plugins.setup_manager({{ root = \"{root}\", config = \"{cfg}\" }})\n\
             btv.plugins.recommend({{ {{ \"file://{repo}\", name = \"theta\" }} }})\n",
            root = q(&root),
            cfg = q(&cfg),
            repo = q(&repo)
        ),
    ...
    assert!(poll_true(&rpc, "return vim.bo.filetype == 'btvpluginswelcome'").await);
    assert!(
        feed_until(&rpc, "<CR>", "return btv.plugins._loaded.theta == true").await,
...
async fn open_customize(rpc: &Rpc) -> bool {
    poll_true(rpc, "return vim.bo.filetype == 'btvpluginswelcome'").await
        && feed_until(rpc, "c", "return vim.bo.filetype == 'btvpluginscustomize'").await
}
...
async fn welcome_offer_summarizes_instead_of_listing_the_set() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins.recommend({{ {{ \"file://{r1}\", name = \"alpha\" }},\n\
               {{ \"file://{r2}\", name = \"beta\" }} }})\n\
             btv.plugins.bootstrap()",
            r1 = q(&repo1),
            r2 = q(&repo2)
...
async fn welcome_untick_excludes_a_plugin() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins.recommend({{ {{ \"file://{r1}\", name = \"uno\" }},\n\
               {{ \"file://{r2}\", name = \"dos\" }} }})\n\
             btv.plugins.bootstrap()",
            r1 = q(&repo1),
            r2 = q(&repo2)
    ...
    assert!(
        feed_until(&rpc, "<CR>", "return btv.plugins._loaded.uno == true").await,
        "confirming installs the ticked plugin"
    );
    tokio::time::sleep(Duration::from_millis(150)).await;
    assert_ne!(
        lua_bool(&rpc, "return btv.plugins._loaded.dos == true").await,
        Some(true),
...
async fn welcome_starts_cursor_on_first_item() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins.recommend({{ {{ \"file://{r1}\", name = \"ichi\" }},\n\
               {{ \"file://{r2}\", name = \"nidan\" }} }})\n\
             btv.plugins.bootstrap()",
            r1 = q(&repo1),
            r2 = q(&repo2)
...
async fn welcome_shows_the_full_source_not_just_name_and_desc() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins.recommend({{ {{ \"file://{repo}\",\n\
...
async fn install_records_task_state_and_fires_on_change() {
    ...
    exec_lua(
        &rpc,
        "_G.changes = 0\n\
         btv.plugins.on_change(function() _G.changes = _G.changes + 1 end)",
    )
    .await;
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins {{ {{ \"file://{repo}\", name = \"tau\" }} }}\n\
             btv.plugins.install():next(function() _G.done = true end)
               :catch(function(e) _G.err = tostring(e and e.message or e) end)",
            repo = q(&repo)
        ),
    ...
    assert!(
        poll_true(
            &rpc,
            "local t = btv.plugins._tasks.tau\n\
...
async fn plugins_command_opens_the_manager_dashboard() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins {{ {{ name = \"vista\", dir = \"{dir}\" }} }}",
            dir = q(&repo)
        ),
    )
    .await;
    assert_eq!(
        lua_bool(&rpc, "return btv.user_command.get().Plugins ~= nil").await,
        Some(true),
    ...
    assert!(
        poll_true(&rpc, "return vim.bo.filetype == 'btvplugins'").await,
...
async fn installing_via_the_manager_prompts_to_restart() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins {{ {{ \"file://{repo}\", name = \"rho\" }} }}",
            repo = q(&repo)
        ),
    )
    .await;
    ...
    assert!(
        poll_true(
            &rpc,
            "local t = btv.plugins._tasks.rho\n\
    ...
    assert!(
        poll_true(&rpc, "return btv.plugins.ui._restart_shown == true").await,
        "installing via the manager should prompt to restart"
    );
}

// The restart notice is a plain, NON-grabbing content float (`btv.ui.float`) — a
...
async fn one_esc_dismisses_the_restart_notice() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins {{ {{ \"file://{repo}\", name = \"phi\" }} }}",
            repo = q(&repo)
        ),
    )
    .await;
...
async fn plugins_welcome_command_reopens_after_marker() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins.recommend({{ {{ \"file://{repo}\", name = \"kappa\" }} }})",
            repo = q(&repo)
        ),
    )
    .await;
    ...
    assert!(
        feed_until(&rpc, "<CR>", "return btv.plugins._loaded.kappa == true").await,
...
// With ServerInit.offer_default_recommended set (the interactive binary), bemtvi's
...
async fn built_in_default_recommended_offers_when_config_registers_none() {
    ...
    assert!(
        poll_true(&rpc, "return #btv.plugins._recommended > 0").await,
    ...
    let n = exec_lua(&rpc, "return #btv.plugins._recommended").await;
    let n = n.as_i64().expect("recommended count");
    let want = format!("{n} plugins from github.com/bemtvi, github.com/rafamadriz.");
    let mut summarized = false;
    for _ in 0..200 {
    ...
    assert!(feed_until(&rpc, "c", "return vim.bo.filetype == 'btvpluginscustomize'").await);
    let mut listed = false;
    for _ in 0..200 {
        let ls = lines(&rpc).await;
        if ls.iter().filter(|l| l.contains('☑')).count() == n as usize
            && ls.iter().any(|l| l.contains("bemtvi/bemtvi-tree"))
        {
            listed = true;
            break;
        }
    ...
    feed_until(
        &rpc,
        "<Esc>",
        "return vim.bo.filetype ~= 'btvpluginscustomize'",
    )
    .await;
}
...
async fn default_recommended_is_off_unless_opted_in() {
    let (rpc, _i) = start().await;
    assert_eq!(
        lua_bool(&rpc, "return #btv.plugins._recommended == 0").await,
        Some(true),
...
async fn plugin_clean_forgets_shada_namespace() {
    ...
    exec_lua(&rpc, r#"btv.shada.plugin("orphan"):set("seen", true)"#).await;
    assert_eq!(
        lua_bool(
            &rpc,
            r#"return vim.tbl_contains(btv.shada.namespaces(), "orphan")"#
        )
        .await,
        Some(true),
    ...
    exec_lua(&rpc, "btv.plugins.clean()").await;
    assert!(
        poll_true(
            &rpc,
            r#"return not vim.tbl_contains(btv.shada.namespaces(), "orphan")"#
        )
        .await,
        "clean() forgot the removed plugin's shada namespace"
    );
    assert!(
        exec_lua(&rpc, r#"return btv.shada.plugin("orphan"):get("seen")"#)
            .await
            .is_nil(),
...
async fn a_failed_dependency_load_does_not_wedge_the_dependent() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins.add({{ {{ name = \"leafy\", dir = \"{}\", lazy = true,\n\
             \x20 dependencies = {{ \"ghost/ghost\" }} }} }})\n\
             _G.first = nil\n\
             btv.plugins.load(\"leafy\"):next(\n\
    ...
    assert_eq!(
        lua_bool(&rpc, "return btv.plugins._loading.leafy ~= true").await,
        Some(true),
...
/// `btv.plugins.system{...}` clones a plugin into the system dir (via the local-always
...
#[tokio::test]
async fn btv_plugins_system_clones_into_the_system_dir_and_loads() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins.setup_manager({{ root = \"{root}\", system = \"{sys}\", config = \"{cfg}\" }})\n\
             btv.plugins.system({{ \"file://{repo}\", name = \"conn\" }})",
    ...
    assert!(
        ...
        "btv.plugins.system must load the plugin into the current session",
    );
    ...
    assert_eq!(
        lua_bool(&rpc, "return btv.plugins._system['conn'] ~= nil").await,
        Some(true),
        "the plugin must be registered in the system tier",
    );
    assert_eq!(
        lua_bool(&rpc, "return btv.plugins._specs['conn'] == nil").await,
        Some(true),
        "a system plugin must not become a managed spec (sync/clean must ignore it)",
    );
}

/// `btv.plugins.promote(name)` moves an already-declared managed plugin into the system
...
#[tokio::test]
async fn btv_plugins_promote_moves_a_managed_plugin_into_the_tier() {
    ...
    exec_lua(
        ...
        &format!(
            "btv.plugins.setup_manager({{ root = \"{root}\", system = \"{sys}\", config = \"{cfg}\" }})\n\
             btv.plugins({{ {{ \"file://{repo}\", name = \"promo\" }} }})\n\
             btv.plugins.sync()",
            root = q(&base.join("install")),
            sys = q(&sysdir),
            cfg = q(&base.join("config")),
    ...
    assert!(
        poll_true(&rpc, "return btv.plugins._loaded['promo'] == true").await,
    ...
    exec_lua(
        &rpc,
        "_G.PROMOTED = nil\nbtv.plugins.promote('promo'):next(function() _G.PROMOTED = true end)",
    )
    .await;
    assert!(
    ...
    assert_eq!(
        lua_bool(&rpc, "return btv.plugins._system['promo'] ~= nil").await,
        Some(true),
        "the promoted plugin must be registered in the system tier",
    );

    // Unknown plugin rejects loudly.
    exec_lua(&rpc, "_G.REJ = nil\nbtv.plugins.promote('nope'):next(function() _G.REJ = 'ok' end, function() _G.REJ = 'rej' end)").await;
...
async fn eager_local_dir_plugin_sources_plugin_scripts_once() {
    ...
    assert!(
        poll_true(&rpc, "return btv.plugins._loaded.delta == true").await,
...
// after every eager plugin's config — including an ASYNC config that `btv.await`s — has
    ...
    std::fs::write(
        ...
        format!(
            "btv.on(\"PluginsLoaded\", {{}}, function()\n\
             ...
             btv.plugins {{ {{ name = \"sigma\", dir = \"{dir}\", config = function()\n\
               local txt = btv.await(btv.fs.read_text(\"{f}\"))\n\
               _G.sigma_cfg = #txt > 0\n\
             end }} }}\n",
            dir = q(&plugin_dir),
...
// The gap this closes. `btv.plugins` activates an eager spec asynchronously (it awaits the
...
// nothing at all for `bemtvi file.txt`, while `:e file.txt` in the same session works.
//
...
async fn startup_read_is_replayed_to_a_plugin_that_loaded_after_it() {
    ...
    std::fs::write(
        config.join("init.lua"),
        format!(
            "btv.plugins {{ {{ name = \"tardy\", dir = \"{dir}\", config = function()\n\
               local txt = btv.await(btv.fs.read_text(\"{f}\"))\n\
               _G.tardy_cfg = #txt > 0\n\
               ...
               btv.on(\"FileType\", {{}}, function(a) _G.ft = a.match end)\n\
             end }} }}\n",
            dir = q(&plugin_dir),
            f = q(&module)
        ),
    ...
    std::fs::write(
        ...
        format!(
            "_G.cfg_reads, _G.cfg_fts = 0, 0\n\
             btv.on(\"BufReadPost\", {{}}, function() _G.cfg_reads = _G.cfg_reads + 1 end)\n\
             btv.on(\"FileType\", {{}}, function() _G.cfg_fts = _G.cfg_fts + 1 end)\n\
             btv.plugins {{ {{ name = \"tardy\", dir = \"{dir}\", config = function()\n\
               btv.await(btv.fs.read_text(\"{f}\"))\n\
               _G.plugin_reads = 0\n\
               btv.on(\"BufReadPost\", {{}}, function() _G.plugin_reads = _G.plugin_reads + 1 end)\n\
             end }} }}\n",
            dir = q(&plugin_dir),
            f = q(&module)
        ),
...
async fn the_startup_replay_window_closes_at_plugins_loaded() {
    ...
    assert!(poll_true(&rpc, "return btv.plugins._plugins_loaded_fired == true").await);

    let _ = exec_lua(
        &rpc,
        "_G.seen = {}\n\
         btv.on('BufReadPost', {}, function(a) _G.seen[#_G.seen+1] = a.file end)\n\
         return true",
    )
    .await;
...
// (so an `btv.on("PluginLoaded", { pattern = name }, …)` handler targets just that one)
...
async fn plugin_loaded_event_fires_per_plugin_including_lazy() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.on(\"PluginLoaded\", {{ pattern = \"omega\" }}, function(ev)\n\
               _G.omega_evt = ev.data and ev.data.name or ev.match\n\
             end)\n\
             btv.plugins {{ {{ name = \"omega\", dir = \"{dir}\", cmd = \"OmegaGo\",\n\
...
// ----- btv.plugins.on_loaded: the race-free per-plugin hook -------------------

// The raw `PluginLoaded` event only ever hears a load that happens LATER, so hooking
// an already-loaded plugin with it silently never runs. `btv.plugins.on_loaded` closes
...
async fn on_loaded_runs_immediately_when_the_plugin_is_already_loaded() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins {{ {{ name = \"tau\", dir = \"{dir}\" }} }}",
            dir = q(&repo)
        ),
    )
    .await;
    assert!(
        poll_true(&rpc, "return btv.plugins._loaded['tau'] == true").await,
    ...
    let got = exec_lua(
        &rpc,
        "btv.plugins.on_loaded(\"tau\", function(n) _G.tau_hook = n end)\n\
...
async fn on_loaded_waits_for_a_not_yet_loaded_plugin_then_runs_once() {
    ...
    exec_lua(
        &rpc,
        "_G.rho_hits = 0\n\
         btv.plugins.on_loaded(\"rho\", function(n)\n\
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins {{ {{ name = \"rho\", dir = \"{dir}\", cmd = \"RhoGo\",\n\
...
async fn on_loaded_unsubscribe_drops_the_pending_hook() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.psi_hit = false\n\
             local unsub = btv.plugins.on_loaded(\"psi\", function() _G.psi_hit = true end)\n\
             btv.plugins {{ {{ name = \"psi\", dir = \"{dir}\", cmd = \"PsiGo\" }} }}\n\
             unsub()",
            dir = q(&repo)
        ),
    )
    ...
    assert!(
        poll_true(&rpc, "return btv.plugins._loaded['psi'] == true").await,
...
async fn on_loaded_bad_arguments_raise_at_registration() {
    ...
    let got = exec_lua(
        &rpc,
        "local ok, err = pcall(btv.plugins.on_loaded, \"tau\", \"not a function\")\n\
    ...
    let got = exec_lua(
        &rpc,
        "local ok, err = pcall(btv.plugins.on_loaded, function() end)\n\
         return tostring(ok) .. '|' .. tostring(err)",
    )
    .await;
    let got = got.as_str().unwrap_or_default();
...
// Phase 1 writes `<config>/bemtvi-lock.json` after every install/update/sync. See
...
async fn sync_writes_a_lockfile_recording_the_cloned_commit() {
    ...
    exec_lua(
        ...
        &format!(
            "btv.plugins {{ {{ \"file://{repo}\", name = \"alpha\" }} }}\n\
             btv.plugins.sync():catch(function(e) _G.err = tostring(e and e.message or e) end)",
            repo = q(&repo)
        ),
    )
    .await;

    let lock = cfg.join("bemtvi-lock.json");
    assert!(
        poll_true(&rpc, "return btv.plugins.locked().alpha ~= nil").await,
    ...
    assert_eq!(
        exec_lua(&rpc, "return btv.plugins.locked().alpha.commit")
            .await
            .as_str(),
        Some(want.as_str())
    );
...
async fn the_lockfile_is_pretty_printed_with_sorted_keys() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins {{ {{ \"file://{a}\", name = \"zeta\" }}, {{ \"file://{b}\", name = \"alpha\" }} }}\n\
             btv.plugins.sync():catch(function(e) _G.err = tostring(e and e.message or e) end)",
            a = q(&a),
            b = q(&b)
        ),
    ...
    assert!(
        poll_true(
            &rpc,
            "return btv.plugins.locked().zeta ~= nil and btv.plugins.locked().alpha ~= nil"
        )
        .await,
        "err={:?}",
        exec_lua(&rpc, "return _G.err").await
    ...
    let text = std::fs::read_to_string(cfg.join("bemtvi-lock.json")).unwrap();
    assert!(
...
async fn a_dev_dir_plugin_is_never_locked() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins {{ {{ \"file://{repo}\", name = \"alpha\" }}, {{ dir = \"{dev}\", name = \"devplug\" }} }}\n\
             btv.plugins.sync():catch(function(e) _G.err = tostring(e and e.message or e) end)",
            repo = q(&repo),
            dev = q(&devdir)
        ),
    ...
    assert!(cfg.join("bemtvi-lock.json").exists());
    assert_eq!(
        lua_bool(
            &rpc,
            "return btv.plugins.locked().alpha ~= nil and btv.plugins.locked().devplug == nil"
        )
        .await,
        Some(true),
...
async fn a_malformed_lockfile_fails_loud() {
    ...
    std::fs::create_dir_all(&cfg).unwrap();
    std::fs::write(cfg.join("bemtvi-lock.json"), "{ this is not json").unwrap();

    exec_lua(
        &rpc,
        "btv.plugins._read_lock()\n  :next(function() _G.ok = true end)\n  \
...
async fn plugin_lock_command_is_wired() {
    let (rpc, _i) = start().await;
    assert_eq!(
        lua_bool(&rpc, "return btv.user_command.get().PluginLock ~= nil").await,
        Some(true)
    );
}
...
async fn install_reproduces_the_locked_commit_not_the_remote_tip() {
    ...
    let declare = format!(
        "btv.plugins {{ {{ \"file://{repo}\", name = \"alpha\" }} }}\n\
         btv.plugins.sync():next(function() _G.done = true end)\n\
           :catch(function(e) _G.err = tostring(e and e.message or e) end)",
        repo = q(&repo)
    );
    exec_lua(
...
async fn an_explicit_spec_commit_outranks_the_lockfile() {
    ...
    exec_lua(
        ...
        &format!(
            "btv.plugins.setup_manager({{ root = \"{root}\", config = \"{cfg}\" }})\n\
             btv.plugins {{ {{ \"file://{repo}\", name = \"alpha\", commit = \"{second}\" }} }}\n\
             btv.plugins.install():next(function() _G.done = true end)
               :catch(function(e) _G.err = tostring(e and e.message or e) end)",
            root = q(&root),
            cfg = q(&cfg),
...
async fn sync_reproduces_the_lock_while_update_advances_past_it() {
    ...
    exec_lua(
        &rpc,
        "_G.status = nil\nbtv.plugins._update('alpha'):next(function(s) _G.status = s end)",
    )
    .await;
    ...
    assert_eq!(
        exec_lua(&rpc, "return btv.plugins.locked().alpha.branch")
            .await
            .as_str(),
        Some("main")
    );
    ...
    exec_lua(
        ...
        "_G.upd, _G.uerr = nil, nil\n\
         btv.plugins.update():next(function(n) _G.upd = n end)\n\
    ...
    assert!(
        poll_true(
            &rpc,
            &format!("return btv.plugins.locked().alpha.commit == \"{tip}\"")
        )
        .await,
...
async fn update_of_a_detached_plugin_with_no_recorded_branch_fails_loud() {
    ...
    let root = base.join("install");
    std::fs::create_dir_all(&cfg).unwrap();
    // A lock entry with NO branch (hand-written, or from an older bemtvi): there is then no
    ...
    exec_lua(
        ...
        &format!(
            "btv.plugins.setup_manager({{ root = \"{root}\", config = \"{cfg}\" }})\n\
             btv.plugins {{ {{ \"file://{repo}\", name = \"alpha\" }} }}\n\
             btv.plugins.install():next(function()\n\
               btv.plugins._update('alpha', {{ advance = true }}):next(function(s) _G.status = s end)\n\
                 :catch(function(e) _G.uerr = tostring(e and e.message or e) end)\n\
...
async fn restore_moves_a_checkout_back_to_the_locked_commit() {
    ...
    exec_lua(
        ...
        "_G.res, _G.rerr = nil, nil\n\
         btv.plugins.restore():next(function(r) _G.res = r end)\n\
    ...
    exec_lua(
        &rpc,
        "_G.res2 = nil\nbtv.plugins.restore():next(function(r) _G.res2 = r end)",
    )
    .await;
...
async fn status_reports_drift_against_the_lockfile() {
    ...
    exec_lua(
        &rpc,
        "_G.rows = nil\nbtv.plugins.status():next(function(r) _G.rows = r end)",
    )
    .await;
    ...
    exec_lua(
        &rpc,
        "_G.rows2 = nil\nbtv.plugins.status():next(function(r) _G.rows2 = r end)",
    )
    .await;
...
async fn restore_leaves_a_dev_dir_plugin_alone() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins {{ {{ dir = \"{dev}\", name = \"devplug\" }} }}\n\
             _G.res = nil\n\
             btv.plugins.restore():next(function(r) _G.res = r end)\n\
               :catch(function(e) _G.rerr = tostring(e and e.message or e) end)",
            dev = q(&devdir)
...
async fn plugin_restore_command_is_wired() {
    let (rpc, _i) = start().await;
    assert_eq!(
        lua_bool(&rpc, "return btv.user_command.get().PluginRestore ~= nil").await,
        Some(true)
    );
}
...
async fn the_dashboard_offers_restore_and_reaches_it() {
    ...
    exec_lua(
        &rpc,
        "_G.__drift = nil\n\
         btv.plugins.status():next(function(r)\n\
...
async fn a_lockfile_entry_for_an_undeclared_plugin_is_pruned() {
    ...
    assert_eq!(
        lua_bool(
            &rpc,
            "return btv.plugins.locked().alpha ~= nil and btv.plugins.locked().ghost == nil"
        )
        .await,
        Some(true),
...
async fn a_tag_bump_invalidates_the_lock_entry() {
    ...
    exec_lua(
        ...
        &format!(
            "btv.plugins.setup_manager({{ root = \"{root}\", config = \"{cfg}\" }})\n\
             btv.plugins {{ {{ \"file://{repo}\", name = \"alpha\", tag = \"v2\" }} }}\n\
             btv.plugins.sync():next(function() _G.done = true end)\n\
               :catch(function(e) _G.err = tostring(e and e.message or e) end)",
            root = q(&root),
            cfg = q(&cfg),
    ...
    assert!(
        poll_true(
            &rpc,
            &format!(
                "local e = btv.plugins.locked().alpha\n\
                 return e ~= nil and e.commit == \"{v2}\" and e.tag == \"v2\""
            )
        )
        .await,
        "the lock should now record v2: {:?}",
        exec_lua(&rpc, "return btv.json.encode(btv.plugins.locked())").await
    );
}
    ...
    let declare = |tag: &str| {
        format!(
            "_G.done, _G.err = nil, nil\n\
             btv.plugins {{ {{ \"file://{repo}\", name = \"alpha\", tag = \"{tag}\" }} }}\n\
             btv.plugins.sync():next(function() _G.done = true end)\n\
               :catch(function(e) _G.err = tostring(e and e.message or e) end)",
            repo = q(&repo)
        )
...
async fn a_branch_change_invalidates_the_lock_entry() {
    ...
    exec_lua(
        ...
        &format!(
            "btv.plugins.setup_manager({{ root = \"{root}\", config = \"{cfg}\" }})\n\
             btv.plugins {{ {{ \"file://{repo}\", name = \"alpha\", branch = \"side\" }} }}\n\
             btv.plugins.sync():next(function() _G.done = true end)\n\
               :catch(function(e) _G.err = tostring(e and e.message or e) end)",
            root = q(&root),
            cfg = q(&cfg),
...
async fn a_dev_dir_override_keeps_the_recorded_entry() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins {{ {{ \"file://x/alpha\", name = \"alpha\", dir = \"{dir}\" }} }}\n\
             btv.plugins.lock():next(function() _G.done = true end)\n\
               :catch(function(e) _G.err = tostring(e and e.message or e) end)",
            dir = q(&repo)
        ),
    )
...
async fn a_session_with_no_declared_plugins_leaves_the_lockfile_alone() {
    ...
    let before =
        "{\n  \"ghost\": { \"commit\": \"0123456789abcdef0123456789abcdef01234567\" }\n}\n";
    std::fs::write(cfg.join("bemtvi-lock.json"), before).unwrap();
    ...
    assert_eq!(
        std::fs::read_to_string(cfg.join("bemtvi-lock.json")).unwrap(),
        before,
...
/// pulling a teammate's updated `bemtvi-lock.json` and syncing must land you on the commits
...
/// empty map from an empty list, and `btv.json` picks the list form — writing `[]` into a
...
async fn a_lock_entry_with_no_commit_fails_loud() {
    ...
    exec_lua(
        &rpc,
        "_G.err = nil\n\
         btv.plugins._read_lock():next(function() _G.ok = true end)\n\
           :catch(function(e) _G.err = tostring(e and e.message or e) end)",
    )
    .await;
    ...
    let msg = msg.as_str().unwrap_or("");
    assert!(
        msg.contains("alpha") && msg.contains("bemtvi-lock.json"),
...
// goes through `btv._feedkeys`, which parses raw vim key-notation — it knows nothing
// about `<leader>`, which `btv.keymap.set` expands at set time. So an unexpanded
...
async fn keys_lazy_plugin_runs_its_mapping_on_the_first_press() {
    ...
    exec_lua(
        ...
        &format!(
            "vim.g.mapleader = \" \"\n\
             _G.kappa_fired = 0\n\
             btv.plugins {{ {{ name = \"kappa\", dir = \"{dir}\", keys = \"<leader>e\",\n\
               config = function()\n\
    ...
    exec_lua(
        &rpc,
        "return btv.buf.set_lines(0, 0, -1, false, { 'hello world' })",
    )
    .await;
    ...
    assert_eq!(
        exec_lua(&rpc, "return btv.mode().mode").await,
        Value::from("n"),
...
async fn keys_lazy_trigger_does_not_re_enter_itself_after_the_load() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins {{ {{ name = \"iota\", dir = \"{dir}\", keys = \"x\",\n\
    ...
    exec_lua(
        &rpc,
        "return btv.buf.set_lines(0, 0, -1, false, { 'abcdef' })",
    )
    .await;
    ...
    assert!(
        poll_true(
            &rpc,
            "return #(btv.buf.lines(0, 0, -1, false)[1] or '') == 5"
        )
        .await,
...
// `btv.plugins.load` is retryable by design, so the same key has to wake the plugin
...
async fn a_failed_keys_lazy_load_re_arms_its_trigger() {
    ...
    exec_lua(
        &rpc,
        "vim.g.mapleader = \" \"\n\
         btv.plugins { { \"bemtvi/nope\", keys = \"<leader>q\" } }",
    )
    .await;
...
async fn ft_lazy_plugin_with_an_async_config_still_gets_the_filetype_event() {
    ...
    std::fs::write(&py, "x = 1\n").unwrap();
    exec_lua(&rpc, &format!("btv.cmd('edit {}')", q(&py))).await;
...
//
// Every management verb takes an optional `plugins` scope — `btv.plugins.update{ plugins
...
async fn install_scoped_to_one_plugin_leaves_the_others_alone() {
    ...
    run_verb(&rpc, "btv.plugins.install({ plugins = 'alpha' })").await;
    assert!(
...
async fn update_scoped_to_one_plugin_moves_only_it() {
    ...
    run_verb(&rpc, "btv.plugins.install()").await;
    assert!(
    ...
    run_verb(&rpc, "btv.plugins.update({ plugins = 'beta' })").await;
    assert!(
        poll_true(&rpc, "return _G.done == 1").await,
    ...
    let lock = std::fs::read_to_string(cfg.join("bemtvi-lock.json")).unwrap();
    assert!(
...
async fn a_scoped_verb_rejects_an_unknown_plugin_name() {
    ...
    run_verb(&rpc, "btv.plugins.update({ plugins = 'ghost' })").await;
    assert!(
        poll_true(&rpc, "return _G.err ~= nil").await,
...
async fn the_plugin_commands_take_an_optional_plugin_name() {
    ...
    assert_eq!(
        lua_bool(
            &rpc,
            "local cands = btv.user_command.get().PluginUpdate.complete({})\n\
             local seen = {}\n\
...
async fn scoped_clean_removes_just_that_clone() {
    ...
    // dirs) can't express.
    run_verb(&rpc, "btv.plugins.clean({ plugins = 'alpha' })").await;
    assert!(
        poll_true(
            &rpc,
...
async fn scoped_clean_refuses_to_delete_a_dev_dir_checkout() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins {{ {{ name = \"delta\", dir = \"{dir}\" }} }}",
            dir = q(&repo)
        ),
    )
    .await;
    run_verb(&rpc, "btv.plugins.clean({ plugins = 'delta' })").await;
    assert!(
        poll_true(&rpc, "return _G.err ~= nil").await,
...
async fn a_scoped_verb_says_why_it_skipped_the_plugin_you_named() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins {{ {{ \"file://{a}\", name = \"alpha\", enabled = false }} }}",
            a = q(&repo)
        ),
    )
    .await;
...
async fn the_dashboard_binds_both_verb_scopes_including_the_moved_refresh() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins {{ {{ name = \"alpha\", dir = \"{dir}\" }} }}",
            dir = q(&repo)
        ),
    )
    .await;
    ...
    assert_eq!(
        lua_bool(
            ...
            "local want = { I=1, U=1, S=1, R=1, X=1, i=1, u=1, s=1, r=1, x=1, ['<C-r>']=1 }\n\
             for _, m in ipairs(btv.keymap.buf_get(0, 'n')) do want[m.lhs] = nil end\n\
...
/// Capture every `btv.notify` into `_G.notes`, so a test asserts on the exact text a
/// plugin-manager path reported rather than racing the message line (a later repaint
/// overwrites it). `btv.notify` is looked up dynamically at each call site in the
/// prelude, so replacing it here is enough.
async fn capture_notifies(rpc: &Rpc) {
    exec_lua(
        rpc,
        "_G.notes = {}\n\
         btv.notify = function(m) _G.notes[#_G.notes + 1] = tostring(m) end",
    )
    .await;
}
...
async fn an_eager_plugin_that_is_not_installed_reports_it() {
    ...
    exec_lua(
        &rpc,
        "btv.plugins { { \"owner/gamma\", name = \"gamma\" } }",
    )
    .await;
...
async fn missing_eager_plugins_are_reported_together_once() {
    ...
    exec_lua(
        &rpc,
        "btv.plugins { { \"owner/one\", name = \"one\" }, { \"owner/two\", name = \"two\" } }",
    )
    .await;
...
async fn an_unknown_spec_key_is_reported() {
    ...
    exec_lua(
        &rpc,
        "btv.plugins { { \"owner/delta\", name = \"delta\", cmds = \"DeltaGo\" } }",
    )
    .await;
...
async fn an_unknown_spec_key_does_not_drop_the_rest_of_the_list() {
    ...
    exec_lua(
        &rpc,
        "btv.plugins { { \"owner/first\", name = \"first\" },\n\
    ...
    assert_eq!(
        lua_bool(
            &rpc,
            "return btv.plugins._specs[\"last\"] ~= nil and btv.plugins._specs[\"bad\"] ~= nil"
        )
        .await,
        Some(true),
...
async fn a_fully_specified_spec_reports_no_unknown_key() {
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.plugins {{ {{ \"file://{a}\", name = \"omega\", desc = \"d\", branch = \"main\",\n\
...
async fn a_trigger_command_name_that_cannot_be_typed_is_reported() {
    ...
    exec_lua(
        &rpc,
        "btv.plugins { { \"owner/mdp\", name = \"mdp\", cmd = \"MarkdownPreview \" },\n\
    ...
    assert_eq!(
        lua_bool(&rpc, "return btv.plugins._specs[\"after\"] ~= nil").await,
        Some(true),
