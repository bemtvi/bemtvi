# `nx.picker` preview pane — the unified widget's Phase 3 — phased plan
...
> (`nx.complete` docs sidebar — their only consumer). Tests: `nxvim-server`'s
> `picker.rs` (preview content/geometry/location/placeholder/confirm) and `nxvim`'s
...
> (`docs/specs/2026-06-14-nx-ui-float-widget.md`): the **preview pane**, rendered
> natively by kind, that completes `nx.picker`. Lives at
> `docs/plans/2026-06-14-nx-picker-preview-pane.md` alongside the other plans.

## Context

The unified float-list widget is the one Rust component completion, the picker, and
`nx.ui.select` render through — *a float containing a selectable, match-highlighted
list*, with an optional preview and an optional prompt. **Phase 1** (the list +
placement, backing `nx.ui.select`) and **Phase 2** (the prompt input-grab, the Rust
...
1. **Picker-only.** This phase ships the `file` + `location` kinds in the
   **editor-placement** (centered box) *pane-within* layout — what "completes the
   ...
   layout back the **completion docs sidebar**, whose only consumer is `nx.complete`
...
`nxvim-core` stays **pure and synchronous**. Core owns only: the per-item *preview
...
+ highlight spans — lives in `nxvim-server`'s redraw layer, where the host FS and the
...
- `Menu` (`menu.rs:151`) grows `preview: bool` (the picker declares a preview pane);
  `open_picker` (`menu.rs:300`) gains a `preview: bool` parameter. `nx.ui.select`'s
...
### Bridge + Lua (`nxvim-lua`)

- `nx.picker.source` spec accepts `preview = "file" | "location"`; `nx.picker.open`
  passes it to `nx._picker_open` (a new bool/string arg) → server `open_picker`.
- `nx._picker_push` (`picker.lua`): when the source declares a preview kind, extract
...
- `project_menu` (`redraw.rs:701`): when `m.preview.is_some()`, split the centred box
  ...
  in `width - preview.width - 1` columns. `nx.ui.select` and preview-less pickers omit
  the `preview` key entirely (clients render exactly as today).

### Wire + clients

- `nxvim-view` `MenuData` (`view.rs:324`) grows `preview: Option<MenuPreview>`
...
### Tests (`crates/nxvim-server/tests/picker.rs`, black-box)
...
- Reuse, don't fork, the highlighter. `highlights_for` (`treesitter.rs:103`) is keyed
  ...
  (`nxvim-core/src/syntax.rs`): `highlight_text(language, text, first_line, last_line)
...
3. `cargo run -p nxvim -- .` → `nx.picker.open("files")` shows the highlighted file
...
5. Regression: `nx.ui.select` and a preview-less picker render byte-identically to
