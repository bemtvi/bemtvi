// Playwright verifier for the browser edit-host's `btv.git` leg
// (docs/plans/2026-07-24-native-git-gix.md) against a REAL `bemtvi --daemon --listen` over
// WebTransport. A browser `btv.git.*` op has no in-browser git engine — the op crosses the wire
// to the daemon as one `git_op` request, runs there through `bemtvi_git::run_git_job` against the
// real repo, and its typed result returns and resolves the op's promise in the tick. The browser
// twin of the native off-tick `btv.git` actor leg.
//
// Faithfulness (not a no-op — the daemon repo exists ONLY on the daemon's disk):
//   (1) btv.git.head returns the daemon repo's branch (main);
//   (2) btv.git.show returns a file's HEAD blob from the daemon's object store, NOT the edited
//       working-tree content, proving it read git objects over the wire;
//   (3) btv.git.discover on a non-repo path REJECTS with err.code == "ENOREPO";
//   (4) btv.git.clone (a Phase-2 MUTATION verb) runs daemon-side over the wire and lands a real
...
//   (5) btv.git.status carries its `ignored` OPT-IN over the wire — off, no `!!` entries; on, the
...
// (target/debug/bemtvi), a Chromium for Playwright, and `git` on PATH. Run: node verify-git-op.mjs
...
const root = mkdtempSync(join(tmpdir(), "bemtvi-gitop-"));
...
const noRepo = mkdtempSync(join(tmpdir(), "bemtvi-norepo-"));
...
const cloneDir = join(mkdtempSync(join(tmpdir(), "bemtvi-gitclone-")), "cloned");

// ── Spawn the real daemon (cwd = the repo) ───────────────────────────────────────────────────
const daemon = spawn(BEMTVI, ["--daemon", "--listen", "127.0.0.1:0"], { cwd: root, stdio: ["ignore", "pipe", "pipe"] });
let uri = null;
let daemonOut = "";
...
try {
  ...
  await luaResult(page, `_G.__b, _G.__berr = nil, nil
     btv.git.head("${trackedFile}"):next(
       function(h) _G.__b = h.branch end,
       function(e) _G.__berr = e.code end)
     return 1`);
  const branch = await until(page,
    () => window.__bemtvi.execLua("return tostring(_G.__b or _G.__berr)").then((r) => r.result),
    (v) => /main|E[A-Z]/.test(String(v)));
  check("btv.git.head returns the daemon repo's branch (over WebTransport)",
  ...
  await luaResult(page, `_G.__s, _G.__serr = nil, nil
     btv.git.show("${trackedFile}", "HEAD"):next(
       function(bytes) _G.__s = bytes end,
       function(e) _G.__serr = e.code end)
     return 1`);
  const blob = await until(page,
    () => window.__bemtvi.execLua("return tostring(_G.__s or _G.__serr)").then((r) => r.result),
    (v) => /a\\nb\\nc|EDITED|E[A-Z]/.test(String(v)));
  check("btv.git.show returns the HEAD blob from the daemon's object store (not the worktree)",
  ...
  await luaResult(page, `_G.__code = nil
     btv.git.discover("${noRepo}"):next(
       function(_) _G.__code = "RESOLVED?!" end,
       function(e) _G.__code = e.code end)
     return 1`);
  const code = await until(page,
    () => window.__bemtvi.execLua("return tostring(_G.__code)").then((r) => r.result),
    (v) => !/^nil$/.test(String(v)));
  check("btv.git.discover outside a repo rejects with err.code == ENOREPO over the wire",
  ...
  await luaResult(page, `_G.__c, _G.__cerr = nil, nil
     btv.git.clone("${root}", "${cloneDir}"):next(
       function(dir) _G.__c = dir end,
       function(e) _G.__cerr = e.code end)
     return 1`);
  const cloned = await until(page,
    () => window.__bemtvi.execLua("return tostring(_G.__c or _G.__cerr)").then((r) => r.result),
  ...
  check("btv.git.clone (mutation verb) resolves over the wire + lands the worktree on the daemon disk",
  ...
  const readStatus = () => until(page,
    () => window.__bemtvi.execLua("return tostring(_G.__st or _G.__sterr)").then((r) => r.result),
    (v) => !/^nil$/.test(String(v)), 15000);

  await luaResult(page, statusCall(""));
  const plain = String(await readStatus());
  check("btv.git.status default does NOT report ignored paths over the wire",
  ...
  check("btv.git.status { ignored = true } crosses the wire (build/ collapsed to one !! entry)",
...
console.log(failures === 0
  ? "\nALL PASS — browser btv.git runs on a real bemtvi --daemon over WebTransport (head, show-from-object-store, ENOREPO reject, clone, status+ignored)"
  : `\n${failures} FAILED`);
