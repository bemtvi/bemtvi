// verify-http-mount.mjs — the serverless-browser `nx.http.mount` leg, end to end.
//
// A tab cannot bind a TCP port, so the web build serves a plugin's mounted subroutes through
// a Service Worker (web/nx-sw.js) that intercepts `/plugin/*` on the page's own origin and
...
try {
  ...
  const swRes = await fetch(`${ORIGIN}/nx-sw.js`);
  check("nx-sw.js is served from the root", swRes.status === 200, `status ${swRes.status}`);
  check(
    "nx-sw.js carries Service-Worker-Allowed: /",
  ...
  check("nx.http.mount resolves on the browser build", !reported.includes("ERROR"), reported);
  check(
...
console.log(failures === 0 ? "\nALL PASS — nx.http.mount serves through the Service Worker" : `\n${failures} FAILED`);
