...
//! job). It backs `btv.glob` in Lua and is the one definition of a glob for every
//! in-tree caller.
//!
...
//! separator handling would then be decided at build time. bemtvi needs it decided
...
//! reaches all the way out to `btv.glob` (Lua strings are byte strings).
//!
...
impl Glob {
    ...
    /// `btv.glob.to_regex`): a glob that mismatches is far easier to debug against the
...
/// `btv.glob.match`. Identical in cost to `compile(..)?.is_match(..)` after the first
