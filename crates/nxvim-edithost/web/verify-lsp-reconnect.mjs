...
// handle dropped from `nx.lsp.clients()`, and its unfinished `$/progress` task dropped with it
...
// So, after a drop:
//
//   * the dead pre-drop client is gone from `nx.lsp.clients()` (a leaked handle makes a buffer
//     report two servers where one process is running — `:LspInfo` disagreeing with
//     `nx.lsp.clients()`, exactly the failure `retire_lsp_server`'s doc comment describes);
...
async function lua(page, expr) {
  const r = await page.evaluate(
    (e) => window.__nxvim.execLua(`return tostring(${e})`).then((r) => String(r.result)),
    expr,
  );
  const m = r.match(/"((?:[^"\\]|\\.)*)"/);
  return m ? m[1] : r;
...
// The current `nx.daemon.status()` phase. Note "disconnected" *contains* "connected" as a
// substring, so match the whole word.
async function status(page) {
  const r = await page.evaluate(() =>
    window.__nxvim.execLua("return nx.daemon.status()").then((r) => String(r.result)),
  );
...
// The ids `nx.lsp.clients()` lists, sorted — a leaked handle shows as an extra id rather than a
...
const PROGRESS = `(function()
  local out = {}
  for _, p in ipairs(nx.lsp.progress()) do out[#out + 1] = p.client_id .. ":" .. p.title end
  table.sort(out)
  return table.concat(out, ",")
end)()`;

// ── The daemon's project file, and the mock server's script ───────────────────────────────
const root = mkdtempSync(join(tmpdir(), "nxvim-lsp-reconnect-"));
...
try {
  ...
  const opened = await until(page, () => window.__nxvim.lines(), (v) => v === "fn main() {}");
  ...
  await page.evaluate(
    ([bin, script]) =>
      window.__nxvim.execLua(
        `nx.lsp.config("mock", { cmd = { ${JSON.stringify(bin)}, "--__lsp-mock", ${JSON.stringify(script)} }, filetypes = { "rust" } })
         nx.lsp.enable({ "mock" })`,
      ),
    [NXVIM, mockJson],
  );
  ...
  check(
    "reconnect: the pre-drop client handle is retired (nx.lsp.clients() lists only the respawn)",
    after === "2",
  ...
  const diags = await untilLua(page, "#nx.diagnostic.get(0)", "1");
  check(
