-- ~~~ bemtvi daemon connection status: a reconnecting remote link, surfaced ~~~
--
-- The edit-host / daemon split runs the editor LOCALLY and only fs/process/watch/LSP
-- on a remote `bemtvi --daemon`. An ssh link can drop (a laptop sleeping past QUIC's
-- idle timeout, a flaky hop) — bemtvi does NOT tear the session down: it re-dials the
...
--
--   * `btv.daemon.status()` -> "connected" | "reconnecting" | "disconnected", or
...
--     BEMTVI_CONFIG=examples/daemon-status \
--       cargo run -p bemtvi -- examples/daemon-status/sample.txt
