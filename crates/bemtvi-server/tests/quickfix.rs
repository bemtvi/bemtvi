...
//! the list in one `exec_lua` and reads it back in a *second* one (the `btv._qflist`
...
async fn enter_in_dock_hosted_qf_jumps_into_the_main_layer() {
    ...
    exec_lua(
        &rpc,
        &format!("btv.dock.open{{ side = 'bottom', size = 10, buf = {qfbuf} }}"),
    )
    .await;
...
/// Phase 2: with `'qfdock'` on (the default — the bemtvi way), `:copen` hosts the
...
/// A window-scoped location list keeps vim's behavior — `btv.qf.send_to_loclist` opens
...
async fn send_to_loclist_opens_a_split_and_replaces_the_window_list() {
    ...
    exec_lua(&rpc, "_G.w = btv.win.current()").await;
    ...
    exec_lua(
        &rpc,
        &format!(
            r#"btv.qf.send_to_loclist({{ {{ filename = "{a}", lnum = 2, col = 1, text = "hit a" }} }}, {{ title = "A" }})"#
        ),
    )
    .await;
    ...
    exec_lua(
        &rpc,
        &format!(
            r#"btv.qf.send_to_loclist({{ {{ filename = "{b}", lnum = 3, col = 1, text = "hit b" }} }}, {{ title = "B" }})"#
        ),
    )
    .await;
...
/// Phase 5: the `'qfdock'` option reads and writes through `btv.o` (the example's
/// toggle relies on it), defaulting on.
#[tokio::test]
async fn qfdock_option_reads_and_writes_via_btv_o() {
    let (rpc, _incoming) = start().await;
    assert_eq!(
        exec_lua(&rpc, "return btv.o.qfdock").await.as_bool(),
        Some(true),
        "qfdock defaults on"
    );
    exec_lua(&rpc, "btv.o.qfdock = false").await;
    assert_eq!(
        exec_lua(&rpc, "return btv.o.qfdock").await.as_bool(),
        Some(false),
        "btv.o.qfdock round-trips"
    );
}
...
async fn send_and_add_to_qflist_use_one_dock_tab() {
    ...
    exec_lua(
        &rpc,
        r#"btv.qf.send_to_qflist({ { filename = "a.c", lnum = 1, text = "one" } }, { title = "Q" })"#,
    )
    .await;
    ...
    exec_lua(
        &rpc,
        r#"btv.qf.add_to_qflist({ { filename = "b.c", lnum = 2, text = "two" } })"#,
    )
    .await;
...
async fn add_to_loclist_appends_to_the_window_list() {
    ...
    exec_lua(
        &rpc,
        &format!(
            r#"btv.qf.send_to_loclist({{ {{ filename = "{path}", lnum = 1, text = "first" }} }}, {{ title = "L" }})"#
        ),
    )
    .await;
    let rd = redraw_after(&rpc, &mut incoming, "<Esc>").await;
    ...
    exec_lua(
        &rpc,
        &format!(
            r#"btv.qf.add_to_loclist({{ {{ filename = "{path}", lnum = 2, text = "second" }} }})"#
        ),
    )
    .await;
    redraw_after(&rpc, &mut incoming, "<Esc>").await;
...
/// Phase 3: with `:set noqfdock`, `btv.qf.send_to_loclist` falls back to the classic
...
async fn send_to_loclist_without_qfdock_replaces_and_splits() {
    ...
    exec_lua(
        &rpc,
        &format!(
            r#"btv.qf.send_to_loclist({{ {{ filename = "{path}", lnum = 2, col = 1, text = "hit" }} }}, {{ title = "Results" }})"#
        ),
    )
    .await;
    let rd = redraw_after(&rpc, &mut incoming, "<Esc>").await;
...
async fn quickfix_enter_is_rebindable() {
    ...
    exec_lua(
        &rpc,
        "btv.autocmd.create('FileType', { pattern = 'qf', callback = function(a)\n\
           btv.keymap.set('n', 'o', btv.qf.actions.jump, { buffer = a.buf })\n\
         end })",
    )
    .await;
...
/// The `btv.qf` location-list nav wrappers (`lopen`/`lclose`/`lnext`/…) drive the
/// `:l*` ex-commands, mirroring the quickfix wrappers but acting on the current
/// window's location list. Here the list is built with `btv.qf.setloclist`, opened
/// and closed entirely through the Lua surface — no `:l*` keystrokes.
#[tokio::test]
async fn btv_qf_loclist_wrappers_open_navigate_and_close() {
    let (rpc, _incoming) = start().await;
    // Split-window open/navigate/close mechanics — opt out of the dock default.
    exec_lua(&rpc, "vim.cmd('set noqfdock')").await;
    exec_lua(
        &rpc,
        r#"btv.qf.setloclist(0, {
             { filename = "a.c", lnum = 10, col = 5, text = "boom" },
             { filename = "b.c", lnum = 3, text = "later" },
           }, " ")"#,
    )
    .await;
    let before = win_count(&rpc).await;
    exec_lua(&rpc, "btv.qf.lopen()").await;
    assert_eq!(
        win_count(&rpc).await,
        before + 1,
        "btv.qf.lopen adds a window"
    );
    // The location-list window is focused; its buffer holds the rendered entries.
    let rendered = lines(&rpc).await;
    assert_eq!(rendered[0], "a.c|10 col 5| boom");
    assert_eq!(rendered[1], "b.c|3| later");
    exec_lua(&rpc, "btv.qf.lclose()").await;
    assert_eq!(win_count(&rpc).await, before, "btv.qf.lclose removes it");
}
...
// --- Phase 4: list stack, location lists, btv.qf / diagnostics ---------------
...
async fn diagnostic_setloclist_fills_a_navigable_loclist() {
    ...
    exec_lua(
        ...
        r#"local b = vim.api.nvim_get_current_buf()
           btv._set_diagnostics(b, {
             { lnum = 2, col = 0, message = "boom", severity = 1 },
           })
           btv.diagnostic.setloclist({ open = false })"#,
    )
    .await;
...
//
// End-to-end coverage for the direct named-list surface: `btv.qf.list(name, items)`
// populates a window-independent list, `btv.qf.show(name)` opens/focuses its own
// bottom-dock tab, `btv.qf.drop(name)` removes it. These prove the Lua -> op -> core
...
async fn named_list_drop_closes_its_tab_and_forgets_it() {
    ...
    exec_lua(&rpc, r#"btv.qf.drop("d")"#).await;
    ...
    exec_lua(&rpc, r#"btv.qf.show("d")"#).await;
    for _ in 0..200 {
...
async fn named_list_survives_closing_the_window_it_was_shown_from() {
    ...
    exec_lua(
        &rpc,
        &format!(
            r#"_G.from = btv.win.current()
               btv.qf.list("s", {{ {{ filename = "{path}", lnum = 2, text = "kept" }} }}, {{ title = "S" }})
               btv.qf.show("s")"#
        ),
    )
    .await;
...
async fn two_named_lists_coexist_as_separate_tabs() {
    ...
    exec_lua(
        &rpc,
        &format!(
            r#"btv.qf.list("a", {{ {{ filename = "{pa}", lnum = 1, text = "AAA" }} }}, {{ title = "A" }})
               btv.qf.show("a")"#
        ),
    )
    .await;
    ...
    exec_lua(
        &rpc,
        &format!(
            r#"btv.qf.list("b", {{ {{ filename = "{pb}", lnum = 1, text = "BBB" }} }}, {{ title = "B" }})
               btv.qf.show("b")"#
        ),
    )
    .await;
    ...
    exec_lua(&rpc, r#"btv.qf.show("a")"#).await;
    assert!(
        poll_first_line(&rpc, &format!("{pa}|1| AAA")).await,
...
async fn named_list_repaints_an_open_tab_in_place() {
    ...
    exec_lua(
        &rpc,
        &format!(
            r#"btv.qf.list("r", {{ {{ filename = "{path}", lnum = 1, text = "first" }} }})
               btv.qf.show("r")"#
        ),
    )
    .await;
    ...
    exec_lua(
        &rpc,
        &format!(r#"btv.qf.list("r", {{ {{ filename = "{path}", lnum = 3, text = "second" }} }})"#),
    )
    .await;
    assert!(
