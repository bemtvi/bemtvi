...
try {
  ...
  await page.evaluate(async () => {
    try {
      const btv = await (await navigator.storage.getDirectory()).getDirectoryHandle(".bemtvi");
      await btv.removeEntry("treesitter", { recursive: true });
    } catch {}
  });
  await page.reload();
  await page.waitForFunction(() => window.__bemtvi !== undefined, null, { timeout: 15000 });
  ...
  cdnFetches = [];
  await page.evaluate(() => window.__bemtvi.feed(":e demo.zig<CR>"));
  await page.evaluate(() => window.__bemtvi.feed('ggdGipub fn main() !void {}<Esc>'));
  ...
  await page.evaluate(() => window.__bemtvi.feed(":TSInstall zig<CR>"));
  ...
  await page.evaluate(() => window.__bemtvi.feed("ggdGiconst x = foo + Bar;<Esc>"));
  ...
  const cached = await page.evaluate(async () => {
    ...
    try {
      const dir = await (await (await navigator.storage.getDirectory())
        .getDirectoryHandle(".bemtvi")).getDirectoryHandle("treesitter");
  ...
  cdnFetches = [];
  await page.evaluate(() => window.__bemtvi.feed(":e demo.toml<CR>"));
  await page.evaluate(() => window.__bemtvi.feed('ggdGititle = "bemtvi"<CR>[package]<CR>version = "0.1.0"<Esc>'));
  const tomlBefore = await page.evaluate(() =>
    [...document.querySelectorAll("#grid .win .row span[style]")].filter((s) => /color\s*:/.test(s.getAttribute("style"))).length);
  check("uninstalled: toml renders plain before :TSInstall", tomlBefore === 0, String(tomlBefore));

  await page.evaluate(() => window.__bemtvi.feed(":TSInstall toml<CR>"));
