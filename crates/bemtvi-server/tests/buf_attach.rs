//! Black-box tests for `btv.buf.attach` (the `nvim_buf_attach` `on_bytes` / `on_reload`
...
use bemtvi_rpc::Rpc;
use bemtvi_test_harness::{exec_lua, feed, start_with_file as open};
use rmpv::Value;
...
async fn an_unsupported_callback_fails_loud() {
    ...
    let ok = exec_lua(
        &rpc,
        "return (pcall(btv.buf.attach, 0, { on_lines = function() end }))",
    )
    .await;
...
async fn no_callback_fails_loud() {
    let (rpc, _inc) = open("a\n").await;
    let ok = exec_lua(&rpc, "return (pcall(btv.buf.attach, 0, {}))").await;
    assert_eq!(ok.as_bool(), Some(false));
}

// ----- btv.buf.changedtick ----------------------------------------------------

// `btv.buf.changedtick` is the *pull* half of the same change signal `on_bytes`
...
async fn changedtick_advances_on_an_edit_and_holds_still_otherwise() {
    ...
    let tick0 = read(&rpc, "tostring(btv.buf.changedtick(0))").await;
    ...
    feed(&rpc, "l");
    assert_eq!(
        read(&rpc, "tostring(btv.buf.changedtick(0))").await,
        tick0,
    ...
    feed(&rpc, "ix");
    feed(&rpc, "<Esc>");
    let tick1 = read(&rpc, "tostring(btv.buf.changedtick(0))").await;
    ...
    feed(&rpc, "u");
    let tick2 = read(&rpc, "tostring(btv.buf.changedtick(0))").await;
    assert_ne!(tick2, tick1, "undo advances changedtick");

    // An unknown buffer reports 0 rather than erroring.
    assert_eq!(
        read(&rpc, "tostring(btv.buf.changedtick(99999))").await,
        "0",
...
async fn changedtick_is_per_buffer_and_matches_getbufinfo() {
    ...
    let other = read(&rpc, "tostring(btv.buf.current())").await;
    exec_lua(&rpc, &format!("_G.__first = {other} - 1")).await;

    let before = read(&rpc, "tostring(btv.buf.changedtick(_G.__first))").await;
    feed(&rpc, "iedit-the-new-one");
    feed(&rpc, "<Esc>");
    assert_eq!(
        read(&rpc, "tostring(btv.buf.changedtick(_G.__first))").await,
        before,
    ...
    let cur = read(&rpc, "tostring(btv.buf.changedtick(0))").await;
    assert_eq!(
        read(
            &rpc,
            "tostring(vim.fn.getbufinfo(btv.buf.current())[1].changedtick)"
        )
        .await,
        cur,
    ...
    assert_eq!(
        read(
            &rpc,
            "tostring(vim.fn.getbufinfo(btv.buf.current())[1].changed)"
        )
        .await,
        "1",
