...
// through `window.__bemtvi` and assert the chunk's side effects truly landed (a set option, a
...
try {
  ...
  await page.evaluate(() => window.__bemtvi.feed(":luafile"));
  await page.evaluate(() => window.__bemtvi.pressEnter());

  const ranGlobal = await until(page, () => window.__bemtvi.execLua("return _G.__example_ran or ''").then((r) => r.result), (v) => /yes/.test(String(v)));
  ...
  await page.evaluate(() => window.__bemtvi.feed(":Hi<CR>"));
  const cmdRan = await until(page, () => window.__bemtvi.execLua("return _G.__cmd_ran or ''").then((r) => r.result), (v) => /hi/.test(String(v)));
  ...
  await page.evaluate(() => window.__bemtvi.feed(":source"));
  await page.evaluate(() => window.__bemtvi.pressEnter());
  const second = await until(page, () => window.__bemtvi.execLua("return _G.__second or ''").then((r) => r.result), (v) => /B/.test(String(v)));
  ...
  await page.evaluate(() => window.__bemtvi.feed(":luafile /nope.lua"));
  await page.evaluate(() => window.__bemtvi.pressEnter());
  await sleep(200);
