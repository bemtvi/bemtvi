# Async `btv.lsp.*` verbs — return promises so LSP actions can be sequenced

**Goal.** Make the `btv.lsp.*` language verbs (`definition`, `references`, `hover`,
...
btv.lsp.references():next(function(items)  -- items = the resolved locations
  -- ...
end)
```
...
- Each verb returns an `btv.promise`.
- The promise **resolves** (never rejects) — resolve-only avoids the
  ...
  (`btv.keymap.set("n", "gd", btv.lsp.definition)`), which is the common case. A
...
- Resolve **value**:
  - navigation (`definition`/`declaration`/`type_definition`/`implementation`/
    ...
    shape `btv.lsp._show_locations` items use); a single-jump goto resolves a
    1-element list.
...
- Settlement reuses `CallbackArgs::LspReply { err, result }` → `btv._run_cb(id,
...
- Lua: a small `lsp_promise(issue)` helper in `prelude/lsp.lua` allocates a
  `btv._next_cb_id()`, stores a resolver in `btv._cb_fns`, calls `issue(id)`, and
  returns `btv.promise.new(...)`. Each verb becomes
  `function() return lsp_promise(function(id) btv._lsp_buf(KIND, id) end) end`.

### Settlement points in `on_lsp_reply` (`crates/bemtvi-server/src/lsp/request.rs`)
...
- `bemtvi-lua`: add `cb_id: u64` to `LspOp::{BufRequest, Format, Rename,
...
- `bemtvi-server`: `register_lsp_request(kind, cb_id)`; `PendingLspReq.cb_id`;
...
- `prelude/lsp.lua`: `lsp_promise` helper; make `definition`/`declaration`/
  ...
  their `btv.ui.input` prompt into the request promise (cancel → resolve `nil`).
  `code_action` unchanged this phase (still fire-and-forget) — noted as Phase 2.
- Tests (`crates/bemtvi-server/tests/lsp*`): use the LSP mock to assert
