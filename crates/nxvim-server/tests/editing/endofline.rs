...
async fn read_detects_whether_the_last_line_is_terminated() {
    ...
    for (tag, bytes, expected) in [
        ...
        let eol = exec_lua(&rpc, "return nx.bo[0].endofline").await;
        assert_eq!(
            eol.as_bool(),
            Some(expected),
        ...
        let fixeol = exec_lua(&rpc, "return nx.bo[0].fixendofline").await;
        assert_eq!(
            fixeol.as_bool(),
            Some(true),
...
async fn a_file_without_a_final_newline_gains_one_by_default() {
    // vim's `'fixeol'` is on out of the box and nxvim matches it: the missing newline
    // is supplied on write. This is the *current* nxvim behavior too — the point of the
...
async fn an_empty_file_stays_empty_across_a_write() {
    // vim writes 0 bytes for a 0-byte file (its `ML_EMPTY` case) — nxvim used to write
...
async fn a_buffer_emptied_by_editing_keeps_the_terminator_its_file_had() {
    ...
    // bit nxvim deliberately does without. That bit is also why vim can undo back to a
...
async fn a_write_that_supplied_the_newline_updates_endofline() {
    // nxvim keeps the flag accurate across a write (vim leaves `&eol` stale at 0 here).
...
async fn the_options_are_settable_from_lua_and_queryable() {
    ...
    exec_lua(&rpc, "nx.bo[0].fixendofline = false").await;
    assert_eq!(
        exec_lua(&rpc, "return nx.bo[0].fixendofline")
            .await
            .as_bool(),
        Some(false),
        "nx.bo write reaches the core"
    );
    redraw_after(&rpc, &mut incoming, ":w<CR>").await;
    assert_eq!(
        std::fs::read(&path).expect("re-read"),
...
async fn the_noeol_flag_stays_off_buffers_it_would_say_nothing_about() {
    ...
    // them would put `[noeol]` on a stock `nxvim` with no file at all.
