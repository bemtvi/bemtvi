//! Native buffer text search, exposed to Lua as `btv.buf.search` (the bridge fn
//! `btv._buf_search`). It runs over the buffer line **mirror** the server pushes into
//! the VM (`btv._bufs[buf].lines`) — never live core state — so a plugin can locate a
...
//!   * `engine = "vim"`  — the embedded vim regexp engine (`bemtvi_regex`).
//!
...
impl Compiled {
    fn compile(pattern: &str, plain: bool, engine: &str, ignorecase: bool) -> Result<Self, String> {
        ...
        match engine {
            ...
            "vim" => bemtvi_regex::VimRegex::compile(pattern)
...
fn slice(s: &str, start: usize, end: usize) -> mlua::Result<&str> {
    s.get(start..end).ok_or_else(|| {
        mlua::Error::runtime(format!(
            "btv.regex: match byte range {start}..{end} is not a valid slice (off a char boundary or reversed)"
        ))
    })
}
...
/// `btv.regex(pat, opts?)` — a compiled pattern object for matching **Lua strings**
...
fn expand_str_repl(template: &str, whole: &str, caps: &[String]) -> mlua::Result<String> {
    ...
    while let Some(c) = chars.next() {
        ...
        match chars.next() {
            ...
            Some(d @ '0'..='9') => {
                ...
                if nth == 0 || (caps.is_empty() && nth == 1) {
                    ...
                    return Err(mlua::Error::runtime(format!(
                        "btv.regex gsub: replacement refers to %{nth} but the pattern has {} capture(s)",
                        caps.len()
                    )));
                }
            }
            Some(other) => {
                return Err(mlua::Error::runtime(format!(
                    "btv.regex gsub: invalid replacement item '%{other}'"
                )))
            }
            None => {
                return Err(mlua::Error::runtime(
                    "btv.regex gsub: replacement string ends with a lone '%'",
                ))
            }
        }
    }
...
fn repl_value_to_string(v: Value) -> mlua::Result<Option<String>> {
    match v {
        ...
        other => Err(mlua::Error::runtime(format!(
            "btv.regex gsub: replacement value must be a string, number, or nil/false, got {}",
            other.type_name()
        ))),
    }
}
...
fn compute_repl(
    ...
    match repl {
        ...
        other => Err(mlua::Error::runtime(format!(
            "btv.regex gsub: replacement must be a string, table, or function, got {}",
            other.type_name()
        ))),
    }
}
