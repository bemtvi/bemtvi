//! Behavior tests for `nx.ui.open` (alias `vim.ui.open`) — hand a file path or
//! URL to the OS opener. The opener argv prefix is chosen by platform in Rust
//! (`nx._ui_opener`: `open` / `explorer` / `xdg-open`); the Lua wrapper appends
//! the uri and runs it off-tick through `nx.run` (the promise-only process API).
//!
...
//! `nx._ui_opener` with a `sh` command that records the uri it was handed into a
...
/// Override `nx._ui_opener` so the "opener" is `sh -c 'printf %s "$1" > out' sh`.
...
fn record_opener_into(out: &std::path::Path) -> String {
    format!(
        "nx._ui_opener = function() return {{ 'sh', '-c', 'printf %s \"$1\" > {}', 'sh' }} end",
        out.display()
    )
}

#[tokio::test]
async fn nx_ui_open_runs_the_platform_opener_with_the_uri() {
    let (rpc, _incoming) = start().await;
    let out = temp_dir("ui_open").join("opened");
    let uri = "https://example.com/page?q=1";

    exec_lua(&rpc, &record_opener_into(&out)).await;
    exec_lua(&rpc, &format!("nx.ui.open('{uri}')")).await;
...
#[tokio::test]
async fn nx_ui_open_returns_a_promise_of_the_exit_result() {
    let (rpc, _incoming) = start().await;
    // nx.ui.open is promise-only (the nx.run shape): it resolves to { code, ... }.
    // Use `true` as the opener so the exit code is a clean 0.
    exec_lua(&rpc, "nx._ui_opener = function() return { 'true' } end").await;
    exec_lua(
        &rpc,
        "_G.res = nil\n\
         nx.ui.open('whatever'):next(function(r) _G.res = r end)",
    )
    .await;
...
#[tokio::test]
async fn nx_ui_open_rejects_a_non_string_uri() {
    let (rpc, _incoming) = start().await;
    // Fail loud (ADR / project convention): a non-string uri raises, not no-ops.
    let msg = exec_lua(
        &rpc,
        "local ok, err = pcall(nx.ui.open, 123); return tostring(ok) .. '|' .. tostring(err)",
    )
    .await;
