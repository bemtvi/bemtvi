...
//! (the `btv.lsp.semantic_tokens.*` / `btv.lsp.inlay_hint.*` buffer-state surface,
//! design §"buffer nouns", with `vim.lsp.*` aliases).
//!
//! Wired like `lsp_config.rs`: the scripted mock server (`bemtvi --__lsp-mock`)
//! stands in for a real language server, `$BEMTVI_LSP_CMD` overrides the spawn
...
/// Open a `.rs` buffer (filetype `rust`), attach, and `btv.lsp.enable` a mock
...
/// reaches the `btv.lsp.inlay_hint.get` read mirror — exercising the full
/// enable→request→reply→`btv._set_inlay_hints`→getter chain (the mirror receiver
...
async fn inlay_hint_enable_requests_and_get_reads_the_mirror() {
    ...
    // Off by default.
    assert!(
        await_lua_eq(&rpc, "btv.lsp.inlay_hint.is_enabled()", "false").await,
    ...
    assert!(
        await_lua_eq(&rpc, "btv.lsp.inlay_hint.is_enabled()", "true").await,
        "is_enabled should report on after enable"
    );
    assert!(
        await_lua_eq(&rpc, "#btv.lsp.inlay_hint.get({ bufnr = 0 })", "1").await,
        "the decoded hint should reach the get() mirror"
    );
    assert!(
        await_lua_eq(
            &rpc,
            "(btv.lsp.inlay_hint.get({ bufnr = 0 })[1] or {}).inlay_hint.label",
            ": i32"
        )
        .await,
    ...
    assert!(
        await_lua_eq(&rpc, "#btv.lsp.inlay_hint.get({ bufnr = 0 })", "0").await,
...
/// exercising the `btv._set_semantic_tokens` receiver and the position getter.
#[tokio::test]
async fn semantic_tokens_decode_into_the_get_at_pos_mirror() {
    ...
    assert_eq!(
        exec_lua(&rpc, "return #btv.lsp.semantic_tokens.get_at_pos(0, 0, 0)")
            .await
            .as_i64(),
        Some(0),
...
/// `references` (always a list) resolves into `btv.picker` — the multi-hit reply
...
async fn references_open_the_location_picker() {
    ...
    let mut items = None;
    for _ in 0..80 {
        exec_lua(&rpc, "btv.lsp.references()").await;
...
async fn single_definition_jumps_without_a_picker() {
    ...
    for _ in 0..80 {
        exec_lua(&rpc, "btv.lsp.definition()").await;
        bemtvi_test_harness::barrier(&rpc).await;
...
async fn document_symbol_opens_the_symbol_picker() {
    ...
    let mut items = None;
    for _ in 0..80 {
        exec_lua(&rpc, "btv.lsp.document_symbol()").await;
        if let Some(rows) = poll_menu_items(&rpc, &mut incoming).await {
            if rows.len() == 2 {
                items = Some(rows);
                break;
...
async fn workspace_symbol_opens_the_symbol_picker() {
    ...
    for _ in 0..80 {
        ...
        exec_lua(&rpc, "btv.lsp.workspace_symbol('Wid')").await;
        if let Some(rows) = poll_menu_items(&rpc, &mut incoming).await {
            if !rows.is_empty() {
                items = Some(rows);
                break;
...
async fn rename_applies_the_workspace_edit() {
    ...
    for _ in 0..80 {
        exec_lua(&rpc, "btv.lsp.rename('bar')").await;
        bemtvi_test_harness::barrier(&rpc).await;
...
async fn check_rename_sync_round_trips(encoding: &str) {
    ...
    exec_lua(&rpc, "btv.lsp.rename('aa')").await;
    let mut renamed = false;
    for _ in 0..80 {
        bemtvi_test_harness::barrier(&rpc).await;
...
async fn code_action_lists_then_applies_the_chosen_action() {
    ...
    for _ in 0..80 {
        exec_lua(&rpc, "btv.lsp.code_action()").await;
        if let Some(rows) = poll_menu_items(&rpc, &mut incoming).await {
...
/// does, since bemtvi syncs a `didChange` per key — must not repaint while you type.
...
async fn lsp_folding_range_folds_the_buffer() {
    ...
    feed(&rpc, ":set foldexpr=v:lua.btv.lsp.foldexpr()<CR>");
...
// ===== Phase 1: async `btv.lsp.*` verbs return promises ===================
...
async fn references_promise_resolves_with_the_item_list() {
    ...
    exec_lua(
        ...
        r#"
        _G.ref_items = nil
        btv.lsp.references():next(function(items) _G.ref_items = items end)
        "#,
    )
    .await;
    assert!(
    ...
    // Each item carries the fields a picker row / a `btv.picker.edit` jump needs.
    assert!(
        await_lua_eq(
            &rpc,
...
async fn superseded_request_settles_its_promise() {
    ...
    exec_lua(
        ...
        btv.lsp.references():next(function() _G.sup_a = true end)
        btv.lsp.references():next(function(items) _G.sup_b = items end)
        "#,
    )
    .await;
    assert!(
...
async fn verb_with_no_server_resolves_nil() {
    ...
    // No mock, no `btv.lsp.enable` — the buffer has no server attached.
    std::env::remove_var("BEMTVI_LSP_CMD");
    ...
    exec_lua(
        ...
        _G.ns_done = false
        _G.ns_res = "unset"
        btv.lsp.format():next(function(res)
            _G.ns_res = res
            _G.ns_done = true
        end)
        "#,
...
async fn code_action_cancel_resolves_nil() {
    ...
    exec_lua(
        ...
        _G.cancel_res = "unset"
        btv.lsp.code_action():next(function(res)
            _G.cancel_res = res
            _G.cancel_done = true
        end)
        "#,
...
// ===== `btv.lsp.code_action(opts)` — kind filter + one-shot apply =====
...
async fn code_action_only_is_sent_to_the_server() {
    ...
    exec_lua(
        &rpc,
        r#"btv.lsp.code_action({ context = { only = { "source.fixAll", "quickfix" } } })"#,
    )
    .await;
...
async fn code_action_only_with_no_match_resolves_nil() {
    ...
    exec_lua(
        ...
        btv.lsp.code_action({ context = { only = { "source.organizeImports" } }, apply = true })
            :next(function(res)
                _G.nomatch_res = res
...
/// An option bemtvi doesn't model fails LOUD rather than being silently dropped — a
...
async fn code_action_rejects_unsupported_options() {
    ...
    for (expr, want) in [
        (
            "btv.lsp.code_action({ filter = function() end })",
            "unsupported option 'filter'",
        ),
        (
            "btv.lsp.code_action({ only = { 'source.fixAll' } })",
            "unsupported option 'only'",
        ),
        (
            "btv.lsp.code_action({ context = { diagnostics = {} } })",
            "unsupported option 'context.diagnostics'",
        ),
        (
            "btv.lsp.code_action({ context = { only = 'source.fixAll' } })",
            "must be a list of kind strings",
        ),
        (
            "btv.lsp.code_action({ apply = 'yes' })",
...
async fn code_action_sends_the_visual_selection_range() {
    ...
    let title = echoed_range(&rpc, &mut incoming, "ggvjl", "btv.lsp.code_action()").await;
    assert!(
    ...
    assert_eq!(
        exec_lua(&rpc, "return btv.mode().mode").await.as_str(),
        Some("n"),
...
async fn code_action_visual_line_selection_spans_whole_lines() {
    ...
    let title = echoed_range(&rpc, &mut incoming, "ggVj", "btv.lsp.code_action()").await;
    assert!(
...
async fn code_action_without_a_range_is_a_point_at_the_cursor() {
    ...
    let title = echoed_range(&rpc, &mut incoming, "ggjll", "btv.lsp.code_action()").await;
    assert!(
...
/// end-exclusive (the `btv.win.select_range` convention) — and it wins over both the
...
async fn code_action_explicit_range_wins_over_the_selection() {
    ...
    let title = echoed_range(
        ...
        &mut incoming,
        "ggvl",
        "btv.lsp.code_action({ range = { start_row = 1, start_col = 1, end_row = 2, end_col = 2 } })",
    )
    .await;
    assert!(
...
async fn code_action_range_carries_the_diagnostics_it_covers() {
    ...
    let (rpc, mut incoming) = open_with_server(&dir, "aaa\nbbb\nccc\n").await;
    assert!(
        await_lua_eq(&rpc, "#btv.diagnostic.get(0)", "1").await,
        "the mock's diagnostic should land"
    );

    let cursor_only = echoed_range(&rpc, &mut incoming, "gg0", "btv.lsp.code_action()").await;
    assert!(
    ...
    let selected = echoed_range(&rpc, &mut incoming, "ggjVk", "btv.lsp.code_action()").await;
    assert!(
...
async fn code_action_rejects_a_malformed_range() {
    ...
    for (expr, want) in [
        (
            "btv.lsp.code_action({ range = { start_row = 0 } })",
            "opts.range must be a table",
        ),
        (
            "btv.lsp.code_action({ range = { 0, 0, 1, 1 } })",
            "opts.range must be a table",
        ),
        (
            "btv.lsp.code_action({ range = { start_row = 0, start_col = 0, end_row = 1, end_col = -1 } })",
            "opts.range must be a table",
        ),
        (
            "btv.lsp.code_action({ range = 'lines' })",
            "opts.range must be a table",
        ),
    ] {
        let got = exec_lua(
...
/// bemtvi answered it with method-not-found, so every such refactor failed outright.
#[tokio::test]
async fn a_servers_apply_edit_creates_the_new_file_and_is_answered_applied() {
    ...
    assert_eq!(
        exec_lua(&rpc, "return tostring(btv.bo[0].modified)")
            .await
            .as_str(),
        Some("true"),
    ...
    assert_eq!(
        exec_lua(&rpc, "return tostring(btv.buf.name(0))")
            .await
            .as_str(),
        Some("helper.rs"),
    ...
    // placeholder bemtvi itself wrote (the disk baseline was re-snapshotted for exactly
    ...
    // …and bemtvi's own placeholder write never came back as somebody else's change: a
    ...
    assert!(
        ...
        "bemtvi's own placeholder write must not be reported as an external change: {msg:?}"
    );
...
",
    ...
    let name = await_lua_contains(&rpc, "btv.buf.name(0)", "moved.rs").await;
    assert!(
...
",
    ...
    barrier(&rpc).await;
    assert!(
        await_lua_contains(&rpc, "#btv.buf.list()", "2")
            .await
            .contains('2'),
    ...
    let buffers = await_lua_contains(&rpc, "#btv.buf.list()", "1").await;
    assert_eq!(
        buffers, "1",
...
async fn edits_addressed_to_a_renamed_file_reach_the_renamed_buffer() {
    ...
    assert!(
        await_lua_eq(&rpc, "#btv.buf.list()", "1").await,
        "the rename must not strand a second buffer for the same file: {:?}",
        exec_lua(
            &rpc,
            "local t = {} for _, b in ipairs(btv.buf.list()) do \
             t[#t + 1] = tostring(btv.buf.name(b)) end return table.concat(t, ', ')"
        )
        .await
    );
    let name = await_lua_contains(&rpc, "btv.buf.name(0)", "moved.rs").await;
...
async fn a_create_of_an_already_open_file_empties_its_document_first() {
    ...
    barrier(&rpc).await;
    assert!(
        await_lua_eq(&rpc, "btv.bo[0].endofline", "true").await,
...
/// A `workspace/applyEdit` is a **request**: the server is blocked until bemtvi
...
///
/// The stall is real, not simulated: the session's `btv.fs` job leg is a
...
async fn a_stalled_file_operation_gives_up_instead_of_blocking_the_server() {
    ...
    std::env::set_var("BEMTVI_WORKSPACE_FS_TIMEOUT_MS", "400");
    ...
    let (rpc, _incoming) = spawn(ServerInit {
        file: Some(file_path.to_string_lossy().into_owned()),
        fs_jobs: Some(bemtvi_server::RemoteFsJobs::connect(
            host_reader,
            host_writer,
        )),
        ..Default::default()
    ...
    std::env::remove_var("BEMTVI_WORKSPACE_FS_TIMEOUT_MS");
    std::env::remove_var("BEMTVI_LSP_CMD");
}
...
async fn a_definition_in_an_unopened_file_lands_on_the_definition() {
    ...
    exec_lua(&rpc, "btv.lsp.definition()").await;
    let name = await_lua_contains(&rpc, "btv.buf.name(0)", "b.rs").await;
    assert!(
        name.contains("b.rs"),
...
// ===== `btv.lsp.apply_workspace_edit` / `btv.lsp.show_document` (the Lua entry) =====
...
/// The whole `WorkspaceEdit` surface is reachable from Lua — `btv.lsp.apply_workspace_edit`
/// (and its `vim.lsp.util` alias), which a plugin or an `btv.lsp.commands` handler uses
...
async fn the_lua_entry_applies_resource_operations_too() {
    ...
    let edit = format!(
        r#"btv.lsp.apply_workspace_edit({{ documentChanges = {{
            {{ kind = "create", uri = "{new}" }},
...
/// `create` gets: bemtvi's rope always carries a trailing newline, every position in an
...
/// reaches for it, and it used to be `nil` (the example in `btv.lsp.commands`'s own
...
/// dialog bemtvi opens for an annotation), returning what was seen either way.
...
/// all (a config that broke `btv.lsp._confirm_edit`). Declining is the right fallback —
...
async fn a_confirm_that_cannot_be_asked_declines_instead_of_pretending() {
    ...
    exec_lua(
        &rpc,
        r#"btv.lsp._confirm_edit = function() error("no ui here") end"#,
    )
    .await;
...
/// the only numbering it can act on — bemtvi declares `failureHandling: abort`, so the
...
/// `btv.lsp.apply_workspace_edit` reads utf-16 columns — which is the whole point on a
...
async fn a_lua_edits_columns_are_read_at_the_encoding_the_caller_names() {
    ...
    let edit = format!(
        r#"btv.lsp.apply_workspace_edit({{ changes = {{
            ["{a}"] = {{ {{ range = {{ start = {{ line = 0, character = 4 }},
                                       ["end"] = {{ line = 0, character = 7 }} }},
                           newText = "bar" }} }},
        }} }})"#,
...
async fn a_confirm_that_rejects_still_answers_the_waiting_server() {
    ...
    exec_lua(
        &rpc,
        r#"btv.ui.confirm = function() return btv.promise.reject("no ui here") end"#,
    )
    .await;
...
/// rope's phantom `\n` is bemtvi bookkeeping standing in for vim's implicit newline after
...
async fn a_buffer_that_loses_its_terminator_syncs_the_removal() {
    ...
    assert!(
        await_lua_eq(&rpc, "btv.bo[0].endofline", "false").await,
        "the flag is off"
    );
    tokio::time::sleep(Duration::from_millis(80)).await;
    barrier(&rpc).await;
...
async fn a_formatter_that_terminates_the_file_sets_endofline() {
    ...
    exec_lua(
        ...
        r#"
        _G.fmt_done = false
        btv.lsp.format():next(function() _G.fmt_done = true end,
