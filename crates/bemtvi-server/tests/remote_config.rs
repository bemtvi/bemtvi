...
use bemtvi_server::{materialize_remote_config_into, RemoteConfigBundle};
use bemtvi_test_harness::temp_dir;
...
fn materialize_mirrors_files_and_rebases_the_roots() {
    ...
    let bundle = RemoteConfigBundle {
        config_dir: Some("/remote/home/.config/bemtvi".to_string()),
        runtimepath: vec![
            "/remote/home/.config/bemtvi".to_string(),
            "/remote/home/.config/bemtvi/pack/vendor/start/myplugin".to_string(),
        ],
        files: vec![
            (
                "/remote/home/.config/bemtvi/init.lua".to_string(),
                b"btv.o.tabstop = 7\n".to_vec(),
            ),
            (
                "/remote/home/.config/bemtvi/lua/mymod.lua".to_string(),
                b"return 42\n".to_vec(),
            ),
            (
                "/remote/home/.config/bemtvi/pack/vendor/start/myplugin/plugin/myplugin.lua"
                    .to_string(),
                b"-- the plugin\n".to_vec(),
            ),
        ],
    ...
    let want_cfg = cache.join("remote/home/.config/bemtvi");
    assert_eq!(
        config_dir.as_deref(),
    ...
    assert_eq!(
        ...
        vec![
            want_cfg.clone(),
            cache.join("remote/home/.config/bemtvi/pack/vendor/start/myplugin"),
        ],
