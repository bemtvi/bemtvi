...
// WebTransport/QUIC to a real `bemtvi --daemon --listen`) and the change source (Node rewriting
...
// (target/debug/bemtvi), and a Chromium for Playwright. Run:  node verify-watch.mjs
...
const root = mkdtempSync(join(tmpdir(), "bemtvi-watch-"));
...
try {
  ...
  await page.evaluate((f) => window.__bemtvi.feed(":e " + f), noteFile);
  ...
  const reloaded = await until(page, () => window.__bemtvi.lines(), (v) => v === "alpha\nbeta\ngamma");
  check(
  ...
  await page.evaluate((f) => window.__bemtvi.feed(":e " + f), docFile);
  await page.evaluate(() => window.__bemtvi.feed("<CR>"));
  await until(page, () => window.__bemtvi.lines(), (v) => v === "first");
  ...
  const handled = await until(page, () => window.__bemtvi.lines(), (v) => v === "second\nthird");
  check(
  ...
  const reason = await page.evaluate(() =>
    window.__bemtvi.execLua("return vim.g.fcs_reason").then((r) => r.result),
  );
  check(
