-- bemtvi — the editor's own built-in colorscheme: `:colorscheme bemtvi`.
--
-- A One Dark palette, the truecolor sibling of the look the GUI and the
-- browser edit-host ship by default, so a bare terminal `:colorscheme bemtvi`
...
-- user `colors/bemtvi.lua` on the runtimepath still shadows it.
--
-- Written against the canonical `btv.hl.define(ns, name, opts)` highlight setter
