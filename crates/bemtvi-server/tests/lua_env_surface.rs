//! Behavior tests for the small `btv.*` surfaces the nvim-lspconfig port needed and
//! bemtvi did not have: `btv.cwd` / `btv.stdpath` / `btv.pid` / `btv.version` (editor and
//! host facts a config must be able to ask for without reaching into `vim.fn`, which
//! ADR 0002 keeps off the alias whitelist), and the two JSON values Lua cannot express
//! on its own — `btv.json.null` and `btv.json.empty_object()`.
//!
//! The JSON pair is the load-bearing one: an LSP `init_options` that means `{}` or
//! `null` and encodes as `[]` or a missing key is a message the server reads
//! differently, and the failure looks like "the server started but does nothing"
//! rather than an error. See docs/plans/2026-07-29-nvim-lspconfig-native-port.md.
//!
//! Black-box per the project conventions: a real server over RPC, driven with
//! `nvim_exec_lua`.

use bemtvi_rpc::{Incoming, Rpc};
use bemtvi_server::ServerInit;
use bemtvi_test_harness::{exec_lua, start_attached};
use tokio::sync::mpsc::UnboundedReceiver;

async fn start() -> (Rpc, UnboundedReceiver<Incoming>) {
    start_attached(ServerInit::default(), 80, 24).await
}

async fn lua_str(rpc: &Rpc, code: &str) -> String {
    exec_lua(rpc, code)
        .await
        .as_str()
        .map(str::to_owned)
        .unwrap_or_default()
}

#[tokio::test]
async fn cwd_is_the_editors_working_directory_and_follows_cd() {
    let (rpc, _incoming) = start().await;

    let cwd = lua_str(&rpc, "return btv.cwd()").await;
    assert!(
        cwd.starts_with('/'),
        "btv.cwd() must be absolute, got {cwd:?}"
    );
    assert!(
        !cwd.ends_with('/') || cwd == "/",
        "btv.cwd() must not carry a trailing separator, got {cwd:?}"
    );

    // It reads the editor's effective directory rather than a value frozen at startup:
    // a config resolving a fallback root after a `:cd` has to see the new one.
    exec_lua(&rpc, "btv.cmd('cd /')").await;
    let after = lua_str(&rpc, "return btv.cwd()").await;
    assert_eq!(after, "/", "btv.cwd() must follow `:cd`");
}

#[tokio::test]
async fn stdpath_names_the_xdg_directories() {
    let (rpc, _incoming) = start().await;

    for what in ["config", "data", "cache", "state"] {
        let dir = lua_str(&rpc, &format!("return btv.stdpath('{what}')")).await;
        assert!(
            dir.starts_with('/'),
            "btv.stdpath({what:?}) must be an absolute path, got {dir:?}"
        );
        assert!(
            !dir.ends_with('/'),
            "btv.stdpath({what:?}) must not carry a trailing separator, got {dir:?}"
        );
    }

    // The point of asking rather than composing $HOME by hand: the answers differ, so a
    // plugin's cache does not land in the user's config.
    let data = lua_str(&rpc, "return btv.stdpath('data')").await;
    let cache = lua_str(&rpc, "return btv.stdpath('cache')").await;
    assert_ne!(data, cache, "data and cache must be distinct directories");
}

#[tokio::test]
async fn pid_is_this_process_and_version_names_bemtvi() {
    let (rpc, _incoming) = start().await;

    let pid = exec_lua(&rpc, "return btv.pid()").await.as_i64().unwrap();
    assert_eq!(
        pid,
        i64::from(std::process::id()),
        "btv.pid() must be the editor process a server would watch, not some other id"
    );

    // Servers carry this into `clientInfo` / vendor telemetry; it must say what the
    // editor actually is.
    let version = lua_str(&rpc, "return btv.version()").await;
    assert!(
        version.starts_with("bemtvi "),
        "btv.version() must name bemtvi, got {version:?}"
    );
}

#[tokio::test]
async fn json_null_encodes_as_null_where_a_nil_would_drop_the_key() {
    let (rpc, _incoming) = start().await;

    // The distinction the sentinel exists for: `nil` removes the key entirely, which a
    // protocol peer reads as "unset, use your default" rather than "explicitly nothing"
    // — and here it leaves nothing behind at all, so the table encodes as an empty
    // array rather than as an object with a null in it.
    let dropped = lua_str(&rpc, "return btv.json.encode({ token = nil })").await;
    assert_eq!(dropped, "[]", "a nil value leaves no key to encode");

    let explicit = lua_str(&rpc, "return btv.json.encode({ token = btv.json.null })").await;
    assert_eq!(explicit, r#"{"token":null}"#);

    // One shared sentinel, so a caller can identify it on the way back out.
    let same = exec_lua(&rpc, "return btv.json.null == btv.json.null").await;
    assert_eq!(same.as_bool(), Some(true));

    // Nested, and alongside real values.
    let nested = lua_str(
        &rpc,
        "return btv.json.encode({ a = 1, b = { c = btv.json.null } })",
    )
    .await;
    assert!(
        nested.contains(r#""c":null"#),
        "btv.json.null must survive nesting, got {nested}"
    );
}

#[tokio::test]
async fn empty_object_encodes_as_an_object_where_a_bare_table_is_an_array() {
    let (rpc, _incoming) = start().await;

    // An empty Lua table is both an empty array and an empty object; the codec has to
    // pick one, and picks `[]`.
    let bare = lua_str(&rpc, "return btv.json.encode({ models = {} })").await;
    assert_eq!(bare, r#"{"models":[]}"#);

    let marked = lua_str(
        &rpc,
        "return btv.json.encode({ models = btv.json.empty_object() })",
    )
    .await;
    assert_eq!(marked, r#"{"models":{}}"#);

    // A fresh table each call, so it can be filled in afterwards — and a table with
    // entries encodes as an object regardless.
    let distinct = exec_lua(
        &rpc,
        "return btv.json.empty_object() ~= btv.json.empty_object()",
    )
    .await;
    assert_eq!(distinct.as_bool(), Some(true));

    let filled = lua_str(
        &rpc,
        r#"local t = btv.json.empty_object()
           t.a = 1
           return btv.json.encode({ m = t })"#,
    )
    .await;
    assert_eq!(filled, r#"{"m":{"a":1}}"#);
}

#[tokio::test]
async fn the_json_sentinels_reach_an_lsp_config_unchanged() {
    let (rpc, _incoming) = start().await;

    // The reason both exist: they are written into an LSP config's `init_options`, and
    // what matters is the JSON that crosses at `initialize`. `btv.lsp.get_config` is the
    // same resolved table the dispatcher hands the spawn, so encoding it is the honest
    // end-to-end check short of a live server.
    exec_lua(
        &rpc,
        r#"btv.lsp.config("sentinel_probe", {
             init_options = {
               token = btv.json.null,
               memory = { file_store = btv.json.empty_object() },
             },
           })"#,
    )
    .await;

    let encoded = lua_str(
        &rpc,
        "return btv.json.encode(btv.lsp.get_config('sentinel_probe').init_options)",
    )
    .await;
    assert!(
        encoded.contains(r#""token":null"#),
        "the null must survive the config merge, got {encoded}"
    );
    assert!(
        encoded.contains(r#""file_store":{}"#),
        "the empty object must survive the config merge, got {encoded}"
    );
}

#[tokio::test]
async fn a_sentinel_overrides_the_value_a_preset_already_put_there() {
    let (rpc, _incoming) = start().await;

    // The shape the sentinels actually ship in: an `lsp/<name>.lua` preset (or an
    // earlier `btv.lsp.config` call) already fills the key, and the user's config
    // overrides it with "explicitly nothing" / "explicitly an empty object". That runs
    // through `btv.tbl.deep_extend`, whose map-merge rule treats an EMPTY table as a
    // mergeable map — so a sentinel merged onto a populated table contributed no keys
    // and vanished, leaving the preset's value on the wire. Silently: the server starts
    // and answers with a configuration nobody asked for, which is the exact failure the
    // sentinels exist to prevent.
    exec_lua(
        &rpc,
        r#"btv.lsp.config("preset_probe", {
             init_options = {
               auth = { token = "from-the-preset" },
               memory = { file_store = { size = 10 } },
             },
           })
           btv.lsp.config("preset_probe", {
             init_options = {
               auth = btv.json.null,
               memory = { file_store = btv.json.empty_object() },
             },
           })"#,
    )
    .await;

    let encoded = lua_str(
        &rpc,
        "return btv.json.encode(btv.lsp.get_config('preset_probe').init_options)",
    )
    .await;
    assert!(
        encoded.contains(r#""auth":null"#),
        "the null must REPLACE the preset's table, not merge into it, got {encoded}"
    );
    assert!(
        encoded.contains(r#""file_store":{}"#),
        "the empty object must REPLACE the preset's map, got {encoded}"
    );

    // And the merge is not one-way: a real value written over a sentinel wins too, so a
    // preset that nulls a key stays overridable.
    exec_lua(
        &rpc,
        r#"btv.lsp.config("unnull_probe", { init_options = { auth = btv.json.null } })
           btv.lsp.config("unnull_probe", { init_options = { auth = { token = "mine" } } })"#,
    )
    .await;
    let back = lua_str(
        &rpc,
        "return btv.json.encode(btv.lsp.get_config('unnull_probe').init_options)",
    )
    .await;
    assert_eq!(back, r#"{"auth":{"token":"mine"}}"#);
}

#[tokio::test]
async fn is_null_identifies_a_sentinel_that_has_been_copied() {
    let (rpc, _incoming) = start().await;

    // `btv.json.null` is one shared table, but every path that *stores* it copies it —
    // `btv.tbl.deepcopy`, and so `btv.tbl.deep_extend`, which is what `btv.lsp.config`
    // merges with. The copy carries the mark but is a different table, so `==` against
    // the sentinel answers false exactly where a caller would ask: on the way back out
    // of a config. `btv.json.is_null` reads the mark instead.
    let copied_eq = exec_lua(
        &rpc,
        "return btv.tbl.deepcopy({ t = btv.json.null }).t == btv.json.null",
    )
    .await;
    assert_eq!(
        copied_eq.as_bool(),
        Some(false),
        "a copy is a different table — this is why identity is not the test"
    );

    for (expr, want) in [
        ("btv.json.null", true),
        ("btv.tbl.deepcopy({ t = btv.json.null }).t", true),
        ("btv.json.empty_object()", false),
        ("{}", false),
        ("nil", false),
        ("false", false),
        ("\"null\"", false),
    ] {
        let got = exec_lua(&rpc, &format!("return btv.json.is_null({expr})")).await;
        assert_eq!(
            got.as_bool(),
            Some(want),
            "btv.json.is_null({expr}) must be {want}"
        );
    }

    // The round trip a config does: write the sentinel, read it back off the resolved
    // config, recognize it.
    exec_lua(
        &rpc,
        r#"btv.lsp.config("is_null_probe", { init_options = { token = btv.json.null } })"#,
    )
    .await;
    let recognized = exec_lua(
        &rpc,
        "return btv.json.is_null(btv.lsp.get_config('is_null_probe').init_options.token)",
    )
    .await;
    assert_eq!(recognized.as_bool(), Some(true));
}

#[tokio::test]
async fn a_decoded_empty_object_stays_an_object_through_a_round_trip() {
    let (rpc, _incoming) = start().await;

    // Read-modify-write is the ordinary shape for any JSON a plugin owns (a workspace
    // `.bemtvi/config.json`, a lockfile, an HTTP body): decode, edit, encode back. An
    // empty JSON object decoded into a bare Lua table is indistinguishable from an
    // empty array, so `{"pylsp":{}}` came back out as `{"pylsp":[]}` — the file
    // silently rewritten into a shape its own schema rejects. Decoding marks it, so
    // what the document said is what it still says on the way out.
    let round = lua_str(
        &rpc,
        r#"return btv.json.encode(btv.json.decode('{"lsps":{"pylsp":{}}}'))"#,
    )
    .await;
    assert_eq!(round, r#"{"lsps":{"pylsp":{}}}"#);

    // Both ends of the ambiguity, so the fix does not just flip which one is wrong: an
    // empty ARRAY must still round-trip as an array.
    let array = lua_str(
        &rpc,
        r#"return btv.json.encode(btv.json.decode('{"a":[]}'))"#,
    )
    .await;
    assert_eq!(array, r#"{"a":[]}"#);

    let top = lua_str(&rpc, "return btv.json.encode(btv.json.decode('{}'))").await;
    assert_eq!(top, "{}");
    let top_array = lua_str(&rpc, "return btv.json.encode(btv.json.decode('[]'))").await;
    assert_eq!(top_array, "[]");

    // It is still an ordinary table to read and to fill in — the mark answers "array or
    // object?", it does not make the value special to a plugin walking the config.
    let usable = lua_str(
        &rpc,
        r#"local c = btv.json.decode('{"lsps":{"pylsp":{}}}')
           local p = c.lsps.pylsp
           assert(type(p) == "table" and next(p) == nil, "decoded {} must be an empty table")
           p.settings = { plugins = {} }
           return btv.json.encode(c)"#,
    )
    .await;
    assert_eq!(usable, r#"{"lsps":{"pylsp":{"settings":{"plugins":[]}}}}"#);
}

#[tokio::test]
async fn a_decoded_empty_object_is_a_value_when_merged() {
    let (rpc, _incoming) = start().await;

    // The mark decoding attaches is the same one `btv.json.empty_object()` carries, so it
    // inherits that value's merge rule: `btv.tbl.deep_extend` treats it as a LEAF that
    // replaces, not as the empty map it otherwise looks like. Pinned here because it is
    // the visible consequence of the decode-side mark — a config layered over another
    // (`btv.lsp.config`, a workspace template merged into a project config) reads a
    // document's `{}` as "an empty object", which is what the document says.
    let replaced = lua_str(
        &rpc,
        r#"local base = { lsps = { pylsp = { settings = { x = 1 } } } }
           local over = btv.json.decode('{"lsps":{"pylsp":{}}}')
           return btv.json.encode(btv.tbl.deep_extend("force", base, over))"#,
    )
    .await;
    assert_eq!(replaced, r#"{"lsps":{"pylsp":{}}}"#);

    // And it is replaced by a real value in turn, so nothing gets stuck empty.
    let refilled = lua_str(
        &rpc,
        r#"local base = btv.json.decode('{"lsps":{"pylsp":{}}}')
           local over = { lsps = { pylsp = { settings = { x = 1 } } } }
           return btv.json.encode(btv.tbl.deep_extend("force", base, over))"#,
    )
    .await;
    assert_eq!(refilled, r#"{"lsps":{"pylsp":{"settings":{"x":1}}}}"#);
}
