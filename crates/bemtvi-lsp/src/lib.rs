//! The bemtvi LSP client.
//!
//! Unlike the treesitter syntax worker (a separate, crash-isolated bemtvi
...
//! `rust-analyzer` just closes a pipe; it cannot segfault bemtvi, so the
...
//! here; `bemtvi-server` reaches them through this crate's surface. The
...
// helper `bemtvi-server` reuses directly (`crate::convert`). `LspRequest::Raw`/
...
// through the transport (`bemtvi-server` injects a daemon-backed one for the
