...
//! `btv_input` / `btv_command`, asserting on `nvim_buf_get_lines` and on what
...
use bemtvi_test_harness::{attach, buf_lines, spawn};
use rmpv::Value;
...
async fn write_goes_back_through_the_injected_host_fs() {
    ...
    rpc.request("btv_input", vec![Value::from("AX<Esc>")])
        .await
        .expect("input");
...
async fn bare_session_routes_a_later_write_through_the_injected_host_fs() {
    ...
    rpc.request("btv_input", vec![Value::from("ihello<Esc>")])
        .await
        .expect("input");
    rpc.request("btv_command", vec![Value::from("write /virtual/out.txt")])
        .await
        .expect("write");
