...
//! hermetic), drive it with `btv_input`, quit, then **respawn** a second server
...
use bemtvi_server::{is_store_file, PersistState, RedbFileStore, ServerInit, ShadaStore};
...
async fn input_namespace_history_survives_a_workspace_restart() {
    // `btv.ui.input{ history = "<ns>" }` rings (e.g. the DAP repl's `dap>` prompt recall)
    ...
    // proving the namespace ring was restored from the workspace store.
    {
        ...
        exec_lua(
            &rpc,
            "_G.r = nil
             btv.ui.input({ prompt = '> ', history = 'repl' }):next(function(t) _G.r = t end)",
        )
        .await;
        feed(&rpc, "<Up><CR>");
        assert_eq!(
...
async fn persisthistory_none_does_not_persist() {
    ...
    // Session 1: disable history persistence, then run a search and quit.
    {
        let (rpc, incoming) =
            start_attached(init_with_store(&dir, Some(file.clone())), 80, 25).await;
        exec_lua(&rpc, "btv.o.persisthistory = 'none'").await;
        feed(&rpc, "/gamma<CR>");
        assert_eq!(cursor(&rpc).await, (3, 0));
        feed(&rpc, ":qa!<CR>");
        await_server_exit(incoming).await;
...
async fn persisthistory_global_targets_the_global_store() {
    ...
    let cfg = config_with("shada_ptg_cfg", "btv.o.persisthistory = 'global'");
...
async fn persisthistory_rejects_an_invalid_value() {
    ...
    exec_lua(&rpc, "btv.cmd('set persisthistory=global')").await;
    assert_eq!(
        exec_lua(&rpc, "return btv.o.persisthistory").await.as_str(),
        Some("global")
    );
    exec_lua(&rpc, "btv.cmd('set persisthistory=bogus')").await;
    assert_eq!(
        exec_lua(&rpc, "return btv.o.persisthistory").await.as_str(),
        Some("global"),
...
/// reason bemtvi avoids neovim's "random file from another session opens on `<C-o>`"
...
async fn plugin_namespace_survives_a_restart() {
    // A plugin that opts in via `btv.shada.plugin()` keeps its own isolated key/value
    ...
    {
        let (rpc, _incoming) = start_attached(init_with_store(&dir, None), 80, 25).await;
        let count = exec_lua(&rpc, r#"return btv.shada.plugin("demo"):get("count")"#).await;
        assert_eq!(count.as_u64(), Some(3), "number survives the restart");

        let recent = exec_lua(
            &rpc,
            r#"return btv.json.encode(btv.shada.plugin("demo"):get("recent"))"#,
        )
        .await;
        assert_eq!(
            recent.as_str(),
        ...
        let keys = exec_lua(
            &rpc,
            r#"return table.concat(btv.shada.plugin("demo"):keys(), ",")"#,
        )
        .await;
        ...
        let other = exec_lua(&rpc, r#"return btv.shada.plugin("other"):get("count")"#).await;
...
async fn plugin_namespace_is_assigned_from_location() {
    ...
    // with an `@<abs-path>` name under a registered rtp entry — exactly how bemtvi
    // sources a real plugin — and assert each attributes to its own dir.
    let (rpc, _incoming) = start_attached(ServerInit::default(), 80, 25).await;
    let summary = exec_lua(
        &rpc,
        r#"
        btv._add_rtp("/virt/plugins/alpha")
        btv._add_rtp("/virt/plugins/beta")
        ...
        as_plugin("/virt/plugins/alpha/lua/a.lua", [[ btv.shada.plugin():set("x", 1) ]])
        local alpha_self =
          as_plugin("/virt/plugins/alpha/plugin/init.lua", [[ return btv.shada.plugin():get("x") ]])

        -- beta sees nothing of alpha's data — a plugin can't reach another's slice.
        local beta_x =
          as_plugin("/virt/plugins/beta/lua/b.lua", [[ return btv.shada.plugin():get("x") ]])

        -- A sourced file may NOT claim a DIFFERENT namespace (it would break isolation).
        local forced_ok =
          as_plugin("/virt/plugins/alpha/lua/c.lua", [[ return pcall(btv.shada.plugin, "forced") ]])
        ...
        local self_ok, self_ns =
          as_plugin("/virt/plugins/alpha/lua/d.lua", [[ return pcall(function() return btv.shada.plugin("alpha").namespace end) ]])
        ...
        btv._add_rtp(cfg)
        local user_ns =
          as_plugin(cfg .. "/lua/u.lua", [[ return btv.shada.plugin().namespace ]])

        -- A plugin loaded by the manager keys on the REGISTERED name, even when it
        -- differs from the install directory's basename.
        btv.plugins._specs["registered-name"] = { name = "registered-name", _dir = "/virt/managed/pkgdir" }
        btv._add_rtp("/virt/managed/pkgdir")
        local managed_ns =
          as_plugin("/virt/managed/pkgdir/lua/m.lua", [[ return btv.shada.plugin().namespace ]])

        return ("alpha_native=%s alpha_self=%s beta_x=%s forced_ok=%s self_ok=%s self_ns=%s user_ns=%s managed_ns=%s"):format(
          tostring(btv._shada_plugin_get("alpha", "x")),
          tostring(alpha_self),
          tostring(beta_x),
          tostring(forced_ok),
          tostring(self_ok),
...
async fn browser_bundled_plugin_attributes_via_its_named_chunk() {
    // Regression for the python web demo crash "btv.shada.plugin: this caller attributes to
    ...
    // runtimepath. A plugin that calls `btv.shada.plugin()` at load (e.g. bemtvi-tree's session
    ...
    let summary = exec_lua(
        ...
        -- chunk, so its source is `@init.lua`; with no rtp entry for it, btv.shada.plugin()
        -- can't attribute. Forge the exact bundle source name via `load(..., "@init.lua")`.
        assert(load(
          'package.preload["oldtree"] = function(...)\n' ..
          '  return btv.shada.plugin().namespace\n' ..
          'end',
          "@init.lua"))()
        ...
        btv._add_rtp("/bemtvi-plugins/webtree")
        package.preload["webtree"] = assert(load(
          'btv.shada.plugin():set("v", 42)\nreturn btv.shada.plugin().namespace',
          "@/bemtvi-plugins/webtree/lua/webtree/init.lua"))
        ...
        return ("old_ok=%s old_err=%s new_ns=%s stored=%s"):format(
          ...
          tostring(new_ns),
          tostring(btv._shada_plugin_get("webtree", "v")))
        "#,
    )
    .await;
    assert_eq!(
...
async fn plugin_namespace_tolerates_a_trailing_slash_rtp_entry() {
    // A runtimepath entry carried in with a TRAILING SLASH (e.g. `BEMTVI_CONFIG=foo/`) must
    ...
    let got = exec_lua(
        &rpc,
        r#"
        btv._add_rtp("/virt/slashed/")        -- registered WITH a trailing slash
        local function as_plugin(path, body)
          return assert(loadstring(body, "@" .. path))()
        end
        -- A file under it resolves to the dir's basename, not an error.
        return as_plugin("/virt/slashed/lua/x.lua", [[ return btv.shada.plugin().namespace ]])
        "#,
    )
    .await;
    assert_eq!(
...
async fn plugin_namespace_enforces_a_size_budget() {
    ...
    let summary = exec_lua(
        &rpc,
        r#"
        local s = btv.shada.plugin("budget-demo")
        -- ~900 KB fits.
...
async fn plugin_namespaces_can_be_listed_and_forgotten() {
    // btv.shada.namespaces() audits what plugins have stored; btv.shada.forget(ns)
    // prunes one namespace (its data stops being persisted) without touching another.
    let (rpc, _incoming) = start_attached(ServerInit::default(), 80, 25).await;
    let out = exec_lua(
        &rpc,
        r#"
        btv.shada.plugin("alpha"):set("k", 1)
        btv.shada.plugin("beta"):set("k", 2)
        local before = table.concat(btv.shada.namespaces(), ",")
        btv.shada.forget("alpha")
        local after = table.concat(btv.shada.namespaces(), ",")
        return ("before=%s after=%s gone=%s kept=%s"):format(
          before, after,
          tostring(btv.shada.plugin("alpha"):get("k")),
          tostring(btv.shada.plugin("beta"):get("k")))
        "#,
    )
    .await;
    assert_eq!(
...
async fn picker_filter_history_survives_a_restart() {
    ...
    // `<C-Up>` away on Monday. It rides `btv.shada.plugin`, which flushes on the
...
"#,
    ...
    // picker opens pre-filled with it, which is the point of persisting at all.
    {
        ...
        assert_eq!(
            exec_lua(&rpc, r#"return #btv.picker.history("include")"#)
                .await
                .as_u64(),
            Some(0),
