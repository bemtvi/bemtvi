...
Rust**. The engine itself — `bemtvi-snippets`, a standalone plugin capable of
...
the existing Rust tabstop session (`btv.snippet`) with variable/transform/accept
...
`btv.snippet` (the Rust session) **stays** as the batteries-included default most
...
> `btv.*` Lua plugin *is* one. — architecture.md guiding principle #3
...
| luasnip needs | neovim API | bemtvi today |
| --- | --- | --- |
| anchor tabstop/mirror regions that shift with edits | `nvim_buf_set_extmark` + gravity | **PARTIAL** — marks exist, gravity ignored |
| read a region's current byte range | `nvim_buf_get_extmark_by_id` | **EXISTS** — `btv.buf.extmarks` |
...
| read clipboard/selection/date for variables | `getreg`, `strftime`, … | **EXISTS** — `btv.reg.get`, `os.date`, `btv.uuid`, `btv.buf.text`, `btv.mark.list` |
| appear in / expand from the completion menu | `additionalTextEdits` + snippet accept | **GAP** (no Lua-source accept hook) |

bemtvi already has extmarks, region reads, and the entire variable-resolution
...
- **VSCode JSON loading.** Read `package.json`'s `contributes.snippets` map,
  read each `*.json`/`*.code-snippets` with async `btv.fs`, normalize `body`
  (string | string[] → joined) and `prefix` (string | string[] → fan out),
  map VSCode language ids → bemtvi filetypes.
...
- **Variable resolution.** `CURRENT_*` via `os.date`, `TM_FILENAME*` via
  `btv.buf.name`, `TM_SELECTED_TEXT`/`CLIPBOARD` via `btv.reg.get`,
  `TM_CURRENT_LINE`/`WORD` via `btv.current_line`/`btv.buf.text`, `UUID` via
  `btv.uuid`, `RANDOM` in Lua. All read-only, all already available.
- **Transforms.** Regex via `btv.regex` (the vendored vim engine), format-string
...
- **The session state machine** — tabstop order, current stop, choice popups
  (via `btv.ui.select`/`btv.complete`), `<Tab>`/`<S-Tab>` jump logic, nested
  snippets, history/undo grouping policy.
- **Completion candidates** — a normal `btv.complete.source` offering triggers
...
### P1 — `btv.buf.set_text` (precise range edit) — **the keystone**

**Gap.** `btv.buf.set_lines` (`api.lua:411`) is line-granular and rewrites whole
...
```lua
-- btv.buf.set_text(buffer, start_row, start_col, end_row, end_col, replacement)
...
right-gravity, end left-gravity). `btv.buf.set_extmark` *accepts*
...
**Spec.** Honor the two flags already accepted by `btv.buf.set_extmark`:

```lua
btv.buf.set_extmark(buf, ns, row, col, {
  end_row = r, end_col = c,
...
### P3 — `btv.buf.attach{ on_bytes }` (edit-notification channel)
...
```lua
-- btv.buf.attach(buffer, { on_bytes = function(buf, tick, sr, sc, byte_off,
...
`btv.buf.extmarks` *inside* the callback reflect the post-edit tick (the callback
...
**Spec.** Add an optional `on_accept` to the `btv.complete.source` item shape:
...
**Update (already exists).** The public **`btv.win.set_cursor(win, line, col)`**
...
already made in the codebase (cursor placement *is* allowed, via `btv.win.set_cursor`,
...
**Deferred (placeholder select).** bemtvi has no vim-style *Select* mode (the core
`Mode::Select` is the `btv.ui.select` widget, not v_CTRL-G), so "type replaces the
...
```lua
-- btv.win.set_cursor(win, row, col)         -- 0-based; the caret move
-- btv.snippet.enter(range_or_pos, mode)     -- OR an engine-scoped helper:
...
Recommendation: expose `btv.win.set_cursor` (the clean generic form; ADR 0002's
...
generic cursor API nil is preferred, ship the engine-scoped `btv.snippet.enter`
...
primitive **`btv.win.select_range(win, s_row, s_col, e_row, e_col[, opts])`** (0-based,
byte cols, end-exclusive) — the select-mode sibling of `btv.win.set_cursor`. The range is
...
The default is the vim-faithful `"normal"`; the `bemtvi-snippets` engine passes
...
Select (`g<C-h>`) is unimplemented — bemtvi has no blockwise Visual either. Linewise
...
bemtvi has no vim-style **Select mode**: the core `Mode::Select` variant is the
`btv.ui.select` *widget*, not neovim's `v_CTRL-G` selection where a printable key
deletes the highlighted text and enters Insert. With only P5 (`btv.win.set_cursor`),
...
The `bemtvi-snippets` plugin ships without this and documents the limitation; it can
...
- **Lua primitive:** one call to enter it over a range, e.g.
  `btv.win.select_range(win, s_row, s_col, e_row, e_col)` (0-based, byte cols) — the
  select-mode sibling of `btv.win.set_cursor`. The engine calls it (instead of
...
1. **P1 `btv.buf.set_text`** — nothing works without precise range edits. **[landed]**
...
3. **P4 `on_accept`** — the completion-menu seam; independent of P1/P2, small.
   **[landed]** — a `btv.complete.source` item's `on_accept = function(item, ctx)`
...
4. **P3 `on_bytes` attach** — mirror/transform sync; needed for transforms and
   ...
   milestone (which works on P1+P2+P5 + `TextChangedI`). **[landed]** — `btv.buf.attach`
...
5. **P5 cursor/mode** — the one ADR decision; needed for jumps. **[landed — no new
   code]** `btv.win.set_cursor` already existed and is sanctioned; verified it serves
...
6. **P6 Select mode** — placeholder auto-select. **[landed]** A real core `Mode::Select`
   ...
   the default and parks in Insert), entered via the new `btv.win.select_range` Lua
   primitive. Generic — a rename/paired-edit widget wants select-and-replace too — not
   snippet-specific.

**All six primitives (P1–P6) are landed**, and the `bemtvi-snippets` plugin
...
- **Milestone 3 (+P6):** type-over-default placeholder selection. **Done** — the
  engine jumps to a non-empty placeholder with `btv.win.select_range` so the first
...
- **Not** extending `btv.snippet` (the Rust session) with variables/transforms.
...
- Select mode was **not** part of the original required set (P1–P5): the engine
  ...
  `btv.win.select_range` primitive, designed on its own merits (a generic
...
- **P1:** feed keys to place a cursor, `exec_lua` a `btv.buf.set_text` over a
...
- **P2:** set a zero-width extmark left/right gravity, `set_text` inside it,
  assert the mark's range via `btv.buf.extmarks` grew (vs. the default-gravity
...
stand-in for the real `bemtvi-snippets` plugin, living in the harness (not an
