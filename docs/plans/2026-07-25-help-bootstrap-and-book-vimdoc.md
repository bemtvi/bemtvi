# `:help` on demand — bootstrapping nxvim-help and shipping the book as vimdoc

Today `:help` in a fresh install prints a pointer at a plugin the user has to go install by
hand, and the ~415 KB of prose in [the nxvim book](2026-06-19-documentation-site.md) exists
only as a website. This plan closes both gaps at once, without giving up the single binary:

```
:help nx.buf.line()
  → "Help isn't installed yet. Fetch nxvim-help and the nxvim docs from GitHub? [y/N]"
  → y
  → (async) clone nxvim/nxvim-help @ pinned commit, nxvim/nxvim-docs @ tag v0.5.0
  → re-dispatch  →  the help window opens on *nx.buf.line()*
```
...
built from, and a GitHub Actions job publishes it to a generated `nxvim/nxvim-docs` repo,
tagged per nxvim release.

**Delivery.** The existing "plugin not installed" arm for `:help` becomes a bootstrap that
installs `nxvim-help` *and* `nxvim-docs` through `nx.plugins`, then retries the command.
...
`nx.plugins` already clones through `nx.git_local` — **first-party gix, no `git` binary**
(`crates/nxvim-lua/src/prelude/plugins.lua:6`) — and it is already remote-aware by
...
- `nxvim-help` discovers them with **zero code**, because its index is
  `nx.runtime_file("doc/*.txt", true)` over the runtimepath — registration is by
...
or remove this?"* with a mechanism `nx.plugins` already answers.

Being fair to the archive: the machinery is not from scratch. `crates/nxvim-ts/src/install.rs`
...
build, the way `crates/nxvim-edithost/build-plugins.sh` already amalgamates the recommended
plugin set into one bundle. See *Three worlds* below.

## Why not vendor the docs into nxvim-help

`nxvim-help` is the **viewer**. The prose has two upstream sources, both in this repo: the
...
(`crates/nxvim-server/src/lib.rs:468`) keeps its current shape — `$NXVIM_RTP`, then the
...
(`generate.py:190`), which is why `book/src/api/nx.buf.md` reads `-&gt;` where the source
...
```
#  `nx.buf`                  → *nx.buf*
## `nx.buf.name(bufnr)`      → *nx.buf.name()*
```

Dotted `nx.*` names are what users actually type and will not collide with anything else on
the runtimepath. Guide and feature chapters get an `nxvim-` prefix on every section tag
(`*nxvim-picker*`, `*nxvim-docks*`) so they do not squat generic topics — the concern
`nxvim-help/lua/nxvim-help/helptags.lua` documents at length about its scanner.
...
(or the job runs `:NxHelptags`) and the repo commits it.
...
time — a SHA pin is a chicken-and-egg. Instead the Action tags `nxvim-docs` with the nxvim
version, and the binary pins:

```
{ "nxvim/nxvim-docs", tag = <nxvim version> }        -- env!("CARGO_PKG_VERSION")
{ "nxvim/nxvim-help", commit = "<pinned sha>" }      -- independent repo, SHA is fine
```

`nx.plugins` already supports both (`commit`/`tag`/`version` all pin, `commit` wins —
`plugins.lua:202`), and a pin is never auto-updated. The `nxvim-help` pin lives beside the
...
The seam already exists. `crates/nxvim-server/src/excmd.rs:211` is a `:help`/`:h` arm that
fires only when no plugin has registered `:help`, and currently echoes *"add it with
:Plugins (nxvim/nxvim-help)"*. That echo becomes the bootstrap; `:helptags` at `:220` gets
the same treatment. When the plugin *is* installed, nothing changes — the user-command arm
below already wins.

The bootstrap itself is Lua in the prelude (`nx._help_bootstrap()`), because installing
plugins is what `nx.plugins` does and this must not become a second installer in Rust.

Four requirements, none optional:

- **Ask once.** Fetching and executing code from the network on a keystroke needs consent.
  Prompt with `nx.ui.confirm`, persist the answer in shada, and expose a config option so
...
- **Fail loud.** Offline, or the fetch rejects: report what it tried to fetch and print the
  exact `nx.plugins` spec to install by hand. Never a `:help` that quietly does nothing —
  that is the no-silent-stubs rule.
- **Async.** A clone on the editor thread would freeze it. This rides `nx.async` like every
...
After the install lands, the bootstrap adds the runtimepath entries (`nx._add_rtp`), sources
the plugin, and **re-dispatches the original command with its argument** — the user typed
`:help nx.buf.line()` and should get that topic, not a bare help window.
...
local-always seams (`nx.fs_local` / `nx.git_local`) were built for exactly this case, so it
...
`doc/*.txt`. A browser session must not print "install the nxvim-help plugin" — either help
...
3. Push the generated tree to `nxvim/nxvim-docs` and tag it with the release version.
...
**Phase 3 — the Actions job and the `nxvim-docs` repo.** Publishing and tagging, with the
...
**Phase 4 — the bootstrap.** The `excmd.rs` arms, `nx._help_bootstrap()`, consent + shada,
...
network. `build-plugins.sh` sets the precedent with `NXVIM_PLUGINS_BASE` (default
`https://github.com`, overridable to a `file:///` mirror); the bootstrap needs the same
seam so a test can point it at a throwaway local repo. **Verify first** that `nx.git_local`
...
1. Does `nx.git_local` clone from a `file://` URL? (Hermetic tests depend on it.)
2. Does a mid-session `nx.plugins` install refresh the daemon's local runtimepath cache?
3. Does panvimdoc's auto-tagging produce the tags described above from `##`
   `` `nx.buf.name(bufnr)` `` headings, or does it need explicit tag markers in the
   generated markdown?
