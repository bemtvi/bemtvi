# `btv.ui.float` — the content-float sibling (hover / signature help)
...
+ the `btv.ui.float` Lua primitive (tested in `bemtvi-server/tests/ui_float.rs`);
LSP hover / signature help rerouted into the float with `btv.lsp.buf.hover` /
`signature_help` verbs (tested in `bemtvi/tests/lsp_float.rs` via `bemtvi-lsp::mock`);
...
[float-widget spec](../specs/2026-06-14-btv-ui-float-widget.md): the **list-less
...
> floats are not this widget … Keep them as `btv.ui.float` (rendered content), a
> sibling of the list-widget on the shared `FloatConfig` placement layer."*

`btv.ui.float` is the one missing `btv.ui` primitive — `select` / `input` /
...
- The full LSP **hover/signatureHelp request→reply cycle already works**:
  `btv._lsp_buf(kind)` queues `LspOp::BufRequest`, `dispatch.rs` issues it,
...
- The `btv.ui.select` bridge (`_ui_select` → `Shared::ui_selects` →
  `effects.rs` drain → `Editor::open_menu`) is the exact plumbing the
  `btv.ui.float` bridge mirrors.
...
### Server redraw (`bemtvi-server/src/redraw.rs`)
...
### Clients (3)
- `bemtvi-tui/src/render.rs`: `render_content_float()` — bordered box, optional
  title on the top border, plain lines (reuse the float/menu border helpers).
- `bemtvi-gui/src/render.rs`: `build_content_float()` — bg quads + glyphs + border.
- `bemtvi-edithost/web/index.html`: `renderContentFloat()` — a bordered div of rows.

### LSP wiring (`bemtvi-server/src/lsp/request.rs`)
...
### Lua surface
- New `prelude/lsp.lua` (minimal slice of btv-lsp Phase A): `btv.lsp.buf.hover()` /
  `btv.lsp.buf.signature_help()` → `btv._lsp_buf(5/6)` (kinds per `LspReqKind`),
  aliased onto `vim.lsp.buf.*`. Register in the prelude loader after `ui.lua`.
- `prelude/ui.lua`: `btv.ui.float(contents, opts)` — `contents` a string or list;
  `opts = { border, title, relative = "cursor"|"editor" }` → `btv._ui_float`.
...
### Tests (`bemtvi-server/tests/`, black-box)
- `ui_float.rs`: `btv.ui.float({...})` projects a `float` redraw sub-map with the
  lines; the next key dismisses it; `relative="editor"` centers.
- LSP `hover`/`signatureHelp` via `bemtvi-lsp::mock` (`$BEMTVI_LSP_CMD`): the reply
...
### Example config
- `examples/ui-float/` — `init.lua` mapping `K` → `btv.lsp.buf.hover()` and a
  command calling `btv.ui.float`, with a sample file, verified end-to-end.

## Phasing (each independently testable)
1. Core `ContentFloat` + view projection + redraw `float` key + the 3 client
   renderers + `btv.ui.float` Lua primitive + `ui_float.rs` test. (Float works
