// bemtvi-regex host shim: implements every editor-side function the vendored
...
void *xmalloc(size_t size)
{
  ...
  if (p == NULL) {
    fprintf(stderr, "bemtvi-regex: out of memory allocating %zu bytes\n", size);
    abort();
  }
  return p;
}
...
void *xcalloc(size_t count, size_t size)
{
  ...
  if (p == NULL) {
    fprintf(stderr, "bemtvi-regex: out of memory allocating %zu*%zu bytes\n", count, size);
    abort();
  }
  return p;
}
...
void *xrealloc(void *ptr, size_t size)
{
  ...
  if (p == NULL) {
    fprintf(stderr, "bemtvi-regex: out of memory reallocating %zu bytes\n", size);
    abort();
  }
  return p;
}
...
enum { BTVRE_ERRBUF = 512 };
static char last_error[BTVRE_ERRBUF];
...
{
  snprintf(last_error, sizeof(last_error), "bemtvi-regex internal error: %s", s);
  have_error = true;
  called_emsg++;
}
...
{
  semsg("bemtvi-regex internal error: %s", where);
}

void siemsg(const char *fmt, ...)
{
  char buf[BTVRE_ERRBUF];
  va_list ap;
  va_start(ap, fmt);
...
// verbose-mode message redirection is presentational in nvim; bemtvi-regex
...
const char *btvre_take_last_error(void)
{
  if (!have_error) {
    return NULL;
  }
...
static char *btvre_get_line_checked(buf_T *buf, linenr_T lnum, colnr_T *len)
{
  const char *line = buf->btv_get_line(buf->btv_ud, lnum, len);
  if (line == NULL) {
    ...
    fprintf(stderr, "bemtvi-regex: line provider returned NULL for line %" PRIdLINENR "\n", lnum);
    abort();
  }
  return (char *)line;
}
...
{
  return btvre_get_line_checked(buf, lnum, NULL);
}

colnr_T ml_get_buf_len(buf_T *buf, linenr_T lnum)
{
  colnr_T len = 0;
  btvre_get_line_checked(buf, lnum, &len);
  return len;
}
...
static btvre_mark_lookup_fn mark_lookup = NULL;
static void *mark_lookup_ud = NULL;

void btvre_set_mark_provider(btvre_mark_lookup_fn lookup, void *userdata)
{
  mark_lookup = lookup;
...
void btvre_set_interrupt(bool value)
{
  got_int = value;
}
...
{
  // got_int is set directly by the host (btvre_set_interrupt); nothing to poll.
}
...
uint64_t btvre_profile_setlimit(int64_t ms)
{
...
void getvvcol(win_T *wp, pos_T *pos, colnr_T *start, colnr_T *cursor, colnr_T *end, int flags)
{
  ...
  if (wp->w_buffer != NULL && pos->lnum >= 1
    ...
    colnr_T len = 0;
    char *line = btvre_get_line_checked(wp->w_buffer, pos->lnum, &len);
...
buf_T *btvre_buf_new(btvre_get_line_fn get_line, void *userdata, linenr_T line_count)
{
  buf_T *buf = xcalloc(1, sizeof(buf_T));
  buf->b_ml.ml_line_count = line_count;
  buf->b_p_ts = 8;
  buf->b_p_isk = xstrdup(isk_default);
  buf->btv_get_line = get_line;
  buf->btv_ud = userdata;
  buf_init_chartab(buf, false);
  return buf;
}

void btvre_buf_free(buf_T *buf)
{
  if (buf == NULL) {
    return;
  }
...
void btvre_buf_set_line_count(buf_T *buf, linenr_T line_count)
{
  buf->b_ml.ml_line_count = line_count;
}

bool btvre_buf_set_iskeyword(buf_T *buf, const char *iskeyword)
{
  char *old = buf->b_p_isk;
  buf->b_p_isk = xstrdup(iskeyword);
  if (buf_init_chartab(buf, false) == FAIL) {
    emsg("bemtvi-regex: invalid 'iskeyword' option string");
    xfree(buf->b_p_isk);
    buf->b_p_isk = old;
    buf_init_chartab(buf, false);
    return false;
...
void btvre_buf_set_tabstop(buf_T *buf, int64_t tabstop)
{
  buf->b_p_ts = tabstop;
}

win_T *btvre_win_new(buf_T *buf)
{
...
void btvre_win_set_cursor(win_T *win, linenr_T lnum, colnr_T col)
{
...
void btvre_buf_set_visual(buf_T *buf, linenr_T start_lnum, colnr_T start_col, linenr_T end_lnum,
...
void btvre_set_current(buf_T *buf, win_T *win)
{
  curbuf = buf;
  curwin = win;
...
void btvre_set_regexpengine(int64_t engine)
{
  p_re = engine;
}
