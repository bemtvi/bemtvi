...
In a native `--connect-daemon bemtvi://…` session the editor is *born remote*:
`run_edit_host_session` (`crates/bemtvi/src/main.rs`) injects the daemon's
`host_proc` / `host_fs_async` / `fs_jobs` into `ServerInit`, so **every** `btv.fs`
and `btv.run` (and thus every process spawn) routes to the daemon (main.rs comment:
*"every fs/process/LSP/Lua-fs path is routed to the daemon"*).

But the `btv.plugins` manager (`crates/bemtvi-lua/src/prelude/plugins.lua`) is a
**local-VM** concern: it clones repos with `btv.run` (git), discovers `plugin/`
scripts and checks presence with `btv.fs`, and then adds each plugin dir to the
runtimepath via `btv._add_rtp` and loads it with `require` — and `_add_rtp` /
...
- **no `--remote-config`** (local config, default): the local `init.lua` declares
  `btv.plugins`, but clone/discover/source hit the daemon. It should behave exactly
  like a local launch — plugins managed on local disk.
- **`--remote-config`**: the remote config is fetched and *materialized locally*
  (`remote_config.rs`), then run locally. Its `btv.plugins` declarations should
...
file editing (buffers, `btv.fs`/`btv.run` a plugin calls while running), which
...
- `btv.fs.*` management ops → a local `StdLuaFs` (native: real disk; wasm: MEMFS).
- git spawns (`btv.run`) → a local `StdHostProc`.

Runtime plugin code keeps using session-routed `btv.fs` / `btv.run` (remote in a
...
- Lua seams `btv._local_fs_op(job, id)` / a `local` argument on `btv._system_async`,
  bound in install.rs to push the op with `local = true`. `btv._fs_op` /
  `btv._system_async` default `local = false` (unchanged runtime behavior).
...
`btv._local_fs_op`) and a `local_run` (git over the local spawn). Replace the
manager's management-path `btv.fs.*` / `btv.run` calls (`source_runtime`,
`source_config`, `clone`, the present-checks in `load` / `activate_eager`) with the
local shim. Nothing else in `btv.*` changes — runtime plugin code is untouched.
...
- install.rs: `btv._local_fs_op`; `local` arg on the spawn bridge.
...
- Test `crates/bemtvi-server/tests/plugins.rs` (or a new daemon test): a daemon
  session whose remote fs is a *different* tree; a `btv.plugins` local-dir plugin
...
- `web/verify-fs-op.mjs` gains a check: against a real `bemtvi --daemon`, a session
  `btv.fs.exists` sees a daemon-only file (true) while `btv._local_fs_op` exists on
...
- `docs/edit-host-split.md` (split-brain filesystem section): a "Plugin management is
  always local" note — `btv.plugins` clones/discovers/sources on the local disk in every
