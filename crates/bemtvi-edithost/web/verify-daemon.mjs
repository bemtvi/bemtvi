// Playwright verifier for the browser edit-host driving a REAL `bemtvi --daemon --listen`
...
// <dir>` through window.__bemtvi and assert the bytes round-trip end to end.
//
// Prereqs: ./build.sh (dist/eh.mjs + eh.wasm + vendor/msgpack), `cargo build -p bemtvi`
// (target/debug/bemtvi), and a Chromium for Playwright. Run:  node verify-daemon.mjs
...
const root = mkdtempSync(join(tmpdir(), "bemtvi-daemon-"));
...
try {
  ...
  const opened = await until(page, () => window.__bemtvi.lines(), (v) => v === FILE_CONTENT);
  check(
  ...
  await page.evaluate(() => window.__bemtvi.feed("GoDELTA-OVER-WIRE<Esc>"));
  ...
  await page.evaluate((d) => window.__bemtvi.feed(":e " + d), sub);
  await page.evaluate(() => window.__bemtvi.feed("<CR>"));
...
console.log(failures === 0
  ? "\nALL PASS — browser edit-host opens/saves/lists files on a real bemtvi --daemon over WebTransport"
  : `\n${failures} FAILED`);
