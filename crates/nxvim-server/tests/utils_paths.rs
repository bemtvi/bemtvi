//! Behavior tests for the `nx.utils` path helpers (`joinpath` / `normalize` /
//! `relpath`) and `nx.fs.which` — the pure path math and the async executable lookup
...
//! `nx.fs.which` settles OFF the editor tick (like every `nx.fs` op), so those tests
...
/// `nx.fs` op settles on a later tick, so the global its chain sets is nil until the
...
async fn joinpath_collapses_separators_at_every_seam() {
    ...
    let joined = lua_str(
        &rpc,
        r#"return nx.utils.joinpath("/a/b", "node_modules/.bin", "eslint")"#,
    )
    .await;
    ...
    let messy = lua_str(&rpc, r#"return nx.utils.joinpath("/a/", "/b/", "c")"#).await;
    ...
    let rooted = lua_str(&rpc, r#"return nx.utils.joinpath("/a", "/b")"#).await;
    assert_eq!(rooted.as_deref(), Some("/a/b"));

    // A relative first component stays relative.
    let rel = lua_str(&rpc, r#"return nx.utils.joinpath("a", "b")"#).await;
...
async fn joinpath_skips_empty_components_and_handles_the_root() {
    ...
    let gap = lua_str(&rpc, r#"return nx.utils.joinpath("/a", "", "c")"#).await;
    assert_eq!(gap.as_deref(), Some("/a/c"));

    let nil_gap = lua_str(&rpc, r#"return nx.utils.joinpath("/a", nil, "c")"#).await;
    assert_eq!(nil_gap.as_deref(), Some("/a/c"));

    // The root is the one first component whose trailing separator IS the path.
    let from_root = lua_str(&rpc, r#"return nx.utils.joinpath("/", "usr", "bin")"#).await;
    assert_eq!(from_root.as_deref(), Some("/usr/bin"));

    let nothing = lua_str(&rpc, r#"return nx.utils.joinpath()"#).await;
...
async fn joinpath_rejects_a_non_string_component() {
    ...
    let err = lua_str(
        &rpc,
        r#"local ok, e = pcall(nx.utils.joinpath, "/a", 7) return tostring(e)"#,
    )
    .await
    .unwrap_or_default();
    assert!(
...
async fn normalize_resolves_dot_and_dotdot_lexically() {
    ...
    for (input, want) in cases {
        let got = lua_str(&rpc, &format!("return nx.utils.normalize({input})")).await;
...
async fn normalize_expands_a_leading_tilde() {
    ...
    let got = lua_str(&rpc, r#"return nx.utils.normalize("~/work/x")"#).await;
...
async fn relpath_compares_whole_components_not_string_prefixes() {
    let (rpc, _incoming) = start().await;
    let inside = lua_str(&rpc, r#"return nx.utils.relpath("/a/b", "/a/b/c/d")"#).await;
    assert_eq!(inside.as_deref(), Some("c/d"));

    let same = lua_str(&rpc, r#"return nx.utils.relpath("/a/b", "/a/b")"#).await;
    assert_eq!(same.as_deref(), Some("."));

    // The bug a plain prefix-match has: `/a/bc` is NOT inside `/a/b`.
    let sibling = lua_str(&rpc, r#"return nx.utils.relpath("/a/b", "/a/bc")"#).await;
    assert_eq!(
        sibling, None,
        "a sibling sharing a name prefix is not inside"
    );

    let outside = lua_str(&rpc, r#"return nx.utils.relpath("/a/b", "/x/y")"#).await;
    assert_eq!(outside, None);

    // Everything is inside the root.
    let from_root = lua_str(&rpc, r#"return nx.utils.relpath("/", "/a/b")"#).await;
...
async fn which_finds_an_executable_on_path_and_answers_nil_for_a_missing_one() {
    ...
    exec_lua(
        &rpc,
        r#"nx.async(function() _G.sh = nx.await(nx.fs.which("sh")) or false end)()"#,
    )
    .await;
    ...
    // config write `nx.await(nx.fs.which(x)) or fallback`).
    exec_lua(
        &rpc,
        r#"nx.async(function()
             local hit = nx.await(nx.fs.which("nxvim-no-such-program-xyzzy"))
...
async fn which_accepts_an_explicit_path_only_when_it_is_executable() {
    ...
    exec_lua(
        ...
        &format!(
            r#"nx.async(function()
                 _G.exe = nx.await(nx.fs.which({exe})) or false
                 _G.plain = nx.await(nx.fs.which({plain})) == nil and "nil" or "found"
               end)()"#,
