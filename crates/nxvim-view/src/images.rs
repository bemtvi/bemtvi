...
/// client's event loop fulfils it with `nxvim_image_read` and feeds the bytes
...
impl RemoteImages {
    ...
    /// Receive an `nxvim_image_read` reply (routed from the client's event loop):
...
/// editor RPC (`nxvim_image_read`), which the client can't read off its own
...
/// Decode an `nxvim_image_read` reply into the bytes [`RemoteImages::deliver`]
