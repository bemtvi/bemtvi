...
completion menu,"* nxvim hands you a completion engine and says *"give it items."*
...
nxvim's architecture is different (see [Architecture](architecture.md)), so its
...
as **first-class APIs**: [`nx.view`](features/ui-primitives.md) (dockable content
...
- [Async & promises](async.md) — rule 3 in practice (`nx.promise` / `async`/`await`).
