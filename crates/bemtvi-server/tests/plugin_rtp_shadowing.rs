...
//!
//! Repro of the reported "btv.plugins race — plugins get deleted" bug: init.lua splits
//! its plugin declarations across two `btv.plugins{}` calls — some inline, the rest in a
//! `lua/plugins.lua` pulled in with `require("plugins")` (the standard neovim idiom).
//! The first call eagerly loads its plugins, and each load runs `btv._add_rtp`, which
...
async fn a_loaded_plugin_does_not_shadow_the_users_require_of_a_config_module() {
    ...
    std::fs::write(
        ...
        "_G.USER_MODULE_WON = true\n\
         btv.plugins({ { dir = \"/no/such/dir\", name = \"declared-in-user-plugins\", enabled = false } })\n",
    )
    .unwrap();
    ...
    std::fs::write(
        ...
        format!(
            "btv.plugins.setup_manager({{ root = \"{root}\", config = \"{cfg}\" }})\n\
             btv.plugins({{ {{ dir = \"{plug}\", name = \"shadowplug\" }} }})\n\
    ...
    assert_eq!(
        exec_lua(
            &rpc,
            "return btv.plugins._specs['declared-in-user-plugins'] ~= nil"
        )
        .await
        .as_bool(),
        Some(true),
    ...
    assert_eq!(
        exec_lua(&rpc, "return btv.plugins._loaded['shadowplug'] == true")
            .await
            .as_bool(),
        Some(true),
