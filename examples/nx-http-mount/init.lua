-- ~~~ nxvim nx.http.mount playground: a live markdown preview, served from the editor ~~~
--
-- Run it (from the repo root):
--
--     NXVIM_CONFIG=examples/nx-http-mount \
--       cargo run -p nxvim -- examples/nx-http-mount/sample.md
--
...
--
-- `nx.http.mount(opts)` publishes a plugin's subroute on the editor's ONE origin and
...
-- ephemeral default:
--
--     nx.o.httpport = 8080          -- 0 (default) picks a free one
--     nx.o.httphost = "0.0.0.0"     -- careful: exposes every mount to the LAN
--
...
<head>
<meta charset="utf-8">
<title>nxvim — live markdown preview</title>
<style>
    ...
    on_request = function(req, respond)
      ...
      elseif req.path == "/source" then
        respond({
          ...
          body = table.concat(nx.buf.lines(0, 0, -1), "\n"),
        })
      ...
      elseif req.path == "/info" then
        local info = {
          name = nx.buf.name(0),
          lines = #nx.buf.lines(0, 0, -1),
          mount = req.name,
          method = req.method,
        }
        respond({
          headers = { ["content-type"] = "application/json" },
          body = nx.json.encode(info, { pretty = req.query.pretty == "1" }),
        })
...
nx.keymap.set("n", "<leader>p", function()
  ...
  nx.ui.open(preview:url())
end)

-- \u — print the URL (and the editor's origin, shared by every mount).
nx.keymap.set("n", "<leader>u", function()
  if not preview then
    return show("the preview is not mounted (yet?)")
  end
  show(("%s   (origin: %s)"):format(preview:url(), nx.http.origin()))
end)
...
show("nx.http.mount demo loading — mounting the preview…")
