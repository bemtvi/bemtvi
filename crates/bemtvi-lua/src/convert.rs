...
//! the `nvim_set_hl` / `btv.run` bridges lean on.
...
/// on wasm32 (`lua_Integer` is `ptrdiff_t`, and wasm32 pointers are 32-bit). bemtvi's
...
/// (`btv._buf_set_option` / `_set_global_option` / `_win_set_option` /
...
///
/// This is a deliberate, parity-exact port of `bemtvi_core::highlight::parse_color`
/// (+ its `named_color` table): the `nvim_set_hl` write-through uses it to build
/// the *same-turn* `btv._hl_defs` mirror row, which must match byte-for-byte the
...
/// it here. bemtvi-lua intentionally carries no `bemtvi-core` dependency (it stays
...
/// The metatable field `btv.json.null` / `btv.json.empty_object()` mark themselves with.
...
const JSON_MARK: &str = "__bemtvi_json";
...
/// Mark `t` as a JSON object — the Rust twin of `btv.json.empty_object()`, for the
...
/// nil. Covers the scalar-and-table shapes bemtvi's synchronous getters return.
...
///
/// An EMPTY object carries the `btv.json.empty_object()` mark, so decoding is the
...
fn json_to_lua_at(lua: &Lua, value: &serde_json::Value, depth: usize) -> mlua::Result<mlua::Value> {
    ...
    Ok(match value {
        ...
        J::Object(map) => {
            ...
            // picks `[]`. Mark it (the `btv.json.empty_object()` mark) so a decode ->
...
/// `capabilities` from `btv._lsp_start`) to JSON for `LspOp::Start`. `None`
...
/// Flatten an `btv.run`-family `spec.env` table (`{ VAR = value }`) into the
/// `(key, value)` pairs the event-loop actor layers onto the child's inherited
/// environment — shared by the `btv._system_async` bridge and its stream /
