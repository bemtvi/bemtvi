...
const srv = spawn(process.execPath, [`${here}serve.mjs`, String(PORT)], { stdio: "inherit", env: { ...process.env, BEMTVI_SERVE_ROOT: DEMO_SITE } });
...
try {
  ...
  // The demo init.lua sets `bemtvi-diff` `signs = true`, so a fresh conflict view shows
  // the per-hunk gutter signs — no need to override here.
  await writeOpfs(page, "init.lua", DEMO_INIT);
  await page.reload();
  await page.waitForFunction(() => window.__bemtvi !== undefined, null, { timeout: 20000 });
  await page.evaluate(() => window.__bemtvi.ready);
  const lua = (c) => page.evaluate((x) => window.__bemtvi.execLua(x).then((r) => r.result), c);

  const loaded = await lua(`return tostring(package.loaded["bemtvi-diff"] ~= nil and package.loaded["bemtvi-keys-helper"] ~= nil)`);
  check("demo: bemtvi-diff + keys-helper loaded", /true/.test(String(loaded)), `loaded=${JSON.stringify(loaded)}`);
  ...
  const signs = await page.evaluate(() => {
    const diffPanes = (window.__bemtvi.frame()?.windows || [])
  ...
  await page.evaluate(() => window.__bemtvi.feed("c"));
  await sleep(700);
  const kh = await page.evaluate(() => {
    ...
    return {
      ...
      vpW: window.innerWidth, hasMaps: /choose_ours|choose_both|choose|bemtvi-diff/.test(text),
    };
  });
