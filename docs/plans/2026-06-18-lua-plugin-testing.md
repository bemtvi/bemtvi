# Lua plugin testing: `nx.test` + `nxvim --test-plugin`

Status: in progress (started 2026-06-18)

## Why

nxvim's prime directive is that every feature that can be an `nx.*` Lua plugin
...
harness (`crates/nxvim-test-harness`) — unreachable from a plugin author's Lua
repo. Building `nxvim-keys-helper` surfaced this: the plugin is real, but its
...
- **API:** `nx.test.describe / it / expect`, native to `nx.*` (not a busted/
...
- **Runner:** `nxvim --test-plugin [dir]` (dir defaults to cwd). Discovers
...
- **Scope of first cut:** state assertions *and* Lua UI/redraw inspection
  (floats, messages, statusline), so `nxvim-keys-helper` ships a real suite.
...
nx.test.describe("nxvim-keys-helper", function()
  ...
  nx.test.it("shows the leader menu on pause", function(t)
    ...
    nx.test.expect(t:float().text).to_contain("+file")
...
(`nx._bufs`, `nx._cur_cursor`, …) are refreshed by the server **before each Lua
...
The Lua API therefore runs each `it` body as an **`nx.async` coroutine**, and the
context's driving methods `nx.await` internally:

- `t:feed(keys)` → queue via `nx._feedkeys`, then `nx.await` one tick
  (`nx.on_next_tick`) so the keys drain and the mirrors refresh before the next
  line runs. Deterministic (synchronous) key handling settles in one tick;
  async effects (timers/debounce) need `t:wait_for`.
- `t:wait_for(pred[, opts])` → `nx.await(nx.wait_for(pred, opts))`.
...
context with feed/cmd/exec/wait_for/sleep + state reads), `nx.now_ms()` native,
`crates/nxvim/src/test_runner.rs` + the `--test-plugin` flag, per-test fresh-slate
isolation (`enew!`), and `crates/nxvim/tests/test_plugin_runner.rs` (pass/fail/
isolation coverage). UI reads (`t:float`/`t:message`/`t:statusline`) are present but
read a `nx._ui` mirror that Phase 2 populates.

Original scope:

1. **`nx.test` prelude module** (`crates/nxvim-lua/src/prelude/test.lua`):
   ...
   - `nx.test._run()` → runs every registered test as an async coroutine in
     sequence, captures `{name, status, ms, error}` into `nx.test._results`,
     resets registry. Failures are caught per-test (one bad test ≠ aborted run).
   - Gated to load only in test mode (cheap module, but keep `nx.test` absent in
     normal runs so it can't leak into plugin runtime).
2. **Headless runner** (`crates/nxvim/src/test_runner.rs` + arg in `main.rs`):
   - Parse `--test-plugin [dir]`. Boot a server thread over a duplex + connect
     an RPC client (the harness `spawn`/`attach` pattern, on `nxvim-rpc` so the
   ...
   - Discover `<dir>/test/**/*_spec.lua` (sorted). For each, `exec_lua` to load
     it (registers tests). Then `exec_lua("nx.test._run()")` and poll
     `nx.test._results` to completion (each poll advances a tick; the server's
     own timer loop advances awaits autonomously) with a wall-clock cap.
   - Format the report; exit code from pass/fail counts.
3. **Self-test:** a tiny fixture plugin under `crates/nxvim/tests/fixtures/` plus
...
Landed: `LuaRuntime::set_ui_mirror` populates `nx._ui = { float, message, cmdline,
...
**Gated** behind an `EditHost.test_mode` flag flipped by a new `nx_enable_test_mode`
RPC (the runner sends it after attach), which also installs `nx.test` (kept out of a
normal session via `nx._install_test`) — so neither the API nor the per-redraw mirror
...
pending mapping-prefix between tests). Dogfood: `nxvim-keys-helper/test/popup_spec.lua`
...
1. Server mirrors the projected UI into a Lua-readable snapshot at redraw time
   (or before Lua entry): `nx._ui = { float = { lines, text, title, chunks },
...
3. Dogfood: write `test/popup_spec.lua` (+ group-naming, availability-filter
   cases) in the `nxvim-keys-helper` repo; run `nxvim --test-plugin` green; note
   it in that repo's README.

### Phase 3 — hermetic seams + docs ✅ DONE (2026-06-19)

Landed: `nx.test.clipboard.seed/peek/clear` (op over `Editor::clipboard_seed` +
`nx._ui.clipboard` mirror via `Editor::clipboard_contents`; the runner installs an
in-memory `MemClipboard` in test mode so `"+`/`"*` round-trip), `nx.test.tempdir()`
(`nx._test_tempdir` native), `nx.now_ms()` (phase 1). Author guide:
...
clock for `nx.timer` (real time + `t:wait_for`/`t:sleep` cover debounce/timeout; a
fake timer wheel is a larger change, intentionally not stubbed).

Original scope:

1. Optional Lua seams: `nx.test.clipboard` (seed/peek, over the `"+`/`"*`
...
- **Poll vs. push for results:** start with polling (simplest, advances ticks);
  revisit a `nx_test_done` notification if latency matters.
...
- **`nx.test` visibility:** only register it in test mode to avoid leaking a test
