//! Black-box tests for the native `btv.buf.search` API: plain / pcre / vim lookups
...
async fn an_invalid_pattern_fails_loud() {
    ...
    let code = "local ok, err = pcall(btv.buf.search, 0, '(unterminated')\n\
                return ok";
