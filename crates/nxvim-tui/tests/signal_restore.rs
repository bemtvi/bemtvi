...
const CHILD_ENV: &str = "NXVIM_TUI_SIGNAL_RESTORE_CHILD";
...
fn signal_restore_child() {
    ...
    // What the editor does at startup, in the same order (`nxvim_tui::run`).
    nxvim_tui::install_signal_restore();
...
fn terminal_is_restored_when_the_graceful_path_never_answers() {
    ...
    let killed = run_child("watchdog", &[("NXVIM_EXIT_GRACE_MS", "500")]);
...
fn a_second_signal_kills_immediately_without_waiting_out_the_grace() {
    ...
    let killed = run_child("double", &[("NXVIM_EXIT_GRACE_MS", "600000")]);
