...
/// Convert a neutral [`bemtvi_view::Style`] into the ratatui [`Style`] the renderer
...
fn rt(s: bemtvi_view::Style) -> Style {
...
/// Map a neutral float [`bemtvi_view::Border`] to the ratatui [`BorderType`] used
/// to draw it. `Solid` (neovim's space border) renders as the nearest line style.
fn bt(b: bemtvi_view::Border) -> BorderType {
    match b {
        bemtvi_view::Border::Single => BorderType::Plain,
        bemtvi_view::Border::Rounded => BorderType::Rounded,
        bemtvi_view::Border::Double => BorderType::Double,
        bemtvi_view::Border::Solid => BorderType::QuadrantInside,
    }
}
...
pub(crate) fn render(
    ...
    // The floating selectable-list menu (`btv.ui.select`) floats the same way,
    ...
    let menu_cursor = match (&view.menu, focused_inner) {
        ...
        // no gutter); the `btv.picker` overlay (`editor_relative`) anchors to the
    ...
    // The list-less content float (`btv.ui.float`; LSP hover / signature help). A
...
fn pad_rect(area: Rect, pad: bemtvi_view::Padding) -> Rect {
...
fn render_window(
    ...
    let anim_line_bg: Vec<(u16, bemtvi_view::Style)>;
    ...
    let frame_line_bg: &[(u16, bemtvi_view::Style)];
    let cursor_row: u16;
...
/// Deliberately NOT shared with the GUI (`bemtvi_view::fit`): the TUI measures
...
/// The `hl_mode` wire codes (mirror the server's [`bemtvi_core::HlMode`]):
...
/// (the `btv.dock` dock title), then one ` {count} {name}{+} ` cell per tab (the
...
/// Draw the floating selectable-list menu (`btv.ui.select`) as a bordered overlay
...
fn render_menu(
    ...
    // The picker box's title (`btv.picker.open{ title = … }`), centered on the top
    ...
    // fills the rest. A promptless `btv.ui.select` has neither — the list is the
    ...
    // picker / `btv.ui.select` / completion popup renders exactly as before.
...
/// Render the list-less content float (`btv.ui.float`; LSP hover / signature help):
...
fn render_content_float(
    frame: &mut Frame,
    text_area: Rect,
    float: &bemtvi_view::ContentFloatData,
    styles: &[bemtvi_view::Style],
    theme: FloatTheme,
) {
...
/// `popup` within `text_area`. The geometry itself is [`bemtvi_view::doc_box`] —
