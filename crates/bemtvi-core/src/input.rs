...
//! `btv_input`. [`parse_keys`] turns such a string into a sequence of [`Key`]
...
/// `Key` in a `HashMap` (see `bemtvi-server`'s `keymap.rs`). It is the sole
...
/// argument of neovim's `btv_input_mouse`. `Wheel` is the scroll wheel (its
...
/// What a mouse button did — the `action` argument of `btv_input_mouse`. For an
...
/// A single mouse gesture at a screen cell — bemtvi's analogue of a parsed
...
pub struct MouseEvent {
    ...
    /// any clock itself, keeping `bemtvi-core` time-free and deterministic in tests.
    pub stamp_ms: u64,
}

impl MouseEvent {
    /// Parse the `btv_input_mouse(button, action, modifier, row, col)` arguments
    ...
    /// boundary rather than being silently coerced. `grid` is not taken: bemtvi is
    ...
    pub fn parse(
        ...
        let button = match button {
            ...
            "x1" => MouseButton::X1,
            "x2" => MouseButton::X2,
            other => return Err(format!("btv_input_mouse: unknown button {other:?}")),
        };
        let action = match button {
            MouseButton::Wheel => match action {
                ...
                other => {
                    return Err(format!(
                        "btv_input_mouse: wheel action must be up/down/left/right, got {other:?}"
                    ))
                }
            },
            ...
            _ => match action {
                ...
                other => {
                    return Err(format!(
                        "btv_input_mouse: button action must be press/drag/release, got {other:?}"
                    ))
                }
            },
        };
...
/// Parse the `modifier` argument of `btv_input_mouse` — a run of modifier chars
...
fn parse_mouse_modifier(modifier: &str) -> Result<(bool, bool, bool), String> {
    ...
    for c in modifier.chars() {
        match c.to_ascii_uppercase() {
            ...
            'S' => shift = true,
            '-' => {} // optional separator
            other => return Err(format!("btv_input_mouse: unknown modifier {other:?}")),
        }
    }
    Ok((ctrl, alt, shift))
}
...
/// chord) has no encoding bemtvi can emit — neovim renders those with K_SPECIAL
/// multi-byte sequences bemtvi doesn't model — so it is left as its `<...>`
...
///
/// This is what `vim.fn.getcharstr` / `vim.on_key` hand to Lua: bemtvi has no
...
fn parse_special(inner: &str) -> Option<Key> {
    ...
    let code = match rest.to_ascii_lowercase().as_str() {
        ...
        other => {
            // A single character. bemtvi follows neovim's model: a *modified* key
