...
async fn getcwd(rpc: &bemtvi_rpc::Rpc) -> Option<String> {
...
/// `bemtvi --workspace proj aaa` from a parent cwd: the server cds into `proj` at boot, then
...
async fn workspace_cd_at_boot_resolves_a_relative_file_against_the_root() {
    ...
    // Launch from the parent, as `bemtvi --workspace proj aaa` does: `workspace_dir` is the
