...
// error message (a command error / `:echoerr` / `nx.err_write`) paints the cmdline
...
async function feedAwaitMessage(page, keys, needle) {
  ...
  for (let i = 0; i < 60; i++) {
    const msg = await page.evaluate(() => window.__nxvim.message());
    if (msg.includes(needle)) return msg;
    await sleep(50);
  }
  return await page.evaluate(() => window.__nxvim.message());
}
...
try {
  ...
  let err = await page.evaluate(() => window.__nxvim.messageError());
  ...
  // ---- 2. nx.err_write flags the message red ----
  await feedAwaitMessage(page, ":lua nx.err_write('lua boom')<CR>", "lua boom");
  err = await page.evaluate(() => window.__nxvim.messageError());
  color = await cmdlineColor(page);
  check("nx.err_write: messageError() is true", err === true, `err=${err}`);
  check("nx.err_write: cmdline row painted red", !!color, `color=${color}`);
