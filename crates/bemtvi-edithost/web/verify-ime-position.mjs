...
try {
  ...
  const usingEditContext = await page.evaluate(() => window.__bemtvi.ime().editContext);
  ...
  await page.evaluate(() => window.__bemtvi.feed("ifirst line<CR>second line<CR>third line here<Esc>gg0"));
  await sleep(200);

  let s = await state(page);
  check("cursor cell tagged with #btv-cursor", s.cur !== null, JSON.stringify(s));
  ...
  await page.evaluate(() => window.__bemtvi.feed("jj0fh")); // line 3, onto the 'h' of "here"
  await sleep(200);
  s = await state(page);
  ...
  await page.evaluate(() => window.__bemtvi.feed("<Esc>2G0wi"));
  await sleep(200);
  s = await state(page);
  ...
  await page.evaluate(() => window.__bemtvi.feed("<Esc>ggVGd"));   // clear buffer
  await page.evaluate(() => window.__bemtvi.feed("i"));            // insert mode
  ...
  await sleep(200);
  let lines = await page.evaluate(() => window.__bemtvi.lines());
  ...
  await sleep(200);
  lines = await page.evaluate(() => window.__bemtvi.lines());
  ...
  await page.evaluate(() => window.__bemtvi.feed("<Esc>ggVGd i"));
  await page.evaluate(() => document.getElementById("ime").focus());
