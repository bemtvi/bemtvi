...
/// The line-break convention bemtvi tracks. `LF_CR` recognizes both Unix (`\n`)
...
pub enum BufferKind {
    ...
    /// (`prelude/explorer.lua`, vim's netrw) fills it via `BufReadCmd` + `btv.fs` and
    /// holds it read-only with the `'modifiable'` option, so the core has no
    /// directory-buffer kind.
    #[default]
    Ordinary,
    /// A **plugin-owned view** (`btv.view`) — a plugin-controlled content surface (the
...
pub struct Buffer {
    ...
    /// (`btv._bufs`): the server folds the batch into one replaced line span so the
    ...
    /// `btv.view.create{ name = … }`, surfaced as the buffer name in the statusline and
...
impl Buffer {
    ...
    /// `save_text().ends_with('\n')`.
    ///
    /// bemtvi updates the flag on write; **vim leaves `'eol'` stale** (probed: reading
    ...
    /// Whether the bound file changed on disk since bemtvi last read or wrote it —
...
/// How a buffer's bound file on disk compares to the snapshot bemtvi took when it
...
/// directory's listing from the entries its fetch already read (the `btv.fs` op and the
