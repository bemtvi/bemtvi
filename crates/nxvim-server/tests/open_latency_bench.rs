...
//! tests). Run with:
//!   NXVIM_DATA_DIR=$HOME/.local/share/nxvim \
//!     cargo test -p nxvim-server --test open_latency_bench -- --ignored --nocapture
use nxvim_rpc::{Incoming, Rpc};
use nxvim_server::ServerInit;
use nxvim_test_harness::{feed, start_attached, temp_dir, window0_field};
use rmpv::Value;
...
async fn open_latency() {
    if !nxvim_ts::installed_parsers()
        .iter()
        .any(|p| p.lang == "python")
    {
        eprintln!(
            "skip: python grammar not installed under {} \
             (set NXVIM_DATA_DIR or install it)",
            nxvim_ts::data_dir().display()
        );
        return;
    }
