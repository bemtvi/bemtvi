...
// Wait for window.__bemtvi to exist + be ready (after the initial load and after a reload).
...
try {
  ...
  await page.evaluate(async () => {
    try { await (await navigator.storage.getDirectory()).removeEntry(".bemtvi", { recursive: true }); } catch {}
  });

  // ── Set cross-session state in session #1 ────────────────────────────────────────────
  // A register: type a marker line and yank it (linewise) into register `a`.
  await page.evaluate(() => window.__bemtvi.feed("ggdGiHELLO-REG-LINE<Esc>"));
  await page.evaluate(() => window.__bemtvi.feed('"ayy'));
  // Search history: a distinctive `/` search (matches the buffer, so it's a real search).
  await page.evaluate(() => window.__bemtvi.feed("/HELLO-REG<CR>"));
  await page.evaluate(() => window.__bemtvi.feed("<Esc>"));
  // Ex history: a harmless ex command.
  await page.evaluate(() => window.__bemtvi.feed(":set wrap<CR>"));
  // Plugin shada: a plugin opts in (btv.shada.plugin) and stores a value in its own
  ...
  const pluginSet = await page.evaluate(() =>
    window.__bemtvi
      .execLua('btv.shada.plugin("verify-plugin"):set("greeting", "PLUGIN-PERSISTED")')
      .then((r) => r.result),
  );
  ...
  const flushed = await page.evaluate(() => window.__bemtvi.shadaFlush().then(() => true).catch((e) => String(e)));
  ...
  const blob = await page.evaluate(async () => {
    try {
      const dir = await (await navigator.storage.getDirectory()).getDirectoryHandle(".bemtvi");
  ...
  const freshLines = await page.evaluate(() => window.__bemtvi.lines());
  ...
  await page.evaluate(() => window.__bemtvi.feed("<Up>"));
  const searchLine = await page.evaluate(() => window.__bemtvi.cmdline());
  await page.evaluate(() => window.__bemtvi.feed("<Esc>"));
  check(
  ...
  await page.evaluate(() => window.__bemtvi.feed(":"));
  await page.evaluate(() => window.__bemtvi.feed("<Up>"));
  const exLine = await page.evaluate(() => window.__bemtvi.cmdline());
  await page.evaluate(() => window.__bemtvi.feed("<Esc>"));
  check(
  ...
  const pluginVal = await page.evaluate(() =>
    window.__bemtvi
      .execLua('return tostring(btv.shada.plugin("verify-plugin"):get("greeting"))')
      .then((r) => r.result),
  );
  check(
    "restore: plugin shada survived the reload (btv.shada.plugin get)",
