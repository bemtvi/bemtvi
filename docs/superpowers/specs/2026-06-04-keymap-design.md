...
self.emit_lifecycle_events();`. The merge replaces the bulk with the matcher drive
`main` uses (`self.feed_matcher(key)` → `for step in self.keymaps.feed(mode, key) {
self.apply_step(step); }`), with **two** interceptions kept ahead of it: the
insert-mode popup guard (B5) and the `g`-prefix recognizer (B4).

**B2 — The non-`g` LSP keys become default mappings.** At startup the server installs
defaults through the registry with `default = true` (D6): `K`→`Hover` (normal), and the
...
already exist on the branch (`lsp.rs`); the B2 defaults carry
`Native(Lsp(LspReqKind::Hover))` etc. No key-feeding, so the `<cmd>` caveat never
...
while open, keys navigate/accept/dismiss), *not* a keymap — it stays a separate,
higher-priority insert interception. The ordering contract in insert mode:
1. **if a completion popup is open** → route the key through `completion_menu_key`
   (popup navigation/accept/dismiss); a key it doesn't claim dismisses the menu and
   falls through;
2. **else** → the keymap matcher (`feed`) — where the trigger defaults live;
3. **else** → `editor.input`.

The triggers `<C-Space>`/`<C-x><C-o>`/`<C-k>` are native default maps (B2);
`<C-x><C-o>` is a two-key sequence the matcher handles, so `lsp_pending_ctrl_x`
**retired**. The popup-open guard (step 1) is load-bearing: it consumes navigation keys
