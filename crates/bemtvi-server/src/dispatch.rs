//! The `nvim_*` / `bemtvi_*` RPC surface: the request/notification handler and
...
/// `btv.test.clipboard` seam can seed/peek it. Single-threaded (server thread), so a
/// `RefCell` suffices — `Clipboard` requires only `Send`.
#[derive(Default)]
struct MemClipboard(std::cell::RefCell<Option<(String, bool)>>);

impl bemtvi_core::Clipboard for MemClipboard {
...
impl EditHost {
    ...
    pub(crate) async fn handle(&mut self, message: Incoming) {
        ...
        match message {
            Incoming::Request { id, method, params } => {
                // `bemtvi_image_read` is answered off-tick (a daemon round-trip for a
                ...
                // `nvim_exec_lua` / `btv_command` chunk that fed keys) is processed
                // before the repaint, so the frame reflects the fed keys' effects.
                // `btv_input` already drained its own, so this is a no-op there.
                self.drain_feedkeys();
                self.redraw();
            }
    ...
    pub(crate) fn dispatch(&mut self, method: &str, params: &[Value]) -> Result<Value, String> {
        match method {
            "btv_ui_attach" => {
                ...
                // default in the bundled `bemtvi` colorscheme — a rich terminal reaches
                ...
                // `colors_name` is `"bemtvi"`, so a reconnect's re-attach won't reapply.
            ...
            "btv_ui_try_resize" => {
                // Clamped at the boundary like `btv_ui_attach` (see `geom`).
                let w = geom(params.first(), 80);
                let h = geom(params.get(1), 24);
                self.ui = Some((w, h));
                self.editor.resize(w, h);
            ...
            // `btv_input_mouse(button, action, modifier, grid, row, col)`: a mouse
            // gesture at a global screen cell. bemtvi is single-grid, so `grid`
            ...
            "bemtvi_panel_is_open" => {
                // Whether a focus-locked bottom panel (`:messages` / `:ls` / a scripted
                // `btv.panel.open`) is currently up. Read-only — clients use it for chrome,
            ...
            "nvim_exec_lua" => {
                ...
                // Also refresh the current-buffer snapshot (`btv._cur_buf`), so a getter
            ...
            "btv_enable_test_mode" => {
                ...
                // `btv.test` framework into Lua (absent otherwise) and start mirroring
                // the projected UI into `btv._ui`. Gated here so a normal editor session
                ...
                // btv.test.clipboard seam.
                self.editor.set_clipboard(Box::<MemClipboard>::default());
                self.lua
                    .install_test_api()
                    .map_err(|e| format!("btv_enable_test_mode: {e}"))?;
            ...
            "nvim_buf_get_name" => {
                ...
                // like neovim's `term://…`), a plugin view (`btv.view`) reports its
    ...
    /// Per the no-silent-stub rule, a `relative`/`anchor`/`border` value bemtvi
...
/// (`btv_ui_attach` / `btv_ui_try_resize`), the per-window `nvim_win_set_width` /
/// `_height`, and the `btv_input_mouse` row/col. Such a value flows into the core
/// layout and then directly sizes allocations and bounds row-building loops in
/// `bemtvi-view` (e.g. `Vec::with_capacity(height)` and the `while rows.len() <
...
/// is still `MAX` — so a single hostile or buggy `btv_ui_try_resize(MAX, MAX)`
...
/// `btv._geom.margin`); a 2-array is the literal `[vertical, horizontal]`; a 4-array is
