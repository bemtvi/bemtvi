...
//! store. bemtvi itself reads no project file; the namespace always arrives on the CLI.
//!
...
//! root to Lua through `btv.workspace`. Those two surfaces are covered here too.

use bemtvi_server::{valid_namespace, workspace_namespace, workspace_shada, ServerInit};
use bemtvi_test_harness::{exec_lua, serial_lock, start_attached, temp_dir};
use std::path::Path;
...
async fn store_redirects_into_the_namespace_subfolder() {
    ...
    let ns_dir = state.join("bemtvi").join("shada").join("ns").join(ns);
    ...
    let global_dir = state.join("bemtvi").join("shada");
...
#[tokio::test]
async fn btv_workspace_reflects_the_seeded_identity() {
    // `btv.workspace.dir()` / `.active()` and `btv.shada.namespace()` read the identity the
    ...
    assert_eq!(
        exec_lua(&rpc, "return btv.workspace.dir()").await.as_str(),
        Some("/srv/ada/proj"),
    );
    assert_eq!(
        exec_lua(&rpc, "return btv.workspace.active()")
            .await
            .as_bool(),
        Some(true),
    );
    assert_eq!(
        exec_lua(&rpc, "return btv.shada.namespace()")
            .await
            .as_str(),
        Some("-srv-ada-proj"),
    );
    ...
    assert!(exec_lua(&rpc, "return btv.shada.namespace()")
        .await
        .is_nil());
}
