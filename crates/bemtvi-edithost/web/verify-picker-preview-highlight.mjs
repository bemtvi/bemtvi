...
//
// Faithfulness (not a no-op): the file is seeded through the real btv.fs/OPFS seam, the real
...
try {
  ...
  check("seeded demo.rs under cwd via btv.fs (OPFS)", /Ok\("ok"\)/.test(String(seeded)), `seed=${JSON.stringify(seeded)}`);
  ...
  check("file picker lists the seeded demo.rs (btv.fs walk)", /demo\.rs/.test(items), `items=${JSON.stringify(items)}`);
