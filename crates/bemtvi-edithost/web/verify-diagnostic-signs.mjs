...
// (`btv.diagnostic.set`) run the same render store on every build, so this drives one
...
const set = readStr(
  eh_exec_lua(
    h,
    'local ns = btv.ns.create("diag-test")\n' +
      'btv.diagnostic.set(ns, 0, { { lnum = 0, col = 0, severity = 1, message = "boom" } })\n' +
      "return 1",
  ),
);
check("lua: btv.diagnostic.set ran without error", set.startsWith("ok"), `got ${JSON.stringify(set)}`);
