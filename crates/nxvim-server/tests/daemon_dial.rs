...
//!
//! `connect_daemon` returns a [`DaemonClient`](nxvim_server::DaemonClient) whose seams
...
//! [`run_daemon_io`](nxvim_server::run_daemon_io) over an in-process duplex serving a
//! real temp file, so a seam op that reaches it returns that file's bytes.

use nxvim_server::{connect_daemon, FsRead, HostFsAsync};
use nxvim_test_harness::write_temp;

/// Stand up a real daemon over an in-process duplex and dial it, exactly as the binary's
/// stdio/ssh path does.
fn dial() -> nxvim_server::DaemonClient {
