...
1. **File picker shows nothing.** `btv.picker` `files` shells out to `rg --files`
...
the callback**, leaving `btv.run` / `btv.run_stream` pending forever (a silent
hang, against the no-silent-stubs rule).

## Decisions

- **Prefer `rg`, then `grep`, fall back to an `btv.fs` walk/grep ONLY on the pure
  web client.** The transport-agnostic seam is `btv.fs` (readdir/read_text),
  ...
  case, so the fix lives in Lua (`btv.fs`), not a server-side tokio fetch.
...
- `btv.fs.walk(dir[, opts])`: a recursive, transport-agnostic file enumeration
  (public, reused by the grep phase). Prunes `.git`/dotfiles by default, capped.
- `picker.lua` `files`: a fallback chain `rg --files` → `find` → `btv.fs.walk`. Each
  step runs only when the previous produced nothing; the binaries need a real shell,
  so the pure web client lands on the `btv.fs` walk.
- Tests: native integration test drives `btv.fs.walk` over a temp dir (hermetic,
...
- `live_grep` falls back `rg --vimgrep` → `grep -rnI` → `btv.fs.grep` (a public,
  reusable walk + in-Lua **plain-substring** match). The binaries need a real shell,
  so the pure web client lands on `btv.fs.grep` over OPFS. Each step runs only when the
  previous found nothing. (Performance is a non-issue — the pure-client trees are small
  and the picker caps results.)
- `btv.fs.grep(dir, query[, opts])` added alongside `btv.fs.walk` (both reusable by
  plugin authors).
- **Preview path resolution** (a correctness fix the grep fallback surfaced): `rg`,
  `grep`, and `btv.fs` all emit cwd-RELATIVE paths, but the off-tick fs read carries no
...
Tests: `btv.fs.grep` over a temp dir (hermetic, native LuaFs); a daemon-wire test that a
