...
//
// Flow: spawn a real `nxvim --daemon --listen`, open the page with `?daemon=<uri>`, open a file
// over the wire, then force the link to drop (window.__nxvim.debugDropDaemon — a sleep/network
// blip stand-in) and assert: the local buffer survives, `nx.daemon.status()` goes
...
// (target/debug/nxvim), and a Chromium for Playwright. Run:  node verify-reconnect.mjs
...
// The current `nx.daemon.status()` phase the page reports. `execLua` renders the result in
...
// Poll `nx.daemon.status()` until it reads `want` (or timeout), returning the last phase.
...
const root = mkdtempSync(join(tmpdir(), "nxvim-reconnect-"));
...
try {
  ...
  const opened = await until(page, () => window.__nxvim.lines(), (v) => v === "one");
  ...
  const survived = await page.evaluate(() => window.__nxvim.lines());
  ...
  const reloaded = await until(page, () => window.__nxvim.lines(), (v) => v === "changed during the outage");
  check(
  ...
  await page.evaluate(() => window.__nxvim.feed("Gosaved after reconnect<Esc>"));
