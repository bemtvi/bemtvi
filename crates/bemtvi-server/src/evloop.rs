//! [`EventLoop`]: the async Lua runtime's background actor — bemtvi's event loop.
//!
...
//!
//! Shape mirrors `bemtvi-lsp`'s `LspManager`/`run_supervisor`: a handle that is
...
/// exactly like [`LspManager`](bemtvi_lsp::LspManager)'s command path, so the
...
pub enum LoopCommand {
    ...
    Spawn {
        ...
        stdin: Vec<u8>,
        /// Stream stdout incrementally (`btv.run_stream`'s streamed stdout) rather than
        /// delivering it whole with the exit (`vim.system`). See [`ProcSpec`].
        stream: bool,
        /// Run on the actor's **local** [`HostProc`] instead of the session's — the
        /// `btv.plugins` manager's git, so plugins clone onto the local disk even in a
        /// daemon session. `false` for `btv.run`/`btv.run_stream` (session routing).
        local: bool,
    },
    ...
    /// [`LoopOp::Kill`](bemtvi_lua::LoopOp::Kill)).
    Kill { id: u64 },
    /// Spawn `argv` as a **duplex** child (`btv.process.open`): stdin stays open for
    ...
    /// Open a TCP client connection (`btv.socket.connect`): on success
    ...
    /// armed).
    FsEventStop { id: u64 },
    /// Run an off-tick `btv.fs` op (`btv._fs_op`) through the actor's [`FsBackend`] — local
    ...
    /// — the `btv.plugins` manager's clone / discover / source ops, which must see the local
    /// disk (plugins load into the local Lua VM). `false` for `btv.fs.*` (session routing).
    Fs { id: u64, job: FsJob, local: bool },
    /// Run an off-tick `btv.git.*` op (`btv._git_op`) through the actor's [`GitBackend`] —
    /// `bemtvi_git::run_git_job` on the blocking pool locally, or the daemon's `git_op` leg
    ...
    /// `btv.git_local` twin the plugin manager uses); `false` follows the session routing.
    Git { id: u64, job: GitJob, local: bool },
    /// Run an off-tick `btv.http.fetch` request (`btv._http_fetch`) through the actor's
    ...
    /// `local` forces it onto the actor's LOCAL `ureq` (`btv.http.fetch_local`) even when the
    ...
    /// `btv.http.mount` — publish a plugin's subroute at `/plugin/<name>/*` on the editor's
...
pub enum LoopEvent {
    ...
    /// batching): the Lua side (`btv.process` consumer, e.g. a DAP client) owns the
    ...
    /// failure or a kill). Fires exactly once, after which the `btv.process` handle
    /// is dead.
    ...
    /// re-stats a known buffer); the Lua `btv.fs.watch` surface reads them. `kind` is
    ...
    /// An off-tick `btv.fs` op ([`LoopCommand::Fs`]) settled: the typed result the
    /// promise registered under `id` resolves / rejects with. The server runs the
    /// matching `btv._run_cb` on its one thread (the marshalling to Lua happens there).
    FsResult {
        id: u64,
        result: Result<FsValue, FsError>,
    },
    /// An off-tick `btv.git.*` op ([`LoopCommand::Git`]) settled: the typed result the
    /// promise registered under `id` resolves / rejects with. The server runs the matching
    /// `btv._run_cb` on its one thread (the marshalling to Lua happens there).
    GitResult {
        id: u64,
        result: Result<GitValue, GitError>,
    },
    /// An off-tick `btv.http.fetch` request ([`LoopCommand::Http`]) settled: the typed
    /// result the promise registered under `id` resolves / rejects with. The server runs
    /// the matching `btv._run_cb` on its one thread (the marshalling to Lua happens there).
    HttpResult {
        id: u64,
...
/// Where the actor runs off-tick `btv.fs` ops. One path per session topology — never
...
///   [`StdLuaFs`](bemtvi_lua::StdLuaFs) on the blocking pool (a quick syscall).
...
/// Where the actor runs off-tick `btv.git.*` ops. The git sibling of [`FsBackend`],
/// but simpler: [`run_git_job`](bemtvi_git::run_git_job) discovers the repo from the
/// job's path itself, so the local variant carries no per-op state.
/// - [`Local`](GitBackend::Local) — native-bare / the `btv.git_local` twin: run the job
...
/// Where the actor runs off-tick `btv.http.fetch` requests. The HTTP sibling of
...
///   the network — the same reason `btv.fs` / processes route there). An `await`, not a park.
#[derive(Clone)]
        ...
        match self {
            HttpBackend::Local => {
                tokio::task::spawn_blocking(move || crate::http::run_http_request(&request))
                    ...
                    .unwrap_or_else(|e| {
                        Err(HttpError {
                            message: format!("btv.http: request task failed: {e}"),
                        })
                    })
            }
...
pub struct EventLoop {
    ...
    /// Where off-tick `btv.fs` ops run — local disk on the blocking pool, or the daemon's
    /// `luafs_op` leg (see [`FsBackend`]). Cloned into the actor when it starts. Preserves
    /// the lazy start — a session that never touches `btv.fs` (or a timer / process) spawns
    /// nothing.
    fs: FsBackend,
    /// Where off-tick `btv.http.fetch` requests run — a local `ureq` round-trip, or the
    ...
    /// [`Spawn`](LoopCommand::Spawn) / [`Fs`](LoopCommand::Fs) ops — the `btv.plugins`
    ...
    local_fs: FsBackend,
    /// Where off-tick `btv.git.*` ops run (see [`GitBackend`]). `git` follows the session
    /// routing; `local_git` is the `btv.git_local` twin (the plugin manager's repos, always
    /// on local disk). Cloned into the actor when it starts.
    git: GitBackend,
    local_git: GitBackend,
    /// Where a streaming `btv.fs.watch` is armed: `None` is the local disk (this machine's
    ...
    /// watch has no `local`-flagged twin (`btv.fs.watch` has no plugin-manager caller).
...
/// the daemon's legs) plus the **local** twins the `btv.plugins` manager needs even in a
...
pub struct LoopBackends {
    /// Where child processes are spawned (`btv.run`, `:!`, `vim.system`).
    pub host_proc: Arc<dyn HostProc>,
    /// Where off-tick `btv.fs` ops run.
    pub fs: FsBackend,
    /// Where `btv.http.fetch` requests run.
    pub http: HttpBackend,
    /// Where `btv.git.*` ops run.
    pub git: GitBackend,
    /// Where a streaming `btv.fs.watch` is armed: `None` = this machine's `notify` backend,
    ...
    /// `btv.plugins` manager's git + discovery, which stay on the local disk even in a
...
async fn run_evloop(
    ...
    // The still-open stdin sinks of live duplex children (`btv.process.open`), keyed
    ...
    // Live TCP connections (`btv.socket`): a close signal + the write sink, keyed by
    ...
    // The `btv.http.mount` routes + listener. Cheap to hold: it binds nothing until the first
    ...
    while let Some(cmd) = cmd_rx.recv().await {
        ...
        // opens and closes many `btv.process` children doesn't accumulate dead
        ...
        match cmd {
            ...
            LoopCommand::FsEventStart {
                ...
                // event per change — its consumer re-stats. A Lua `btv.fs.watch` (id <
                    ...
                    FsBackend::Local(lua_fs) => {
                        ...
                        tokio::spawn(async move {
                            ...
                            .unwrap_or_else(|e| {
                                ...
                                Err(FsError {
                                    code: "EIO".to_string(),
                                    message: format!("btv.fs op task failed: {e}"),
                                })
                            });
            ...
            LoopCommand::Git { id, job, local } => {
                ...
                // `btv.git_local` twin) always takes the local backend. The daemon `Remote`
                ...
                match backend {
                    GitBackend::Local => {
                        tokio::spawn(async move {
                            let result =
                                tokio::task::spawn_blocking(move || bemtvi_git::run_git_job(&job))
                                    .await
                                    .unwrap_or_else(|e| {
                                        ...
                                        Err(GitError {
                                            code: "EIO".to_string(),
                                            message: format!("btv.git op task failed: {e}"),
                                        })
                                    });
            ...
            LoopCommand::Http { id, request, local } => {
                ...
                // (`btv.http.fetch_local`) forces the local `ureq` even in a daemon session.
                let event_tx = event_tx.clone();
...
/// Arm a native filesystem watch for the Lua `btv.fs.watch` surface: like
...
/// wants a longer settle composes `btv.utils.debounce` on top.)
///
...
/// difference between LSP's `Created` and `Changed` (`btv.lsp._register_capability`'s
...
/// The `btv.fs.watch` change class for a `notify` [`EventKind`]: `"create"` /
