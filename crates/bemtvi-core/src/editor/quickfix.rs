...
//! engine itself rides on [`bemtvi_regex`] (vim's vendored `regexp.c`) and so lives
...
/// The most lists vim (and bemtvi) keep in a quickfix/location-list stack; older
...
pub struct NamedList {
    /// The stable name this list is addressed by (the `btv.qf.show(name)` key), and
...
pub enum QfWhich {
    ...
    /// (`btv.qf.show(name)`). Quickfix-flavored but a distinct instance per name.
    Named(NamedListId),
}
...
impl Editor {
    ...
    /// Open or focus the named list `name`'s bottom-dock tab (`btv.qf.show`) — the
    ...
    /// Forget the named list `name` (`btv.qf.drop`): close its dock tab if shown, drop
    ...
    /// `btv.qf.drop` for a `kind = "list"` dynamic list.
    ...
    /// `buftype` option reports. bemtvi models the kinds it actually distinguishes:
    ...
    /// non-file scratch surface (a plugin `btv.view`, a built-in panel/listing like
    ...
    /// scratch buffer. This is what `btv.decor`'s `bufs.buftype` filter keys off (so a
    ...
    pub fn buffer_buftype(&self, buf: BufferId) -> &'static str {
        if self.qf_context_of_buffer(buf).is_some() {
            ...
            // A surface that exists but isn't a listed document — an `btv.view`, a
    ...
    pub(crate) fn qf_refresh_window(&mut self, which: QfWhich) {
        ...
        let name = match which {
            ...
            // (`btv.qf.list{ title }`), falling back to the list's name.
            QfWhich::Named(id) => {
    ...
    /// the bemtvi way (`'qfdock'`). The first such list opens the dock; subsequent
    ...
    /// `btv.qf.{send,add}_to_{loc,qf}list` (the bemtvi port of telescope's send/add-to-list
    /// actions).
    ///
...
impl Editor {
    ...
    pub(crate) fn ex_vimgrep(&mut self, which: QfWhich, args: &str, action: QfAction) {
        ...
        // predicate (`crate::glob::is_glob`, the same one `btv.glob.is_glob` exposes),
...
fn qf_type_hl(typ: u8) -> Option<&'static str> {
    match typ {
        ...
        // `N` is vim's "note"; bemtvi's diagnostics map HINT → `N`.
...
mod engine {
    use super::QfEntry;
    use bemtvi_regex::{Engine, PatternKind, VimRegex};
    use std::path::Path;
    ...
    const CONV: [u8; 14] = *b"fbnlecktmrpvso";
