...
//!
//! `bemtvi-core` defines the *interface* — and the plain data it exchanges — for
//! an in-process treesitter backend; the implementation lives in `bemtvi-ts`.
...
pub trait SyntaxEngine {
    ...
    /// native `btv.treesitter.foldexpr`. Empty when there is no grammar, no
