...
//! `btv_input` and asserts on the buffer the child's output is mirrored into. PTY
...
async fn alternate_screen_fills_the_window_after_scrolling() {
    ...
    let script = r#"btv.terminal.open{ cmd = {'sh','-c',
...
/// bemtvi answers terminal status queries (like a real terminal) so apps that probe
...
/// its output; bemtvi must write back a CPR (`ESC[row;colR`), which `cat` then echoes,
...
/// Phase 5 — the `btv.terminal` Lua control surface. `btv.terminal.open{ cmd = ... }`
...
#[tokio::test]
async fn btv_terminal_open_runs_a_command_programmatically() {
    let _guard = serial_lock().lock().await;
    let (rpc, _incoming) = start().await;

    exec_lua(&rpc, "btv.terminal.open{ cmd = 'cat' }").await;
    assert_eq!(
        mode(&rpc).await,
        "t",
        "btv.terminal.open enters terminal mode"
    );
    feed(&rpc, "hi<CR>");
    wait_lines(&rpc, "cat to echo 'hi'", |ls| has_line(ls, "hi")).await;
}
...
#[tokio::test]
async fn btv_terminal_open_list_cmd_preserves_argument_spaces() {
    let _guard = serial_lock().lock().await;
    let (rpc, _incoming) = start().await;

    exec_lua(
        &rpc,
        r"btv.terminal.open{ cmd = {'printf', 'hello world\n'} }",
    )
    .await;
...
async fn cursor_keys_use_application_mode_when_the_child_enables_it() {
    ...
    exec_lua(
        &rpc,
        r#"btv.terminal.open{ cmd = {'sh', '-c', 'printf "\\033[?1h"; exec cat -v'} }"#,
    )
    .await;
...
/// `btv.terminal.open{ cwd = ... }` starts the child in the requested directory.
#[tokio::test]
async fn btv_terminal_open_respects_an_explicit_cwd() {
    let _guard = serial_lock().lock().await;
    let (rpc, _incoming) = start().await;

    let dir = temp_dir("btv_term_cwd");
    ...
    exec_lua(
        &rpc,
        &format!("btv.terminal.open{{ cmd = 'pwd', cwd = {dir:?} }}"),
    )
    .await;
...
async fn terminal_inherits_the_parent_environment() {
    ...
    std::env::set_var("BEMTVI_TERM_ENV_TEST", "hello-from-parent");
    let (rpc, _incoming) = start().await;

    command(&rpc, "terminal printenv BEMTVI_TERM_ENV_TEST").await;
...
/// `:terminal` advertises a `TERM`/`COLORTERM` that describe **bemtvi's own vt100 emulator**
/// — not whatever the editor happened to inherit. bemtvi renders the child into its own
...
async fn terminal_advertises_its_emulator_term_over_a_bogus_ambient_one() {
    ...
    // `env` prints every variable as `KEY=value`; the child must see bemtvi's values, not
    // the bogus `unknown` / absent ones it would have inherited.
    command(&rpc, "terminal env").await;
    wait_lines(
        &rpc,
        "the child to see bemtvi's emulator TERM/COLORTERM",
...
async fn ctrl_c_trims_a_flooding_terminal_and_marks_the_cut() {
    ...
    exec_lua(
        &rpc,
        r#"btv.terminal.open{ cmd = {'sh','-c','seq 1000; trap "" INT; cat'} }"#,
    )
    .await;
...
async fn deleting_a_live_terminal_buffer_kills_the_child() {
    ...
    exec_lua(
        &rpc,
        "btv.terminal.open{ cmd = {'sh', '-c', 'echo $$; exec sleep 600'} }",
    )
    .await;
