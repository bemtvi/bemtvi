-- ~~~ nxvim nx.http playground: a fetch-modeled, promise-only HTTP client ~~~
--
-- Run it (from the repo root; needs network access for the demo endpoints):
--
--     NXVIM_CONFIG=examples/nx-http \
--       cargo run -p nxvim -- examples/nx-http/sample.txt
--
-- `nx.http.fetch(url[, opts])` is modeled on the browser's `fetch`: it returns a
...
--
-- The same API works everywhere nxvim runs: natively it's a `ureq` round-trip off the
...
--    chain it with `:next` (or await it inside `nx.async`, below).
--------------------------------------------------------------------------------
nx.keymap.set("n", "<leader>h", function()
  show("GET https://example.com …")
  nx.http.fetch("https://example.com")
    :next(function(res)
...
--    field out with `res:json()`. Shown with the `nx.async` / `nx.await` style, which
--    reads like straight-line code.
--------------------------------------------------------------------------------
nx.keymap.set("n", "<leader>j", nx.async(function()
  show("GET api.github.com/search …")
  local ok, res = pcall(nx.await, nx.http.fetch("https://api.github.com/search/repositories", {
    query = { q = "language:rust stars:>10000", per_page = 1 },
    headers = { ["User-Agent"] = "nxvim-nx-http-example" },
  }))
  if not ok then
...
nx.keymap.set("n", "<leader>p", function()
  ...
  nx.http.fetch("https://httpbin.org/anything", {
    method = "POST",
    body = { editor = "nxvim", feature = "nx.http" },
  })
    :next(function(res)
      if res.ok then
...
nx.keymap.set("n", "<leader>x", function()
  ...
  nx.http.fetch("http://127.0.0.1:1/nope", { timeout = 2000 })
    :next(function(res)
...
nx.keymap.set("n", "<leader>r", function()
  ...
  nx.http.fetch("https://httpbin.org/redirect/1", { redirect = "manual" })
    :next(function(res)
...
--------------------------------------------------------------------------------
-- 6. <leader>l — LOCAL fetch. `nx.http.fetch_local` is identical to `nx.http.fetch`
--    but always runs on THIS machine's network (never routed to a daemon in a remote
--    session) — the HTTP analogue of the plugin manager's local-only `nx.fs`. In a
--    plain local session it behaves exactly like `\h`.
--------------------------------------------------------------------------------
nx.keymap.set("n", "<leader>l", function()
  show("GET https://example.com (forced local) …")
  nx.http.fetch_local("https://example.com")
    :next(function(res)
...
show("nx.http demo loaded — try  \\h  \\j  \\p  \\x  \\r  \\l  (needs network)")
