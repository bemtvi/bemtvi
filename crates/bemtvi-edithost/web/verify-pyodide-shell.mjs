...
async function readBuf(page) {
  const raw = await page.evaluate((c) => window.__bemtvi.execLua(c).then((r) => String(r.result)), READ_BUF);
...
try {
  ...
  const run = async (line, re) => {
    await page.evaluate((k) => window.__bemtvi.feed(k), `${line}<CR>`);
    return waitFor(page, re);
  };

  // Open the shell (bare :terminal). First run loads Pyodide.
  await page.evaluate(() => window.__bemtvi.feed(":terminal<CR>"));
  const banner = await waitFor(page, /\/ \$ $/m);
  check("shell: opens with a prompt after Pyodide loads", /bemtvi shell/.test(banner), `buf=${JSON.stringify(banner.slice(-80))}`);
