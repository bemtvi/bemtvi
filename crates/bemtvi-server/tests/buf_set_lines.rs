//! Black-box tests for the native `btv.buf.set_lines` API (alias `nvim_buf_set_lines`):
...
async fn set_and_read(rpc: &Rpc, args: &str) -> String {
    exec_lua(rpc, &format!("btv.buf.set_lines(0, {args})")).await;
    lines(rpc).await
}
...
async fn the_edit_marks_the_buffer_modified() {
    let (rpc, _inc) = open("a\nb\n").await;
    exec_lua(&rpc, r#"btv.buf.set_lines(0, 0, 1, false, { "A" })"#).await;
    assert_eq!(
...
async fn the_promise_resolves_after_the_edit_is_visible() {
    ...
    exec_lua(
        ...
        r#"btv.async(function()
             btv.await(btv.buf.set_lines(0, 1, 2, false, { "B" }))
...
async fn a_nomodifiable_buffer_fails_loud() {
    ...
    let ok = exec_lua(
        &rpc,
        r#"return (pcall(btv.buf.set_lines, 0, 0, 1, false, { "X" }))"#,
    )
    .await;
...
async fn a_non_string_replacement_line_fails_loud() {
    ...
    let ok = exec_lua(
        &rpc,
        "return (pcall(btv.buf.set_lines, 0, 0, 1, false, { 42 }))",
    )
    .await;
    assert_eq!(ok.as_bool(), Some(false));
}
