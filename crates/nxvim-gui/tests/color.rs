...
//! on desaturated chrome but glaring on a saturated nxvim-line mode block (a blue
//! NORMAL fill rendered orange, while green/purple — with R == B — looked fine).

use nxvim_gui::{color_to_rgba, srgb_to_color, srgb_to_color_rgba};
...
fn quad_conversion_preserves_channel_order() {
    ...
    // completely different hue (the nxvim-line NORMAL block bug).
    assert_paths_agree(0x5f_af_ff); // nxvim-line NORMAL blue
