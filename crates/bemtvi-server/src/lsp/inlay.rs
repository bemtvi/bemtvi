...
use bemtvi_lsp::lsp_types::{Position, Range};
use bemtvi_lsp::{InlayHintData, ServerKey};
use bemtvi_lua::InlayHintMirrorData;
use rmpv::Value;
...
/// [`InlayHintMirrorData`] list the `btv._inlay_hints` mirror holds (one entry per
...
impl EditHost {
    ...
    pub(crate) fn request_inlay_hints(&mut self, buffer: BufferId) {
        ...
        for (key, _enc) in self.lsp_capable_servers(buffer, LspReqKind::InlayHints) {
            ...
            self.fx.lsp_request(
                key,
                token,
                bemtvi_lsp::LspRequest::InlayHint {
                    uri: uri.clone(),
                    range,
                },
            );
    ...
    pub(crate) fn on_inlay_hints_reply(
        ...
        // Push the read mirror (`btv._inlay_hints`) and then resolve any lazy
    ...
    /// Rebuild `btv._inlay_hints[bufnr]` from every attached server's cache, each
    ...
    fn issue_inlay_resolves(&mut self, buffer: BufferId, server_key: &ServerKey, tick: u64) {
        ...
        let mut jobs: Vec<(usize, usize, bemtvi_lsp::serde_json::Value)> = Vec::new();
        ...
        for (line, idx, hint) in jobs {
            ...
            self.fx.lsp_request(
                server_key.clone(),
                token,
                bemtvi_lsp::LspRequest::ResolveInlayHint { hint },
            );
        }
    }
