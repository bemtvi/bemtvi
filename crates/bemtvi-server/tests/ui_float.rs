//! Behavior tests for `btv.ui.float` — the list-less **content float**, the
//! sibling of the selectable-list widget on the shared float layer
//! (`docs/specs/2026-06-14-btv-ui-float-widget.md`, "What stays out of this
//! widget").
//!
...
async fn float_projects_its_content_lines_and_border() {
    ...
    exec_lua(
        &rpc,
        "btv.ui.float({ 'alpha', 'beta', 'gamma' }, { border = 'rounded' })",
    )
    .await;
...
async fn styled_chunk_lines_carry_per_segment_highlights() {
    ...
    exec_lua(
        &rpc,
        "btv.hl.define(0, 'FloatKey', { fg = '#7dcfff', bold = true })\n\
         btv.hl.define(0, 'FloatDesc', { fg = '#565f89' })\n\
         btv.ui.float({ { { 'w', 'FloatKey' }, { '  write', 'FloatDesc' } } }, { border = 'none' })",
    )
    .await;
...
async fn the_next_key_dismisses_the_float() {
    ...
    exec_lua(&rpc, "btv.ui.float({ 'transient' }, {})").await;
...
async fn a_lua_mapped_key_also_dismisses_the_float() {
    ...
    exec_lua(
        &rpc,
        "btv.keymap.set('n', 'x', function() _G.hit = (_G.hit or 0) + 1 end)",
    )
    .await;
    exec_lua(&rpc, "btv.ui.float({ 'transient' }, {})").await;
    poll_float(&rpc, &mut incoming, |f| matches!(f, Value::Map(_)))
...
async fn a_persistent_float_survives_keystrokes() {
    ...
    // key (the which-key shape: it observes keys via btv.on_key while staying open).
    exec_lua(
        &rpc,
        "_G.wk = btv.ui.float({ 'pending: f g h' }, { persist = true })",
    )
    .await;
    poll_float(&rpc, &mut incoming, |f| matches!(f, Value::Map(_)))
        .await
...
async fn update_replaces_the_persistent_float_in_place() {
    ...
    exec_lua(
        &rpc,
        "_G.wk = btv.ui.float({ 'first' }, { persist = true })",
    )
    .await;
    poll_float(&rpc, &mut incoming, |f| matches!(f, Value::Map(_)))
        .await
...
async fn close_dismisses_the_persistent_float() {
    ...
    exec_lua(&rpc, "_G.wk = btv.ui.float({ 'open' }, { persist = true })").await;
    poll_float(&rpc, &mut incoming, |f| matches!(f, Value::Map(_)))
        .await
        .expect("the float opens");
...
async fn editor_placement_centers_the_float() {
    ...
    exec_lua(
        &rpc,
        "btv.ui.float({ 'centered content here' }, { relative = 'editor' })",
    )
    .await;
...
async fn split_keeps_editor_floats_over_the_whole_editor() {
    ...
    exec_lua(
        &rpc,
        "btv.ui.float({ 'which-key here' }, { relative = 'bottom' })",
    )
    .await;
    let map = poll_float(&rpc, &mut incoming, |f| matches!(f, Value::Map(_)))
        .await
...
async fn unknown_border_is_rejected_loud() {
    ...
    exec_lua(&rpc, "btv.ui.float({ 'x' }, { border = 'bogus' })").await;
...
/// The `btv.component` "float" backend: the Vue-shaped component model rendered onto a
