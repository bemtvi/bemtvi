...
try {
  ...
  await page.evaluate(() => window.__bemtvi.feed("iline1<CR>line2<CR>line3<Esc>"));
  let cur = await page.evaluate(() => window.__bemtvi.cursor());
  ...
  cur = await page.evaluate(() => window.__bemtvi.cursor());
  ...
  await page.evaluate(() => window.__bemtvi.feed("G")); // back to the last line
  cur = await page.evaluate(() => window.__bemtvi.cursor());
  check("setup: back on the last line", cur && cur.row === 2, JSON.stringify(cur));
  await page.evaluate(() => window.__bemtvi.execLua("vim.o.timeout = false"));
  ...
  await page.evaluate(() => window.__bemtvi.feed("h"));
  const ggh = await page.evaluate(() => window.__bemtvi.execLua("return _G.GGH"));
