# `btv.fs` off-tick + the daemon `luafs` leg over WebTransport
...
>   `glob`) and `btv._readdir` were **removed outright**, not given async parity — there is
>   no point keeping `vim.fn.*` sugar that can't behave like neovim's synchronous version.
>   `btv.fs` (async) is the sole fs API; `btv.lsp.enable`'s `find_root` walks it directly.
> - **The low-level per-op `luafs` leg + `RemoteLuaFs` are GONE.** They only existed to
>   back those sync builtins. Native-daemon `btv.fs` now uses the **`luafs_op`** leg (whole
...
Status: **Phases 1–3b done (2026-06-16/17); Phase 3c (the `vim.fn` / `btv._readdir`
async-parity tail) is moot — those sync builtins were removed, not made async (see the
banner above).** `btv.fs` now works in every world — native, native daemon,
browser+daemon, and serverless browser (OPFS) — including the streaming
`btv.fs.watch` over the daemon. Makes `btv.fs` (and the `LuaFs` seam under it)
...
[`btv.fs` API plan](2026-06-16-btv-fs-api.md)'s "Plumbing" posture (one-shot ops run
*synchronously inline* — that plan already named this as the planned next step:
*"Offloading a heavy op to the loop actor à la `btv._system_async` — fully off-tick
— remains a future optimization … the promise surface already permits it without
an API change."*).

## Problem

`btv.fs.*` one-shot ops are **promise-shaped but not actually async**: each
`btv._fs_*` bridge calls `resolve_lua_fs(&sh).<op>()` **inline on the editor thread**
and wraps the already-computed result in a pre-resolved promise
(`crates/bemtvi-lua/src/install.rs` `_fs_*`; `crates/bemtvi-lua/src/prelude/fs.lua`
...
2. **On the browser it can't reach the daemon at all.** `resolve_lua_fs` defaults
   ...
   `btv.fs` hits emscripten's in-memory **MEMFS**, not the host. A synchronous
...
The fix for (2) **is** the fix for (1): route `btv.fs` through the **off-tick**
...
`vim.system` / `btv.run` is the exact dual-path shape to mirror — an off-tick op
...
`btv.fs` gets the identical treatment, with a `FsJob` op-descriptor in place of the
...
### The op descriptor + result (shared, `bemtvi-lua/src/ops.rs`)
...
`+ Send`), and the construction sites (`main.rs`, `bemtvi-gui/session.rs`,
`daemon.rs`). The synchronous editor-thread callers (`vim.fn` fs builtins,
`host.rs::glob_paths`, the `btv._readdir`/`btv._read_file` legacy bridges) keep
...
`btv.fs` API); only `btv.fs` goes off-tick.
>
...
`btv.fs.x()` → `fs.lua` allocates a `cb_id`, registers the promise's
resolve/reject in `btv._cb_fns`, and calls `btv._fs_op(job, cb_id)` which pushes
...
already models. Net: a browser Lua plugin's `btv.fs.readdir(remote_path)` returns the
daemon's listing.

### `fs.lua` rewrite

`settle()` stops running the bridge inline. Each op becomes the
`btv.ui.input`/`btv.run` shape: build a promise, `btv._next_cb_id()`, store
`resolve`/`reject` (the bridge fires `btv._run_cb(id, false, err, value)` — `err` the
`{code,message}` table or nil), and call `btv._fs_op(<job table>, id)`. `exists`
keeps its never-reject contract (maps an `ENOENT` reject to `resolve(false)` in the
wrapper). **No `btv.fs.*` surface change** — `btv.await`/`:next`/`:catch` are
...
- **Phase 1 — native off-tick (foundation, hermetic). DONE (2026-06-16).**
  `FsJob`/`FsValue`/`FsError` (`bemtvi-lua/src/ops.rs`) + `LoopOp::Fs` +
  ...
  one `btv._fs_op(job, cb_id)` bridge replacing the 13 inline `btv._fs_*` bridges;
  ...
  `btv._readdir`/`_read_file` stay inline (Arc derefs like Rc). The fs handle is
...
- **Phase 2 — the wasm `luafs` leg over WebTransport (the goal). DONE (2026-06-16).**
  A browser `btv.fs.*` op now routes to the connected daemon and back. Pieces:
  - **Wire codec** (`bemtvi-lua/src/fswire.rs`): `FsJob`/`FsValue`/`FsError` ⟷
  ...
  - **Daemon leg** (`daemon.rs`): a new `luafs_op` method served on the **same**
    ...
    decodes the `FsJob` and runs the **whole** [`run_fs_job`](../../crates/bemtvi-lua/src/luafs.rs)
  ...
  - **Seam** (`edithost.rs` + `effects.rs` + `lib.rs`): wasm-only
    ...
    serverless-OPFS `btv.fs` route is Phase 3 — never a silent MEMFS hit). Inbound:
  ...
  - **Wasm FFI** (`bemtvi-edithost`): a `Sink.fs_ops` queue, `WasmEffects::fs_op`,
  ...
  - **The choice the plan flagged**: gated on **daemon-connected** (`has_remote_proc`,
    ...
    `true` on wasm — OPFS) — gating on the latter would have hung serverless `btv.fs` on
    a wire with no daemon. Serverless routing stays Phase 3.
  - **Test** (`web/verify-fs-op.mjs`, Playwright against a real `bemtvi --daemon`):
...
- **Phase 3a — serverless-OPFS `btv.fs`. DONE (2026-06-17).** The decision was
  *route to OPFS* (not keep failing loud): a serverless browser `btv.fs.*` op now runs
  against the Origin Private File System — the same sandbox `:e`/`:w` persist to — so a
  plugin's `btv.fs` works in the common no-daemon mode, not just with a daemon. Pieces:
...
- **Phase 3b — `btv.fs.watch` over the daemon wire. DONE (2026-06-17).** A browser
  `btv.fs.watch` now streams daemon-side changes; daemon-only (serverless OPFS has no
  ...
  - **Daemon leg** (`serve_luafs_watch_daemon_on`): a `select!` loop owning a per-stream-
    ...
    `btv.fs.watch` rides — and forward its `LoopEvent::FsEvent` as `luafs_change [id, kind,
  ...
  - **Seam** (`edithost.rs`/`effects.rs`/`lib.rs`): wasm-only
    ...
    `fs_watch_error(id, message)` drive the stream's `btv._run_fs_watch` pump.
  ...
  - **Native edit-host** (added later, closing the native-daemon half): `RemoteFsWatch`
    ...
    **re-arms** every live watch on a re-dial (a fresh daemon knows about none of them) —
    ...
    Tests: `crates/bemtvi-server/tests/daemon_fs_watch.rs`.
  - **Test** (`web/verify-fs-watch.mjs`, Playwright + real daemon): a file Node creates
    ...
    spawns `target/debug/bemtvi` — a daemon-side server change needs `cargo build -p
    bemtvi`, not just the lib, or the spawned daemon is stale and drops the new method.)
- **Phase 3c — MOOT (resolved 2026-06-17, not as written).** Rather than route the
  `vim.fn` fs builtins / `btv._readdir` through the async path *or* leave them sync, they
  were **removed entirely** ("no blocking IO at all" — see the top banner and
  [[no-blocking-io-fs-async-only]]). `btv.fs` is the only fs API; the one in-tree consumer,
  `btv.lsp.enable`'s `find_root`, became async on `btv.fs.readdir`.

## Risks / decisions to confirm

- **Ordering.** Off-tick ops resolve in completion order, not call order. `btv.fs` is
...
- **`vim.fn` fs stays sync.** Keeping `isdirectory`/`glob`/… inline means browser
  `vim.fn` fs still hits MEMFS. Acceptable: `btv.fs` is the supported async surface;
