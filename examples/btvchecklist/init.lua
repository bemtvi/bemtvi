-- ~~~ bemtvi btv.view.component playground: a modal floating checkbox dialog ~~~
--
-- Run it (from the repo root):
--
--     BEMTVI_CONFIG=examples/btvchecklist cargo run -p bemtvi
--
-- This is the use case `btv.view` floats were built for — a centered floating checkbox
-- list you navigate with <Tab>, toggle with <Space>, and confirm/cancel — written with
-- `btv.view.component`, the Vue-shaped component model:
--
--   * setup(ctx, props)  runs ONCE: it makes reactive state and binds keys. (It may be
--                        async — `btv.await` a fetch right inside it. See ASYNC below.)
--   * render(state)      is PURE: state -> { lines, decor }. The framework re-runs it
--                        automatically every time the reactive state changes.
--
-- Note what is GONE versus driving the raw handle: no `btv.schedule` to wait for the
...
btv.hl.define(0, "BtvChecklistOn", { fg = "#a6e3a1" })
btv.hl.define(0, "BtvChecklistHint", { fg = "#6c7086" })

local Checklist = btv.view.component({
  setup = function(ctx, props)
    ...
    -- ctx.keymap_set mirrors btv.keymap.set(mode, lhs, rhs, opts), with buffer + nowait
  ...
  render = function(view)
    ...
    for i, it in ipairs(view.items) do
      ...
      if it.checked then
        decor[#decor + 1] =
          { line = i - 1, col = 0, end_row = i - 1, end_col = 3, hl_group = "BtvChecklistOn" }
      end
    end
    lines[#lines + 1] = ""
  ...
  Checklist.mount({
    ...
    props = {
      ...
      on_done = function(chosen)
        if chosen == nil then
          _G.btvchecklist_result = "<cancelled>"
          btv.notify("checklist cancelled", 2)
        else
          _G.btvchecklist_result = table.concat(chosen, ", ")
          btv.notify("enabled: " .. _G.btvchecklist_result, 2)
        end
      end,
    },
  })
...
btv.keymap.set("n", "<leader>c", demo, { desc = "Open the checklist dialog" })
...
--
--   setup = function(ctx, props)
--     local entries = btv.await(btv.fs.readdir(props.path))   -- suspends; resumes with data
