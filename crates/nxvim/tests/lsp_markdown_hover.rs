...
//!
//! Wired like `lsp_config.rs`: the mock (`nxvim --__lsp-mock`) stands in via
//! `$NXVIM_LSP_CMD` (process-global env ⇒ `serial_lock`), driven by a `rust`-filetype
//! buffer.
...
/// Retry `nx.lsp.hover()` until the float window carries a line containing `want`,
