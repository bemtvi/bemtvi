...
async fn sleep_blocks_the_editor_for_the_requested_duration() {
    ...
    rpc.request("btv_command", vec![Value::from("sleep 150m")])
        .await
...
async fn wo_wrap_funnel_actually_wraps_not_just_stores() {
    // `vim.wo.wrap = true` (the `btv.wo` funnel a plugin / component uses, e.g.
    ...
    // observable-only `btv._wo_store`, so the read-back said `true` while the display
