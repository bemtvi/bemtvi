...
try {
  ...
  await page.evaluate(() => window.__bemtvi.feed(":e /broken.png<CR>"));
  const offFrame = await until(
    page,
    () => { const w = (window.__bemtvi.frame()?.windows || []).find((x) => x.focused); return w ? { img: !!w.image, lines: window.__bemtvi.lines() } : null; },
  ...
  await page.evaluate(() => window.__bemtvi.feed(":set imagepreview<CR>"));
  await page.evaluate(() => window.__bemtvi.feed(":e /viaset.png<CR>"));
  const viaSet = await until(
    page,
    () => { const w = (window.__bemtvi.frame()?.windows || []).find((x) => x.focused); return w && w.image ? { path: w.image.path } : null; },
    (v) => v != null,
  );
  check(
    "`:set imagepreview` enables previews (the ex-command path), not just btv.o",
    viaSet != null && /viaset\.png$/.test(viaSet.path),
    `viaSet=${JSON.stringify(viaSet)}`,
  );

  // Keep the option on via the canonical btv.* surface for the remaining checks.
  await page.evaluate(() => window.__bemtvi.execLua("btv.o.imagepreview = true"));

  // ── 2. Option ON: opening the PNG carries the marker AND an empty buffer ──────────────
  await page.evaluate(() => window.__bemtvi.feed(":e /sample.png<CR>"));
  const marker = await until(
    page,
    () => {
      const w = (window.__bemtvi.frame()?.windows || []).find((x) => x.focused);
      return w && w.image ? { path: w.image.path, size: w.image.size, lines: window.__bemtvi.lines() } : null;
    },
    (v) => v != null,
  );
  check(
  ...
  await page.evaluate(() => window.__bemtvi.feed(":e /corrupt.png<CR>"));
  ...
  await page.evaluate(async () => {
    ...
    let w = await fh.createWritable(); await w.write(new TextEncoder().encode("btv.o.imagepreview = true\n")); await w.close();
    const png = Uint8Array.from(atob("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=="), (c) => c.charCodeAt(0));
  ...
  await page.waitForFunction(() => window.__bemtvi !== undefined, null, { timeout: 15000 });
  await page.evaluate(() => window.__bemtvi.ready);
  await page.evaluate(() => window.__bemtvi.feed(":e /viaconfig.png<CR>"));
  ...
  check(
    "config: `btv.o.imagepreview` in /init.lua enables previews after boot (the reported scenario)",
  ...
  await seedPng(4);
  await page.evaluate(() => window.__bemtvi.feed(":e /refresh.png<CR>"));
  ...
  await page.evaluate(() => window.__bemtvi.feed(":e /refresh.png<CR>")); // re-edit → version bumps → re-fetch
