# `btv.complete` — the native completion engine on the unified float-list widget — phased plan

> Working checklist for **Phase 4** of the unified float-list widget
> (`docs/specs/2026-06-14-btv-ui-float-widget.md`) — the last widget phase. The
> widget core (Phase 1, `btv.ui.select`), the prompt input-grab (Phase 2,
> `btv.picker`), and the preview pane (Phase 3) are done; this phase drives the
...
> sources. Lives at `docs/plans/2026-06-15-btv-complete-completion-engine.md`
> alongside the other `docs/plans/*.md`.

## Context

The unified widget (`docs/specs/2026-06-14-btv-ui-float-widget.md`) is one Rust
...
(`btv.complete` / `btv.picker` / `btv.ui.select`) are thin drivers; the widget does
...
`crates/bemtvi-server/src/lsp/completion.rs` already implements a **native LSP
...
type — `crates/bemtvi-server/src/lib.rs:1590`).

Decision (with the user, 2026-06-15): **build `btv.complete` on the unified widget
...
`bemtvi-core` stays **pure and synchronous**. Core owns: the completion `Menu`
...
async — LSP requests, `btv.spawn`, debounce timers, Lua source dispatch — lives in
`bemtvi-server` and the Lua wrapper, exactly as the picker splits it.

## Core idea: completion is a third `Menu` kind, not a fourth widget

Extend the existing `Menu` (`crates/bemtvi-core/src/editor/menu.rs`), do **not** add
a sibling. The widget already renders a cursor-anchored, match-highlighted,
selectable list (the `btv.ui.select` shape). Completion is that shape with two
...
unchanged. `fuzzy::rank` (`crates/bemtvi-core/src/fuzzy.rs:29`) is the same matcher
the picker uses.
...
`crates/bemtvi-core/src/editor/complete.rs`; the `CompleteSetupReq` op
...
`prelude/complete.lua` (`btv.complete.setup{}`, `buffer` only, unknown sources +
`btv.complete.source{}` fail loud); the completion menu renders through the existing
Cursor-placement `project_menu` (no new client code). Tests:
`crates/bemtvi-server/tests/complete.rs` (9, incl. the example). Example:
...
- **Manual trigger** — `btv.complete.trigger()` (Lua API) and a `keys.trigger`
  ...
  `complete_triggers` queue (`btv._complete_trigger` → drained in `effects.rs`).
...
- **Noselect (cmp-style)** — the auto-opened popup highlights **nothing** until you
  ...
  `false` for auto-typing, `true` for an explicit `btv.complete.trigger()`.
...
- **New `crates/bemtvi-core/src/editor/complete.rs`** (declared in `editor/mod.rs`):
...
- New `crates/bemtvi-lua/src/prelude/complete.lua` (registered like `picker.lua` /
  `ui.lua`): `btv.complete.setup { sources = { { "buffer", min_chars = 3 } },
  auto = true, keys = { next=…, prev=…, confirm=…, abort=… } }`. Only `"buffer"`
  is recognized this phase; any other source name **errors loud** ("btv.complete
  source 'X' not yet implemented") per the no-silent-stub rule. `keys` notation is
  parsed server-side. `btv.complete.source{}` (plugin sources) is **not** added
  yet — calling it errors loud.

### Tests (`crates/bemtvi-server/tests/complete.rs`, black-box)

- `btv.complete.setup` buffer source; type a prefix matching an existing buffer
...
- `cargo build --workspace`; `cargo test -p bemtvi-server --test complete`;
...
`btv.complete.source{}` API pulled forward from Phase 4-E** — pure async plumbing
...
- **Core** (`bemtvi-core`, pure/sync): `CompleteCtx { buf, row, col, prefix }` snapshot;
...
- **Server** (`bemtvi-server`, native): the settle fixpoint drains `complete_query_changes`
  → `lua.run_complete_run(gen, ctx)` → `apply_lua_effects` (so the source's debounce
  timer / `btv.spawn` actually starts), added to the convergence + recursion-limit clears;
...
- **Lua** (`bemtvi-lua`): `CompletePush { gen, label, insert }` op + `complete_pushes` /
  `complete_finishes` Shared queues + `btv._complete_push` / `btv._complete_finish` bridges;
  `run_complete_run` passes the ctx as primitives (the crate can't see `CompleteCtx`).
  `btv._complete_setup` gained a `has_async` arg.
- **Prelude** (`prelude/complete.lua`): `btv.complete.source { name, complete, debounce }`
  registration (reserved built-in names fail loud); `setup{}` validates against built-ins
  **and** registered sources, collects the active async sources into `btv._complete`, and
  passes `has_async`. `btv._complete_run(gen, ctx)` debounces each source via `btv.timer`,
  builds `push`/`done`/`ctx.on_cancel`, batches pushes (`FLUSH_N = 256`), reaps a superseded
  run, and reduces all sources' `done()` to one `btv._complete_finish(gen)`.
  `btv.complete.debounce` default = 120 ms.
...
- **Debounce** via the evloop (a `btv.timer`, as the picker does): coalesce
  keystrokes; a new keystroke cancels the in-flight request. Default ~120 ms,
  configurable (`btv.complete.setup{ debounce = … }`).
...
- **The `lsp` source is server-native, not Lua** — LSP plumbing + the encoding-aware
  `textEdit`/`additionalTextEdits` accept live in `bemtvi-server` (Lua's mutation API
  ...
  server applies the edit. (`crates/bemtvi-server/src/lsp/completion.rs`, rewritten.)
...
- **Retired**: `CompletionMenu`, `pmenu_value`, `completion_menu_key`, `lsp_menu_*`,
  ...
  → `complete_manual_trigger` (a no-op until `btv.complete.setup{}` enables the engine
...
- **Tests** — `crates/bemtvi/tests/lsp_complete.rs` (new, in the `bemtvi` crate for
  `CARGO_BIN_EXE_bemtvi`): drives the scripted mock LSP (`bemtvi --__lsp-mock`) via the
  `$BEMTVI_LSP_CMD` env hook + the raw `btv._lsp_start` bridge (the `vim.lsp.*` user API
...
requires `btv.complete.setup{ sources = { { "lsp" } } }` (opt-in) rather than working
...
- **Config**: `CompleteConfig.docs` (default on; `btv.complete.setup{ docs = false }` to
...
- **Projection + clients**: `EditHost::project_complete_docs` (native-only) emits a `docs`
  ...
  and clamped to the viewport; `bemtvi-view` parses it into `MenuData.docs`
...
- **Tests** (`crates/bemtvi/tests/lsp_complete.rs`, +2): the inline-`documentation` path
...
`docs/plans/2026-06-15-btv-snippet-engine.md`.

- **Snippet expansion on accept**: LSP `insertTextFormat = Snippet`
  (`$1` / `${1:default}` / `$0` / `${1|a,b|}` / mirrors) parses
  (`bemtvi_core::parse_snippet`) and expands into a tabstop session
...
- **Trigger-char gating** — `btv.complete.source { trigger = { chars = { ":" } } }`. The
  ...
  trigger context. Lua-side (`prelude/complete.lua`) `btv._complete_run` gates per source:
...
- **Plugin-source docs** — a row pushed as `{ text, insert, doc }` carries **inline**
  ...
  `btv._complete_resolve(id)` → the callback's `respond` → `complete_resolve_dones` →
...
- **wasm async parity** — the completion async path is pure core + Lua + shared ops (no
  ...
  proved the substrate). Confirmed: `cargo build -p bemtvi-server --no-default-features`
  and the full emscripten `crates/bemtvi-edithost/build.sh` both build clean (no new
...
- **Tests** — `crates/bemtvi-server/tests/complete.rs` (+5, 22 total): trigger-char wake +
...
- `btv.complete.source { name, complete, debounce }` **already landed in Phase 4-B**
...
1. `cargo build --workspace`; `cargo test -p bemtvi-server --test complete`.
2. `cargo clippy --all-targets -- -D warnings`; `cargo fmt --all -- --check`.
3. `cargo run -p bemtvi -- examples/ui-complete/sample.txt` with the example config:
