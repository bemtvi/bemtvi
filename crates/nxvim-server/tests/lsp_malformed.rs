...
//! `nx._lsp_apply_workspace_edit` (the Lua entry behind
...
use nxvim_test_harness::{attach, command, exec_lua, lines, spawn, temp_dir};
    ...
    let pcall = exec_lua(
        ...
        &format!(
            "local ok, err = pcall(function() \
               nx._lsp_apply_workspace_edit({{ changes = {{ ['file://{abs}'] = {{ \
