...
// The config seeds diagnostics from a BufReadPost/BufEnter autocmd via btv.diagnostic.set.
...
try {
  ...
  await page.evaluate((cfg) => window.__bemtvi.execLua(cfg + `
    _G.__events = {}
    btv.autocmd.create({ "BufReadPost" }, { pattern = "*sample.txt",
      callback = function() _G.__events[#_G.__events + 1] = "BufReadPost" end })
    btv.autocmd.create({ "BufNewFile" }, { pattern = "*sample.txt",
  ...
  for (let i = 0; i < 100; i++) {
    if ((await page.evaluate(() => window.__bemtvi.cmdline())) === ":eo") break;
    await sleep(30);
  }
  ...
  const lines = await (async () => {
    for (let i = 0; i < 200; i++) {
      const v = await page.evaluate(() => window.__bemtvi.lines());
      if (/prnit/.test(String(v))) return v;
      await sleep(40);
    }
    return await page.evaluate(() => window.__bemtvi.lines());
  })();
  ...
  const events = await page.evaluate(() =>
    window.__bemtvi.execLua("return table.concat(_G.__events, ',')"));
