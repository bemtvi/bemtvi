# Porting nvim-lspconfig to bemtvi — the real, total, async port

**Repo under port:** `~/work/nxvim-plugins/nxvim-lspconfig` (a fork of
`neovim/nvim-lspconfig` @ `b7b92094`), which now claims the `bemtvi/bemtvi-lspconfig`
name. The earlier 22-server hand-written port has been renamed to
`bemtvi/bemtvi-lspconfig-base` (`~/work/nxvim-plugins/nxvim-lspconfig-base`) and is
**superseded** by this work.

**Goal:** every one of the 407 upstream server configs runs on bemtvi's own
surfaces — `btv.lsp` / `btv.fs` / `btv.run` / `btv.utils` / `btv.command` / `btv.ui` —
...
`:checkhealth`) with no bemtvi analogue and no reason to exist here. The user
commands they define are re-implemented natively.

## 2. The translation table (upstream API → bemtvi native)

| upstream | bemtvi | status |
| --- | --- | --- |
...
| `vim.fs.dirname` / `basename` | `btv.utils.dirname` / `basename` | ✅ exists |
| `vim.fs.parents` | `btv.utils.ancestors` | ✅ exists |
| `vim.fs.joinpath` | `btv.utils.joinpath` | **new (core prelude)** |
| `vim.fs.normalize` | `btv.utils.normalize` | **new (core prelude)** |
| `vim.fs.relpath` | `btv.utils.relpath` | **new (core prelude)** |
| `vim.fn.executable` / `exepath` | `btv.fs.which(name)` → promise | **new (core, cross-crate)** |
| `vim.uv.fs_stat` | `btv.fs.stat` / `btv.fs.exists` | ✅ exists, async |
| `vim.system(cmd)` | `btv.run{}` → promise | ✅ exists |
| `vim.lsp.rpc.start(argv, d)` | `cmd` fn just **returns the argv** | drop the wrapper |
| `vim.api.nvim_buf_get_name` | `btv.buf.name(bufnr)` | ✅ exists |
| `nvim_buf_create_user_command` | `btv.user_command.buf_create` | ✅ exists |
| `vim.notify` | `btv.notify` | ✅ exists |
| `vim.lsp.get_clients` | `btv.lsp.clients` | ✅ exists |
| `vim.json.decode` | `btv.json.decode` | ✅ exists |
| `vim.ui.select` / `input` | `btv.ui.select` / `btv.ui.input` (promise) | ✅ exists |
| `vim.keymap.set` | `btv.keymap.set` | ✅ exists |
| `vim.cmd.edit` | `btv.cmd("edit …")` | ✅ exists |
| `vim.uri_from_bufnr` / `uri_to_fname` | `btv.uri.*` | **new (plugin util)** |
| `vim.fn.has('nvim-0.11.x')` | deleted — bemtvi is not versioned by nvim | n/a |
| `vim.deprecate` | deleted | n/a |

## 3. Gaps in `btv.lsp` this port must close (Phase 1)

`btv.lsp`'s resolved-config reader (`crates/bemtvi-lua/src/prelude/lsp.lua`)
...
4. **`get_language_id(bufnr, ft)`** (8 configs) — the `didOpen` `languageId`.
   The filetype already crosses `btv._lsp_start` as `ft`; apply the hook to it
   Lua-side. No Rust change.
5. **`before_init(init_params, config)`** (7 configs) — mutates
   `initializationOptions` before `initialize`. `btv.lsp` builds `init_options`
...
6. **`cmd_env`** (7 configs) — environment for the spawned server. **Rust,
   ...
   `Command::envs()` in `bemtvi-lsp/src/transport.rs`, and the daemon leg.
...
Plus `btv.fs.which` (Rust, cross-crate: `FsJob::Which` through `install.rs`
native + `fswire.rs` daemon leg + the wasm Worker — the tier-1 remote rule), and
`btv.utils.joinpath` / `normalize` / `relpath` (pure Lua, prelude — CLAUDE.md
puts path munging in public `btv.utils.*`).

Phase 4 surfaced five more, closed in `128eb1e2`: `btv.cwd()`, `btv.stdpath(what)`,
`btv.pid()`, `btv.version()` — editor and host facts previously reachable only
through `vim.fn`, which ADR 0002 keeps off the whitelist — and the two JSON values
Lua cannot express, `btv.json.null` and `btv.json.empty_object()`. The JSON pair is
...
- **Phase 1 — close the canonical gaps.** `btv.utils` path helpers; `btv.fs.which`
  end-to-end (native + daemon + wasm); `btv.lsp` config-key support per §3 with a
  loud rejection for anything still unmodelled. Harness tests in
  `crates/bemtvi-server/tests/`. *No plugin code yet.*
- **Phase 2 — the plugin skeleton.** Strip `lua/lspconfig/`, `plugin/`, `test/`,
  ...
  went out with the plumbing.)** Write `lua/bemtvi-lspconfig/util.lua` (the
  async native helpers of §2) and `lua/bemtvi-lspconfig/init.lua` (`setup()`,
  carried forward from `-base` as an additive convenience over the primary
  `btv.lsp.enable` path). Native `:LspStart` / `:LspStop` / `:LspRestart` /
  `:LspInfo` / `:LspLog` via `btv.command`.
- **Phase 3 — the 284 pure-data configs.** Scripted rewrite: strip the
  `---@type vim.lsp.Config` / `lspconfig.settings.*` annotations, rewrite
  `vim.lsp.config(…)` doc examples to `btv.lsp.config(…)`, verify every file
...
- **Phase 4 — the 114 code-bearing configs.** Hand-ported in batches, grouped by
  ...
  - *Pass 3* (core `b2cda7cd`) — the **client-side LSP helper surface**, and the eight
    ...
    **21** configs called `client:exec_cmd` / `client:supports_method` /
    ...
    - Core (`btv.lsp`): `client.offset_encoding` — readable AND writable, the write
      ...
      and `btv.utils.uri_from_path` / `uri_from_buf` / `uri_to_path`. The config key
      `offset_encoding` moved off the unsupported list.
    - Plugin: `open_floating_preview` → `btv.ui.float` (a non-persistent float already
      IS "transient, unfocusable, sized to its contents"); `locations_to_items` +
      `setqflist` → `btv.lsp.locations_to_items` + `btv.qf`; `vim.cmd.edit(uri_to_path)`
      → `btv.lsp.show_document` (reuses a buffer opened under a relative name).
      ...
      path bemtvi already implements. ts_ls' source-action command likewise asks for the
      ...
      left as code that cannot run (`btv.lsp` does not route server-initiated messages
    ...
    - Also fixed: three pass-2 call sites passing the encoding **positionally** to
      `btv.lsp.show_document` / `apply_workspace_edit` (neovim's signature, not bemtvi's),
      which silently defaulted to utf-16.
    - Tests: `crates/bemtvi/tests/lsp_client_api.rs` (11, over the mock server — the
      ...
      `bemtvi --test-plugin` (15): every config loads, every `cmd` / `root_dir` /
      `before_init` / `get_language_id` runs with the arguments `btv.lsp` really passes,
...
- **Phase 5 — verification & ship.** Harness tests over a representative slice
  ...
  vimdoc regeneration; bump the `bemtvi/bemtvi-lspconfig` pin in
  `crates/bemtvi-edithost/build-plugins.sh` (**currently pinned at `e9d13fff`,
  a SHA that now lives only in `bemtvi-lspconfig-base` — the rename broke it**).
  - Measured against what already existed, the *core* half of the slice was
    ...
    `workspace_required`, `utils_paths.rs` has `btv.fs.which`, and the plugin's
  ...
  - `examples/lspconfig/` (`init.lua` + `sample.lua`), verified end-to-end against a
    ...
    while `btv` does **not** (the `diagnostics.globals` override taking), the nine
    plugin keymaps installed buffer-local, `:LspInfo` showing the `.git`-tier root,
    407 available, `for_filetype("lua")`, and `:LspStop`.
  - Doc pipeline per `WRITING-VIMDOCS.md`: authored `doc/bemtvi-lspconfig.md`,
    generated `doc/bemtvi-lspconfig.txt` (23 tags), `scripts/gen-vimdoc.sh` +
    ...
    `:help bemtvi-lspconfig` opens the page, `:help bemtvi-lspconfig-writing-a-config`
    lands on its heading.
  - **Corrections the verification forced.** (a) `single_file_support` was listed in
    §3 as a Phase 1 item and in `configs_spec`'s known-key set, but `btv.lsp` never
    ...
    five `vim.log.levels` → `btv.log.levels`. (c) The doc comments themselves were
    ...
    `capabilities` deep-merge over the base, and bemtvi does **not** advertise
    `snippetSupport`, so that advice is load-bearing here too), and lua_ls/emmylua_ls
    retargeted at bemtvi: **PUC 5.4, not LuaJIT**, and `globals = { 'btv', 'vim' }`.
  ...
  - **The per-server reference, restored.** Phase 2's wholesale `scripts/` strip took
    ...
    `scripts/docgen.lua` is now a native port running under `bemtvi --lua` against
    `btv.fs`/`btv.cwd`/`btv.await`, with two deliberate departures from upstream: a
    **tag per server** (`:help lspconfig-clangd` lands on clangd; upstream tagged
    ...
    byte-identical run to run. Determinism needed upstream's hack in bemtvi spelling:
    the table is rendered by *loading* the config, and a handful read host facts at
    load time (`btv.stdpath`, `btv.version`, `btv.env.get`, `~`), so those are frozen to
  ...
  - **A core bug the example surfaced** (`crates/bemtvi/tests/lsp_lifecycle.rs`, 2):
    an explicit `:LspStop` left `btv.lsp.clients()` listing a server `:LspInfo` said
    ...
    `btv.lsp.clients()`, so the phantom made the *next* `:LspStop` claim it stopped
