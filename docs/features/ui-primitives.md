...
bemtvi gives plugins a small, layered set of primitives for building rich
...
| --- | --- | --- |
| A stateful UI that redraws itself (file tree, dashboard, dialog) | **`btv.view.component`** / **`btv.component`** | A Vue-shaped reactive component: state + a pure render. |
| Plugin-owned content you update by hand (a list, a report) | **`btv.view`** | An inert buffer you set lines on and mount in a dock / split / float. |
| To ask the user something (text, choice, yes/no) | **`btv.ui.input` / `select` / `confirm`** | Async prompt widgets — return a promise. |
| To pop up read-only content (a hint, a tooltip) | **`btv.ui.float`** | A bordered content overlay, dismissed by the next key. |
...
`btv.component` is a **Vue-shaped** component model: you write reactive **state** and
...
Both may be **async** — call `btv.await(...)` straight inside them (each runs in its
own `btv.async` coroutine), so a `setup` that loads from disk, or a `render` that
fetches to display, reads top-to-bottom.

```lua
local Counter = btv.view.component({
  setup = function(ctx)
...
- **`"view"`** (the default, and what `btv.view.component` selects) — a
  **focus-taking, navigable** [`btv.view`](#views--the-content-surface) buffer, mounted in a dock, split,
...
- **`"float"`** — a **non-focus** [popup-content](#popup-content) float (the
  ...
  **empty render hides** the float — so a component shows and hides purely by what
  it returns. Reach it with `btv.component{ surface = "float", … }`.

```lua
-- A self-dismissing toast on the non-focus float surface.
local Toast = btv.component({
  surface = "float",
  setup = function(ctx)
    local s = ctx.reactive({ text = ctx.props.text })
    btv.timer(function() s.text = nil end, 1500)   -- clear -> empty render -> hides
    return s
  end,
  render = function(s)
...
setup errors are caught and surfaced through `btv.notify` rather than crashing the
editor.

## Views — the content surface

`btv.view` is the surface components are built on, and is useful on its own. A view
...
```lua
local v = btv.view.create({ name = "files", filetype = "btvfiles" })
v:set_lines({ "  init.lua", "  README.md" })
v:set_userdata({ { path = "init.lua" }, { path = "README.md" } })  -- parallel to lines
v:on_select(function(line, data) btv.open(data.path, { where = "main" }) end)
...
| `:place_in(win)` | Adopt a reserved restore slot (used by `btv.view.on_restore`, below). |
...
[`btv.shada.plugin()`](../plans/2026-06-26-plugin-shada-namespaces.md) store, keyed
by the same id.
...
btv.view.on_restore(function(id, place)
  local state = btv.shada.plugin():get("view:" .. id)
  local v = btv.view.create({ name = "Files", filetype = "btvfiles", persist = id })
  v:set_lines(render(state))
  v:on_select(...)
  place(v)
end)
...
`btv.shada.plugin()` — so two plugins can both use `persist = "main"` without colliding, and
...
declared. You do not arm anything for it: keep `bemtvi-tree` behind `cmd = "BtvTree"` and the
...
`btv.shada.plugin(namespace)` takes. **GC:** the editor never deletes your stored state —
...
dance above is what `btv.view.component` (see [Components](#components--reactive-uis) above)
...
```lua
local Files = btv.view.component({
  setup = function(ctx)
...
Files.mount({ name = "Files", filetype = "btvfiles", persist = "main", dock = "left", size = 30 })
```

`ctx.store` is this component's `btv.shada.plugin()` slice; mutate it on every change and the
...
The `btv.ui` widgets are the common interactions, prebuilt. The three input ones are
**non-blocking** and **promise-only** (ADR 0002): the call returns at once and you
react with `:next(fn)`, or `await` it inside `btv.async`. (The `vim.ui.*`
muscle-memory aliases keep neovim's callback shape.)

```lua
-- input: a one-line prompt. Resolves to the text, or nil on <Esc>.
btv.ui.input({ prompt = "Rename to: ", default = vim.fn.expand("%:t") })
  :next(function(name) if name and name ~= "" then btv.notify(name) end end)

-- select: a floating chooser. Resolves to the chosen item, or nil on cancel.
btv.ui.select({ "apple", "banana", "cherry" }, { prompt = "Pick:" })
  :next(function(item) if item then btv.notify("picked " .. item) end end)

-- confirm: a yes/no dialog. Resolves to a boolean.
btv.ui.confirm("Quit without saving?", { default = false })
  :next(function(ok) if ok then btv.cmd("qa!") end end)
```

- **`btv.ui.input(opts)`** — `prompt` label + `default` prefill, over the command
  line. `""` on an empty `<CR>`, `nil` on `<Esc>`.
- **`btv.ui.select(items, opts)`** — a floating list. `format_item` maps an item to
  ...
  ```lua
  btv.keymap.set("select", "<C-j>", btv.ui.select_actions.next)   -- actions: next,
  btv.keymap.set("select", "q", function() end)                  -- prev, first, last,
  ```                                                            -- confirm, cancel

- **`btv.ui.confirm(message, opts)`** — yes/no. `default = true` (the default) makes
  `<CR>` Yes and shows `[Y/n]`; `false` makes it decline and shows `[y/N]`. For a
  multi-choice menu use `select`.
- **`btv.ui.open(uri)`** — hand a path/URL to the OS opener (`open` / `explorer` /
  `xdg-open`), off-tick; resolves to the run result `{ code, stdout, stderr }`
  (like `btv.run`, it *resolves* rather than rejects — a missing opener is
  `code = -1`).

### Popup content

`btv.ui.float(contents, opts)` is the **popup-content** surface — a box of read-only
...
| **Popup content** | A lightweight display overlay (`btv.ui.float`) — *not* a window | ❌ | never | ❌ |
| **Popup window** | A transient, read-only *real* window over a scratch buffer — auto-dismissed but scrollable | ✅ | never | ✅ |
| **List widget** | The floating selectable list — picker, `btv.ui.select`, completion | server widget | ✅ | ✅ (preview) |
...
constructor — `btv.lsp.hover` / `signature_help` open it internally.

## Placing things

Every windowed surface — floating windows, `btv.view`, components, pickers, the
...
| `border` | `"none"` / `"single"` / `"rounded"` / `"double"` / `"solid"` (plus `"shadow"` for `btv.view` floats). |
...
Lua: bemtvi's Lua API deliberately omits the mutating `vim.api.nvim_*` entity surface
(ADR 0002), so a plugin opens its float through `btv.view:mount{ float = … }` or a
...
`btv.win.config`) reads a window's float config back (`{ relative = "" }` for a tiled
window), and `btv.win.gettype(win)` returns `"popup"` for a float. A float is a real
...
picker, and `btv.ui.select` are thin engines over the same widget, differing only in
...
reactivity core (dependency-tracked `reactive` / `computed`) over the `btv.view`
...
[float-list widget spec](../specs/2026-06-14-btv-ui-float-widget.md), and the
