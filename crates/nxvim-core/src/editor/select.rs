...
//! is no Normal-mode keystroke to enter it, only [`nx.win.select_range`] (backed by
//! [`Editor::select_range_in_window`]), the Select sibling of `nx.win.set_cursor`.
//!
...
impl Editor {
    ...
    /// `nx.win.select_range`. Focuses `id` first (the selection is the focused
