-- bemtvi:prelude/glob — the `btv.glob.*` surface: shell/gitignore-style path patterns,
...
-- match runs in Rust.
--
-- The canonical engine is `bemtvi_core::glob`; these are thin wrappers over its
-- `btv._glob*` bridges. The syntax and the two deliberate defaults (`*` stops at `/`;
-- path style is an explicit option, never the host's) are documented on
-- `btv.glob.match` below, since that is the entry point most callers read first.

btv.glob = btv.glob or {}

-- `btv.glob.match(pattern, path, opts)` -> boolean: does `path` match the glob
-- `pattern`? The one-shot form — it goes through the same compiled-regex cache as
-- `btv.glob.compile`, so calling it in a loop over many paths does not recompile.
...
--
--   * **`*` stops at `/`.** `btv.glob.match("*.lua", "a/b/c.lua")` is `false`; say
...
--     machine bemtvi runs on: a daemon or remote session can hand a Unix-hosted
...
--
-- ```lua
-- btv.glob.match("*.lua", "init.lua")            --> true
-- btv.glob.match("*.lua", "conf/init.lua")       --> false  ( `*` stops at `/` )
-- btv.glob.match("**/*.lua", "conf/init.lua")    --> true
-- btv.glob.match("*.lua", "conf/init.lua", { basename = true })  --> true
-- btv.glob.match("src/**/*.{rs,toml}", "src/a/b/Cargo.toml")     --> true
-- ```
function btv.glob.match(pattern, path, opts)
  return btv._glob_match(pattern, path, opts)
end

-- `btv.glob.compile(pattern, opts)` -> glob: compile `pattern` into a reusable glob
-- object. Equivalent to `btv.glob.match` for a single test, but worth holding onto
...
-- documented on `btv.glob.match`. Raises on an invalid pattern.
--
...
-- is why `btv.glob.to_regex`, whose job is to hand the translation to another engine,
-- refuses a `basename` glob rather than answering with a regex that means something
-- else.)
--
-- ```lua
-- local ignore = btv.glob.compile("**/target/**")
-- if ignore:test(path) then return end
-- ```
function btv.glob.compile(pattern, opts)
  return btv._glob(pattern, opts)
end

-- `btv.glob.set(patterns, opts)` -> globset: compile a **list** of globs into one
...
-- patterns wants. `opts` is as documented on `btv.glob.match` and applies to every
...
--
-- ```lua
-- local ignore = btv.glob.set({ "**/.git/**", "**/target/**", "*.tmp" })
-- local kept = btv.tbl.filter(function(p) return not ignore:test(p) end, paths)
-- ```
function btv.glob.set(patterns, opts)
  return btv._glob_set(patterns, opts)
end

-- `btv.glob.any(patterns, path, opts)` -> boolean: does any glob in `patterns` match
-- `path`? The one-shot form of `btv.glob.set` — cached the same way, so a repeated
...
--
-- ```lua
-- btv.glob.any({ "*.rs", "*.toml" }, "Cargo.toml")  --> true
-- ```
function btv.glob.any(patterns, path, opts)
  -- Checked here rather than left to the bridge: the two branches below would name
  -- two DIFFERENT functions (`btv.glob.match` / `globset:test`) for the same mistake,
  -- neither of them the one the caller actually wrote.
  if type(path) ~= "string" then
    error(("btv.glob.any: path must be a string, got %s"):format(type(path)), 2)
  end
  if type(patterns) == "string" then
    return btv._glob_match(patterns, path, opts)
  end
  return btv._glob_set(patterns, opts):test(path)
end

-- `btv.glob.filter(patterns, paths, opts)` -> list: the entries of `paths` matching
-- any glob in `patterns`, in their original order. `patterns` may be a single
-- pattern string or a list, as in `btv.glob.any`. One compiled set for the whole
-- sweep.
--
-- ```lua
-- btv.async(function()
--   local rs = btv.glob.filter({ "**/*.rs" }, btv.await(btv.fs.walk(".")))
-- end)
-- ```
function btv.glob.filter(patterns, paths, opts)
  local set = type(patterns) == "string" and btv._glob_set({ patterns }, opts)
    or btv._glob_set(patterns, opts)
  local out = {}
  for i, path in ipairs(paths) do
    ...
    if type(path) ~= "string" then
      error(("btv.glob.filter: paths[%d] must be a string, got %s"):format(i, type(path)), 2)
    end
...
-- `btv.glob.to_regex(pattern, opts)` -> string: the regex source `pattern` translates
...
-- dialect `btv.regex` speaks). `opts` is as documented on `btv.glob.match`.
--
...
--
-- ```lua
-- btv.glob.to_regex("*.lua")  --> "(?-u)^[^/]*\\.lua$"
-- ```
function btv.glob.to_regex(pattern, opts)
  return btv._glob_to_regex(pattern, opts)
end

-- `btv.glob.is_glob(s)` -> boolean: does `s` carry a glob metacharacter (`*` `?` `[`
...
--
-- ```lua
-- btv.glob.is_glob("src/*.rs")   --> true
-- btv.glob.is_glob("src/lib.rs") --> false
-- ```
function btv.glob.is_glob(s)
  return btv._glob_is_glob(s)
end

-- `btv.glob.split(list)` -> a list of patterns: split one **comma-separated line** of
...
--
-- ```lua
-- btv.glob.split("src/**, docs/**")   --> { "src/**", "docs/**" }
-- btv.glob.split("**/{a,b}/**")       --> { "**/{a,b}/**" }   ( one pattern )
-- btv.glob.split("  , *.lock ,, ")    --> { "*.lock" }
-- ```
function btv.glob.split(list)
  return btv._glob_split(list)
end
