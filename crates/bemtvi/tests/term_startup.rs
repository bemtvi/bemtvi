//! Tier 3: startup against a terminal that answers *some* of what bemtvi asks it.
//!
...
//! then ate the user's first keystroke. That is the "bemtvi is slow to open inside
...
/// `~/.config/bemtvi` (hermetic, per the suite convention).
fn empty_config_dir() -> PathBuf {
    static SEQ: AtomicU64 = AtomicU64::new(0);
    let n = SEQ.fetch_add(1, Ordering::Relaxed);
    let dir = bemtvi_test_harness::temp_root()
        .join(format!("bemtvi_term_cfg_{}_{n}", std::process::id()));
    std::fs::create_dir_all(&dir).expect("create empty config dir");
    dir
}

/// A fresh empty data dir, so startup never reads — or writes — the developer's
/// own `~/.local/share/bemtvi` (plugins, shada, treesitter grammars all live
/// there). Isolating the config dir alone is not enough: `stdpath("data")` keys
/// off `XDG_DATA_HOME`, and a *fresh* data dir is a **first run**, which opens
/// the recommended-plugins welcome — a focus-grabbing float that swallows the
/// keystrokes these tests type and clones real plugins over the network. So the
/// dir is pre-marked as already-asked (the same `.recommended-prompted` marker
/// `btv.plugins.bootstrap` writes), putting the session in the state a returning
/// user is in: a plain editor, no first-run UI, nothing fetched.
fn empty_data_dir() -> PathBuf {
    static SEQ: AtomicU64 = AtomicU64::new(0);
    let n = SEQ.fetch_add(1, Ordering::Relaxed);
    let dir = bemtvi_test_harness::temp_root()
        .join(format!("bemtvi_term_data_{}_{n}", std::process::id()));
...
impl FakeTerminal {
    /// Spawn `bemtvi` on a pty and answer its capability queries the way tmux does:
    ...
    fn spawn_tmux_like(file: &str) -> FakeTerminal {
        ...
        // point, and an inherited `BEMTVI_OSC52` / `BEMTVI_KITTY_KEYBOARD` would skip it.
        cmd.env_remove("BEMTVI_OSC52");
        cmd.env_remove("BEMTVI_KITTY_KEYBOARD");
...
fn sample_file(name: &str, body: &str) -> PathBuf {
    let path = bemtvi_test_harness::temp_root().join(format!("{}_{name}", std::process::id()));
