...
//! editor: a global screen cell goes in via `btv_input_mouse`, and we assert on
...
/// A malformed `btv_input_mouse` call (unknown action for the button) is
...
async fn left_mouse_can_be_mapped_like_any_key() {
    ...
    exec_lua(
        ...
        btv.keymap.set('n', '<LeftMouse>', function()
...
async fn double_click_fires_a_mapped_2leftmouse_and_suppresses_selection() {
    ...
    exec_lua(
        ...
        r#"
        _G.opened = 0
        btv.keymap.set('n', '<2-LeftMouse>', function() _G.opened = _G.opened + 1 end)
        return true
    "#,
    )
    .await;
...
async fn a_buffer_local_mouse_map_is_scoped_to_its_buffer() {
    ...
    exec_lua(
        ...
        r#"
        _G.local_hits = 0
        btv.keymap.set('n', '<2-LeftMouse>', function() _G.local_hits = _G.local_hits + 1 end,
          { buffer = 0 })
        return true
    "#,
    )
...
async fn a_mouse_map_reads_getmousepos_for_the_clicked_cell() {
    ...
    exec_lua(
        ...
        _G.clicked_line = nil
        _G.clicked_col = nil
        btv.keymap.set('n', '<RightMouse>', function()
...
async fn ctrl_left_mouse_can_be_mapped_and_places_cursor() {
    ...
    exec_lua(
        ...
        _G.ctrl_hits = 0
        _G.line_at_click = nil
        btv.keymap.set('n', '<C-LeftMouse>', function()
...
async fn a_modifier_distinguishes_the_left_mouse_map() {
    ...
    exec_lua(
        ...
        _G.plain = 0
        _G.ctrl = 0
        btv.keymap.set('n', '<LeftMouse>', function() _G.plain = _G.plain + 1 end)
        btv.keymap.set('n', '<C-LeftMouse>', function() _G.ctrl = _G.ctrl + 1 end)
        return true
    "#,
    )
    .await;
...
async fn right_mouse_can_be_mapped_and_suppresses_the_default() {
    ...
    exec_lua(
        ...
        r#"
        _G.right_hits = 0
        btv.keymap.set('n', '<RightMouse>', function() _G.right_hits = _G.right_hits + 1 end)
        return true
    "#,
    )
    .await;
...
async fn middle_mouse_can_be_mapped_and_suppresses_the_paste() {
    ...
    exec_lua(
        ...
        r#"
        _G.mid_hits = 0
        btv.keymap.set('n', '<MiddleMouse>', function() _G.mid_hits = _G.mid_hits + 1 end)
        return true
    "#,
    )
    .await;
...
async fn shift_left_mouse_can_be_mapped_and_suppresses_extend() {
    ...
    exec_lua(
        ...
        r#"
        _G.shift_hits = 0
        btv.keymap.set('n', '<S-LeftMouse>', function() _G.shift_hits = _G.shift_hits + 1 end)
        return true
    "#,
    )
    .await;
...
async fn left_drag_can_be_mapped_and_suppresses_select() {
    ...
    exec_lua(
        ...
        r#"
        _G.drags = 0
        btv.keymap.set('n', '<LeftDrag>', function() _G.drags = _G.drags + 1 end)
        return true
    "#,
    )
    .await;
...
async fn left_release_can_be_mapped() {
    ...
    exec_lua(
        ...
        r#"
        _G.releases = 0
        btv.keymap.set('n', '<LeftRelease>', function() _G.releases = _G.releases + 1 end)
        return true
    "#,
    )
    .await;
...
async fn right_drag_can_be_mapped() {
    ...
    exec_lua(
        ...
        r#"
        _G.rdrag = 0
        btv.keymap.set('n', '<RightDrag>', function() _G.rdrag = _G.rdrag + 1 end)
        return true
    "#,
    )
    .await;
...
async fn scroll_wheel_can_be_mapped_and_suppresses_scroll() {
    ...
    exec_lua(
        ...
        r#"
        _G.scrolls = 0
        btv.keymap.set('n', '<ScrollWheelDown>', function() _G.scrolls = _G.scrolls + 1 end)
        return true
    "#,
    )
    .await;
...
async fn a_shift_scroll_wheel_is_a_distinct_map() {
    ...
    exec_lua(
        ...
        _G.plain = 0
        _G.shift = 0
        btv.keymap.set('n', '<ScrollWheelUp>', function() _G.plain = _G.plain + 1 end)
        btv.keymap.set('n', '<S-ScrollWheelUp>', function() _G.shift = _G.shift + 1 end)
        return true
    "#,
    )
    .await;
...
async fn double_right_click_fires_a_mapped_2rightmouse() {
    ...
    exec_lua(
        ...
        r#"
        _G.dbl = 0
        btv.keymap.set('n', '<2-RightMouse>', function() _G.dbl = _G.dbl + 1 end)
        return true
    "#,
    )
    .await;
...
async fn wheel_over_a_dock_scrolls_it_without_focus() {
    ...
    exec_lua(
        &rpc,
        "btv.dock.open{ side = 'left', size = 20, buf = vim.api.nvim_get_current_buf() }",
    )
    .await;
    feed(&rpc, "<C-w><C-w>l");
...
// ── Phase 9b: clickable btv.statusline segments (on_click) ───────────────────

/// A left-click on a `btv.statusline` segment whose spec carries `on_click` fires
...
async fn statusline_segment_on_click_fires() {
    ...
    exec_lua(
        ...
        btv.statusline.setup{ left = { "clicky" } }
        "#,
    )
    .await;
    let row = lone_status_row(&rpc).await;
...
/// The global bar also honours `btv.statusline` **segment** `on_click` (resolved at
...
/// screen dimension (`btv_ui_try_resize`) and mouse coordinate (`btv_input_mouse`)
