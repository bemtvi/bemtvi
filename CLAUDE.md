...
- `crates/nxvim-test-harness/src/lib.rs` — the shared black-box test harness (a `publish = false` dev-dependency of `nxvim` and `nxvim-server`): server spawn/attach, the RPC-driving conveniences, the redraw accessors, and temp-fs / lock / clipboard helpers every integration suite shares. Its `lua51`/`luajit` features thread the backend through to `nxvim-server` so a single backend resolves through the dev-dependency cycle.
