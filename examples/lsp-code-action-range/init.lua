-- ~~~ bemtvi btv.lsp.code_action over a RANGE — the visual selection ~~~
--
-- Run it (from the repo root) — needs `gopls` on your PATH:
--
--     BEMTVI_CONFIG=examples/lsp-code-action-range \
--       cargo run -p bemtvi -- examples/lsp-code-action-range/sample.go
--
...
-- over a non-empty one. bemtvi takes that range from, in order: an explicit
...
btv.lsp.config("gopls", {
  ...
  on_attach = function(_client, bufnr)
    ...
    -- trick: pressed in Visual, `btv.lsp.code_action()` finds the live selection and
    ...
    -- END-EXCLUSIVE (the `btv.win.select_range` convention). It wins over both the
    ...
    btv.keymap.set("n", "<leader>cx", function()
      btv.lsp.code_action({
    ...
    btv.keymap.set({ "n", "v" }, "<leader>cr", function()
      btv.lsp.code_action({ context = { only = { "refactor" } } })
    end, { buffer = bufnr })
