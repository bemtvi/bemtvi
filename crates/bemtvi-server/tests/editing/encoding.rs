...
// and — after `:w` — on the exact bytes back on disk. bemtvi keeps the rope UTF-8
// and `'fileencoding'` governs the on-disk form; these prove the round-trip.
//
// Note on trailing newlines: bemtvi always maintains a trailing newline in the rope
