...
//!
//! ```sh
//! cargo test -p bemtvi-ts --test spell_capture -- --ignored --nocapture
//! ```
...
impl TempDataDir {
    fn new() -> Self {
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("bemtvi_ts_spell_{pid}_{nanos}"));
...
fn comment_with_spell_capture_paints_as_comment_not_spell() {
    ...
    bemtvi_ts::install::install(&data.0, "toml")
...
fn lua_match_predicate_gates_the_bash_shebang_rule() {
    ...
    bemtvi_ts::install::install(&data.0, "bash")
