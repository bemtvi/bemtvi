//! Behavior tests for `btv.picker` — the fuzzy finder over the unified float-list
//! widget (`docs/specs/2026-06-14-btv-ui-float-widget.md`, Phase 2: the prompt
...
async fn open_with_an_initial_query_opens_already_filtered() {
    // `btv.picker.open(name, { query = "ap" })` pre-fills the prompt and kicks the
    ...
    exec_lua(&rpc, "btv.picker.open('fruits', { query = 'ap' })").await;
    let menu = menu_of(&poll_menu(&rpc, &mut incoming).await.expect("menu opens"));
...
async fn open_with_a_title_projects_it_on_the_box() {
    // `btv.picker.open(name, { title = … })` puts a title on the picker box's top
    ...
    exec_lua(
        &rpc,
        "btv.picker.open('fruits', { title = 'Pick a fruit' })",
    )
    .await;
    let menu = menu_of(&poll_menu(&rpc, &mut incoming).await.expect("menu opens"));
    assert_eq!(
...
async fn multiselect_false_disables_tab_marking() {
    // `btv.picker.open(name, { multiselect = false })` makes `<Tab>` a no-op — a
    // single-choice picker (the cmdline file completer rides this).
    let dir = temp_dir("picker_multiselect_off");
    let (rpc, mut incoming) = start(&dir, STATIC_SRC).await;

    exec_lua(&rpc, "btv.picker.open('fruits', { multiselect = false })").await;
    poll_menu(&rpc, &mut incoming).await.expect("menu opens");
    feed(&rpc, "<Tab>");
...
async fn clicking_a_picker_row_in_a_split_hits_via_editor_absolute_geometry() {
    ...
    exec_lua(&rpc, "_G.picked = nil; btv.picker.open('fruits')").await;
    let menu = menu_of(&poll_menu(&rpc, &mut incoming).await.expect("menu opens"));
...
"#,
    ...
    exec_lua(&rpc, "btv.picker.open('locs')").await;
    poll_menu(&rpc, &mut incoming).await.expect("menu opens");
    ...
    // It opened as a bottom-dock tab (the default bemtvi way).
...
async fn escape_closes_the_picker_without_confirming() {
    ...
    exec_lua(&rpc, "_G.picked = 'unset'; btv.picker.open('fruits')").await;
    poll_menu(&rpc, &mut incoming).await.expect("menu opens");
    ...
    assert_eq!(
        exec_lua(&rpc, "return btv._picker == nil").await,
        Value::Boolean(true),
...
"#;
    ...
    exec_lua(&rpc, "btv.picker.open('echo')").await;
    poll_menu(&rpc, &mut incoming).await.expect("menu opens");
...
"#;
    let (rpc, mut incoming) = start(&dir, src).await;
    exec_lua(&rpc, "_G.runs = 0; btv.picker.open('counted')").await;
    poll_menu(&rpc, &mut incoming).await.expect("menu opens");
...
"#;
    let (rpc, mut incoming) = start(&dir, src).await;
    exec_lua(&rpc, "btv.picker.open('many')").await;
...
"#;
    ...
    exec_lua(&rpc, "btv.picker.open('huge')").await;
    // Poll until all 100k have streamed in (total reflects the full set).
    let mut total = 0u64;
    for _ in 0..100 {
        total = exec_lua(&rpc, "return btv._picker and btv._picker.nitems or 0")
            .await
            .as_u64()
            .unwrap_or(0);
        if total >= 100000 {
...
"#;
    let (rpc, mut incoming) = start(&dir, src).await;
    exec_lua(&rpc, "btv.picker.open('deferred')").await;
    poll_menu(&rpc, &mut incoming).await.expect("menu opens");
...
async fn streaming_source_appears_as_rows_arrive() {
    // A source that streams via `btv.run_stream` (the async-iterator path):
...
btv.picker.source {
  ...
  items = btv.async(function(ctx)
    for batch in btv.await_each(btv.run_stream { cmd = "printf", args = { "one\ntwo\nthree\n" } }) do
...
"#;
    let (rpc, mut incoming) = start(&dir, src).await;
    exec_lua(&rpc, "btv.picker.open('stream')").await;
...
"#;
    let (rpc, mut incoming) = start(&dir, src).await;
    exec_lua(&rpc, "btv.picker.open('narrow')").await;
    poll_menu(&rpc, &mut incoming).await.expect("menu opens");
...
btv.picker.source {
  ...
  items = function(ctx)
    ...
    return btv.promise.new(function(resolve)
      btv.timer(function() ctx.push { text = tag }; resolve() end, 40)
    end)
  end,
  confirm = function(item) end,
}
"#;
    ...
    exec_lua(&rpc, "btv.picker.open('leak')").await;
    poll_menu(&rpc, &mut incoming)
        .await
...
async fn prompt_position_defaults_to_top_and_is_configurable_to_bottom() {
    ...
    exec_lua(&rpc, "btv.picker.open('fruits', { prompt_pos = 'bottom' })").await;
...
"#;
    let (rpc, mut incoming) = start(&dir, src).await;
    exec_lua(&rpc, "btv.picker.open('many', { width = 30, height = 12 })").await;
    let menu = menu_of(&poll_menu(&rpc, &mut incoming).await.expect("opens"));
...
async fn edit_helper_opens_a_file_and_jumps_to_the_location() {
    // The shipped confirm action `btv.picker.edit` (used by `files` / `live_grep`)
    ...
    exec_lua(
        &rpc,
        &format!(
            "btv.picker.edit({{ path = '{}', row = 3, col = 2 }})",
            file.display()
        ),
    )
    .await;
...
async fn picker_file_open_records_a_jump_so_ctrl_o_returns() {
    // The finder's location-less file confirm (`files` source → `btv.picker.edit`
    // with no `row`) opens through `btv._open_switchbuf`, which — like `:edit` — is
    ...
    exec_lua(
        &rpc,
        &format!("btv.picker.edit({{ path = '{}' }})", file.display()),
    )
    .await;
    assert_eq!(
        lines(&rpc).await,
...
async fn picker_size_is_configurable_in_cells_and_viewport_fractions() {
    ...
    exec_lua(
        &rpc,
        "btv.picker.open('fruits', { width = 30, height = 12 })",
    )
    .await;
    ...
    exec_lua(
        &rpc,
        "btv.picker.open('fruits', { width = '50vw', height = '25vh' })",
    )
    .await;
...
async fn picker_align_and_margin_place_the_box_in_a_corner_with_a_gap() {
    ...
    exec_lua(
        &rpc,
        "btv.picker.open('fruits', { width = 20, height = 6, align = 'top-left' })",
    )
    .await;
    ...
    exec_lua(
        &rpc,
        "btv.picker.open('fruits', { width = 20, height = 6, align = 'top-left', margin = 4 })",
    )
    .await;
    let menu = menu_of(
        &poll_menu(&rpc, &mut incoming)
    ...
    exec_lua(
        &rpc,
        "btv.picker.open('fruits', { width = 20, height = 6, align = 'bottom-right', margin = { 2, 5 } })",
    )
    .await;
...
async fn buffers_picker_rows_carry_the_ls_buffer_info() {
    ...
    let alpha_nr = exec_lua(&rpc, &format!("return vim.fn.bufnr('{}')", alpha.display()))
        ...
        .as_u64()
        .expect("alpha bufnr");
    let beta_nr = exec_lua(&rpc, "return btv.buf.current()")
        .await
        .as_u64()
        .expect("beta bufnr");

    exec_lua(&rpc, "btv.picker.open('buffers')").await;
    let menu = menu_of(&poll_menu(&rpc, &mut incoming).await.expect("menu opens"));
    let items = menu_items(&menu);
    let row = |name: &str| {
        items
...
async fn buffers_picker_lists_unnamed_buffers_like_ls() {
    ...
    let ls_rows = exec_lua(&rpc, "return #btv.buf.list({ focused = true })")
        .await
        .as_u64()
...
async fn buffers_picker_is_scoped_to_the_focused_layer() {
    ...
    exec_lua(&rpc, "btv.dock.open({ side = 'bottom', size = 12 })").await;
    exec_lua(&rpc, "btv.dock.focus('bottom')").await;
...
btv.picker.source({{
  ...
  confirm = function(item, mode, layer) btv.picker.edit(item, mode, layer) end,
}})
"#,
        target.display()
    );
    ...
    exec_lua(&rpc, "btv.picker.open('myfiles')").await;
    poll_menu(&rpc, &mut incoming).await.expect("menu opens");
    feed(&rpc, "<CR>");
    bemtvi_test_harness::barrier(&rpc).await;
    ...
    exec_lua(&rpc, "btv.dock.focus('left')").await;
    assert_eq!(
        lines(&rpc).await,
        vec!["dock"],
...
async fn shipped_sources_declare_their_confirm_layer() {
    ...
    let (rpc, _incoming) = start(&dir, "").await;
    assert_eq!(
        exec_lua(&rpc, "return btv.picker._sources.files.layer").await,
        Value::from("main"),
    );
    assert_eq!(
        exec_lua(&rpc, "return btv.picker._sources.live_grep.layer").await,
        Value::from("main"),
    );
    assert_eq!(
        exec_lua(&rpc, "return btv.picker._sources.buffers.layer").await,
        Value::from("active"),
    );
}
...
"#,
    ...
    exec_lua(&rpc, "btv.picker.open('files_test')").await;
    let menu = menu_of(&poll_menu(&rpc, &mut incoming).await.expect("menu opens"));
...
"#,
    ...
    exec_lua(&rpc, "btv.picker.open('tabs_test')").await;
    let menu = menu_of(&poll_menu(&rpc, &mut incoming).await.expect("menu opens"));
    let preview = preview_of(&menu).expect("file picker carries a preview pane");
...
"#,
    ...
    exec_lua(&rpc, "btv.picker.open('loc_test')").await;
    let menu = menu_of(&poll_menu(&rpc, &mut incoming).await.expect("menu opens"));
...
"#,
    ...
    exec_lua(&rpc, "btv.picker.open('ft_test')").await;
    let menu = menu_of(&poll_menu(&rpc, &mut incoming).await.expect("menu opens"));
...
"#,
    ...
    exec_lua(&rpc, "btv.picker.open('scroll_test')").await;
    let menu = menu_of(&poll_menu(&rpc, &mut incoming).await.expect("menu opens"));
    let preview = preview_of(&menu).expect("file picker carries a preview pane");
...
"#,
    ...
    exec_lua(&rpc, "btv.picker.open('reset_test')").await;
    let menu = menu_of(&poll_menu(&rpc, &mut incoming).await.expect("menu opens"));
...
"#,
    ...
    let (rpc, mut incoming) = start(&dir, &src).await;
    exec_lua(&rpc, "btv.picker.open('geom')").await;
    let menu = menu_of(&poll_menu(&rpc, &mut incoming).await.expect("menu opens"));
...
"#,
    ...
    let (rpc, mut incoming) = start(&dir, &src).await;
    exec_lua(&rpc, "btv.picker.open('missing')").await;
    let menu = menu_of(&poll_menu(&rpc, &mut incoming).await.expect("menu opens"));
...
"#,
    ...
    let (rpc, mut incoming) = start(&dir, &src).await;
    exec_lua(&rpc, "_G.picked = nil; btv.picker.open('fc')").await;
    poll_menu(&rpc, &mut incoming).await.expect("menu opens");
...
"#,
    ...
    exec_lua(&rpc, "btv.picker.open('pv')").await;
    let menu = menu_of(&poll_menu(&rpc, &mut incoming).await.expect("menu opens"));
    let preview = preview_of(&menu).expect("preview pane");
    let half = (preview_lines(&preview).len() as u64 / 2).max(1);
    assert_eq!(
...
"#,
    ...
    exec_lua(&rpc, "btv.picker.open('pvh')").await;
    let menu = menu_of(&poll_menu(&rpc, &mut incoming).await.expect("menu opens"));
    let preview = preview_of(&menu).expect("preview pane");
...
async fn buffers_picker_empty_switchbuf_stays_in_current_tab() {
    ...
    exec_lua(&rpc, "btv.o.switchbuf = ''").await;
...
/// A location-less confirm (the `files` shape: `btv.picker.edit` on a `path`-only
...
"#,
    ...
    exec_lua(&rpc, "btv.picker.open('openfiles')").await;
    poll_menu(&rpc, &mut incoming).await.expect("menu opens");
    feed(&rpc, "<CR>");
    bemtvi_test_harness::barrier(&rpc).await;
...
"#,
    ...
    exec_lua(&rpc, "btv.picker.open('openone')").await;
    poll_menu(&rpc, &mut incoming).await.expect("menu opens");
    feed(&rpc, "<CR>");
    bemtvi_test_harness::barrier(&rpc).await;
...
async fn resume_reopens_the_last_picker_with_its_query() {
    // `btv.picker.resume()` replays the closed picker's frozen content: the displayed
    ...
    assert_eq!(
        exec_lua(&rpc, "return btv.picker._last.name")
            .await
            .as_str(),
        Some("fruits"),
    ...
    exec_lua(&rpc, "btv.picker.resume()").await;
...
async fn resume_with_no_prior_picker_is_a_graceful_noop() {
    ...
    let ok = exec_lua(&rpc, "return pcall(btv.picker.resume)").await;
...
async fn a_non_resumable_source_does_not_clobber_the_resume_slot() {
    ...
    let src = format!(
        "{STATIC_SRC}\n\
         btv.picker.source {{ name = 'scratch', resumable = false,\n\
    ...
    exec_lua(&rpc, "btv.picker.open('scratch')").await;
...
"#;
    ...
    let kept = exec_lua(
        &rpc,
        "local n = 0; for _ in pairs(btv.picker._last.picker.items) do n = n + 1 end; return n",
    )
    .await;
    assert_eq!(
        kept,
...
async fn resume_replays_the_snapshot_without_rerunning_a_dynamic_source() {
    ...
    exec_lua(&rpc, "btv.picker.open('shuf')").await;
    let menu = menu_of(&poll_menu(&rpc, &mut incoming).await.expect("opens"));
...
async fn builtin_native_sources_drive_without_config() {
    ...
    exec_lua(&rpc, "btv.picker.open('curbuf')").await;
    ...
    exec_lua(
        &rpc,
        "btv.diagnostic.set(1, 0, { { lnum = 1, col = 0, message = 'boom', \
    ...
    // hard-to-see literal blank (btv.keytrans), so the row is legible.
    exec_lua(
        &rpc,
        "vim.g.mapleader = ' '; btv.keymap.set('n', '<leader>xy', function() end, \
         { desc = 'space leader probe' })",
    )
    .await;
    exec_lua(&rpc, "btv.picker.open('keymaps')").await;
...
async fn keymaps_picker_lists_buffer_local_and_shadows_globals() {
    ...
    exec_lua(
        &rpc,
        "btv.keymap.set('n', 'aa', function() end, \
           { buffer = 0, desc = 'bemtvi-tree: Create a file' })
         btv.keymap.set('n', 'gx', function() end, { desc = 'global probe' })
         btv.keymap.set('n', 'gx', function() end, \
    ...
    assert!(
        rows.iter().any(|r| r.contains("@")
            && r.contains("aa")
            && r.contains("bemtvi-tree: Create a file")),
...
/// `btv.mark.list` reads the server-pushed `btv._marks` mirror, and the built-in
...
async fn builtin_marks_list_and_picker_jump() {
    ...
    // btv.mark.list surfaces mark `a` at the 0-based middle line (1) with its text.
    let info = exec_lua(
        &rpc,
        r#"for _, m in ipairs(btv.mark.list()) do
    ...
    assert_eq!(
        info.as_str(),
        Some("1|zzz middle"),
        "btv.mark.list reports mark a at line 1 (0-based) with the line's text"
    );

    // The picker lists it; filter to the unique word and confirm the jump.
    exec_lua(&rpc, "btv.picker.open('marks')").await;
    poll_menu(&rpc, &mut incoming)
        .await
...
/// (`btv.jumplist.get`), lists the focused window's jump history newest-first with
...
async fn builtin_jumplist_picker_lists_and_jumps() {
    ...
    let ok = exec_lua(&rpc, "return tostring(btv.picker._sources.jumplist ~= nil)").await;
    ...
    exec_lua(&rpc, "btv.picker.open('jumplist')").await;
    poll_menu(&rpc, &mut incoming)
        .await
...
/// chain (`rg -uu -g'!.git'` → `find … -not -path '*/.git/*'` → `btv.fs.walk{hidden}`,
...
async fn builtin_files_picker_lists_ignored_and_hidden_files() {
    ...
    exec_lua(&rpc, "btv.picker.open('files')").await;
    let mut rows = Vec::new();
    for _ in 0..60 {
        if let Some(m) = poll_menu(&rpc, &mut incoming).await {
            rows = menu_items(&menu_of(&m));
...
/// (`rg` → `grep` → the `btv.fs` walk) goes through the same `emit`, and the head is
...
async fn live_grep_stops_at_the_first_working_tool_even_with_no_matches() {
    ...
    let init = r#"
_G.cmds = {}
local real = btv.run_stream
btv.run_stream = function(spec)
...
async fn filter_boxes_are_hidden_until_toggled_then_take_the_typing() {
    ...
    exec_lua(&rpc, "btv.picker.open('paths')").await;
...
async fn the_exclude_box_drops_matching_paths_and_the_include_box_keeps_only_its_own() {
    ...
    exec_lua(&rpc, "btv.picker.open('paths', { exclude = 'target' })").await;
    ...
    exec_lua(&rpc, "btv.picker.open('paths', { include = 'src/**' })").await;
    let rows = rows_until(&rpc, &mut incoming, |r| {
    ...
    // makes one typed pattern mean the same to btv.glob and to ripgrep's `-g`. Without
    // it `*.lock` would never match `vendor/lib.lock` (btv.glob's `*` stops at `/`).
    exec_lua(&rpc, "btv.picker.open('paths', { include = '*.lock' })").await;
    let rows = rows_until(&rpc, &mut incoming, |r| {
...
async fn the_filter_boxes_narrow_live_grep_too() {
    ...
    exec_lua(&rpc, "btv.picker.open('live_grep', { include = 'src/**' })").await;
    feed(&rpc, "zqxmain");
    let rows = rows_until(&rpc, &mut incoming, |r| {
...
async fn a_collapsed_box_with_active_patterns_shows_a_badge() {
    ...
    exec_lua(
        &rpc,
        "btv.picker.open('paths', { include = 'src/**', exclude = 'a, b' })",
    )
    .await;
    let menu = menu_of(&poll_menu(&rpc, &mut incoming).await.expect("opens"));
    assert_eq!(
...
async fn open_seeds_the_boxes_and_can_reveal_them() {
    ...
    exec_lua(
        &rpc,
        "btv.picker.open('paths', { exclude = { 'target', 'vendor' }, filters = 'open' })",
    )
    .await;
    let menu = menu_of(&poll_menu(&rpc, &mut incoming).await.expect("opens"));
    assert!(
...
async fn a_filter_line_is_remembered_and_recalled_with_ctrl_up_down() {
    ...
    let stored = exec_lua(
        &rpc,
        r#"return table.concat(btv.picker.history("exclude"), "|")"#,
    )
    .await;
    assert_eq!(
        stored.as_str(),
...
async fn a_recalled_line_actually_filters() {
    ...
    exec_lua(&rpc, "btv.picker.open('paths', { exclude = '' })").await;
    let rows = rows_until(&rpc, &mut incoming, |r| {
        r.iter().any(|x| x.starts_with("target/"))
    })
    .await;
...
async fn setup_supplies_the_default_filter_and_open_opts_outrank_it() {
    // `btv.picker.setup` is the "stop showing me target/" knob: every filterable
    ...
    let src =
        format!("{FILTERABLE_SRC}\nbtv.picker.setup({{ exclude = {{ 'target/', 'vendor/' }} }})\n");
    let (rpc, mut incoming) = start(&dir, &src).await;
    ...
    exec_lua(&rpc, "btv.picker.open('paths', { exclude = 'README.md' })").await;
...
async fn a_used_line_outranks_the_configured_default() {
    ...
    let src = format!("{FILTERABLE_SRC}\nbtv.picker.setup({{ exclude = 'vendor/' }})\n");
    let (rpc, mut incoming) = start(&dir, &src).await;
...
async fn history_can_be_turned_off_and_forgotten() {
    ...
    close_picker(&rpc, &mut incoming).await;
    assert_eq!(
        exec_lua(&rpc, r#"return #btv.picker.history("exclude")"#)
            .await
            .as_u64(),
        Some(0),
    ...
    exec_lua(&rpc, "btv.picker.setup({ history = 20 })").await;
    exec_lua(&rpc, "btv.picker.open('paths', { exclude = 'target' })").await;
    poll_menu(&rpc, &mut incoming).await;
    close_picker(&rpc, &mut incoming).await;
    assert_eq!(
    ...
    exec_lua(&rpc, "btv.picker.forget_history()").await;
    assert_eq!(
        exec_lua(&rpc, r#"return #btv.picker.history("exclude")"#)
            .await
            .as_u64(),
...
async fn history_is_per_box_and_the_query_has_none() {
    ...
    exec_lua(
        &rpc,
        "btv.picker.open('paths', { include = 'src/**', exclude = 'target' })",
    )
    .await;
    poll_menu(&rpc, &mut incoming).await;
    close_picker(&rpc, &mut incoming).await;
    ...
    assert_eq!(
        exec_lua(
            &rpc,
            r#"return table.concat(btv.picker.history("include"), "|")"#
        )
        .await
        .as_str(),
        Some("src/**")
...
"#;
    let (rpc, mut incoming) = start(&dir, src).await;
    exec_lua(&rpc, "btv.picker.open('counted', { debounce = 120 })").await;
    poll_menu(&rpc, &mut incoming).await;
    assert_eq!(
        exec_lua(&rpc, "return _G.runs").await.as_u64(),
        Some(1),
...
"#;
    let (rpc, mut incoming) = start(&dir, src).await;
    command(&rpc, "set nonumber norelativenumber").await;
    exec_lua(&rpc, "_G.picked = nil; btv.picker.open('rows')").await;
    poll_menu(&rpc, &mut incoming).await;
