...
async fn resolve_capture(rpc: &Rpc, capture: &str) -> Option<Vec<(Value, Value)>> {
    let result = rpc
        .request("bemtvi_resolve_capture", vec![Value::from(capture)])
        .await
...
#[tokio::test]
async fn bemtvi_scheme_completion_match_is_visible_on_the_selected_row() {
    // Regression: in the built-in `bemtvi` scheme the fuzzy-match accent must stay
    ...
    let dir = temp_dir("bemtvi_scheme_match");
    let (rpc, _incoming) = start_with_config(&dir, "vim.cmd.colorscheme('bemtvi')\n").await;
...
async fn a_colorscheme_keeps_the_name_it_gave_itself() {
    ...
    // key off that name — bemtvi-line looks up `lualine/themes/<colors_name>.lua`,
...
#[tokio::test]
async fn builtin_bemtvi_colorscheme_loads_with_no_runtime_file() {
    // `:colorscheme bemtvi` must work with an empty config dir — the scheme is
    ...
    // file-backed scheme.
    let dir = temp_dir("builtin_bemtvi");
    let (rpc, mut incoming) = start_with_config(
        &dir,
        "vim.api.nvim_create_autocmd('ColorScheme', \
           { pattern = 'bemtvi', callback = function(o) print('themed:' .. o.match) end })\n",
    )
    .await;
    let map = redraw_after(&rpc, &mut incoming, ":colorscheme bemtvi<CR>").await;
    assert_eq!(
        field(&map, "message").and_then(Value::as_str),
        Some("themed:bemtvi"),
...
#[tokio::test]
async fn builtin_bemtvi_colorscheme_shows_eob_tildes() {
    ...
    let dir = temp_dir("eob_bemtvi");
    let (rpc, mut incoming) = start_with_config(&dir, "").await;
    let _ = redraw_after(&rpc, &mut incoming, ":colorscheme bemtvi<CR>").await;
...
#[tokio::test]
async fn builtin_bemtvi_colorscheme_themes_the_tabline() {
    ...
    let dir = temp_dir("tabline_bemtvi");
    let (rpc, mut incoming) = start_with_config(&dir, "").await;
    let _ = redraw_after(&rpc, &mut incoming, ":colorscheme bemtvi<CR>").await;
...
#[tokio::test]
async fn truecolor_attach_defaults_in_the_bemtvi_colorscheme() {
    // A client that declares truecolor support (`truecolor = true` in the attach
    // capabilities) with no config-chosen scheme lands on the bundled `bemtvi` One
    ...
    assert_eq!(
        hl_color(&normal, "fg"),
        Some(hex("abb2bf")),
        "a truecolor attach defaults in the bemtvi scheme's Normal fg"
    );
    assert_eq!(hl_color(&normal, "bg"), Some(hex("282c34")));
...
async fn truecolor_attach_respects_a_config_chosen_colorscheme() {
    ...
    // auto-default is skipped. The user's `cat` wins, not the bundled `bemtvi`.
...
async fn user_colors_file_overrides_the_builtin_scheme() {
    // A `colors/bemtvi.lua` on the runtimepath shadows the bundled scheme — the
    // runtimepath is searched first, the built-in is only the fallback.
    let dir = temp_dir("override_bemtvi");
    std::fs::create_dir_all(dir.join("colors")).expect("create colors dir");
    std::fs::write(
        dir.join("colors").join("bemtvi.lua"),
    ...
    assert_eq!(
        hl_color(&normal, "bg"),
        Some(hex("000000")),
        "a user colors/bemtvi.lua must win over the bundled scheme"
    );
}
...
fn write_compiler_fixture(dir: &std::path::Path) {
    ...
    // which won't create parents; the real plugin would `btv.fs.mkdir` it (async,
...
async fn a_new_colorscheme_drops_the_previous_scheme_groups() {
    ...
    // the bundled `bemtvi` One Dark that a truecolor attach defaults in survives
    ...
    exec_lua(
        &rpc,
        "btv.hl.define(0, 'MyPluginGroup', { fg = '#ff00ff' }) return 1",
    )
    .await;
...
/// `btv.hl.palette()` -> the named slot, as the `"#rrggbb"` string the API returns.
async fn palette_slot(rpc: &Rpc, slot: &str) -> String {
    let code = format!("return btv.hl.palette()[{slot:?}]");
    exec_lua(rpc, &code)
        .await
        .as_str()
        .unwrap_or_default()
...
async fn hl_palette_derives_the_builtin_scheme_hues() {
    // `btv.hl.palette()` is how a plugin picks default colors without hardcoding one
    // theme's hex values. Under `:colorscheme bemtvi` every accent slot must resolve
    ...
    for (slot, want) in [
        ...
        assert_eq!(
            ...
            want,
            "palette slot `{slot}` under :colorscheme bemtvi"
        );
    }
}
...
async fn hl_palette_follows_the_active_scheme_and_its_links() {
    ...
    // whose whole chain the theme leaves undefined falls back to the built-in bemtvi
    ...
    assert_eq!(
        ...
        "#21252b",
        "an unmodelled slot falls back to the built-in bemtvi value"
    );
...
async fn hl_fallback_yields_to_a_theme_but_replaces_its_own_stale_default() {
    // `btv.hl.fallback` is how a plugin ships default groups. The half a plain
    // `if not btv.hl.exists(name)` guard gets wrong is the RE-apply: `:colorscheme`
    ...
    let (rpc, mut incoming) = start_with_config(
        ...
        "local function paint()\n\
           btv.hl.fallback('PlugDim', { fg = btv.hl.palette().muted })\n\
         end\n\
         paint()\n\
    ...
    // With no theme, the default derives from the built-in bemtvi palette.
    assert_eq!(
...
async fn hl_fallback_never_clobbers_an_explicit_definition() {
    ...
    assert_eq!(
        exec_lua(
            &rpc,
            "return btv.hl.fallback('PlugKey', { fg = '#111111' })"
        )
        .await,
        Value::Boolean(true),
    ...
    exec_lua(
        &rpc,
        "btv.hl.define(0, 'PlugKey', { fg = '#ff00ff' }) return 1",
    )
    .await;
    assert_eq!(
        exec_lua(
            &rpc,
            "return btv.hl.fallback('PlugKey', { fg = '#222222' })"
        )
        .await,
        Value::Boolean(false),
...
async fn a_colorscheme_handler_sees_the_dropped_groups_as_gone() {
    ...
    let (rpc, mut incoming) = start_with_config(
        &dir,
        "_G.seen = {}\n\
         btv.on('ColorScheme', {}, function(o)\n\
           local d = btv.hl.get(0, { name = 'OnlyDark', link = false })\n\
           _G.seen[o.match] = { exists = btv.hl.exists('OnlyDark'), fg = d.fg }\n\
         end)\n",
    )
    .await;
    ...
    assert_eq!(
        exec_lua(
            &rpc,
            "local d = btv.hl.get(0, { name = 'Normal', link = false }) return d.bg"
        )
        .await
        .as_u64(),
        Some(0x00ef_f1f5),
