...
//! (`btv.helix.smart_case`), not the global `:set ignorecase`/`smartcase`, which it
...
async fn helix_search_is_smart_case_without_touching_global_options() {
    ...
    // Self-contained: the global search options stay at bemtvi's own default (off).
    assert_eq!(
...
/// `btv.helix.enable{ smart_case = false }` turns the default off: a lowercase
...
async fn smart_case_off_makes_helix_search_case_sensitive() {
    ...
    exec_lua(&rpc, "btv.helix.enable{ smart_case = false }").await;
...
async fn smart_case_off_falls_back_to_global_ignorecase() {
    ...
    exec_lua(&rpc, "btv.helix.smart_case(false)").await;
