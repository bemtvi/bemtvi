...
The laptop slept; QUIC's 30 s idle timeout (`crates/bemtvi-server/src/quic.rs`
...
B; `crates/bemtvi-gui/src/session.rs:1-9`). A remote session is a local
`bemtvi_server::run` over an in-process duplex, with its fs / proc / lsp / term seams
pointed at a `DaemonClient` (`crates/bemtvi-server/src/daemon.rs`). The four leg groups
...
1. **Dialer abstraction.** A `Dialer` that can produce a fresh connection on demand:
   ...
   - ssh/stdio: re-spawn the child (`ssh … bemtvi --daemon`) and reconnect over its
...
Retry policy (constants, overridable later by an `btv.o` option): ~5 attempts, backoff
...
- `btv.daemon.status()` → `"connected" | "reconnecting" | "disconnected"` (plus
  `attempt`/`max` detail); a non-daemon session reports `nil`/`"local"` so a component
  hides itself.
- A `DaemonStatusChanged` event (autocmd / `btv.on_*`) so a statusline component
  re-renders on change.
- `bemtvi-line` component (in `~/work/nxvim-plugins/nxvim-line`) rendering
...
- **Phase 3 — Editor integration: status + commands + `btv` API.** Thread the
  ...
  ex-commands (work on the TUI too). Expose `btv.daemon.status()` + the event to Lua.
  Tested via the harness (inject a handle into `ServerInit`).
- **Phase 4 — `bemtvi-line` component.** The colored statusline component in
...
- **Phase 5 — Reconnecting ssh/stdio link in GUI + TUI (DONE).** `connect_daemon_reconnecting`
  ...
  sleep-disconnect scenario** (ssh hop drops → EOF → auto-retry re-spawns `ssh … bemtvi
...
- **Phase 6 — State re-sync + polish (DONE).** On a genuine reconnect (a
  ...
  **re-stats** by threading each buffer's disk baseline through `fs_watch [path, known?]` so the
  ...
  (a colored `btv.daemon.status()` statusline segment, verified end-to-end).
- **Phase 7 — QUIC 4-stream reconnect + web/wasm + keep-alive tuning.**
  - *QUIC 4-stream reconnect (DONE).* The supervisor (`maintain_link` / `reconnect_cycle`)
    ...
    GUI `:connect bemtvi://…`) now thread the `ReconnectHandle` into `ServerInit.daemon_link`.
  ...
  - *Web/wasm supervisor mirror (DONE).* The reconnect supervisor is mirrored in the
    ...
    reconnect would hang). Status rides `btv.daemon.status()` exactly like native, via a new
    ...
    `eh_take_watch_requests` / `drainWatchRequests`). Tested over a **real** loopback `bemtvi
