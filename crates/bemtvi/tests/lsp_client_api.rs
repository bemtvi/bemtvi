...
//! real one, `$BEMTVI_LSP_CMD` overrides the spawn argv, and the mock's `record`
...
/// starting a server — the test drives `btv.lsp.config` / `enable` itself.
...
async fn offset_encoding_reports_what_the_server_negotiated() {
    ...
    // answers bemtvi's advertised `utf-8` preference — and wrong only on lines with a
    ...
    let encoding = exec_lua(
        &rpc,
        "return btv.lsp.clients({ bufnr = 0 })[1].offset_encoding",
    )
    .await;
...
async fn assigning_offset_encoding_renegotiates_the_live_client() {
    ...
    // relabelled the Lua handle, every column bemtvi sent would stay in the old
    ...
    exec_lua(
        &rpc,
        "btv.lsp.clients({ bufnr = 0 })[1].offset_encoding = 'utf-8'",
    )
    .await;
    feed(&rpc, "A?<Esc>");
...
async fn exec_cmd_sends_the_command_to_the_server() {
    ...
    exec_lua(
        &rpc,
        "btv.lsp.clients({ bufnr = 0 })[1]:exec_cmd({\n\
...
async fn exec_cmd_prefers_the_configs_own_command_handler() {
    ...
    .await;
    assert!(await_lua_eq(&rpc, "#btv.lsp.clients({ bufnr = 0 })", "1").await);
...
async fn position_params_counts_the_column_in_the_requested_encoding() {
    ...
    // returned bemtvi's own byte column would be right only for utf-8.
    ...
    let cols = exec_lua(
        ...
        "local out = {}\n\
         ...
         \x20 out[#out + 1] = btv.lsp.position_params({ encoding = enc }).position.character\n\
         end\n\
    ...
    let rest = exec_lua(
        &rpc,
        "local p = btv.lsp.position_params({ encoding = 'utf-8' })\n\
...
async fn text_document_params_names_the_buffers_file() {
    ...
    let (rpc, _incoming) = open_rust(dir.as_path(), "let x = 1\n").await;
    let uri = exec_lua(&rpc, "return btv.lsp.text_document_params(0).uri").await;
...
async fn a_uri_percent_encodes_the_characters_that_would_break_it() {
    ...
    let out = exec_lua(
        &rpc,
        "local u = btv.utils.uri_from_path('/tmp/a b/c#d/é.rs')\n\
         return u .. '|' .. tostring(btv.utils.uri_to_path(u))\n",
    )
    .await;
    assert_eq!(
        out.as_str(),
    ...
    let other = exec_lua(
        &rpc,
        "return tostring(btv.utils.uri_to_path('deno:/https/example.com/mod.ts'))",
    )
    .await;
    assert_eq!(
        other.as_str(),
...
async fn locations_to_items_converts_columns_and_reads_the_line() {
    ...
    // be right: the column comes back as bemtvi's own BYTE offset (the server counted
    // utf-16 units), and each item carries the source line — which for a file no
    // buffer holds means reading it, asynchronously, because bemtvi does no blocking
    // I/O.
    ...
    let out = exec_lua(
        ...
        &format!(
            "_G.items = nil\n\
             ...
             btv.lsp.locations_to_items(locs, {{ encoding = 'utf-16' }}):next(function(items)\n\
             \x20 _G.items = items\n\
             end)\n\
             return 'queued'",
            p = other.display()
...
async fn locations_to_items_prefers_an_open_buffers_unsaved_text() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.items = nil\n\
             btv.lsp.locations_to_items({{ {{ uri = 'file://{p}', range = {{\n\
