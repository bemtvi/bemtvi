...
/// `tbl_deep_extend` semantics, which `btv.lsp.config`'s deep-merge documents).
...
#[tokio::test]
async fn vim_fn_stdpath_returns_an_bemtvi_path() {
    ...
    assert!(
        !msg.is_empty() && msg.contains("bemtvi"),
        "stdpath('cache') should be a non-empty bemtvi path, got {msg:?}"
    );
}
...
async fn buf_local_user_command_is_scoped_to_its_buffer() {
    ...
    // `btv._user_commands` globally, so it fired everywhere.
...
async fn deleting_a_buffer_purges_its_buffer_local_commands_and_keymaps() {
    ...
    let probe = "local kc = 0\n\
         for _, e in ipairs(btv._keymaps) do if e.buffer == 2 then kc = kc + 1 end end\n\
         return tostring(btv._buf_user_commands[2] ~= nil) .. ',' .. tostring(kc)";
    assert_eq!(
...
async fn vim_json_encodes_compact_and_pretty_and_round_trips() {
    ...
    // `vim.json`/`btv.json` is the public codec: a compact encode by default, a
    ...
    let (rpc, mut incoming) = start_with_config(
        ...
        "local v = { a = 1 }\n\
         ...
         local pretty = btv.json.encode(v, { pretty = true })\n\
