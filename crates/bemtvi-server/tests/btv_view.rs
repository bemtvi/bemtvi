//! Behavior tests for `btv.open` / `btv.layer` (the layer-cross surface) and
//! `btv.view` (the plugin-owned, dockable read-only content surface that generalizes
//! the bottom panel). Driven black-box over RPC like `dock.rs`.
//!
//! `btv.open(path, { where = "main" })` opens a file in the main editor area even
//! when fired from a dock keymap; `btv.layer.focus` / `btv.layer.main` cross focus
//! between the main area and the docks. A `btv.view` is an inert (read-only) buffer a
...
/// `btv.open(file, { where = "main" })` fired while a dock is focused opens the file
...
#[tokio::test]
async fn btv_open_where_main_lands_in_the_main_area_from_a_dock() {
    let (rpc, _incoming) = start().await;
    let file = write_temp("btvopen_main", "txt", "file-contents\n");
    ...
    exec_lua(&rpc, &format!("btv.open({:?}, {{ where = 'main' }})", file)).await;
    assert_eq!(
        lines(&rpc).await,
        vec!["file-contents"],
...
/// `btv.open` without `where` (the default) opens in the **current** window — fired
/// from a dock, the file lands inside the dock, not the main area.
#[tokio::test]
async fn btv_open_default_opens_in_the_current_window() {
    let (rpc, _incoming) = start().await;
    let file = write_temp("btvopen_cur", "txt", "in-the-dock\n");

    feed_sync(&rpc, "imain<Esc>").await;
    exec_lua(&rpc, "btv.dock.open{ side = 'left', size = 20 }").await;
    exec_lua(&rpc, &format!("btv.open({:?})", file)).await;
    assert_eq!(
        lines(&rpc).await,
        vec!["in-the-dock"],
...
/// `btv.open` defaults to `'switchbuf'`-honoring reuse: opening a file that is already
...
#[tokio::test]
async fn btv_open_jumps_to_a_window_already_showing_the_file() {
    let (rpc, _incoming) = start().await;
    let file = write_temp("btvopen_jump", "txt", "shared-file\n");
    ...
    assert_eq!(
        lines(&rpc).await,
        vec![""],
        "the other window still holds its scratch: btv.open jumped, it did not edit-in-place"
    );
}

/// `btv.open(path, { reuse = false })` opts back into `:edit` semantics: it loads the
...
#[tokio::test]
async fn btv_open_reuse_false_edits_in_place() {
    let (rpc, _incoming) = start().await;
    let file = write_temp("btvopen_inplace", "txt", "shared-file\n");
    ...
    exec_lua(&rpc, &format!("btv.open({:?}, {{ reuse = false }})", file)).await;
...
/// `btv.open` with an opts table that does NOT say `where = "main"` opens in the
...
#[tokio::test]
async fn btv_open_with_non_main_opts_stays_in_the_current_window() {
    let (rpc, _incoming) = start().await;
    let file = write_temp("btvopen_opts", "txt", "in-the-dock\n");
...
/// `btv.layer.main()` / `btv.layer.focus(side)` round-trip focus between the main
/// area and a dock.
#[tokio::test]
async fn btv_layer_focus_round_trips_main_and_dock() {
    let (rpc, _incoming) = start().await;
    feed_sync(&rpc, "imain<Esc>").await;
    exec_lua(&rpc, "btv.dock.open{ side = 'right', size = 20 }").await;
    feed_sync(&rpc, "idock<Esc>").await;

    exec_lua(&rpc, "btv.layer.main()").await;
    assert_eq!(lines(&rpc).await, vec!["main"], "focused the main area");

    exec_lua(&rpc, "btv.layer.focus('right')").await;
    assert_eq!(lines(&rpc).await, vec!["dock"], "focused the dock back");
}

/// An unknown layer name is reported, not silently ignored.
#[tokio::test]
async fn btv_layer_focus_reports_an_unknown_name() {
    let (rpc, mut incoming) = start().await;
    exec_lua(&rpc, "btv.layer.focus('sideways')").await;
...
/// Regression: btv.cursor.set must move a *view*-backed window's cursor to an exact
/// column. btv.view's own cursor is line-granular (`:set_cursor` lands at col 0), and
...
#[tokio::test]
async fn btv_cursor_set_refines_a_view_window_cursor_column() {
    let (rpc, _incoming) = start().await;
    exec_lua(
        &rpc,
        r#"vw = btv.view.create{ name = "[T]", filetype = "help" }
    ...
    exec_lua(
        ...
        r#"vw:set_lines{ "first line here", "second line here" }
           vw:focus()
           vw:set_cursor(1)
           btv.cursor.set({ 1, 6 }, vw:winid())"#,
    )
    .await;
    assert_eq!(
        cursor(&rpc).await,
        (1, 6),
        "btv.cursor.set refined the view window's cursor column on re-show"
    );
}

/// `btv.view.create` + `:set_lines` produce a buffer whose lines a plugin controls
...
async fn view_create_and_set_lines_controls_the_buffer() {
    ...
    exec_lua(
        &rpc,
        r#"vw = btv.view.create{ name = "btv-tree", filetype = "btvtree" }
...
async fn view_mounts_in_a_left_dock() {
    ...
    exec_lua(
        &rpc,
        r#"vw = btv.view.create{ name = "tree" }
...
async fn tab_mount_plus_split_builds_a_two_pane_tab() {
    ...
    exec_lua(
        &rpc,
        r#"a = btv.view.create{}; a:set_lines{ "left" }
           b = btv.view.create{}; b:set_lines{ "right" }
           a:mount{ tab = true }
...
async fn view_name_shows_in_statusline_and_tab_label() {
    ...
    exec_lua(
        &rpc,
        r#"vw = btv.view.create{ name = "ours" }
           vw:set_lines{ "x" }
           vw:mount{ tab = true }"#,
    )
    .await;
...
/// `btv.buf.name` (alias `nvim_buf_get_name`) — what a Lua statusline plugin reads to
...
/// statusline (e.g. bemtvi-line) shows `[No Name]` for every diff pane / file-tree view.
/// Covers both the current-buffer fast path (`btv.buf.name(0)` via the `_cur_buf`
/// snapshot) and the by-handle path (the `btv._bufs` mirror a status bar reads per
/// window).
#[tokio::test]
async fn view_name_is_returned_by_buf_get_name() {
    ...
    .await;
    // By handle (the `btv._bufs` mirror): the view's buffer reports its name.
    let by_handle = exec_lua(&rpc, r#"return btv.buf.name(vw:bufnr())"#).await;
    assert_eq!(
        by_handle.as_str(),
        Some("ours"),
        "btv.buf.name(bufnr) of a view must be its create name, not [No Name]"
    );
    // The current-buffer fast path: focus the view (its own tick so the `_cur_buf`
    // snapshot refreshes), then read `btv.buf.name(0)`.
    exec_lua(&rpc, r#"vw:focus()"#).await;
    feed_sync(&rpc, "").await;
    let current = exec_lua(&rpc, r#"return btv.buf.name(0)"#).await;
    assert_eq!(
        current.as_str(),
        Some("ours"),
        "btv.buf.name(0) for the focused view must be its create name"
    );
}
...
async fn view_buffers_are_skipped_by_buffer_navigation() {
    ...
    let f1 = write_temp("btvview_nav_a", "txt", "alpha\n");
    let f2 = write_temp("btvview_nav_b", "txt", "beta\n");
    exec_lua(&rpc, &format!(r#"btv.open({f1:?}, {{ where = "main" }})"#)).await;
    exec_lua(&rpc, &format!(r#"btv.open({f2:?}, {{ where = "main" }})"#)).await;
...
async fn view_set_decor_lays_extmarks() {
    ...
    exec_lua(
        &rpc,
        r#"ns = btv.ns.create("btvview-decor")
           vw:set_decor(ns, {
...
/// dock drifted to. Regression test for the bemtvi-dap REPL "cursor jumps up after pressing
/// enter" bug.
#[tokio::test]
...
/// suspends in setup (`btv.await(btv.promise.delay)`), and only renders the message it sets
/// *after* the await resolves — so seeing that message on screen proves the framework
/// awaited setup, not raced past it. This is the `btv.view.component` async contract.
#[tokio::test]
    ...
    exec_lua(
        ...
        r#"_G.async_ran = false
           local C = btv.view.component({
             setup = function(ctx)
               btv.await(btv.promise.delay(5))   -- suspend the lifecycle
               _G.async_ran = true
...
async fn grabbing_view_floats_stack_and_focus_pops() {
    ...
    exec_lua(
        &rpc,
        r#"v1 = btv.view.create{}; v1:set_lines{ "modal-1" }
    ...
    exec_lua(
        &rpc,
        r#"v2 = btv.view.create{}; v2:set_lines{ "modal-2" }
...
/// settled. A statusline plugin renders from inside an event callback (bemtvi-line's
/// segments are invalidated by `TextChanged` / `BufEnter` / `FileType` / …, then re-render
/// reading `btv.buf.name`), and every lifecycle emitter re-seeds the current-buffer
/// snapshot that `btv.buf.name(0)`'s fast path reads. Two of those emitters seeded it from
...
async fn view_name_holds_while_an_autocmd_is_firing() {
    ...
    exec_lua(
        &rpc,
        r#"seen = {}
           vw = btv.view.create{ name = "ours", filetype = "ourview" }
           vw:set_lines{ "x" }
           vw:mount{ split = "vsplit" }
           vw:focus()
           for _, ev in ipairs({ "TextChanged", "BufEnter", "FileType", "BufWritePost" }) do
             btv.on(ev, {}, function()
               ...
               if btv.buf.current() == vw:bufnr() then
                 seen[#seen + 1] = ev .. ":cur=" .. tostring(btv.buf.name(0))
               end
               seen[#seen + 1] = ev .. ":handle=" .. tostring(btv.buf.name(vw:bufnr()))
             end)
           end"#,
    )
    .await;
