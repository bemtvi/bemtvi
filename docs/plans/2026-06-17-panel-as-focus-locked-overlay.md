...
autocmd (the `:ls`/`qf`/`btvdir` ftplugin model), never special-cased in the input loop:
`q`/`<Esc>` dismiss (the shared `btvlisting`/`btvbuffers`/`btvpanel` ftplugin), `<CR>` switches
buffers for `:ls`.
...
`apply_panel_action`, `PANEL_ON_SELECT`, `btv.panel.actions.*`) comes back.

## As built

- **Core** (`crates/bemtvi-core/src/editor/`): new slim `panel.rs` —
  `PanelState { window, prev_window }`, `open_panel` (mount/reuse), `open_script_panel`
  (the `btv.panel.open` entry, over a third singleton `script_panel_bufnr`), `close_panel`,
  ...
  diagnostics-text + `:ls`) becomes a panel; generic listings gained `filetype=btvlisting`.
- **Lua surface** (`btv.panel`): `open{ lines, filetype?, height? }` and `close()` — that is
  the whole API. `PanelOp::{Open,Close}` (ops.rs) drained in `effects.rs` into
  `open_script_panel` / `close_panel`; the `bemtvi_panel_is_open` RPC (dispatch.rs) backs
  chrome + tests. Behavior attaches via a `FileType` autocmd on the chosen filetype
  (default `btvpanel`) — `btv.panel.open` deliberately does **not** return a bufnr (a fresh
...
- **ftplugins** (`prelude/keymap.lua`): one `FileType` autocmd over the pattern list
  `{ btvlisting, btvbuffers, btvpanel }` installs the buffer-local default `q`/`<Esc>` →
  `btv.panel.close`. `:ls`'s `<CR>` closes the panel then `btv.schedule`s the
...
- **Tests**: `tests/editing/listings.rs` rewritten to assert panel behavior via the
  ...
  scripted `btv.panel.open` with a buffer-local `<CR>`. `:ls <CR>` coverage in `buffers.rs`
  still passes (now also closes the panel). Example: `examples/panel/`.

## Incidental fix

`crates/bemtvi-server/src/lsp/edit.rs` `show_code_actions` set the
...
1. **Panels are not documents.** The display buffers were showing in `:ls` and were
   ...
   / `btv.buf.list`) now filters out registry buffers via `is_panel_buffer`. `:lspanels` /
...
(`open_named_panel` is the single home all listings + `btv.panel.open` flow through);
navigating to it via `:lspanels` `<CR>` (which is just `:b <n>`, swapping inside the panel
window) shows its **last content** with no regenerating command run. `btv.panel.open` gained
a `name?` field (default `[Panel]`). New ftplugin: `FileType btvpanels` maps `<CR>` →
`:b <n>`; `btvpanels` joined the `q`/`<Esc>` dismiss pattern list.
...
- **`btv.view`** (the *persistent* dockable surface) is untouched and complementary — it is
  for docked plugin content; `btv.panel` is the *transient grabbing* surface.
- **Scripted-panel filetype churn**: `btv.panel.open` reuses one `script_panel_bufnr`
