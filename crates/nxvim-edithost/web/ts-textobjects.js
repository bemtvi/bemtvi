// In-worker tree-sitter TEXT OBJECTS for the nxvim edit-host.
//
...
// captures). The range logic below is a faithful port of nxvim-ts's
...
// OPFS path where `:TSInstall` caches grammars: /.nxvim/treesitter/<lang>/{parser.wasm,textobjects.scm}.
...
export function createTextObjects() {
  ...
  (async () => {
    ...
    } catch (e) { console.error('[nxvim] textobjects: vendor manifest load failed:', e); }
    try {
    ...
    ready = true;
    readyResolve();
  })().catch((e) => console.error('[nxvim] textobjects: init failed:', e && e.stack ? e.stack : e));
  ...
  async function ensure(reg) {
    ...
    try {
      ...
      console.error('[nxvim] textobjects: failed to load', reg, e);
      langs.set(reg, null);
    } finally {
      loading.delete(reg);
    }
  ...
  return {
    ...
    textObjectsAt(coreLang, text, capture, byte) {
      ...
      try {
        ...
        console.error('[nxvim] textobjects: query failed for', reg, e);
        return null;
      } finally {
        tree.delete();
      }
