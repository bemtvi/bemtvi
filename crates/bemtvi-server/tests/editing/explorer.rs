//! The in-window directory listing — bemtvi's file explorer (vim's netrw).
//!
...
//! a read-only listing buffer over `btv.fs`; `<CR>` opens the entry under the cursor and
//! `-` goes up. These tests drive it the way a user would and assert on the listing
//! buffer and on the file that gets opened.
//!
//! Because the fill is asynchronous (`btv.fs.readdir` settles off the editor tick), the
...
async fn opening_a_directory_lists_its_entries() {
    // `bemtvi <dir>` opens the explorer: a `../` up-entry, then directories first
...
async fn directory_rows_are_highlighted() {
    // The `btv.decor` provider colours directory rows: the `../` up-entry one group,
    ...
    // the `sub/` highlight.
    let map = wait_redraw(&mut incoming, |m| row_has_group(m, 1, "BtvDirDirectory")).await;
    assert!(row_has_group(&map, 0, "BtvDirParent"), "../ row coloured");
    assert!(
        row_has_group(&map, 1, "BtvDirDirectory"),
        "sub/ row coloured"
    );
    assert!(
        !row_has_group(&map, 2, "BtvDirDirectory") && !row_has_group(&map, 2, "BtvDirParent"),
