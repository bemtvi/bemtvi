...
try {
  ...
  check("seed: the tour + init.lua are present in OPFS",
    !!tour && /Welcome to bemtvi/.test(tour) && !!init && /catppuccin/.test(init),
  ...
  for (let i = 0; i < 60 && !tourOpen; i++) {
    if (/Welcome to bemtvi/.test(await page.evaluate(() => window.__bemtvi.lines()))) tourOpen = true;
    else await sleep(100);
  }
  check("seed: the guided tour opens as the startup buffer", tourOpen,
    `lines=${JSON.stringify((await page.evaluate(() => window.__bemtvi.lines())).slice(0, 60))}`);

  // 3. main.py highlights with no :TSInstall — the python grammar is in the offline bundle.
  await page.evaluate(() => window.__bemtvi.feed(":edit /main.py<CR>"));
  let styled = 0;
  for (let i = 0; i < 60; i++) {
