-- btv.cmdline_complete: command-line completion — the unified float-list widget's
-- **fifth orchestration** (docs/specs/2026-06-14-btv-ui-float-widget.md;
...
-- catalog merged with `btv.user_command.get()`, so a plugin command appears exactly
...
-- inline wildmenu it hands off to the full `btv.picker` overlay (a fuzzy finder with
...
-- on confirm splices the chosen path back into the command and runs it (`btv.cmd`),
-- so every file-taking command works — modifiers and all. See
-- `docs/plans/2026-06-26-cmdline-file-path-picker.md`.

btv.cmdline_complete = btv.cmdline_complete or {}
...
-- user commands (`btv.user_command.get()`) are appended in `_cmdline_complete_run`.
--
...
-- `:command`, …) in `bemtvi-server/src/excmd.rs`. When you add a user-facing ex-command
...
local BUILTIN = {
  ...
  { "quitall", ":quitall", "Quit bemtvi, failing if any buffer has unsaved changes." },
  ...
  { "checktime", ":checktime", "Reload buffers changed on disk outside bemtvi." },
  ...
  { "lua", ":lua {chunk}", "Execute {chunk} as Lua in the btv.* runtime." },
  ...
  { "undolist", ":undolist", "List the leafs in the tree of changes." },
  { "earlier", ":earlier [count]", "Go to an older text state (`g-`)." },
  { "later", ":later [count]", "Go to a newer text state (`g+`)." },
...
-- `btv.cmdline_complete.setup{ docs = true }`: enable the engine. `docs` toggles the
...
function btv.cmdline_complete.setup(opts)
  opts = opts or {}
  if type(opts) ~= "table" then
    error("btv.cmdline_complete.setup: expected a table, got " .. type(opts))
  end
  local docs = opts.docs
  if docs == nil then
    docs = true
  end
  btv._cmdline_complete_setup(docs == true)
end

-- Append the registered user commands (`btv.user_command.create`) to `out` so a
...
local function append_user_commands(out, seen)
  ...
  add(btv.user_command.buf_get(0))
  add(btv.user_command.get())
end
...
-- option from core's injected catalog (`btv._options_catalog`, the single source of
...
local function option_candidates()
  local out = {}
  for _, opt in ipairs(btv._options_catalog or {}) do
...
-- authoritative buffer mirror (`btv._bufs`). The name is both the shown label and the
...
-- live runtimepath (found synchronously via `btv.runtime_file`, so a plugin-installed
-- scheme shows the instant it lands on disk) plus the schemes bundled in the binary
-- (`btv._builtin_colorschemes`, injected by the server — the embedded `runtime/colors/`
...
local function colorscheme_candidates()
  ...
  for _, name in ipairs(btv._builtin_colorschemes or {}) do
    add(name)
  end
  for _, path in ipairs(btv.runtime_file("colors/*.lua", true)) do
    add(path:match("([^/]+)%.lua$"))
  end
  table.sort(names)
  local out = {}
...
-- name, from the highlight-registry mirror (`btv._hl_defs`, the same table `btv.hl.get`
-- reads). Sorted for a stable menu.
local function highlight_candidates()
  local names = {}
  for name in pairs(btv._hl_defs or {}) do
    names[#names + 1] = name
  end
  table.sort(names)
  local out = {}
...
-- language name in bemtvi (`language_of_path`), and those are exactly the languages
-- bemtvi highlights, so `btv._filetypes` is the meaningful, offline candidate set. Both
...
local function lsp_server_candidates()
  local names, seen = {}, {}
  for _, client in ipairs(btv.lsp.clients({ bufnr = 0 })) do
...
-- The autocmd events bemtvi emits, plus the accepted aliases (canonicalized on
-- registration). Hand-curated to mirror `docs/autocmd-events.md` — the authoritative
-- catalog, since bemtvi has no runtime registry of the *emitted* set to source from —
...
-- the live `btv._augroups` registry (name -> id). Sorted for a stable menu.
local function augroup_candidates()
  local names = {}
  for name in pairs(btv._augroups or {}) do
    names[#names + 1] = name
  end
  table.sort(names)
  local out = {}
...
-- from the register mirror (`btv._registers`, name -> { text, type }). The single-char
-- name is the label/insert; the docs pane previews the register's first line.
local function register_candidates()
  local names = {}
  for name in pairs(btv._registers or {}) do
    names[#names + 1] = name
  end
  table.sort(names)
  local out = {}
  for _, name in ipairs(names) do
    local reg = btv._registers[name]
...
-- modifier word is complete, `btv._cmdline_complete_run` strips it and completes the
...
-- the core / server dispatch (bemtvi has no `:aboveleft`/`:keepjumps`/… modifiers).
...
-- argument token (`btv._cmdline_set_arg`) — the user runs the filled line with <CR>.
local pending = nil
...
-- relative base joins onto cwd. Always returns an absolute path for `btv.fs`.
local function resolve_dir(base, cwd)
  if base == "" then
    return cwd
  end
  local p = btv.utils.expanduser(base)
  if p:sub(1, 1) ~= "/" then
    p = (cwd:gsub("/$", "")) .. "/" .. p
  end
  -- Strip the trailing "/" the typed base carries (`sub/`) so `btv.fs.readdir` gets a
...
-- directory (the one the typed base points at) via `btv.fs.readdir` (async, so it
...
btv.picker.source({
  ...
  items = btv.async(function(ctx)
    ...
    local ok, entries = pcall(btv.await, btv.fs.readdir(dir))
  ...
  confirm = function(item)
    ...
    if item.is_dir and not dirs_only then
      ...
      -- runs with `btv._picker` already cleared — re-opening inline would race). The
    ...
    btv._cmdline_set_arg(item.path)
  end,
})
...
-- wildmenu (see `btv._cmdline_complete_run`); only a promise-returning one routes
...
btv.picker.source({
  ...
  items = btv.async(function(ctx)
    ...
    local cands = btv.await(spec.promise)
  ...
  confirm = function(item)
    btv._cmdline_set_arg(item.insert or item.text)
  end,
})
...
-- `btv._cmdline_complete_run(line, col)`: the candidate source the server calls
...
--
-- The file-picker case is a SIDE EFFECT (it queues `btv.picker.open`) and returns
...
-- same argument token via `btv._cmdline_set_arg`. `dirs_only` lists directories only.
local function launch_path_picker(before, dirs_only)
  ...
  btv.picker.open("cmdline_files", popts)
...
local function user_command_complete(cmd)
  local rec = btv.user_command.buf_get(0)[cmd] or btv.user_command.get()[cmd]
...
local function launch_fn_picker(before, cmd, promise)
  ...
  btv.picker.open("cmdline_complete_fn", { query = prefix, title = ":" .. cmd })
  return { __picker = true }
end

function btv._cmdline_complete_run(line, col)
  ...
  if wrap_prefix and WRAPPER_COMMANDS[wrap_prefix:match("^(%a%w*)")] then
    local n = #wrap_prefix
    return btv._cmdline_complete_run(line:sub(n + 1), col - n)
  end
