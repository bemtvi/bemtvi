// Playwright verifier for the runtime `:connect nxvim://…` command (the browser twin of
// nxvim-gui's client-side `:connect`). Unlike verify-daemon.mjs — which hands the daemon URI
...
// `:connect nxvim://…` and pressing Enter through the real keydown interception. The proof is
...
// page WITHOUT `?daemon=`, then drive `:connect <uri>` + pressEnter via window.__nxvim and assert
// the off-tick fs seam re-points onto the wire.
//
// Prereqs: ./build.sh (dist/eh.mjs + eh.wasm + vendor/msgpack), `cargo build -p nxvim`
// (target/debug/nxvim), and a Chromium for Playwright. Run:  node verify-connect.mjs
...
const root = mkdtempSync(join(tmpdir(), "nxvim-connect-"));
...
try {
  ...
  check("Worker booted serverless (no ?daemon=); window.__nxvim.ready resolved", true);

  // ── 1. A non-nxvim:// argument is rejected loudly, the wire untouched ──────────────────
  await page.evaluate(() => window.__nxvim.feed(":connect nvim://oops"));
  const rejected = await page.evaluate(() => window.__nxvim.pressEnter());
  check(
    "reject: :connect with a non-nxvim:// URI is intercepted (handled client-side)",
  ...
  check(
    ...
    /nxvim:\/\//.test(statusAfterReject),
    `status=${JSON.stringify(statusAfterReject)}`,
  );

  // ── 2. `:connect nxvim://…` dials the real daemon at runtime ───────────────────────────
  await page.evaluate((u) => window.__nxvim.feed(":connect " + u), uri);
  const connected = await page.evaluate(() => window.__nxvim.pressEnter());
  check(
    "connect: :connect nxvim://… is intercepted (the editor core never sees it)",
  ...
  const opened = await until(page, () => window.__nxvim.lines(), (v) => v === FILE_CONTENT, 8000);
  check(
...
console.log(failures === 0
  ? "\nALL PASS — runtime :connect nxvim://… brings up the daemon fs seam at runtime (no ?daemon= param)"
  : `\n${failures} FAILED`);
