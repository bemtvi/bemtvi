...
//! that hasn't finished within [`GRACE`](unix::GRACE) (`NXVIM_EXIT_GRACE_MS`),
...
pub mod unix {
    ...
    /// polite request. Overridable with `NXVIM_EXIT_GRACE_MS` for a session with slow
    ...
    /// [`GRACE`], or the `NXVIM_EXIT_GRACE_MS` override when it parses.
    fn grace() -> Duration {
        std::env::var("NXVIM_EXIT_GRACE_MS")
            .ok()
