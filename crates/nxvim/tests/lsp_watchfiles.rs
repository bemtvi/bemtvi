...
//!
//! Wired like `lsp_progress.rs`: the scripted mock server (`nxvim --__lsp-mock`)
...
/// registration is acked before `nx.fs.watch` has actually armed the inotify watch, so
...
/// synthetic workspace, and analyses nothing — while every layer of nxvim looks fine.
#[tokio::test]
...
async fn the_watch_chain_works_over_a_daemon_session() {
    ...
    tokio::spawn(async move {
        let _ = nxvim_server::serve_luafs_watch_daemon(daemon_reader, daemon_writer).await;
    });
    let (host_reader, host_writer) = tokio::io::split(eh_end);

    let file_path = dir.join("a.rs");
    std::fs::write(&file_path, "fn main() {}\n").expect("write test file");
    let (rpc, _incoming) = nxvim_test_harness::spawn_with_daemon_lsp(ServerInit {
        file: Some(file_path.to_string_lossy().into_owned()),
        fs_watch: Some(nxvim_server::RemoteFsWatch::connect(
            host_reader,
            host_writer,
        )),
        ..Default::default()
...
async fn an_unregistered_capability_drops_its_watches() {
    ...
    const COUNT: &str = r#"
        local n = 0
        for _, regs in pairs(nx.lsp._registrations) do
