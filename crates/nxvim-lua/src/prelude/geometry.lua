-- nx._geom — the shared window-geometry normalizer.
--
-- One place validates the unified geometry vocabulary every surface (floats /
-- `nvim_open_win`, `nx.view`, pickers, the panel) accepts, and emits the wire
...
function nx._geom.align(a)
  if a == nil then
    return nil
  end
  if type(a) ~= "string" or not ALIGN[a] then
    error("invalid align " .. nx._geom._show(a) .. " (expected one of: " .. ALIGN_LIST .. ")", 3)
  end
  return a
end
...
function nx._geom.size(s)
  ...
  error(
    "invalid size " .. nx._geom._show(s) .. " (expected a number or a 'NNvw'/'NNvh'/'NN%' string)",
    3
  )
end
