...
//! [`HostFs`](bemtvi_core::HostFs) — that would park the single editor thread (the
//! one that processes input *and* paints) on the network. Instead `:w` snapshots the
//! buffer at command time into a [`PendingSave`] (see `bemtvi-core`), and this module
...
use bemtvi_core::{BufferId, FileStat, PendingSave};
...
impl EditHost {
    ...
    fn dispatch_save(&mut self, save: PendingSave) {
        if !self.fx.has_remote_fs() {
            ...
            self.editor
                .echo("bemtvi: off-tick save with no daemon filesystem".to_string());
            return;
        }
        ...
        match result {
            ...
            Err(e) => {
                ...
                if let Some(queued) = self.saves_queued.remove(&save.buffer) {
                    for q in queued {
                        self.editor.echo(format!(
                            "bemtvi: write of \"{}\" abandoned (an earlier write failed)",
                            q.path.display()
                        ));
                    }
                }
    ...
    pub(crate) fn cancel_quit_all_gate(&mut self, seq: u64) {
            ...
            .is_some_and(|g| g.waiting.contains(&seq))
        {
            self.quit_all_gate = None;
            self.editor
                .echo("bemtvi: :wqa aborted (a write failed); editor stays open".to_string());
        }
    }
