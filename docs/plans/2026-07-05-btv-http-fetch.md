# btv.http — a fetch-modeled async HTTP client

`btv.http.fetch(url[, opts])` is the Lua HTTP surface, modeled on the browser's
...
the `btv.fs` off-tick shape.

## Lua surface (`crates/bemtvi-lua/src/prelude/http.lua`)

- `btv.http.fetch(url, opts)` → promise of a `Response`.
...
- `Response`: `{ status, ok, statusText, headers = { [lowercased-name]=value }, body }`
  plus `:text()` (the body) and `:json()` (the body decoded via `btv.json.decode`).
- URL/query helpers (public, lib-backed): `btv.http.encode_query`,
  `btv.http.encode_uri_component`, `btv.http.build_url`. The encoding is the `rust-url`
  crates (`form_urlencoded` + `percent-encoding`) via the `btv._url_encode_*` bridges (in
  `bemtvi-lua`, wasm-safe) — the Lua side only flattens map/list shapes into pairs.

Fetch semantics: a 404/500 resolves with `ok == false`; only a DNS/connect/TLS/timeout
failure rejects. The wrapper bridges through `btv._http_fetch(cb_id, url, method, headers,
body, timeout_ms)` and settles the promise from `btv._cb_fns[cb_id](err, response)`.

## The one-shot off-tick op (the `btv.fs` twin)

Types live in `bemtvi-lua` (`ops.rs`: `HttpRequest` / `HttpResponse` / `HttpError`), so
they stay transport-free and wasm-safe (no `ureq` in `bemtvi-lua`). The wire codec is
...
same reason `btv.fs` / processes route to the daemon.

## Serverless browser (`bemtvi-edithost` + `worker.mjs`)
...
- `HostEffects::http_op` (wasm) records `(id, request)` on the `Sink`; drained by
  ...
  `EditHost::http_result`). Unlike `btv.fs`, **no host gate** — the browser always has
...
## Local-forced fetch (`btv.http.fetch_local`)

`btv.http.fetch_local(url, opts)` is identical to `fetch` but runs on **this machine's**
network even in a daemon session — the HTTP analogue of the plugin manager's local-only
`btv.fs` (`btv._local_fs_op`). (`local` is a Lua keyword, hence the `_local` name rather
than an option.) Implemented as a `local: bool` flag on `LoopOp::Http` /
`LoopCommand::Http` (bridged by `btv._http_fetch` = false / `btv._local_http_fetch` = true):
...
- `crates/bemtvi-server/tests/http.rs` — the native path against a loopback server
  (2xx / json / 404-resolves / POST body / transport-reject).
- `crates/bemtvi-server/tests/daemon_http.rs` — the `http_op` leg over an in-process
  duplex (the actor is `HttpBackend::Remote`, so a resolved response can only have crossed
  the wire).
- `crates/bemtvi-edithost/web/verify-http.mjs` — the serverless browser `fetch()` leg in a
...
- `examples/btv-http/` — a runnable playground (`\h` GET, `\j` JSON, `\p` POST, `\x`
  reject).
