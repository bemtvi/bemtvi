...
//! resolve to the grammar bemtvi actually installs for it.
//!
...
//! then looked for a grammar under that literal name — but bemtvi ships no `jsonc`
...
use bemtvi_core::{resolve_language, BufferId, OpenOutcome};
...
fn the_reported_aliases_resolve_to_their_grammars() {
    for (alias, grammar) in [
        ...
        ("bash", "bash"),
        ("c_sharp", "c_sharp"),
        // ...as does a name bemtvi's tables don't know at all (it may still be an
        // installed grammar — resolution must never lose it).
        ("bemtvi_no_such_lang", "bemtvi_no_such_lang"),
    ] {
        assert_eq!(
