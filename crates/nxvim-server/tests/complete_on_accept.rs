...
//! `nx.complete.source` item carrying `on_accept` OWNS the edit when its row is
    ...
    if ctx.prefix ~= '' then\n\
      ctx.push({\n\
        ...
        on_accept = function(_item, c)\n\
          ...
          nx.buf.set_text(c.buf, c.start_row, c.start_col, c.end_row, c.end_col, { 'REPLACED' })\n\
        end,\n\
      })\n\
    end\n\
  end,\n\
}\n\
nx.complete.setup { sources = { { 'snip' } }, min_chars = 2 }";
...
nx.complete.setup { sources = { { 'plain' } }, min_chars = 2 }";
...
nx.complete.setup { sources = { { 'boom' } }, min_chars = 2 }";
    let (rpc, mut incoming) = start(&dir, init).await;
    type_and_accept(&rpc, &mut incoming, "fo").await;
...
nx.complete.setup { sources = { { 'bad' } }, min_chars = 2 }";
    let (rpc, mut incoming) = start(&dir, init).await;
    feed(&rpc, "ifo");
