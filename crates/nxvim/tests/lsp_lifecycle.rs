...
//!
//! `nx.lsp.stop` / `nx.lsp.restart` kill the server synchronously, but the exit is
...
//! its list from `nx.lsp.clients()`, so a phantom makes the next `:LspStop` report that
...
/// Point `$NXVIM_LSP_CMD` at the binary's mock mode. The caller holds `serial_lock`.
...
async fn start_demo(dir: &Path) -> (Rpc, UnboundedReceiver<Incoming>) {
    ...
    exec_lua(
        &rpc,
        "nx.autocmd.create('LspDetach', { callback = function(a) \
    ...
    .await;
    eventually(&rpc, "#nx.lsp.clients()", 1, "the demo server came up").await;
    (rpc, incoming)
}
...
async fn stop_runs_the_exit_path_and_forgets_the_client() {
    ...
    exec_lua(&rpc, "nx.lsp.stop('demo')").await;

    eventually(&rpc, "#nx.lsp.clients()", 0, "the handle is dropped").await;
...
async fn restart_forgets_the_client_it_replaced() {
    ...
    exec_lua(&rpc, "nx.lsp.restart('demo')").await;

    eventually(&rpc, "#_G.exits", 1, "on_exit ran for the replaced process").await;
    eventually(&rpc, "#nx.lsp.clients()", 1, "exactly one live handle").await;
    std::env::remove_var("NXVIM_LSP_CMD");
}
