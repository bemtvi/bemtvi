...
async fn indent_of_current(rpc: &Rpc) -> String {
    exec_lua(
        rpc,
        r#"local b = nx.buf.current()
           return "ts=" .. tostring(nx.bo[b].tabstop) .. " et=" .. tostring(nx.bo[b].expandtab)"#,
    )
    .await
    .as_str()
    .unwrap_or_default()
...
async fn setglobal_of_a_scopeless_option_fails_loud() {
    ...
    assert_eq!(
        exec_lua(&rpc, "return nx.bo[nx.buf.current()].fileencoding")
            .await
            .as_str()
            .unwrap_or_default(),
        "utf-8",
...
/// without touching the buffer you are in. Before, both fell into the `nx._o_store`
...
async fn a_dock_window_is_born_from_the_window_tier() {
    ...
    exec_lua(&rpc, r#"nx.dock.open{ side = "left", size = 20 }"#).await;
    let so = exec_lua(&rpc, "return nx.wo[nx.win.current()].scrolloff").await;
    assert_eq!(
        so.as_i64(),
        Some(4),
...
async fn foldexpr_and_commentstring_follow_the_global_value() {
    ...
    command(&rpc, "set foldexpr=v:lua.nx.treesitter.foldexpr()").await;
    command(&rpc, "set commentstring=#%s").await;
    edit(&rpc, &file).await;
    assert_eq!(
        set_message(&rpc, &mut incoming, "foldexpr?").await,
        "foldexpr=v:lua.nx.treesitter.foldexpr()",
...
async fn nvim_set_option_value_writes_both_tiers_like_set() {
    ...
    exec_lua(
        ...
        r#"vim.api.nvim_set_option_value("tabstop", 7, { scope = "local" })
           vim.api.nvim_set_option_value("shiftwidth", 5, { buf = nx.buf.current() })"#,
    )
    .await;
    let pinned = exec_lua(
        &rpc,
        r#"local b = nx.buf.current()
           return nx.bo[b].tabstop .. "|" .. nx.bo[b].shiftwidth
...
/// "unknown option … a typo, or an option nxvim doesn't model" warning, which is
/// false (nxvim models `'fileencoding'` fully; it just has no tier) and sends the
/// reader hunting for a misspelling. The write is still rejected rather than dropped
/// into `nx._o_store`, where a later read would hand back a value nothing honors.
///
        ...
        r#"local bad = {}
           for _, r in ipairs(nx._options_catalog) do
             if r.scope == "buffer" and not r.global_tier then
               local seen = {}
               local old = nx.notify
               nx.notify = function(m) seen[#seen + 1] = tostring(m) end
               ...
               elseif nx._o_store[r.name] ~= nil then
...
/// surface: every buffer- or window-scoped option in `nx._options_catalog` must be
/// *routed* by `vim.opt` / `vim.o` to its scope — never fall into the `nx._o_store`
        ...
        r#"local bad = {}
           for _, r in ipairs(nx._options_catalog) do
             if r.scope == "buffer" or r.scope == "window" then
               -- `nx.o` routes by scope; an unrouted name lands in the catch-all.
               if nx._o_route(r.name) == nil then
                 bad[#bad + 1] = r.name .. "(" .. r.scope .. ")"
               end
               if r.abbrev and nx._o_route(r.abbrev) == nil then
...
/// halves used to land in `nx._o_store` — the core honored `:set foldmethod=expr`, but
...
async fn vim_opt_reaches_the_map_backed_buffer_nouns() {
    ...
    exec_lua(
        ...
        r#"vim.opt.foldmethod = "marker"
           vim.opt.foldexpr = "v:lua.nx.treesitter.foldexpr()"
           vim.opt.foldmarker = "<<<,>>>"
...
async fn the_window_mirror_carries_every_routed_window_option() {
    ...
    let out = exec_lua(
        &rpc,
        r#"local w = nx.win.current()
           return table.concat({
             tostring(nx.wo[w].foldcolumn), tostring(nx.wo[w].foldlevel),
             tostring(nx.wo[w].foldenable), tostring(nx.wo[w].breakindent),
             nx.wo[w].showbreak, nx.wo[w].breakindentopt,
             tostring(nx.wo[w].sidescroll), tostring(nx.wo[w].sidescrolloff),
             nx.wo[w].winhighlight,
           }, "|")"#,
    )
    .await;
    assert_eq!(
...
async fn the_buffer_mirror_carries_the_fold_options() {
    ...
    let out = exec_lua(
        ...
        r#"local b = nx.buf.current()
           return table.concat({
             nx.bo[b].foldmethod, tostring(nx.bo[b].foldnestmax),
             tostring(nx.bo[b].foldminlines), nx.bo[b].foldmarker,
           }, "|")"#,
    )
    .await;
        ...
        r#"local bad = {}
           for _, r in ipairs(nx._options_catalog) do
             if r.global_tier and (r.scope == "buffer" or r.scope == "window") then
               local mirror = r.scope == "window" and nx._wo_global or nx._bo_global
               -- `'regexsyntax'` is the one global-local name whose tier IS the
               -- editor-wide option (`nx._go_mirror`), not a per-scope tier table.
...
async fn vim_go_winhighlight_round_trips() {
    ...
    let win = exec_lua(
        &rpc,
        r#"return "[" .. nx.wo[nx.win.current()].winhighlight .. "]""#,
    )
    .await;
...
/// `sidescrolloff` were pushed into `nx._wo_global` and then read back as `nil`.
#[tokio::test]
async fn vim_go_reads_every_mirrored_window_option() {
    ...
    let win = exec_lua(
        &rpc,
        r#"local w = nx.win.current()
           return tostring(nx.wo[w].breakindent) .. "|" .. tostring(nx.wo[w].sidescroll)"#,
    )
    .await;
