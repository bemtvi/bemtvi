//! The native `nx.http.fetch` runner — a blocking [`ureq`] round-trip mapped onto the
//! typed [`HttpRequest`] / [`HttpResponse`] the `nx.http` promise resolves with. Runs on
...
use nxvim_lua::{HttpError, HttpRequest, HttpResponse};
use std::io::Read;
        ...
        Ok(resp) => {
            ...
            if req.redirect == "error" && (300..400).contains(&resp.status()) {
                return Err(HttpError {
                    message: format!(
                        "nx.http: redirect ({}) not followed (redirect = \"error\")",
                        resp.status()
                    ),
                });
            }
...
fn response_from_ureq(resp: ureq::Response) -> Result<HttpResponse, HttpError> {
    ...
    resp.into_reader()
        ...
        .map_err(|e| HttpError {
            message: format!("nx.http: reading response body: {e}"),
        })?;
    Ok(HttpResponse {
        status,
        status_text,
