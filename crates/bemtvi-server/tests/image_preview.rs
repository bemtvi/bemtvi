...
async fn image_file_previews_when_enabled() {
    ...
    // The config knob (`btv.o` is canonical; `vim.o` is the alias) turns it on.
...
async fn set_ex_command_enables_imagepreview() {
    // `:set imagepreview` must enable it just like `btv.o.imagepreview = true` does
...
async fn local_session_marks_not_remote_and_reads_bytes_over_the_rpc() {
    ...
    // bytes are *not* remote — the client decodes `path` directly. `bemtvi_image_read`
    ...
    let reply = rpc
        .request("bemtvi_image_read", vec![Value::from(path.as_str())])
        .await
        .expect("bemtvi_image_read responds");
    assert_eq!(
        reply,
        Value::Binary(b"PNGPLACEHOLDER\n".to_vec()),
        "bemtvi_image_read returns the file's raw bytes"
    );

    // A bad path fails loud (the client shows its `[image: …]` placeholder) — never a
    // silent empty image.
    let err = rpc
        .request("bemtvi_image_read", vec![Value::from("/no/such/img.png")])
        .await;
    assert!(
        err.is_err(),
        "bemtvi_image_read fails loud on an unreadable path (got {err:?})"
    );
}
...
async fn cli_file_arg_previews_after_config_enables_it() {
    // The reported flow: `BEMTVI_CONFIG=… bemtvi photo.png`. The file arg is opened at
    ...
    std::fs::write(dir.join("init.lua"), "btv.o.imagepreview = true\n").expect("write init.lua");
