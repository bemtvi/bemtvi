...
//
//   - every first-party module (catppuccin + the five nxvim-* plugins) loaded from the bundle
...
//   - the statusline RENDERS — nxvim-line projected styled status segments into the redraw
//     frame, including the "NORMAL" mode label (its lualine_a mode component);
//   - the python LSP is configured + enabled — nx.lsp has basedpyright with its cmd.
//
...
try {
  ...
  const luaResult = (code) =>
    page.evaluate((c) => window.__nxvim.execLua(c).then((r) => r.result), code).then(unwrap);

  // 1. Every first-party module loaded from the bundle (package.loaded set by the require()s).
  const PLUGINS = ["catppuccin", "nxvim-keys-helper", "nxvim-tree", "nxvim-line", "nxvim-lspconfig", "nxvim-diff"];
  ...
  // 3. The statusline renders — nxvim-line projected styled status segments (incl. the NORMAL
  ...
  check("demo: nxvim-line statusline renders (NORMAL mode segment in the frame)",
  ...
  const lsp = await luaResult(
    'return tostring(nx.lsp._enabled["basedpyright"]) .. "|" .. ' +
    'tostring((nx.lsp._config["basedpyright"] or {}).cmd and nx.lsp._config["basedpyright"].cmd[1])');
  ...
  await page.evaluate(() => window.__nxvim.feed("ggdGiplugins-ok<Esc>"));
  const edited = await page.evaluate(() => window.__nxvim.lines());
