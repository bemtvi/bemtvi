//! Behavior tests for `btv.http` — the promise-always, `fetch`-modeled HTTP client
...
//! `btv.async` continuation sets (`await_truthy`) rather than a fixed sleep — which would
...
async fn fetch_resolves_with_a_2xx_response() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.res = nil\n\
             btv.http.fetch('{base}/hello'):next(function(r) _G.res = r end)"
        ),
    )
    .await;
...
async fn fetch_json_decodes_the_body() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.name = nil\n\
             _G.count = nil\n\
             btv.async(function()\n\
               local r = btv.await(btv.http.fetch('{base}/data'))\n\
               local j = r:json()\n\
               _G.name = j.name\n\
               _G.count = j.count\n\
             end)()"
    ...
    assert_eq!(exec_lua(&rpc, "return _G.name").await.as_str(), Some("btv"));
...
async fn post_sends_a_json_body() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.body = nil\n\
             btv.http.fetch('{base}/echo', {{ method = 'POST', body = {{ hi = 'there' }} }})\n\
...
async fn fetch_rejects_on_a_transport_failure() {
    ...
    exec_lua(
        ...
        "_G.err = nil\n\
         _G.resolved = false\n\
         btv.http.fetch('http://127.0.0.1:1/nope', { timeout = 1500 })\n\
...
async fn redirect_follow_is_the_default() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.res = nil\n\
             btv.http.fetch('{base}/redirect'):next(function(r) _G.res = r end)"
        ),
    )
    .await;
    assert!(await_truthy(&rpc, "_G.res").await, "fetch never settled");
...
async fn redirect_manual_returns_the_3xx() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.res = nil\n\
             btv.http.fetch('{base}/redirect', {{ redirect = 'manual' }})\n\
...
async fn redirect_error_rejects() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.err = nil\n\
             _G.resolved = false\n\
             btv.http.fetch('{base}/redirect', {{ redirect = 'error' }})\n\
               :next(function() _G.resolved = true end)\n\
               :catch(function(e) _G.err = e.message end)"
        ),
    )
...
async fn query_params_are_appended_and_encoded() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.tgt = nil\n\
             btv.http.fetch('{base}/target', {{ query = {{ q = 'hello world', n = 2 }} }})\n\
...
async fn form_body_is_urlencoded() {
    ...
    exec_lua(
        ...
        &format!(
            "_G.body = nil\n\
             btv.http.fetch('{base}/echo', {{ method = 'POST', form = {{ a = 'x y', b = 'z' }} }})\n\
               :next(function(r) _G.body = r:text() end)"
        ),
    )
    .await;
...
async fn encode_query_helpers_are_public() {
    ...
    assert_eq!(
        exec_lua(&rpc, "return btv.http.encode_uri_component('a b/c')")
            .await
            .as_str(),
        Some("a%20b%2Fc")
    );
    assert_eq!(
        exec_lua(
            &rpc,
            "return btv.http.encode_query({ { 'q', 'a b' }, { 'q', 'c' } })"
        )
        .await
        .as_str(),
        Some("q=a+b&q=c")
    ...
    assert_eq!(
        exec_lua(
            &rpc,
            "return btv.http.build_url('http://h/p?x=1', { y = 2 })"
        )
        .await
        .as_str(),
...
async fn fetch_local_works_like_fetch() {
    let base = spawn_test_server(1);
    let (rpc, _incoming) = start().await;
    // In a bare session `btv.http.fetch_local` is exactly `btv.http.fetch` — it resolves the
    ...
    exec_lua(
        ...
        &format!(
            "_G.res = nil\n\
             btv.http.fetch_local('{base}/hello'):next(function(r) _G.res = r:text() end)"
        ),
    )
    .await;
    assert!(
