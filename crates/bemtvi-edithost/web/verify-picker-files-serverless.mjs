...
// The `btv.picker` `files` source streams `rg --files`, falling back to `find` and then to a
// transport-agnostic `btv.fs` walk. On the serverless build there is no process host, so the
// rg/find spawns fail loud (code -1) and the picker must land on the btv.fs walk over OPFS —
// the bug was that it showed nothing. This seeds a couple of files under the cwd via btv.fs,
// opens the `files` picker, and asserts they appear in the picker's candidate list.
//
// Faithfulness (not a no-op): the files are seeded through the SAME btv.fs/OPFS seam the walk
// reads, the picker runs its real async source through the production tick, and the assertion
// reads the picker's live candidate list (`btv._picker.items`).
//
...
try {
  ...
  // Seed two files under the cwd through the btv.fs/OPFS seam the walk will read back.
  const seeded = await settle(page, "__seed", `btv.async(function()
       ...
       btv.await(btv.fs.write(base .. "/alpha.txt", "a"))
       btv.await(btv.fs.write(base .. "/sub/beta.txt", "b"))
       _G.__seed = "ok"
     end)()`);
  check("seeded alpha.txt + sub/beta.txt under cwd via btv.fs (OPFS)", /ok/.test(String(seeded)), `seed=${JSON.stringify(seeded)}`);

  // Open the real `files` picker — rg/find spawns fail loud (no host), so it must fall back
  // to the btv.fs walk and surface the seeded files.
  await luaResult(page, `btv.picker.open('files')`);
  const items = await pollPickerItems(page);
  check(
    "file picker falls back to the btv.fs walk and lists the seeded files (no rg/find)",
...
console.log(failures === 0
  ? "\nALL PASS — serverless file picker lists files via the btv.fs walk fallback"
  : `\n${failures} FAILED`);
