...
// `bemtvi --daemon --listen` over WebTransport. An async `vim.system` in the browser has no
...
// (target/debug/bemtvi), and a Chromium for Playwright. Run:  node verify-proc.mjs
...
const root = mkdtempSync(join(tmpdir(), "bemtvi-proc-"));
...
try {
  ...
  // The async spawn is driven through the real `btv._system_async` funnel (the same one the
  // public `btv.run` / `vim.system` wrappers call — `btv.run` builds the argv and registers a
  // callback id, then hands off to this funnel; the native proc leg is funnel-only too).
  // A callback id is registered in `btv._cb_fns`, exactly as the wrapper would, so this
  ...
  await luaResult(
    ...
    `_G.__out, _G.__code = nil, nil
     ...
     btv._cb_fns[id] = function(res) _G.__out, _G.__code = res.stdout, res.code end
     btv._system_async(id, { "sh", "-c", "printf hello-from-daemon" }, nil, nil, nil)
     return 1`,
  );
  const out = await until(page, () => window.__bemtvi.execLua("return _G.__out or ''").then((r) => r.result),
  ...
  await luaResult(
    page,
    `local id = btv._next_cb_id()
     btv._cb_fns[id] = function() _G.__wrote = true end
     btv._system_async(id, { "sh", "-c", "printf RAN-ON-DAEMON > ${marker}" }, nil, nil, nil)
     return 1`,
  );
  await until(page, () => window.__bemtvi.execLua("return _G.__wrote and 1 or 0").then((r) => r.result),
    (v) => /1$/.test(String(v)));
  let onDisk = "";
  for (let i = 0; i < 50; i++) {
  ...
  await luaResult(
    ...
    `_G.__killed_code = nil
     _G.__kid = btv._next_cb_id()
     btv._cb_fns[_G.__kid] = function(res) _G.__killed_code = res.code end
     btv._system_async(_G.__kid, { "sh", "-c", "sleep 30" }, nil, nil, nil)
     return 1`,
  );
  // Give the spawn a beat to register on the daemon, then kill it.
  await sleep(200);
  await luaResult(page, "btv._system_kill(_G.__kid, nil) return 1");
  const killedCode = await until(page,
    () => window.__bemtvi.execLua("return tostring(_G.__killed_code)").then((r) => r.result),
...
console.log(failures === 0
  ? "\nALL PASS — browser edit-host runs vim.system on a real bemtvi --daemon over WebTransport (stdout, daemon-side effect, kill)"
  : `\n${failures} FAILED`);
