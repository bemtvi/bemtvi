...
//! ssh/QUIC link to `bemtvi --daemon`). This is the fetch half of "config and plugins
...
//! `BEMTVI_CONFIG`, and the bundle's bytes can only have come from walking that tree
...
//! the prior `BEMTVI_CONFIG` / `BEMTVI_RUNTIMEPATH`.
...
/// value on drop — so a test can scope `BEMTVI_CONFIG` to its temp tree without leaking
...
fn populate_config(dir: &std::path::Path) -> PathBuf {
    std::fs::write(dir.join("init.lua"), "btv.o.tabstop = 7\n").unwrap();
...
fn connect_over_wire() -> RemoteConfig {
    ...
    tokio::spawn(async move {
        let _ = bemtvi_server::serve_config_daemon(daemon_reader, daemon_writer).await;
    });
...
/// the metadata (`false`).
async fn fetch_over_wire(include_files: bool) -> bemtvi_server::RemoteConfigBundle {
    connect_over_wire()
        .fetch(include_files)
        .await
...
async fn config_bundle_ships_the_daemons_config_and_plugins() {
    ...
    let _c = EnvGuard::set("BEMTVI_CONFIG", Some(&cfg));
    let _r = EnvGuard::set("BEMTVI_RUNTIMEPATH", None);
...
async fn config_bundle_is_empty_when_the_daemon_has_no_config() {
    ...
    let _c = EnvGuard::set("BEMTVI_CONFIG", Some(&missing));
    let _r = EnvGuard::set("BEMTVI_RUNTIMEPATH", None);
...
async fn config_bundle_lists_the_daemons_installed_treesitter_languages() {
    ...
    let _c = EnvGuard::set("BEMTVI_CONFIG", Some(&cfg));
    let _r = EnvGuard::set("BEMTVI_RUNTIMEPATH", None);
    let _d = EnvGuard::set("BEMTVI_DATA_DIR", Some(&data));
...
async fn config_bundle_lite_fetch_skips_the_files() {
    ...
    let _d = EnvGuard::set(
        "BEMTVI_DATA_DIR",
        Some(&temp_dir("daemon_config_lite_data")),
    );
...
///
/// Faithful, not a no-op: the daemon's config lives in temp dir A (`BEMTVI_CONFIG`), the
...
/// crossed the daemon wire from A into B — there is no local `~/.config/bemtvi` in play.
#[tokio::test]
async fn a_daemon_session_loads_the_remotes_config_and_plugins() {
    ...
    let _d = EnvGuard::set("BEMTVI_DATA_DIR", Some(&temp_dir("e2e_remote_tsdata")));
    ...
    assert_eq!(
        exec_lua(&rpc, "return btv.o.tabstop").await.as_i64(),
        Some(7),
...
async fn drain_until_exit(
    mut incoming: tokio::sync::mpsc::UnboundedReceiver<bemtvi_rpc::Incoming>,
) {
...
async fn remote_session_init(
    ...
    let (store, remote_shada) = bemtvi_server::resolve_session_shada(
        &client.host_fs,
        ConfigSource::Remote,
        resolved.state_dir.as_deref(),
        namespace,
        bemtvi_server::default_shada(),
    )
    .await;
    assert!(
...
async fn remote_config_session_keeps_shada_on_the_daemon() {
    ...
    let _c = EnvGuard::set("BEMTVI_CONFIG", Some(&temp_dir("remote_shada_cfg")));
    let _r = EnvGuard::set("BEMTVI_RUNTIMEPATH", None);
    let _d = EnvGuard::set("BEMTVI_DATA_DIR", Some(&temp_dir("remote_shada_data")));
    ...
    // native shada dir.
    let remote_dir = state.join("bemtvi/shada/remote");
...
async fn local_config_session_keeps_shada_local() {
    ...
    let _c = EnvGuard::set("BEMTVI_CONFIG", Some(&temp_dir("local_shada_cfg")));
    let _r = EnvGuard::set("BEMTVI_RUNTIMEPATH", None);
    let _d = EnvGuard::set("BEMTVI_DATA_DIR", Some(&temp_dir("local_shada_data")));
    ...
    let shada = state.join("bemtvi/shada");
    assert!(
...
fn store_files_in(dir: &std::path::Path) -> Vec<PathBuf> {
    std::fs::read_dir(dir)
        ...
        .flatten()
        .map(|e| e.path())
        .filter(|p| bemtvi_server::is_store_file(p))
        .collect()
}
...
async fn remote_shada_compacts_and_merges_across_sessions() {
    ...
    let _c = EnvGuard::set("BEMTVI_CONFIG", Some(&temp_dir("remote_shada_compact_cfg")));
    let _r = EnvGuard::set("BEMTVI_RUNTIMEPATH", None);
    let _d = EnvGuard::set(
        "BEMTVI_DATA_DIR",
...
async fn remote_shada_namespace_isolates_projects() {
    ...
    let _c = EnvGuard::set("BEMTVI_CONFIG", Some(&temp_dir("remote_shada_ns_cfg")));
    let _r = EnvGuard::set("BEMTVI_RUNTIMEPATH", None);
    let _d = EnvGuard::set("BEMTVI_DATA_DIR", Some(&temp_dir("remote_shada_ns_data")));
    ...
    let ns_base = state.join("bemtvi/shada/ns");
    ...
    assert!(
        !state.join("bemtvi/shada/remote").exists(),
...
struct HoldShadaWrites {
    inner: bemtvi_server::RemoteHostFs,
    stats: std::sync::Arc<ShadaWriteStats>,
}

impl bemtvi_server::HostFsAsync for HoldShadaWrites {
    fn read(
        &self,
        path: String,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = std::io::Result<bemtvi_server::FsRead>> + Send>,
    > {
    ...
    fn write(
        ...
        Box<
            dyn std::future::Future<Output = std::io::Result<Option<bemtvi_core::FileStat>>> + Send,
        >,
    > {
    ...
    fn watch(&self, path: String, known: Option<bemtvi_core::FileStat>) {
    ...
    fn take_watch_events(
        &self,
    ) -> Option<tokio::sync::mpsc::UnboundedReceiver<bemtvi_server::WatchEvent>> {
...
async fn a_checkpoint_upload_never_overtakes_the_clean_exit_upload() {
    ...
    let _c = EnvGuard::set("BEMTVI_CONFIG", Some(&temp_dir("remote_shada_order_cfg")));
    let _r = EnvGuard::set("BEMTVI_RUNTIMEPATH", None);
    let _d = EnvGuard::set(
        "BEMTVI_DATA_DIR",
        Some(&temp_dir("remote_shada_order_data")),
    );
