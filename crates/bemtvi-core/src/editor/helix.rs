...
//! through the same registry the bundled `btv.helix` plugin binds
...
impl Editor {
    ...
    /// for the `btv.on_key_pending` (which-key) signal while a *native* Helix sub-grammar
    ...
    /// captures (`f`/`a`/`c`/`t`), and any `btv.textobject.map` registry key. An
    ...
    /// The named-verb seam the Helix-keymap plugin binds to (`btv._helix_action` →
    ...
    /// keymap can bind it (`btv.helix.actions.<name>`) — the goto/space menus and any
    ...
    pub fn apply_helix_action(&mut self, name: &str, count: Option<usize>) -> Result<(), String> {
        match name {
            ...
            // The self-contained smart-case search toggle (`btv.helix.smart_case` /
            // `btv.helix.enable{ smart_case = … }`). Settable from any mode — it only
