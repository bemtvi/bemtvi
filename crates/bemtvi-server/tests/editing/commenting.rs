...
#[tokio::test]
async fn explicit_commentstring_override_via_btv_bo() {
    // A plain `.txt` buffer has no filetype default; set one through `btv.bo`.
    let (rpc, _incoming) = start_file("ovr_bo", "txt", "hello\n").await;
    exec_lua(&rpc, "btv.bo.commentstring = '// %s'").await;
    // The override reads back through the mirror.
    assert_eq!(
        exec_lua(&rpc, "return btv.bo.commentstring").await.as_str(),
        Some("// %s"),
    );
    feed(&rpc, "gcc");
...
#[tokio::test]
async fn btv_bo_reflects_the_filetype_default() {
    // With no explicit override, `btv.bo.commentstring` reports the filetype default.
...
async fn a_string_rhs_keymap_can_drive_the_comment_operator() {
    ...
    exec_lua(&rpc, "btv.keymap.set('n', ',c', 'gcc')").await;
    feed(&rpc, ",c");
    assert_eq!(lines(&rpc).await, vec!["// let x = 1;"]);
}
