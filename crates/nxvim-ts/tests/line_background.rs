...
//! `NXVIM_DATA_DIR` pins the engine's search path to the fixture dir.

mod fixture;

use fixture::{install_markdown_grammar, install_rust_grammar, TempDir};
use nxvim_core::{BufferId, OpenOutcome, SyntaxEngine};
use nxvim_ts::Engine;
...
fn fenced_code_block_reports_every_line_as_a_background_line() {
    ...
    // nxvim install uses nvim-treesitter's `@markup.raw.block`. Pin that name so the
