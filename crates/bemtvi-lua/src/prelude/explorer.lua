-- btv.explorer visuals: highlighting for the in-window directory listing (vim's
-- netrw, `filetype=btvdir`). The listing itself — reading entries, `<CR>`/`-`
-- navigation — lives in the core (`editor/explorer.rs`), and its activation keys
-- are wired as buffer-local maps by the `FileType btvdir` autocmd in
-- `prelude/keymap.lua`. This file adds the *decoration* layer, and it does so the
-- way a plugin would: a viewport-scoped `btv.decor` provider (the new native
-- decoration API), not a special case baked into the renderer.
--
-- Loads AFTER `prelude/decor.lua` (it calls `btv.decor.provider`) — see the prelude
-- order in `runtime.rs`.
...
local HL = {
  BtvDirDirectory = { fg = "#89b4fa", bold = true }, -- a sub-directory row (suffixed `/`)
  BtvDirParent = { fg = "#94e2d5" }, -- the leading `../` up-entry
}
for name, spec in pairs(HL) do
  btv.hl.define(0, name, spec)
end

-- The provider: wake once per visible-range change of an `btvdir` window and colour
...
btv.decor.provider({
  name = "explorer",
  bufs = { filetype = { "btvdir" } },
  on_range = function(ctx, publish)
    local marks = {}
    for i, line in ipairs(ctx.lines) do
      local row = ctx.top + i - 1
      if line == "../" then
        marks[#marks + 1] = { row = row, col = 0, end_col = #line, hl = "BtvDirParent" }
      elseif line:sub(-1) == "/" then
        marks[#marks + 1] = { row = row, col = 0, end_col = #line, hl = "BtvDirDirectory" }
      end
    end
    publish(marks)
  end,
...
--
-- This is the netrw mechanism, built on `btv.*`. A local `:e dir` is claimed by a
-- `BufReadCmd` handler (Primitive B of docs/plans/2026-06-25-explorer-lua-port.md),
-- which reads the entries with `btv.fs` and renders them into the opened buffer. A
-- *remote* directory (a daemon / web session) is filled by the server from the entries
-- its off-tick fetch already read (the `btv.fs` op and the file-open fetch are separate
-- legs, and a daemon may wire only the latter) — but in BOTH cases the result is the
-- same: a `nomodifiable`, `filetype=btvdir` buffer named for the directory.
--
...
-- colours the rows. The activation maps are installed by the `FileType btvdir` autocmd
-- (prelude/keymap.lua), which fires the moment either fill path sets the filetype.
-- ============================================================================

-- Escaping a path for use as a bare `:edit` argument is `btv.fname.escape`
-- (vimfn.lua — loaded after this module, referenced only at open time).
local function edit_escape(path)
  return btv.fname.escape(path)
end

-- Render a `btv.fs.readdir` result into listing lines — the Lua twin of the server's
-- `bemtvi_core::dir_listing` (off-tick): `../` first, then sub-directories (suffixed
...
-- read, not an edit — no `[+]`), and mark it `btvdir` (drives the decor provider + the
-- `FileType btvdir` activation maps). A failed read is surfaced loud and leaves the
-- buffer empty rather than a half listing. The LOCAL fill path (the `BufReadCmd`
-- handler); a remote directory is filled server-side (`bemtvi_core::dir_listing`).
local function list_into(buf, dir)
  btv.async(function()
    ...
    if not ok then
      ...
      return btv.notify("explorer: can't read " .. dir .. ": " .. msg, 4)
    end
    ...
    btv.bo[buf].modifiable = true
    vim.api.nvim_buf_set_lines(buf, 0, -1, false, render(entries))
    btv.bo[buf].modifiable = false
    btv.bo[buf].modified = false
    btv.bo[buf].filetype = "btvdir"
  end)()
end
...
function btv.explorer._open(buf)
  local line = entry_under_cursor(buf)
  if not line then
    return
  end
  if line == "../" then
    return btv.explorer._up(buf)
  end
...
--   `btv.autocmd.create("FileType", { pattern = "btvdir", callback = function(a)`
...
btv.explorer.actions.up = function()
  btv.explorer._up(vim.api.nvim_get_current_buf())
end

-- `btv.explorer.enable()`: turn the explorer on. Registers the `BufReadCmd` handler that
...
function btv.explorer.enable()
  if btv.explorer._enabled then
    return
  end
  btv.explorer._enabled = true
  btv.autocmd.create("BufReadCmd", {
    pattern = "*",
