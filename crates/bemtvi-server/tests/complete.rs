//! Behavior tests for `btv.complete` — the native completion engine over the
...
//! `btv.complete.setup{}`, completion is driven over the same msgpack-RPC a UI
...
const BUFFER_INIT: &str = "btv.complete.setup { sources = { { 'buffer', min_chars = 2 } } }";
...
async fn does_not_suggest_the_word_under_the_cursor() {
    ...
    let (rpc, mut incoming) = start(
        &dir,
        "btv.complete.setup { sources = { { 'buffer', min_chars = 2 } }, auto = false }",
    )
    .await;
...
/// `btv.complete.accept{ behavior = … }` is remappable: two keys can accept under
/// different behaviors regardless of the configured default. Here the default is
/// `replace`, but a `<C-j>` mapped to `btv.complete.accept('insert')` keeps the suffix,
/// while a `<C-l>` mapped to `btv.complete.accept('replace')` swaps the whole word.
#[tokio::test]
async fn mapped_accept_keys_choose_behavior() {
    ...
    let (rpc, mut incoming) = start(
        &dir,
        "btv.complete.setup { sources = { { 'buffer', min_chars = 2 } }, auto = false }\n\
         btv.keymap.set('i', '<C-j>', function() btv.complete.accept('insert') end)\n\
...
/// The completion trigger is a Lua keymap installed by `btv.complete.setup` (it is no
...
/// the moved keymap fires `btv.complete.trigger()`.
#[tokio::test]
async fn trigger_key_opens_the_popup() {
    let dir = temp_dir("complete_trigger_key");
    let (rpc, mut incoming) = start(
        &dir,
        "btv.complete.setup { sources = { { 'buffer' } }, auto = false }",
    )
    .await;
    ...
    // The default trigger key opens it — proving the `btv.complete.setup`-installed
    // `<C-Space>` map fires `btv.complete.trigger()`.
    feed(&rpc, "<C-Space>");
    let menu = menu_of(
...
async fn an_unknown_source_fails_loud() {
    ...
    let err = exec_lua(
        &rpc,
        "local ok, e = pcall(function() btv.complete.setup { sources = { { 'made_up' } } } end) \
...
/// `btv.complete.setup { confirm = "first" }` flips the noselect default: a confirm
...
async fn confirm_selected_is_the_default_and_keeps_the_newline() {
    ...
    let (rpc, mut incoming) = start(
        &dir,
        "btv.complete.setup { sources = { { 'buffer', min_chars = 2 } }, confirm = 'selected' }",
    )
    .await;
...
async fn select_menu_keeps_its_full_border() {
    ...
    exec_lua(&rpc, "btv.ui.select({ 'one', 'two' }, {})").await;
...
async fn manual_trigger_opens_even_with_auto_off_and_below_min_chars() {
    ...
    let (rpc, mut incoming) = start(
        &dir,
        "btv.complete.setup { sources = { { 'buffer', min_chars = 5 } }, auto = false }",
    )
    .await;
...
// ---- Phase 4-B: async sources (btv.complete.source{}) --------------------------
...
btv.complete.setup { sources = { { 'buffer', min_chars = 2 }, { 'echo' } } }";
...
btv.complete.setup { sources = { { 'echo' } }, min_chars = 2 }";
    let (rpc, mut incoming) = start(&dir, init).await;
...
/// A source registered with `btv.complete.source{}` **after** `btv.complete.setup{}` has
...
async fn a_source_registered_after_setup_joins_the_live_engine() {
    ...
    let (rpc, mut incoming) =
        start(&dir, "btv.complete.setup { sources = { { 'buffer' } } }").await;
...
/// `btv.complete.setup { exclusive = true }` opts out of auto-join: a registered source
...
btv.complete.setup { sources = { { 'buffer', min_chars = 2 } }, exclusive = true }";
    let (rpc, mut incoming) = start(&dir, init).await;
...
/// Entering Select mode (`btv.win.select_range`, the seam a plugin snippet engine uses
...
async fn entering_select_mode_closes_the_completion_popup() {
    ...
    exec_lua(
        &rpc,
        "btv.win.select_range(0, 0, 0, 0, 2, { on_escape = 'insert' })",
    )
    .await;
    assert!(
        poll_no_menu(&rpc, &mut incoming).await.is_some(),
...
btv.complete.setup { sources = { { 'buffer', min_chars = 2 }, { 'snip' } } }";
    let (rpc, mut incoming) = start(&dir, init).await;
...
btv.complete.setup { sources = { { 'k' } }, min_chars = 1 }";
    let (rpc, mut incoming) = start(&dir, init).await;
...
btv.complete.setup { sources = { { 'k' } }, min_chars = 1 }";
    ...
    exec_lua(&rpc, "btv.o.pummaxwidth = 0").await;
    let uncapped = mu64(
...
/// A `btv.complete.source` item's `kind` field rides the push wire and projects onto
...
btv.complete.setup { sources = { { 'kinded' } }, min_chars = 2 }";
    let (rpc, mut incoming) = start(&dir, init).await;
...
btv.complete.setup { sources = { { 'silent' } }, min_chars = 2 }";
    let (rpc, mut incoming) = start(&dir, init).await;
...
btv.complete.setup { sources = { { 'buffer', min_chars = 3 }, { 'echo', min_chars = 2 } } }";
    let (rpc, mut incoming) = start(&dir, init).await;
...
async fn registering_a_reserved_builtin_name_fails_loud() {
    ...
    let err = exec_lua(
        ...
        "local ok, e = pcall(function() \
           btv.complete.source { name = 'buffer', complete = function() end } end) \
         return (not ok) and e or 'no error'",
    )
    .await;
    assert!(
...
btv.complete.setup { sources = { { 'deferred' } }, min_chars = 2 }";
    let (rpc, mut incoming) = start(&dir, init).await;
...
btv.complete.setup { sources = { { 'snip' } } }";
    let (rpc, mut incoming) = start(&dir, init).await;
...
btv.complete.setup { sources = { { 'doc' } }, docs_wrap = false }";
    let (rpc, mut incoming) = start(&dir, init).await;
    feed(&rpc, "ihe");
    let _ = poll_menu(&rpc, &mut incoming).await.expect("popup opens");
    feed(&rpc, "<C-n>");
...
btv.complete.source {\n\
  ...
  resolve = function(item)\n\
    return btv.promise.resolve { doc = 'resolved docs for ' .. item.text }\n\
  end,\n\
}\n\
btv.complete.setup { sources = { { 'lazy' } }, min_chars = 2 }";
    let (rpc, mut incoming) = start(&dir, init).await;
...
async fn a_resolve_function_must_be_a_function() {
    ...
    let err = exec_lua(
        &rpc,
        "local ok, e = pcall(function() \
           btv.complete.source { name = 's', complete = function() end, resolve = 42 } end) \
         return (not ok) and e or 'no error'",
    )
    .await;
    assert!(
...
btv.complete.setup { sources = { { 'docs' } } }";
    let (rpc, mut incoming) = start(&dir, init).await;
...
async fn docs_sidebar_respects_the_bottom_edge_past_a_top_dock() {
    ...
    exec_lua(&rpc, "btv.dock.open{ side = 'top', size = 5 }").await;
    feed(&rpc, "<C-w><C-w>j");
    bemtvi_test_harness::barrier(&rpc).await;
...
async fn wheeling_the_docs_sidebar_hit_tests_in_global_cells_past_a_dock() {
    ...
    bemtvi_test_harness::command(&rpc, "set nonumber norelativenumber showtabline=0").await;
...
/// bemtvi`) — projected as the `line_bg` layer, *not* as a `@markup.raw.block` span in
...
/// (`<C-Space>` → `btv.complete.trigger()`) opens the popup with row 0 preselected so a
