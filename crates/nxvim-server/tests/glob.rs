//! Behavior tests for the `nx.glob.*` surface — the canonical glob engine
//! (`nxvim_core::glob`: globset parses the pattern, the translated regex is compiled
...
/// `nx.glob.match(pat, path)` as a string, for the many one-line syntax cases.
async fn matches(rpc: &Rpc, pattern: &str, path: &str) -> String {
    eval(rpc, &format!("nx.glob.match({pattern:?}, {path:?})")).await
}
...
async fn literal_separator_false_lets_star_cross_a_separator() {
    ...
    let got = eval(
        &rpc,
        "nx.glob.match('*.lua', 'a/b/c.lua', { literal_separator = false })",
    )
    .await;
...
async fn basename_matches_the_last_component_only_for_separatorless_patterns() {
    ...
    assert_eq!(
        eval(
            &rpc,
            &format!("nx.glob.match('*.lua', 'a/b/c.lua', {base})")
        )
        .await,
        "true"
    );
    ...
    // is the path).
    assert_eq!(
        eval(&rpc, &format!("nx.glob.match('*.lua', 'c.lua', {base})")).await,
        "true"
    );
    assert_eq!(
        eval(&rpc, &format!("nx.glob.match('*.rs', 'a/b/c.lua', {base})")).await,
        "false"
    );
    ...
    assert_eq!(
        eval(
            &rpc,
            &format!("nx.glob.match('b/*.lua', 'a/b/c.lua', {base})")
        )
        .await,
        "false",
    ...
    assert_eq!(
        eval(
            &rpc,
            &format!("nx.glob.match('b/*.lua', 'b/c.lua', {base})")
        )
        .await,
        "true"
    );
...
async fn empty_alternates_opts_into_the_empty_brace_branch() {
    ...
    let got = exec_lua(
        ...
        "local on = { empty_alternates = true }\n\
         return tostring(nx.glob.match('x.{lua,}', 'x.', on))\n\
         \x20 .. '|' .. tostring(nx.glob.match('x.{lua,}', 'x.lua', on))\n\
         \x20 .. '|' .. tostring(nx.glob.match('x.{lua,}', 'x.rs', on))\n\
         \x20 .. '|' .. tostring(nx.glob.match('x.{lua,}', 'x.'))",
    )
    .await;
    ...
    let regexes = exec_lua(
        &rpc,
        "return nx.glob.to_regex('x.{lua,}')\n\
         \x20 .. ' || ' .. nx.glob.to_regex('x.{lua,}', { empty_alternates = true })",
    )
    .await;
...
async fn an_invalid_pattern_raises_naming_the_glob() {
    ...
    let got = exec_lua(
        &rpc,
        "local ok, err = pcall(nx.glob.match, '[z-a].txt', 'b.txt')\n\
...
async fn windows_style_treats_backslash_as_a_separator() {
    ...
    assert_eq!(
        eval(
            &rpc,
            &format!(r#"nx.glob.match('src\\*.rs', 'src\\main.rs', {win})"#)
        )
        .await,
        "true"
    );
    ...
    assert_eq!(
        eval(
            &rpc,
            &format!(r#"nx.glob.match('src\\*.rs', 'src\\a\\main.rs', {win})"#)
        )
        .await,
        "false",
    ...
    assert_eq!(
        eval(
            &rpc,
            &format!(r#"nx.glob.match('src\\**\\*.rs', 'src\\a\\b\\main.rs', {win})"#)
        )
        .await,
        "true"
    );
    ...
    assert_eq!(
        eval(
            &rpc,
            &format!(r#"nx.glob.match('C:/**/*.txt', 'C:\\Users\\me\\a.txt', {win})"#)
        )
        .await,
        "true"
    );
...
async fn unix_style_treats_backslash_as_an_escape_not_a_separator() {
    ...
    assert_eq!(
        eval(&rpc, &format!(r#"nx.glob.match('a\\*b', 'a*b', {unix})"#)).await,
        "true"
    );
    // ...and NOT an arbitrary run of characters, which an unescaped `*` would.
    assert_eq!(
        eval(&rpc, &format!(r#"nx.glob.match('a\\*b', 'axyzb', {unix})"#)).await,
        "false",
    ...
    assert_eq!(
        eval(&rpc, &format!(r#"nx.glob.match('a*b', 'a\\qb', {unix})"#)).await,
        "true",
...
async fn the_two_styles_disagree_on_the_same_input() {
    ...
    let got = exec_lua(
        ...
        "local p, path = 'a*b', 'a\\\\q\\\\b'\n\
         return tostring(nx.glob.match(p, path, { style = 'unix' }))\n\
         \x20 .. '|' .. tostring(nx.glob.match(p, path, { style = 'windows' }))",
    )
    .await;
    assert_eq!(
        got.as_str(),
...
async fn an_unknown_path_style_raises() {
    ...
    let got = exec_lua(
        &rpc,
        "local ok, err = pcall(nx.glob.match, '*.rs', 'a.rs', { style = 'win32' })\n\
         return tostring(ok) .. '|' .. tostring(err)",
    )
    .await;
    let got = got.as_str().unwrap_or_default();
...
async fn ignorecase_is_opt_in_and_independent_of_style() {
    ...
    assert_eq!(
        eval(
            &rpc,
            "nx.glob.match('*.TXT', 'a.txt', { ignorecase = true })"
        )
        .await,
        "true"
    );
    assert_eq!(
        eval(
            &rpc,
            r#"nx.glob.match('SRC\\*.RS', 'src\\a.rs', { style = 'windows' })"#
        )
        .await,
        "false",
...
/// `nx.glob.compile` hands back a reusable object: `:test`, `:pattern` (as written),
...
/// `nx.glob.to_regex` exposes the real translation: anchored, carrying the literal
...
///
/// It is asserted on shape rather than round-tripped through `nx.regex`: globset
/// emits a BYTE-oriented source (`(?-u)`, and `[^/]` can match invalid UTF-8), which
/// the str-based `nx.regex` rejects by design. That is why the engine compiles it
...
async fn to_regex_exposes_the_translation_including_the_options() {
    ...
    let got = exec_lua(
        &rpc,
        "local strict = nx.glob.to_regex('*.lua')\n\
         local loose = nx.glob.to_regex('*.lua', { literal_separator = false })\n\
         return strict .. '\\n' .. loose",
    )
    .await;
    let got = got.as_str().unwrap_or_default();
...
async fn to_regex_refuses_a_basename_glob_instead_of_answering_wrong() {
    ...
    let got = exec_lua(
        ...
        "local ok, err = pcall(nx.glob.to_regex, '*.lua', { basename = true })\n\
         ...
         local with_sep = nx.glob.to_regex('a/*.lua', { basename = true })\n\
         -- and the introspection form still answers for the reducing pattern.\n\
         local introspect = nx.glob.compile('*.lua', { basename = true }):regex()\n\
    ...
    assert_eq!(
        eval(
            &rpc,
            "nx.glob.match('*.lua', 'a/b/c.lua', { basename = true })"
        )
        .await,
        "true"
    );
...
/// `globset:test` — and `any` would blame `nx.glob.match` instead whenever it was
...
async fn any_and_filter_name_themselves_on_a_bad_path_argument() {
    ...
    let got = exec_lua(
        &rpc,
        "local _, list_err = pcall(nx.glob.any, { '*.rs' }, 7)\n\
         local _, str_err = pcall(nx.glob.any, '*.rs', 7)\n\
         local _, filt_err = pcall(nx.glob.filter, '*.rs', { 'a.rs', 'b.rs', 7 })\n\
    ...
    for err in [list_err, str_err] {
        assert!(
            err.contains("nx.glob.any: path must be a string, got number"),
            "both branches of `any` must name `any` itself, got {err:?}"
        );
    }
    assert!(
        filt_err.contains("nx.glob.filter: paths[3] must be a string, got number"),
...
async fn a_glob_set_reports_which_patterns_matched() {
    ...
    let got = exec_lua(
        &rpc,
        "local s = nx.glob.set({ '**/*.rs', '**/Cargo.toml', '*.tmp' })\n\
...
async fn a_mixed_basename_set_attributes_each_hit_correctly() {
    ...
    let got = exec_lua(
        &rpc,
        "local s = nx.glob.set({ '*.lua', 'src/*.rs' }, { basename = true })\n\
...
async fn an_invalid_pattern_fails_the_whole_set() {
    ...
    let got = exec_lua(
        &rpc,
        "local ok, err = pcall(nx.glob.set, { '*.rs', '[z-a]' })\n\
         return tostring(ok) .. '|' .. tostring(err)",
    )
    .await;
    let got = got.as_str().unwrap_or_default();
...
/// `nx.glob.any` takes either a single pattern string or a list, so a caller reading
...
async fn any_accepts_a_single_pattern_or_a_list() {
    ...
    let got = exec_lua(
        &rpc,
        "return tostring(nx.glob.any('*.rs', 'a.rs'))\n\
         \x20 .. '|' .. tostring(nx.glob.any('*.rs', 'a.lua'))\n\
         \x20 .. '|' .. tostring(nx.glob.any({ '*.rs', '*.toml' }, 'Cargo.toml'))\n\
         \x20 .. '|' .. tostring(nx.glob.any({ '*.rs', '*.toml' }, 'a.lua'))",
    )
    .await;
    assert_eq!(got.as_str(), Some("true|false|true|false"));
}

/// `nx.glob.filter` keeps the matching paths in their original order.
#[tokio::test]
async fn filter_keeps_matching_paths_in_order() {
    ...
    let got = exec_lua(
        ...
        "local paths = { 'z.rs', 'a.lua', 'm.rs', 'src/b.rs', 'k.toml' }\n\
         return table.concat(nx.glob.filter({ '*.rs', '*.toml' }, paths), ',')",
    )
    .await;
    assert_eq!(
        got.as_str(),
...
/// `nx.glob.is_glob` is the canonical "is this a pattern or a plain path" predicate.
#[tokio::test]
async fn is_glob_detects_every_metacharacter() {
    ...
    let got = exec_lua(
        ...
        "local out = {}\n\
         ...
         \x20 out[#out + 1] = tostring(nx.glob.is_glob(s))\n\
         end\n\
...
async fn a_non_utf8_path_matches_by_its_bytes() {
    ...
    let got = exec_lua(
        ...
        "local path = 'caf\\xe9/init.lua'\n\
         local g = nx.glob.compile('**/*.lua')\n\
         local s = nx.glob.set({ '**/*.lua', '**/*.rs' })\n\
         return table.concat({\n\
         \x20 tostring(nx.glob.match('**/*.lua', path)),\n\
         \x20 tostring(nx.glob.match('**/*.rs', path)),\n\
         \x20 tostring(g:test(path)),\n\
         \x20 tostring(s:test(path)),\n\
         \x20 table.concat(s:matches(path), '+'),\n\
         \x20 tostring(nx.glob.is_glob(path)),\n\
         ...
         \x20 tostring(nx.glob.match('caf?/*.lua', path)),\n\
         \x20 tostring(nx.glob.match('caf???/*.lua', path)),\n\
         \x20 tostring(nx.glob.match('cafe/*.lua', path)),\n\
         }, ',')",
    )
    .await;
    assert_eq!(
...
/// `nx._glob*` bridge behind it, as a wrong argument type does too.
#[tokio::test]
async fn a_non_utf8_pattern_and_a_wrong_type_both_fail_loud_by_public_name() {
    ...
    let got = exec_lua(
        ...
        "local out = {}\n\
         ...
         err(nx.glob.match, 'caf\\xe9*', 'x')\n\
         err(nx.glob.match, '*.lua', nil)\n\
         err(nx.glob.is_glob, nil)\n\
         err(nx.glob.compile, 42)\n\
         return table.concat(out, '\\n')",
    )
    .await;
    let got = got.as_str().unwrap_or_default();
    for expected in [
        "nx.glob.match: pattern must be valid UTF-8",
        "nx.glob.match: path must be a string, got nil",
        "nx.glob.is_glob: value must be a string, got nil",
        "nx.glob.compile: pattern must be a string, got integer",
    ] {
...
async fn the_compiled_cache_is_keyed_by_the_options_not_just_the_pattern() {
    ...
    let got = exec_lua(
        ...
        "local p, path = '*.LUA', 'a/b/c.lua'\n\
         ...
         out[#out+1] = tostring(nx.glob.match(p, path))\n\
         out[#out+1] = tostring(nx.glob.match(p, path, { literal_separator = false }))\n\
         out[#out+1] = tostring(nx.glob.match(p, path, { ignorecase = true }))\n\
         out[#out+1] = tostring(nx.glob.match(p, path))\n\
         out[#out+1] = tostring(nx.glob.match(p, path, { basename = true, ignorecase = true }))\n\
         out[#out+1] = tostring(nx.glob.match(p, path, { literal_separator = false, ignorecase = true }))\n\
         return table.concat(out, ',')",
    )
    .await;
    assert_eq!(
...
async fn repeated_matching_reuses_the_compiled_regex() {
    ...
    let got = exec_lua(
        ...
        "local n = 0\n\
         for i = 1, 20000 do\n\
         \x20 if nx.glob.match('src/**/*.{rs,toml}', 'src/a/b/mod.rs') then n = n + 1 end\n\
         end\n\
         return n",
    )
    .await;
...
/// A compiled `nx.glob.compile` object and the one-shot `nx.glob.match` share the
...
async fn compile_and_the_one_shot_form_agree_on_every_option_set() {
    ...
    let got = exec_lua(
        ...
        "local cases = {\n\
         ...
         \x20 local one = nx.glob.match(c[1], c[2], c[3])\n\
         \x20 local obj = nx.glob.compile(c[1], c[3]):test(c[2])\n\
         \x20 if one ~= obj then\n\
...
/// `nx.runtime_file` (a.k.a. `nvim_get_runtime_file`) matches its final component
...
async fn runtime_file_lookup_speaks_the_full_dialect_and_sorts() {
    let dir = nxvim_test_harness::temp_dir("glob_rtp");
    ...
    let got = exec_lua(
        ...
        "local function tails(name, all)\n\
         \x20 local out = {}\n\
         \x20 for _, p in ipairs(nx.runtime_file(name, all)) do\n\
    ...
    let first = exec_lua(
        &rpc,
        "local hits = nx.runtime_file('lsp/*.lua', false)\n\
    ...
    let literal = exec_lua(
        ...
        "local function tail(name)\n\
         \x20 local hits = nx.runtime_file(name, true)\n\
    ...
    if non_utf8 {
        ...
        let bytes = exec_lua(
            &rpc,
            "return #nx.runtime_file('lsp/caf?.lua', true)\n\
             \x20 .. ':' .. #nx.runtime_file('lsp/caf???.lua', true)",
        )
        .await;
        assert_eq!(
            bytes.as_str(),
