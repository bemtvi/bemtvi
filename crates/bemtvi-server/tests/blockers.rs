...
use bemtvi_rpc::Rpc;
use bemtvi_test_harness::{command, exec_lua, feed, start_with_config, temp_dir};
use rmpv::Value;
...
async fn missing_vim_fn_fails_loud_with_its_name() {
    ...
    assert_eq!(
        exec_lua(
            &rpc,
            "pcall(vim.fn.another_missing_one); return btv._notimpl_hits['vim.fn.another_missing_one'] == true",
        )
        .await,
        Value::Boolean(true),
    );
...
async fn screenrow_and_screencol_track_the_cursor() {
    ...
    // (so >= 1; bemtvi shows a gutter by default). Read the baseline, then assert
...
// terminal-byte encoding bemtvi can emit — neovim uses K_SPECIAL sequences bemtvi
...
async fn string_width_and_character_builtins() {
    ...
    // keytrans round-trips notation unchanged (bemtvi's internal form IS notation).
    assert_eq!(
...
/// `nvim_exec_lua` takes `args` only as an empty list: bemtvi has no channel that
