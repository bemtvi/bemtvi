//! The daemon wire protocol for the duplex `nx.process` leg (`dproc_*`) and the
//! `nx.socket` leg (`sock_*`) — the remote transports a WASM/web edit-host uses to run
...
async fn dproc_leg_round_trips_a_duplex_child_over_the_wire() {
    let (client, mut inc) = wire_leg(nxvim_server::serve_dproc_daemon_on);
...
async fn sock_leg_round_trips_a_tcp_connection_over_the_wire() {
    ...
    let (client, mut inc) = wire_leg(nxvim_server::serve_sock_daemon_on);
    client.notify(
        "sock_connect",
        vec![
            Value::from(2u64),
