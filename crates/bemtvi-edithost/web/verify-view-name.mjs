...
// a Lua statusline plugin (bemtvi-line) labels each window from `btv.buf.name(bufnr)`,
// but the buffer-name mirror that getter reads was sourced from the path-only
// `buffer_name`, which is empty for an `btv.view` (it has no file path). So every named
...
// `btv.buf.name` returns the name both by handle and for the focused buffer.
//
...
execLua(h, `vw = btv.view.create{ name = "ours" }
            vw:set_lines{ "x" }
...
const byHandle = execLua(h, `return btv.buf.name(vw:bufnr())`);
check("btv.buf.name(view bufnr) is the create name, not [No Name]",
  byHandle.includes("\"ours\""), `result=${JSON.stringify(byHandle)}`);

// Focused-buffer fast path (`btv.buf.name(0)` / `expand("%")` via the _cur_buf snapshot).
execLua(h, `vw:focus()`);
eh_input(h, "<Esc>");
const current = execLua(h, `return btv.buf.name(0)`);
check("btv.buf.name(0) for the focused view is the create name",
  current.includes("\"ours\""), `result=${JSON.stringify(current)}`);

eh_free(h);
console.log(failures === 0
  ? "\nALL PASS — an btv.view's name reaches btv.buf.name on the web build"
  : `\n${failures} FAILED`);
