...
//!   family, …), matched by the embedded [`nxvim_regex`] engine — vim's own
...
pub(crate) enum RegexEngine {
    /// Rust `regex` crate — canonical/PCRE syntax (the historical nxvim default).
    Pcre,
    /// The real vim regexp engine (`nxvim-regex`) — vim's "magic" dialect.
    Vim,
}
...
pub(crate) enum SearchRegex {
    ...
    Pcre(Regex),
    /// A vim-dialect pattern matched by [`nxvim_regex`]. Only built under the
    /// `vim-regex` feature.
...
fn compile_vim(pattern: &str, ignorecase: bool) -> Result<SearchRegex, String> {
    let re = nxvim_regex::VimRegex::compile(pattern)
...
impl VimSearch {
    ...
    fn exec(&self, line: &str, from: usize) -> Option<nxvim_regex::LineMatch> {
        self.re
