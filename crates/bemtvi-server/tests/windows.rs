...
async fn set_current_focuses_the_requested_window() {
    ...
    exec_lua(&rpc, &format!("btv.win.set_current({other})")).await;
    ...
    assert_eq!(
        now, other,
        "btv.win.set_current focuses the requested window"
    );
}
...
async fn open_win_non_string_border_errors_loudly() {
    ...
    let config = Value::Map(vec![
        ...
        (Value::from("height"), Value::from(5u64)),
        // neovim's per-side char-list border spec, which bemtvi doesn't model.
        (
            Value::from("border"),
...
/// drifted. Driven through the private `btv._open_float` / `btv._win_set_config`
...
async fn empty_title_clears_on_the_lua_op_path() {
    ...
    exec_lua(
        &rpc,
        "btv._open_float({ buf = 0, enter = true, relative = 'editor', anchor = 'NW', \
    ...
    exec_lua(
        &rpc,
        &format!("btv._win_set_config({float}, {{ title = '' }}) return 0"),
    )
    .await;
    let cfg = req(&rpc, "nvim_win_get_config", vec![Value::from(float)]).await;
    assert_eq!(
