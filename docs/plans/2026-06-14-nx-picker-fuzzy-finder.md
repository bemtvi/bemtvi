# `nx.picker` — the fuzzy finder on the unified float-list widget — phased plan

> Working checklist for **Phase 2** of the unified float-list widget
> (`docs/specs/2026-06-14-nx-ui-float-widget.md`): the prompt input-grab, the Rust
> fuzzy matcher, dynamic-source forwarding with generation tokens, and incremental
> streaming — which together unlock `nx.picker`. Project convention: lives at
> `docs/plans/2026-06-14-nx-picker-fuzzy-finder.md` alongside the other
> `docs/plans/*.md`.

## Context

The unified float-list widget (`docs/specs/2026-06-14-nx-ui-float-widget.md`) is the
single Rust component that completion, the fuzzy picker, and `nx.ui.select` all
...
the editor. The engines on top (`nx.complete` / `nx.picker` / `nx.ui.select`) are
...
`crates/nxvim-core/src/editor/menu.rs` is a floating, input-grabbing, selectable
list with the `MenuPlacement::{Cursor, Editor}` axis already modeled (so the picker
reuses the same widget). It backs `nx.ui.select` end-to-end across TUI / GUI / web:
...
1. **Streaming is in scope now.** The async spawn path is one-shot today:
   `nxvim-server/src/host.rs` reads the child to completion and emits a single
   `LoopEvent::ProcessExit` carrying the full stdout; `runtime.lua`'s process
   callbacks are one-shot; there is **no `on_stdout` and no `nx.spawn`** (only the
   one-shot `vim.system`). Incremental streaming (`ProcessStdout` events + a
   non-one-shot `on_stdout` dispatch + the `nx.spawn` wrapper) is therefore net-new
...
`nxvim-core` stays **pure and synchronous**. Core owns only: the prompt edit, the
...
`live_gen` — lives in `nxvim-server` and the Lua wrapper. This mirrors how the
...
exact `nx.ui.select` round-trip (`ui.lua:47–82`) generalized from "index" to "key".
...
## Phase 1 — The Rust fuzzy matcher (`nxvim-core`, pure)
...
- Add `nucleo-matcher` (matcher-only sub-crate: no async, no IO — honours core
  ...
  `=x.y.z`; pull into `nxvim-core` via `nucleo-matcher.workspace = true`.
- New pure module `crates/nxvim-core/src/fuzzy.rs`:
...
## Phase 2 — Extend `Menu` into the unified widget (`nxvim-core`, pure/sync)
...
- `open_menu` gains the picker shape (prompt + dynamic + `Editor` placement);
  `nx.ui.select` keeps calling the promptless `Cursor` shape unchanged.

## Phase 3 — Streaming spawn primitive (`nxvim-server` + `nxvim-lua`)
...
- `nxvim-lua`: add `nx.spawn { cmd, args, cwd, on_stdout, on_exit }` over the
...
## Phase 4 — The `nx.picker` engine (Lua wrapper + server orchestration)

- **Lua** — new `crates/nxvim-lua/src/prelude/picker.lua` (registered like
  `ui.lua`): `nx.picker.source { name, items=function(ctx,push,done), dynamic,
  confirm }` and `nx.picker.open(name)`. The wrapper keeps full item tables Lua-side
...
- **Built-in sources** (engine defaults + the runnable example): `files` =
  `nx.spawn rg --files` streaming pushes (static, local match); `live_grep` =
  `dynamic=true`, per-query `nx.spawn rg --vimgrep -- query` with
  `ctx.on_cancel(function() p:kill() end)`; `buffers` = in-memory from `nx.buf.*`,
  no spawn.
- **Live-query UX** (dynamic sources). To make live grep usable: a `debounce` (ms;
  global default 250 via `nx.picker.debounce`, overridable per source and per open)
...
- **Fixed box size (not content-derived).** An `Editor`-placed picker is a *fixed*
  ...
  Frac(f32) }` (nxvim-core) carried on the `Menu`/`MenuView`; the source/`open` set
  ...
  `project_menu`. Default ~80vw × 60vh. `nx.ui.select` stays content-anchored under
  the cursor. Clients render the **full** box height, padding empty rows past the
  item count.
- **`nxvim-view`** (`view.rs:319` `MenuData`): decode `query` + per-row match spans.
...
- **Tests** — new `crates/nxvim-server/tests/picker.rs`, black-box per the spec
  testing section:
...
- **No silent stubs** — `nx.spawn` must fully implement streaming or fail loud; a
  source that "opens" but never streams is exactly the quietly-broken shape the
  project rule forbids.

## Verification

1. `cargo build --workspace`; `cargo test -p nxvim-server --test picker`.
2. `cargo clippy --all-targets -- -D warnings` and `cargo fmt --all -- --check`.
3. `cargo run -p nxvim -- .` → a `<leader>` map to `nx.picker.open("files")`: type
