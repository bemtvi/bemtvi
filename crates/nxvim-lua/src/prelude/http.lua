-- nx.http — a promise-always HTTP client modeled after the browser's `fetch`.
--
-- Part of the pure-Lua `nx.*` prelude (see runtime.rs); the Lua half over the
-- `nx._http_fetch` Rust bridge in install.rs. Loaded AFTER promise.lua — the one
-- entry point returns a PROMISE (there are NO callbacks, matching `nx.run` / `nx.fs`).
--
-- Shape (matches the browser `fetch`): `nx.http.fetch(url[, opts])` returns a promise
...
-- ```lua
-- nx.async(function()
--   local res = nx.await(nx.http.fetch("https://api.example.com/search", {
...
--     nx.print(data.name)
--   else
--     nx.print("HTTP " .. res.status)        -- 404 / 500 / … still resolve here
--   end
-- end)()
-- ```
--
-- Query params (`opts.query`) and form bodies (`opts.form`) are encoded with the
-- `rust-url` crates (`form_urlencoded` / `percent-encoding`) via the `nx._url_encode_*`
-- bridges — an established encoder, not bespoke. The building blocks are public too:
-- `nx.http.encode_query`, `nx.http.encode_uri_component`, `nx.http.build_url`.
--
-- The bridge runs each request OFF the editor tick: `nx._http_fetch` queues a
...
-- ----- Response --------------------------------------------------------------
--
-- The resolved value of `nx.http.fetch`. A plain table the Rust side fills —
...
-- `res:json()` -> the body decoded from JSON into a Lua value. Uses `nx.json.decode`
-- (the shared codec). Raises on malformed JSON — inside `nx.async` that surfaces as a
-- promise rejection, exactly like the browser `response.json()` rejecting.
function Response:json()
  return nx.json.decode(self.body)
end
...
-- `nx.http.encode_uri_component(s)` -> `s` percent-encoded for use INSIDE a URL component
...
-- (`nx._url_encode_component`).
function nx.http.encode_uri_component(s)
  return nx._url_encode_component(tostring(s))
end

-- `nx.http.encode_query(params)` -> an encoded query string with NO leading `?`, built
...
-- (`rust-url`) crate's (`nx._url_encode_query`), so a space is `+` and a `+`/`&`/`=` in a
-- value is escaped correctly — no bespoke encoder.
function nx.http.encode_query(params)
  if type(params) ~= "table" then
    error("nx.http.encode_query: expected a table, got " .. type(params), 2)
  end
  local pairs_out = {}
  ...
  return nx._url_encode_query(pairs_out)
end

-- `nx.http.build_url(url, query)` -> `url` with `query` (a table, see `encode_query`)
...
function nx.http.build_url(url, query)
  if query == nil then
    return url
  end
  local qs = nx.http.encode_query(query)
  if qs == "" then
    return url
  end
...
local function build_headers(headers)
  ...
  if type(headers) ~= "table" then
    error("nx.http.fetch: `headers` must be a table, got " .. type(headers), 3)
  end
...
local function prepare_request(who, url, opts)
  ...
  url = nx.http.build_url(url, opts.query)
  ...
  if opts.form ~= nil then
    ...
    body = nx.http.encode_query(opts.form)
...
-- `bridge` (`nx._http_fetch` or `nx._local_http_fetch`), settling the promise on the reply.
local function do_fetch(who, bridge, url, opts)
  ...
  return nx.promise.new(function(resolve, reject)
    ...
    nx._cb_fns[id] = function(err, response)
      if err ~= nil then
        reject(err)
      else
...
-- `nx.http.fetch(url[, opts])` -> promise of a Response. The one entry point of the
-- `nx.http` client — modeled on the browser's `fetch`, fully async, non-blocking.
--
-- Args:
...
-- `fetch` makes. Await it inside `nx.async`, or chain `:next` / `:catch`:
--
-- ```lua
-- nx.async(function()
--   local res = nx.await(nx.http.fetch("https://api.example.com/search", {
--     query = { q = "hello world", page = 2 },   -- ?q=hello+world&page=2 (encoded for you)
--   }))
--   if res.ok then nx.print(res:json().name) else nx.print("HTTP " .. res.status) end
-- end)()
-- ```
function nx.http.fetch(url, opts)
  return do_fetch("nx.http.fetch", nx._http_fetch, url, opts)
end

-- `nx.http.fetch_local(url[, opts])` — identical to `nx.http.fetch`, but the request runs
-- on THIS machine's network (the native `ureq`, or the browser's own `fetch()`) even when
-- the session's `nx.http` routes to a daemon. The HTTP analogue of the plugin manager's
-- local-only `nx.fs` (`nx._local_fs_op`): reach for it when a request must originate from
-- the client, not the daemon (`local` is a Lua keyword, hence the `_local` name rather than
-- an option). In a bare/local session it is exactly `nx.http.fetch`.
function nx.http.fetch_local(url, opts)
  return do_fetch("nx.http.fetch_local", nx._local_http_fetch, url, opts)
end
