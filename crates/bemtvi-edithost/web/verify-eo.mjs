...
try {
  ...
  const cmd = await (async () => {
    for (let i = 0; i < 100; i++) {
      const v = await page.evaluate(() => window.__bemtvi.cmdline());
      if (v === ":eo") return v;
      await sleep(30);
    }
    return await page.evaluate(() => window.__bemtvi.cmdline());
  })();
  ...
  const finalMsg = await page.evaluate(() => window.__bemtvi.message());
  ...
  for (let i = 0; i < 100; i++) {
    if ((await page.evaluate(() => window.__bemtvi.cmdline())) === ":wo") break;
    await sleep(30);
  }
  await page.keyboard.press("Enter");
