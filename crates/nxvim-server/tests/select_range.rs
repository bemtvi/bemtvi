//! Black-box tests for **Select mode** and its Lua primitive `nx.win.select_range`
...
//!
//! These drive the primitive directly (a plugin would call `nx.win.select_range`
...
async fn select(rpc: &Rpc, s_col: usize, e_col: usize) {
    exec_lua(
        rpc,
        &format!("nx.win.select_range(0, 0, {s_col}, 0, {e_col})"),
    )
    .await;
}
...
async fn on_escape_insert_keeps_the_default_and_parks_in_insert_past_it() {
    ...
    exec_lua(
        &rpc,
        "nx.win.select_range(0, 0, 6, 0, 11, { on_escape = 'insert' })",
    )
    .await;
