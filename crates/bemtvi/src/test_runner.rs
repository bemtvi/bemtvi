//! `bemtvi --test-plugin [dir]` — the headless runner for a plugin's Lua test suite.
//!
...
//! *Lua* framework (`btv.test`, see `crates/bemtvi-lua/src/prelude/test.lua`).
//!
//! Flow: discover `<dir>/test/**/*_spec.lua`, source each over `nvim_exec_lua` so it
//! registers `btv.test.describe/it` cases, kick `btv.test._run()` (asynchronous — the
//! cases await ticks), poll `btv.test` for the results, print a report, and exit
...
use bemtvi_server::{run as run_server, ClipboardProvider, ServerInit};
use rmpv::Value;
...
pub fn run_test_plugin(dir: PathBuf) -> Result<bool> {
    ...
    if files.is_empty() {
        eprintln!(
            "bemtvi --test-plugin: no *_spec.lua files under {}",
            test_dir.display()
        );
        return Ok(false);
    }
...
fn spawn_server(plugin_dir: PathBuf) -> (Rpc, UnboundedReceiver<Incoming>) {
    ...
    std::thread::spawn(move || {
        let init = ServerInit {
            ...
            // The plugin-test runner reads `print` through the message mirror, not stdout.
            lua_stdio: false,
...
async fn run(dir: PathBuf, files: Vec<PathBuf>) -> Result<bool> {
    ...
    // That also makes `btv.ui.caps().keyboard_protocol` true, so a plugin that installs a
    ...
    // Turn on plugin-test mode: installs the `btv.test` framework into Lua and starts
    // the `btv._ui` redraw mirror. Without this the API is absent — the gate that keeps
    // it out of normal editor sessions.
    rpc.request("btv_enable_test_mode", vec![])
        .await
        .map_err(|e| anyhow!("enabling test mode: {e}"))?;

    exec_lua(&rpc, "btv.test.reset()").await?;
    ...
    exec_lua(&rpc, "btv.test._run()").await?;
    ...
    let results = loop {
        let done = exec_lua(&rpc, "return btv.test._done == true").await?;
        if matches!(done, Value::Boolean(true)) {
            break exec_lua(&rpc, "return btv.test._results").await?;
        }
