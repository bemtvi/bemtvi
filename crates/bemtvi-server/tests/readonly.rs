...
//! plugin-owned `btv.view`, the quickfix display, and an image preview. (A live
...
use bemtvi_test_harness::{exec_lua, lines, message_after, start_attached, temp_dir, write_temp};
...
async fn explorer_listing_is_read_only_to_ex_command_edits() {
    ...
    exec_lua(&rpc, &format!("btv.cmd('edit {}')", dir.to_string_lossy())).await;
...
/// A plugin-owned `btv.view` — read-only at the chokepoints via the same `read_only()`
/// path (its `view` marker). (Normal-mode inertness is covered in `btv_view.rs`.)
#[tokio::test]
...
async fn image_preview_is_read_only_to_ex_command_edits() {
    ...
    exec_lua(&rpc, &format!("btv.cmd('edit {path}')")).await;
