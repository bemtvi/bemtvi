# `btv.snippet` — the native snippet engine — phased plan

> Implements the pending half of completion **Phase 4-D**
> (`docs/plans/2026-06-15-btv-complete-completion-engine.md` → *Snippets*) and the
> `btv.snippet` surface from the native-plugin-API spec
...
- **Parser** (`bemtvi-core/src/snippet.rs`, pure): `parse_snippet(&str) -> Result<ParsedSnippet, SnippetError>`.
...
- **Session** (`bemtvi-core/src/editor/snippet.rs`): `Option<SnippetSession>` on `Editor`.
...
- **S-2 — Expansion + tabstop session** ✅: `expand_snippet` / `snippet_jump` /
  ...
  View projection of tabstop highlight spans; TUI/GUI render. `btv.snippet.expand("body")`
  test hook.
- **S-3 — `btv.snippet` Lua API + `snippets` source** ✅: `btv.snippet.setup{ jump_next, jump_prev }`,
  `btv.snippet.add(ft, {{trigger, body}})` (string bodies; **function bodies error loud**,
  deferred to S-5), `btv.snippet.expand(body)`, and the built-in `snippets` completion
  source (offers triggers for the buffer's filetype, expands the body on accept).
  `prelude/snippet.lua` + `btv._snippet_*` bridges + the server `snippet.rs` store/source
...
- **S-5 — Choices + select-default + function bodies (deferred)**: `${1|a,b|}` rendered
  ...
  so typing replaces; `btv.snippet.add` function bodies (dynamic / context-aware); a
...
1. `cargo build --workspace`; `cargo test -p bemtvi-server --test snippet` (new suite).
