...
try {
  ...
  await page.evaluate(() => window.__nxvim.feed("ihello world<Esc>gg0"));
  await page.evaluate(() => window.__nxvim.feed("<C-w>v"));
  await sleep(300);

  const winCount = await page.evaluate(() => (window.__nxvim.frame().windows || []).length);
