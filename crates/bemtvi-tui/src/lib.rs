...
///
/// An **empty** payload passes: `ESC ] 52 ; c ; ESC \` is OSC 52's "clear the
/// clipboard", and it is what our own encoder produces for an empty yank — a
/// whitelist that rejected it would silently drop a sequence the server legitimately
/// sent. It carries nothing, so it smuggles nothing.
///
/// A rejected sequence is dropped without a word: the only channel this client has
/// is the terminal it is protecting, so "reporting" a hostile escape would mean
/// painting attacker bytes onto the screen.
///
/// `pub` for the black-box tests (`tests/osc52.rs`), like the other reply parsers here.
pub fn is_osc52(seq: &str) -> bool {
