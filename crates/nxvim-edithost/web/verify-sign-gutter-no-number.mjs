// Repro/regression for the bug "nxvim-diff gutter signs don't show on the web build":
// a window that reserves a sign column (`signcolumn=yes`) but has NO line numbers
// (`nonumber`) — like an `nx.view` diff pane — dropped its signs on wasm. The redraw
...
try {
  ...
  // extmark on line 1 — the exact shape an nx.view diff pane paints (signs, no numbers).
  await page.evaluate(() => window.__nxvim.feed("ggdGialpha<Esc>"));
  await luaResult(`
    vim.wo.number = false
    vim.wo.relativenumber = false
    vim.wo.signcolumn = "yes"
    nx.hl.define(0, "MySignHl", { fg = "#89b4fa", bold = true })
    local ns = nx.ns.create("sign-gutter-test")
    nx.buf.set_extmark(0, ns, 0, 0, { sign_text = "▶", sign_hl_group = "MySignHl" })
    return true
  `);
