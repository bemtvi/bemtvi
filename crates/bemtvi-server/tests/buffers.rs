...
///
/// Taking the *latest* message is right for persistent state, but bemtvi arms a native
...
async fn editing_a_nonexistent_file_does_not_storm_the_file_watch() {
    ...
    static COUNTER: AtomicU64 = AtomicU64::new(0);
    let missing = bemtvi_test_harness::temp_root().join(format!(
        "bemtvi_missing_{}_{}.txt",
        std::process::id(),
        COUNTER.fetch_add(1, Ordering::Relaxed)
    ));
    ...
    // keystrokes (`btv_input`), not an `btv_command` RPC — edit a missing file.
...
async fn splitting_onto_the_file_you_are_already_editing_reads_nothing() {
    ...
    // path: the new window inherits the buffer, so there is nothing to read. bemtvi routed
...
async fn a_save_that_cannot_be_made_atomic_leaves_the_original_intact() {
    ...
    let dir =
        bemtvi_test_harness::temp_root().join(format!("bemtvi_atomic_ro_{}", std::process::id()));
...
async fn write_through_a_symlink_keeps_the_link_and_updates_the_target() {
    ...
    let link = bemtvi_test_harness::temp_root().join(format!(
        "bemtvi_buf_symlink_link_{}.txt",
        std::process::id()
    ));
...
/// to lack: the alternate as a live **handle** (`btv.buf.alternate()` /
...
async fn the_mirror_carries_the_alternate_handle_and_each_buffers_line() {
    ...
    assert_eq!(
        exec_lua(&rpc, "return btv.buf.alternate()").await.as_u64(),
        Some(a_nr)
    );
    assert_eq!(
    ...
    assert!(
        exec_lua(&rpc, "return btv.buf.alternate()").await.is_nil(),
...
async fn buf_attach_callback_effects_drain_in_the_same_batch() {
    ...
    // The `nvim_buf_attach` change channel: the server fires `btv._buf_changed`
    ...
    exec_lua(
        &rpc,
        "btv._buf_changed = function(buf, tick) print('ATTACH-DRAINED') end",
    )
    .await;
    feed(&rpc, "ix");
    assert_eq!(
