...
//! that exist only because the line isn't finished. bemtvi's default is a debounce:
...
//! until `InsertLeave`).
//!
//! Driven through the client-set surface (`btv.diagnostic.set`), which shares the
...
//! half of the same gate is covered in `crates/bemtvi/tests/lsp_features.rs`.
//!
...
async fn open_with_a_diagnostic(rpc: &Rpc) {
    ...
    exec_lua(
        rpc,
        r#"btv.diagnostic.set(btv.ns.create("insert_pause"), 0, {
             { lnum = 1, col = 0, end_lnum = 1, end_col = 4,
               severity = btv.diagnostic.severity.ERROR, message = "before" },
           })"#,
    )
    .await;
}
    ...
    exec_lua(
        ...
        r#"btv.diagnostic.set(btv.ns.create("insert_pause"), 0, {
             { lnum = 3, col = 0, end_lnum = 3, end_col = 4,
               severity = btv.diagnostic.severity.ERROR, message = "after" },
           })"#,
    )
    .await;
}
...
async fn a_clear_during_insert_is_held_too() {
    ...
    exec_lua(
        &rpc,
        r#"btv.diagnostic.reset(btv.ns.create("insert_pause"), 0)"#,
    )
    .await;
    assert_eq!(
        painted_rows(&rpc, &mut incoming).await,
    ...
    exec_lua(
        ...
        r#"btv.diagnostic.set(btv.ns.create("insert_pause"), 0, {
             { lnum = 5, col = 0, end_lnum = 5, end_col = 4,
               severity = btv.diagnostic.severity.ERROR, message = "newest" },
           })"#,
    )
    .await;
    ...
    exec_lua(
        ...
        r#"btv.diagnostic.set(btv.ns.create("insert_pause"), 0, {
             { lnum = 1, col = 5, end_lnum = 1, end_col = 6,
               severity = btv.diagnostic.severity.ERROR, message = "the 2" },
           })"#,
    )
    .await;
