//! Behavior tests for permanent **docks** — bemtvi's VSCode-style edge panels —
...
//! `btv.dock.open/close/focus` create and address them; `<C-w><C-w>` + h/j/k/l
...
async fn single_ctrl_w_splits_within_the_focused_dock() {
    let (rpc, _incoming) = start().await;
    exec_lua(&rpc, "btv.dock.open{ side = 'left', size = 30 }").await;
...
async fn double_ctrl_w_v_from_main_splits_the_last_dock() {
    let (rpc, _incoming) = start().await;
    exec_lua(&rpc, "btv.dock.open{ side = 'right', size = 30 }").await;
...
async fn closing_a_dock_reclaims_its_band() {
    ...
    exec_lua(&rpc, "btv.dock.close('left')").await;
...
async fn closing_the_last_dock_window_with_ctrl_w_c_closes_the_dock() {
    let (rpc, _incoming) = start().await;
    exec_lua(&rpc, "btv.dock.open{ side = 'bottom', size = 8 }").await;
    assert_eq!(win_count(&rpc).await, 2);
...
async fn dock_open_ex_command_opens_a_dock() {
    let (rpc, _incoming) = start().await;
    // The `:DockOpen` ex-command (Lua prelude wrapper over `btv.dock.open`).
...
async fn invalid_dock_side_is_reported_not_silently_ignored() {
    let (rpc, mut incoming) = start().await;
    exec_lua(&rpc, "btv.dock.open{ side = 'sideways', size = 10 }").await;
...
async fn four_docks_keep_a_nondegenerate_main_area() {
    ...
    for side in ["left", "right", "top", "bottom"] {
        exec_lua(
            &rpc,
            &format!("btv.dock.open{{ side = '{side}', size = 6 }}"),
        )
        .await;
    }
...
async fn per_dock_showtabline_override_forces_the_strip() {
    ...
    exec_lua(&rpc, "btv.dock.opt('left').showtabline = 2").await;
...
async fn per_dock_showtabline_zero_hides_even_with_two_tabs() {
    ...
    exec_lua(
        &rpc,
        "btv.dock.open{ side = 'left', size = 20, showtabline = 0 }",
    )
    .await;
...
async fn per_dock_laststatus_hides_the_dock_statusline() {
    ...
    exec_lua(&rpc, "btv.dock.opt('left').laststatus = 0").await;
...
async fn per_dock_laststatus_accepts_the_inline_open_form() {
    let (rpc, mut incoming) = start().await;
    // The option also rides `btv.dock.open{...}` inline, like showtabline/title.
    exec_lua(
        &rpc,
        "btv.dock.open{ side = 'bottom', size = 8, laststatus = 0 }",
    )
    .await;
...
async fn dock_size_option_resizes_the_band() {
    ...
    exec_lua(&rpc, "btv.dock.opt('bottom').size = 15").await;
...
async fn dock_title_projects_and_forces_the_strip() {
    ...
    exec_lua(
        &rpc,
        "btv.dock.open{ side = 'left', size = 20, title = 'EXPLORER' }",
    )
    .await;
...
async fn main_tabline_click_survives_a_top_dock_offset() {
    ...
    exec_lua(&rpc, "vim.cmd('tabnew')").await;
    exec_lua(&rpc, "btv.dock.open{ side = 'top', size = 3 }").await;
...
async fn dock_winhighlight_valid_value_round_trips() {
    ...
    exec_lua(
        &rpc,
        "btv.dock.opt('left').winhighlight = 'Normal:NormalSB'",
    )
    .await;
    let got = exec_lua(&rpc, "return btv.dock.opt('left').winhighlight").await;
    assert_eq!(
        got.as_str(),
...
async fn dock_winhighlight_malformed_entry_is_reported() {
    ...
    exec_lua(
        &rpc,
        "btv.dock.opt('left').winhighlight = 'Normal:NormalSB,bogus'",
    )
    .await;
    let rd = wait_redraw(&mut incoming, |m| {
        map_get(m, "message")
...
async fn dock_winhighlight_remaps_content_highlight_group() {
    ...
    exec_lua(&rpc, "btv.dock.open{ side = 'left', size = 30 }").await;
    feed(&rpc, "ihello<Esc>");
    exec_lua(&rpc, "btv.hl.define(0, 'Foo', { fg = '#ff0000' })").await;
    exec_lua(&rpc, "btv.hl.define(0, 'Bar', { fg = '#00ff00' })").await;
    ...
    exec_lua(&rpc, "btv.dock.opt('left').winhighlight = 'Foo:Bar'").await;
...
async fn dock_winhighlight_overrides_chrome_normal_per_window() {
    ...
    exec_lua(&rpc, "btv.hl.define(0, 'NormalSB', { bg = '#202030' })").await;
    exec_lua(
        &rpc,
        "btv.dock.opt('left').winhighlight = 'Normal:NormalSB'",
    )
...
// `btv.wo` — backing the window-scope claim the example config documents.
#[tokio::test]
async fn btv_wo_winhighlight_remaps_a_plain_window() {
    let (rpc, mut incoming) = start().await;
    exec_lua(&rpc, "btv.hl.define(0, 'NormalSB', { bg = '#202030' })").await;
    exec_lua(&rpc, "btv.wo.winhighlight = 'Normal:NormalSB'").await;
    let rd = wait_redraw(&mut incoming, |m| {
    ...
    assert_eq!(
        region_chrome_bg(&rd, "main", "normal"),
        Some(0x202030),
        "btv.wo.winhighlight remaps the focused (non-dock) window's chrome"
    );
}
...
// round-trips through `btv.dock.opt` (covered above).
#[tokio::test]
async fn dock_winhighlight_remap_renders_the_dock_chrome() {
    let (rpc, mut incoming) = start().await;
    exec_lua(
        &rpc,
        r##"btv.hl.define(0, "NormalSB", { bg = "#181825", fg = "#cdd6f4" })
...
/// `btv.dock.toggle` hides a visible dock (it leaves the window list and the band
...
async fn toggle_hides_then_shows_a_dock_preserving_content() {
    ...
    exec_lua(&rpc, "btv.dock.toggle('left')").await;
    assert_eq!(
        win_count(&rpc).await,
        1,
...
async fn toggle_preserves_internal_splits() {
    ...
    exec_lua(&rpc, "btv.dock.hide('left')").await;
    assert_eq!(win_count(&rpc).await, 1, "both dock windows are hidden");

    exec_lua(&rpc, "btv.dock.show('left')").await;
    assert_eq!(
        win_count(&rpc).await,
        3,
...
/// `btv.dock.open` reveals the same content, whereas closing drops it so a reopen
...
async fn hidden_is_distinct_from_closed() {
    ...
    exec_lua(&rpc, "btv.dock.hide('left')").await;
    exec_lua(&rpc, "btv.dock.open{ side = 'left' }").await;
    assert_eq!(
        lines(&rpc).await,
        vec!["keep"],
...
async fn autohide_dock_collapses_when_focus_leaves() {
    ...
    exec_lua(
        &rpc,
        "btv.dock.open{ side = 'left', size = 20, autohide = true }",
    )
    .await;
    feed(&rpc, "ipanel<Esc>");
    assert_eq!(
...
async fn toggle_on_an_absent_side_is_a_noop() {
    ...
    exec_lua(&rpc, "btv.dock.toggle('right')").await;
    assert_eq!(
        win_count(&rpc).await,
        1,
...
/// The `:DockToggle` ex-command drives the same path as `btv.dock.toggle`.
#[tokio::test]
...
async fn an_untitled_hidden_dock_chip_uses_the_side_keyword() {
    let (rpc, mut incoming) = start().await;
    exec_lua(&rpc, "btv.dock.open{ side = 'bottom', size = 6 }").await;
    exec_lua(&rpc, "btv.dock.hide('bottom')").await;
    let rd = wait_redraw(&mut incoming, |m| !hidden_docks(m).is_empty()).await;
...
/// `btv.buf.list` defaults to every buffer across all layers; `{ focused = true }`
...
async fn buf_list_focused_option_scopes_to_the_focused_layer() {
    ...
    exec_lua(&rpc, "btv.dock.open{ side = 'left', size = 30 }").await; // scratch buffer 2 (dock)
    ...
    assert_eq!(
        lua_u64(&rpc, "return #btv.buf.list()").await,
        Some(2),
        "the default list spans every layer"
    );
    assert_eq!(
        lua_u64(&rpc, "return #btv.buf.list{ focused = true }").await,
        Some(1),
        "the focused list is the dock alone"
    );
    assert_eq!(
        lua_u64(&rpc, "return btv.buf.list{ focused = true }[1]").await,
        Some(2),
...
async fn opening_a_picker_in_a_too_small_dock_does_not_crash() {
    ...
    exec_lua(&rpc, "btv.dock.open({ side = 'bottom', size = 2 })").await;
    ...
    let open = exec_lua(&rpc, "return btv._picker ~= nil").await;
...
async fn quit_in_a_dock_closes_the_dock_not_the_editor() {
    ...
    exec_lua(&rpc, "btv.dock.open{ side = 'left', size = 20 }").await; // focuses the dock
...
/// mapping layer — exactly as neovim reads `<C-w>{cmd}`. bemtvi-tree binds `h`/`l`
...
async fn window_and_layer_nav_ignore_buffer_local_hl_maps() {
    ...
    exec_lua(
        &rpc,
        "_G.hl = {}\n\
         btv.keymap.set('n', 'h', function() _G.hl.h = true end, { buffer = 0 })\n\
         btv.keymap.set('n', 'l', function() _G.hl.l = true end, { buffer = 0 })",
    )
    .await;
...
async fn quitall_surfaces_a_modified_dock_buffer_in_its_dock() {
    ...
    exec_lua(&rpc, "btv.dock.open{ side = 'left', size = 24 }").await;
...
async fn assert_cross(rpc: &Rpc, from: &str, dir: &str, want: &str) {
    if from == "main" {
        ...
        exec_lua(rpc, &format!("btv.dock.focus('{from}')")).await;
    }
...
async fn cw_cw_crosses_spatially_between_docks() {
    ...
    for (side, txt) in [
        ...
        exec_lua(
            &rpc,
            &format!("btv.dock.open{{ side = '{side}', size = 8 }}"),
        )
        .await;
...
async fn cw_cw_cross_no_ops_and_wraps_with_one_axis_open() {
    ...
    exec_lua(&rpc, "btv.dock.open{ side = 'left', size = 8 }").await;
    feed(&rpc, "iLEFT<Esc>");
    exec_lua(&rpc, "btv.dock.open{ side = 'right', size = 8 }").await;
    feed(&rpc, "iRIGHT<Esc>");
