-- nx.test — a native test framework for nxvim plugins.
--
-- Plugins are pure Lua (ADR 0002), so their tests are too: `nx.test` lets a plugin
-- repo carry a `test/*_spec.lua` suite that drives a REAL editor and asserts on its
-- state, run headlessly by `nxvim --test-plugin <dir>`. It is the Lua sibling of the
-- Rust black-box harness (`crates/nxvim-test-harness`): same philosophy (feed vim
-- keys, assert on the resulting buffer / cursor / UI), reachable from a plugin's own
-- repo.
--
--   nx.test.describe("my-plugin", function()
--     nx.test.before_each(function() require("my-plugin").setup({}) end)
...
-- a tick, and the Lua state mirrors (nx._bufs / nx._cur_cursor / …) refresh BEFORE
-- each Lua entry — so a single synchronous chunk that feeds then reads sees stale
-- state. Every `it` body therefore runs inside an `nx.async` coroutine, and the
-- context's driving methods `nx.await` internally: `t:feed` queues the keys then
...
-- popup, a timer) need `t:wait_for(predicate)`. This reuses nxvim's existing
-- async/promise/timer machinery wholesale — no new scheduling primitives.

local M = {}

-- nx.test is GATED. The framework is built here, but exposed only through
-- `nx._install_test`, which the server calls when the `nxvim --test-plugin` runner
-- turns on test mode (the `nx_enable_test_mode` RPC). In a normal editor session
-- `nx.test` stays nil, so a config or plugin can't depend on the test API — and the
-- UI mirror it reads (`nx._ui`) is likewise only populated under `--test-plugin`.
function nx._install_test()
  nx.test = M
end
...
function M.describe(name, fn)
  if type(name) ~= "string" then
    error("nx.test.describe: name must be a string", 2)
  end
  if type(fn) ~= "function" then
    error("nx.test.describe: body must be a function", 2)
  end
...
function M.it(name, fn)
  if type(name) ~= "string" then
    error("nx.test.it: name must be a string", 2)
  end
  if type(fn) ~= "function" then
    error("nx.test.it: body must be a function", 2)
  end
...
local function fail(msg)
  ...
  -- nx.await re-raises promise rejections the same way (a table with `.message`).
  local e = { __nxtest_fail = true, message = msg }
  error(e, 0)
end
...
local function settle(n)
  for _ = 1, (n or 1) do
    nx.await(nx.promise.new(function(resolve)
...
function Ctx:feed(keys, opts)
  opts = opts or {}
  nx._feedkeys(keys, opts.remap ~= false, opts.insert or false)
...
function M.clipboard.seed(text, linewise)
  nx._test_clipboard_seed(text, linewise and true or false)
  settle(1)
end
...
-- touches the filesystem, so runs don't collide. Pair with nx.fs to read/write it.
function M.tempdir()
  return nx._test_tempdir()
end

-- ----- state reads (post-await mirror reads) --------------------------------

function Ctx:lines(first, last)
  return nx.buf.lines(0, first or 0, last or -1, false)
end

function Ctx:line(n)
  local ls = nx.buf.lines(0, (n or 1) - 1, n or 1, false)
  return ls[1]
end

function Ctx:current_line()
  return nx.current_line()
end

function Ctx:cursor()
  return nx.cursor.get()
end

-- The short mode string ("n", "i", "v", …). `nx.mode()` returns the
...
function Ctx:buf()
  return nx._cur_buf and nx._cur_buf.bufnr
end

function Ctx:keymaps(mode)
  return nx.keymap.get(mode or "n")
end

-- ----- UI / redraw inspection (Phase 2: backed by the nx._ui mirror) --------
...
-- once and sets `nx.test._results` / `nx.test._done` when finished. The runner polls
  ...
  nx.async(function()
    ...
    for _, case in ipairs(cases) do
      ...
      if status == "pass" then
        ...
        if not ok then
          if type(err) == "table" and err.__nxtest_fail then
      ...
      results[#results + 1] = {
        ...
        status = status,
        message = message,
        ms = math.floor((nx.now_ms() - started) + 0.5),
      }
    end
    M._results = results
    M._done = true
    ...
    M._results = {
      {
        ...
        name = "<test runner>",
        status = "error",
        message = "nx.test runner crashed: "
