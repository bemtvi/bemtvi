// Reproduce the real nxvim-diff scenario on web: an `nx.view` pane that reserves a sign
...
// verify-sign-gutter-no-number used a plain buffer; the diff panes are nx.view buffers,
...
try {
  ...
  await page.evaluate(() => window.__nxvim.feed("imain<Esc>"));
  ...
  await luaResult(`
    ...
    ns = nx.ns.create("view-sign-test")
...
console.log(failures === 0
  ? "\nALL PASS — an nx.view sign decor renders in the gutter on web"
  : `\n${failures} FAILED`);
