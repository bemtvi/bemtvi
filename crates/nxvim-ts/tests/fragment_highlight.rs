...
/// storm. A fixed path (the same convention as `crates/nxvim/tests/syntax.rs`)
/// so repeat runs overwrite rather than accumulate.
fn fixture_dir() -> &'static Path {
    static DIR: OnceLock<PathBuf> = OnceLock::new();
    DIR.get_or_init(|| {
        let dir = std::env::temp_dir().join("nxvim-ts-fragment-fixture");
...
/// framing makes it whole: `field: Vec<String>` inside `struct __nx { … }` parses
...
fn a_same_line_framing_maps_columns_back() {
    let mut engine = engine();
    engine.set_fragment_context("rust", vec!["fn __nx() { %s; }".to_string()]);
...
fn a_malformed_or_empty_context_list_is_inert() {
    ...
    engine.set_fragment_context("rust", vec!["struct __nx { }".to_string()]); // no `%s`
    assert_eq!(
        group_of(&painted(&mut engine, text, true), "Vec"),
        None,
...
/// A framing whose opener is **pure indentation** (`"class __nx:\n    %s"`, what an
...
fn a_same_line_opener_does_not_shift_later_lines() {
    let mut engine = engine();
    engine.set_fragment_context("rust", vec!["fn __nx() { let __v = %s; }".to_string()]);
