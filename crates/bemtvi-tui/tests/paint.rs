...
fn command_cursor_accounts_for_a_wide_prompt() {
    // A multi-char `btv.ui.input` prompt with wide chars offsets the caret by its
...
/// A `menu` redraw submap (the picker / `btv.ui.select` float-list). `items` are the
...
fn a_picker_row_keeps_the_file_name_when_the_path_overflows() {
    ...
    let long = "crates/bemtvi-core/src/editor/operators.rs"; // 42 chars
    let menu = menu_value(
        &[long, "main.rs"],
        // Highlight "operators" (chars 30..39) on row 0 — it must follow the elision.
        &[&[(30, 39)], &[]],
    ...
    assert!(
        !shown.contains("crates/bemtvi-core"),
