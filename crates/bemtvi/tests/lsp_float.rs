...
//! (`bemtvi --__lsp-mock`, `bemtvi_lsp::mock`) answers `textDocument/hover` and
//! `textDocument/signatureHelp`, the `$BEMTVI_LSP_CMD` env hook overrides the
//! server's spawn argv, and the buffer is bound via the raw `btv._lsp_start`
...
/// Write a mock LSP script (the whole JSON object) and point `$BEMTVI_LSP_CMD` at
...
fn painted_rows(redraw: &[(Value, Value)]) -> Vec<String> {
    let mut view = bemtvi_view::View::default();
    view.update(&[Value::Map(redraw.to_vec())]);
    let buf = bemtvi_tui::paint(&view, 80, 24);
    (0..buf.area.height)
        .map(|y| {
            (0..buf.area.width)
...
async fn a_hover_at_the_bottom_of_the_screen_opens_above_the_cursor() {
    ...
    let (redraw, win) =
        await_doc_float_redraw(&rpc, &mut incoming, "btv.lsp.hover()", "scripted hover").await;
...
async fn a_tall_hover_shrinks_into_the_roomier_side_of_the_cursor() {
    ...
    let (redraw, win) =
        await_doc_float_redraw(&rpc, &mut incoming, "btv.lsp.hover()", "hover line 00").await;
    let cursor_row = cursor_screen_row(&redraw);
    let (_, y, _, h) = window_rect(&win);
    assert!(
        y + h <= cursor_row,
...
async fn a_signature_float_at_the_bottom_of_the_screen_opens_above_the_cursor() {
    ...
    let (redraw, win) =
        await_doc_float_redraw(&rpc, &mut incoming, "btv.lsp.signature_help()", "fn foo(").await;
    let cursor_row = cursor_screen_row(&redraw);
    let (_, y, _, h) = window_rect(&win);
    assert!(
        y + h <= cursor_row,
...
async fn hover_reply_opens_a_float_window() {
    ...
    let win =
        await_doc_float_window(&rpc, &mut incoming, "btv.lsp.hover()", "scripted hover").await;
...
async fn hover_markdown_html_entities_are_decoded_to_plain_text() {
    ...
    // hover. bemtvi renders markdown as plain text, so those entities must be decoded
    ...
    let win = await_doc_float_window(&rpc, &mut incoming, "btv.lsp.hover()", "foo").await;
...
async fn hover_window_scrolls_with_the_wheel_and_a_key_dismisses_it() {
    ...
    let win = await_doc_float_window(&rpc, &mut incoming, "btv.lsp.hover()", "hover line 00").await;
    assert_eq!(
...
async fn signature_help_reply_opens_a_float_window() {
    ...
    let win =
        await_doc_float_window(&rpc, &mut incoming, "btv.lsp.signature_help()", "fn foo(").await;
    assert_eq!(
        window_lines(&win),
...
async fn signature_help_marks_the_active_parameter_line() {
    ...
    let (redraw, win) =
        await_doc_float_redraw(&rpc, &mut incoming, "btv.lsp.signature_help()", "a: i32").await;
    assert_eq!(
        overlay_markers(&win),
        vec![(1, 2, "▸".to_string())],
...
async fn signature_help_marker_follows_the_active_parameter_index() {
    ...
    let win =
        await_doc_float_window(&rpc, &mut incoming, "btv.lsp.signature_help()", "b: i32").await;
    assert_eq!(
        overlay_markers(&win),
        vec![(2, 2, "▸".to_string())],
...
async fn signature_help_splits_on_label_offsets_not_commas() {
    ...
    let win =
        await_doc_float_window(&rpc, &mut incoming, "btv.lsp.signature_help()", "def load(").await;
    assert_eq!(
        window_lines(&win),
        vec![
            "def load(",
...
async fn a_single_parameter_is_not_echoed_after_the_signature() {
    ...
    let (redraw, win) =
        await_doc_float_redraw(&rpc, &mut incoming, "btv.lsp.signature_help()", "fn only").await;
    assert_eq!(
        window_lines(&win),
        vec!["fn only(a: i32)"],
...
/// `btv.lsp._clients`), so the server's advertised trigger chars have reached core.
async fn wait_for_lsp(rpc: &Rpc) {
    for _ in 0..200 {
        if exec_lua(rpc, "return next(btv.lsp._clients) ~= nil").await == Value::Boolean(true) {
            return;
        }
        tokio::time::sleep(std::time::Duration::from_millis(25)).await;
    }
