...
//! `nx.lsp.progress()` and the `LspProgress` autocmd.
//!
//! Wired like `lsp_features.rs`: the scripted mock server (`nxvim --__lsp-mock`)
//! stands in for a real language server, `$NXVIM_LSP_CMD` overrides the spawn argv,
...
/// Write a mock LSP script and point `$NXVIM_LSP_CMD` at the binary's `--__lsp-mock`
...
/// A one-line rendering of `nx.lsp.progress()` — `client|title|message|percentage`
...
/// `end` retires the task: `nx.lsp.progress()` means "busy right now", so a finished
...
async fn an_ended_task_leaves_no_progress() {
    ...
    let (rpc, _incoming) = open_with_server(
        ...
        r#"
        _G.seen_progress = 0
        nx.autocmd.create("LspProgress", { callback = function()
    ...
    assert_eq!(
        await_lua_eq(&rpc, "#nx.lsp.progress()", "0").await,
        "0",
...
async fn lsp_progress_fires_with_the_kind_as_its_pattern() {
    ...
    let (rpc, _incoming) = open_with_server(
        ...
        _G.ends = 0
        _G.end_client = nil
        nx.autocmd.create("LspProgress", { callback = function(a)
          _G.log[#_G.log + 1] = a.match .. ":" .. a.data.kind .. ":" .. (a.data.token or "?")
        end })
        nx.autocmd.create("LspProgress", { pattern = "end", callback = function(a)
          _G.ends = _G.ends + 1
          local c = nx.lsp.client_by_id(a.data.client_id)
...
async fn the_bufnr_filter_narrows_to_that_buffers_clients() {
    ...
    assert_eq!(
        await_lua_eq(&rpc, "#nx.lsp.progress({ bufnr = 0 })", "1").await,
        "1",
    ...
    assert_eq!(
        await_lua_eq(&rpc, "#nx.lsp.progress({ bufnr = 0 })", "0").await,
        "0",
        "a buffer with no attached client should see no progress"
    );
    assert_eq!(
        await_lua_eq(&rpc, "#nx.lsp.progress()", "1").await,
        "1",
...
async fn stopping_a_server_clears_its_progress() {
    ...
    exec_lua(&rpc, "nx.lsp.stop('mock')").await;
    assert_eq!(
        await_lua_eq(&rpc, "#nx.lsp.progress()", "0").await,
        "0",
...
/// visibly busy server beats showing nothing — the case `nx.lsp.progress`'s docstring
...
/// `nx.lsp.progress()` lists clients in a **stable, ascending-client-id** order — the
...
/// nxvim-line renders `tasks[1]` plus `(+N)` for the rest, so an unstable order means
...
async fn progress_is_listed_in_ascending_client_id_order() {
    ...
    exec_lua(
        &rpc,
        r#"
        nx.lsp._set_client(3, "gamma", {}, "utf-8")
        nx.lsp._set_client(2, "beta", {}, "utf-8")
        nx.lsp._set_progress(3, { { token = "g1", title = "Gamma work" } })
        nx.lsp._set_progress(2, { { token = "b1", title = "Beta work" } })
        "#,
    )
    .await;
    ...
    (function()
      local out = {}
      for _, p in ipairs(nx.lsp.progress()) do out[#out + 1] = p.client_name end
      return table.concat(out, ",")
    end)()
    "#;
    let want = "beta,gamma";
