...
// `bemtvi-ts`'s native `install.rs` `NVIM_TS_REF`) so the browser indents.scm is the
...
// mirror / the runtime's `__BEMTVI_TS_BASE`). The nvim-treesitter query directory name
...
// match `bemtvi-ts`'s native `install.rs` `NVIM_TS_TEXTOBJECTS_REF`, so the browser's
...
// (bemtvi-server/src/treesitter.rs) so the browser merges exactly the set native does.
