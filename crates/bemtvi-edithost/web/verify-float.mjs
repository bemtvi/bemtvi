// Focused verifier for content-float (btv.ui.float) border + title rendering in the
...
try {
  ...
  await page.evaluate(() => window.__bemtvi.execLua(
    "btv.ui.float('a wide enough body line', { title = 'd', border = 'double' })"));
  await sleep(150);
  ...
  // ---- Window floats (a real buffer in a float, e.g. btv.view dialogs) ----
  // These render via renderFloat → an opaque chrome box + glyph border + inset
  // content, NOT the btv.ui.float content-float path. The web client used to draw no
