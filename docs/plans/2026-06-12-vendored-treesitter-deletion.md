...
> **native engine** (`nxvim-ts::Engine`, sync, in-core) is untouched and keeps
...
> `Engine::set_query`) survive and are re-fronted by declarative `nx.*` —
...
### D1 — does `nx.treesitter` expose a *Lua* parser/query API?
...
([`engine.rs:191`/`227`](../../crates/nxvim-ts/src/engine.rs)) ←
`Editor::set_ts_query` ([`syntax.rs:194`](../../crates/nxvim-core/src/editor/syntax.rs)).

- **Recommended — keep execution, drop the Lua merge.** Offer
  `nx.treesitter.set_query(lang, name, text)` straight to `Engine::set_query`.
...
- `nxvim-ts::Engine` and the grammar loader/installer — the editor's real
  treesitter. Drives redraw highlighting, indent, fold, `=`.
- `Editor::ts_override` + `ts_start` / `ts_stop` / `ts_reset` / `ts_language_for`
  ([`syntax.rs`](../../crates/nxvim-core/src/editor/syntax.rs)) — the per-buffer
...
- `Engine::set_query` / `set_query_overlay`, `Editor::set_ts_query` — bridge #4's
  execution path (D2 keeps it, re-fronted by `nx.treesitter.set_query`).

## Delete set — the vendored *Lua surface* (not the submodule)

> **Scope note — do not confuse two `vendor` trees.** This plan deletes
> **`crates/nxvim-lua/src/vendor/nvim/`** — copies of neovim's Lua runtime
> files compiled into the `nxvim-lua` crate via `include_str!`. It does **not**
...
them), so all of `crates/nxvim-lua/src/vendor/nvim/` goes except its `LICENSE`,
...
`crates/nxvim-lua/src/vendor/nvim/`.

| File (under `crates/nxvim-lua/src/vendor/nvim/`) | Lines |
| --- | --- |
...
- **[`runtime.rs`](../../crates/nxvim-lua/src/runtime.rs):** the `VENDORED_TS_LUA`
...
- **[`prelude/treesitter.lua`](../../crates/nxvim-lua/src/prelude/treesitter.lua)**
  ...
  direct setter).
- **[`nxvim-ts/src/lua.rs`](../../crates/nxvim-ts/src/lua.rs)** `install()` and its
  call at `runtime.rs:500` → **delete under D1** (keep + re-home under `nx.*` if
  D1 = expose).
...
`_ts_start` / `_ts_stop`
([`install.rs:1186`/`1194`](../../crates/nxvim-lua/src/install.rs)), `TsOp::Start` /
`TsOp::Stop`, the `effects.rs:234–241` drain
([`effects.rs`](../../crates/nxvim-server/src/effects.rs)). Replace with the
`nx.bo.filetype` / `nx.bo.ts_highlight` option-write path writing the same
...
Delete `_ts_set_query` ([`install.rs:1207`](../../crates/nxvim-lua/src/install.rs)),
...
`Engine::set_query` as the execution path, now fed by `nx.treesitter.set_query`
directly (D2).

## Tests

- **[`treesitter_lua.rs`](../../crates/nxvim/tests/treesitter_lua.rs)** (34 uses:
  ...
  under D1 (rewrite against `nx.treesitter` if D1 = expose).
- **[`syntax.rs`](../../crates/nxvim/tests/syntax.rs)** (30 uses): the 7 `start` +
  3 `stop` → **migrate** to `nx.bo.filetype` / `ts_highlight`; the 19 `query.*` +
  1 `get_parser` → migrate to `nx.treesitter.set_query` (D2). These exercise
  nxvim's *own* highlighting, so they survive in migrated form — highlighting
  must keep working.
- **[`autocmds.rs`](../../crates/nxvim-server/tests/autocmds.rs)** (1 use) —
...
1. **Land the survivors' new front doors** — `nx.bo.filetype` / `ts_highlight`
   and `nx.treesitter.set_query` (thin contracts over `ts_override` /
...
4. **Delete the vendored Lua** — all of `crates/nxvim-lua/src/vendor/nvim/`
...
5. **Under D1** — delete `nxvim-ts/src/lua.rs` + `treesitter_lua.rs`.
...
- `nxvim-ts::Engine` and grammar install/loading — the native engine stays.
- `vim.lsp` (prelude-only; its own deletion/refactor is a separate plan).
- Building a full `nx.treesitter` Lua parser API — deferred to D1's first real
  consumer; this plan only removes, plus the two thin `nx.treesitter` /
  `nx.bo` front doors needed to keep highlighting and query-override working.
