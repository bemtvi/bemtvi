...
use bemtvi_test_harness::{drain_latest_redraw, exec_lua, feed, start_attached, window0};
use rmpv::Value;
...
async fn get_extmarks_round_trips_across_chunks() {
    ...
    // Read it back in a *separate* chunk: the server rebuilt btv._extmarks from
...
async fn virt_lines_interleave_above_and_below_their_line() {
    ...
    // intentionally absent in bemtvi (extmark reads/creates exist, mutation doesn't).
...
async fn scroll_accounts_for_virt_lines_to_keep_the_cursor_visible() {
    ...
    // in bemtvi) — so with no virtual lines the whole buffer fits at top=0.
    let body = (1..=h)
        .map(|i| format!("L{i:02}"))
        .collect::<Vec<_>>()
        .join("<CR>");
...
/// An btv-native `line_fill` mark fills the anchored row with a repeated glyph,
