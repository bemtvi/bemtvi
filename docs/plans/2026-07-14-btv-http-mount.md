# btv.http.mount — plugin-defined HTTP, one editor-owned origin

`btv.http.mount(opts)` is the Lua HTTP **server** surface, the inbound twin of
[`btv.http.fetch`](2026-07-05-btv-http-fetch.md). A plugin does **not** bind a port. It
...
btv.http.mount({
  ...
  btv.ui.open(mount:url())        -- http://127.0.0.1:53124/plugin/example/
end)
```

## Why mounts and not ports

The first draft of this plan gave every plugin its own listener (`btv.http.serve{ port }`).
...
a `TcpListener` can. Same Lua, three worlds, which is the same bet `btv.http.fetch` made.

It also removes a whole class of plugin bugs that ports invite: port collisions between two
bemtvi instances, plugins hard-coding `8080`, and every plugin author re-inventing bind-retry.
...
`btv` is **promise-only for one-shot async; a persistent event stream stays handler-based**
(`prelude/process.lua`). A mount is both, so it uses both:

- **The mount is one-shot** → `btv.http.mount` returns a **promise of the handle**. It
  ...
  `winid` trap `btv.schedule` cannot poll (CLAUDE.md). Resolving the handle hands over a URL
...
- **The requests are a persistent stream** → `opts.on_request`, a handler. Same doctrine as
  `btv.process.open` / `btv.socket.connect` / `btv.fs.watch`.
...
return value forces the reply same-tick; real handlers await (`btv.fs.read` a file,
`btv.http.fetch` an upstream, render a buffer). Passing `respond` lets a handler reply now or
later, with no ambiguity about whether a returned promise means "the response" or "the
response's body".

## Lua surface (`crates/bemtvi-lua/src/prelude/httpmount.lua`)

- `btv.http.mount(opts)` → promise of a `Mount`.
  - `opts.name` — **required**; `[%w_-]+`. Mounts at `/plugin/<name>`. The `/plugin/`
    namespace is reserved for mounts so the editor keeps `/` and `/_btv/*` for itself.
...
- `btv.http.origin()` → the base origin, or `nil` when nothing has mounted yet (so it never
...
`bemtvi_lua::mount_root_redirect`), so a plugin serving an index page at its root needs no
...
firewall, their bookmark. So it is an ordinary option (bemtvi's own, not standard vim),
global, settable from `:set` / `btv.o` / `vim.o` like any other:

```lua
btv.o.httphost = "127.0.0.1"   -- default; loopback only
btv.o.httpport = 0             -- default; 0 picks a free ephemeral port
```
...
the `btv.http.configure{}` call an earlier draft had: a Lua-only setter would have let the
...
keeps `:set httpport?` and `btv.http.origin()` in agreement — an option that reads `9000`
...
`btv.http.fetch` routes to the daemon (the daemon owns the network; it dodges CORS and
...
Types live in `bemtvi-lua` (`ops.rs`), transport-free and wasm-safe, beside
...
The **per-request id space** is the one genuinely new thing versus `btv.socket`, whose
...
workspace convention and gated behind `bemtvi-server`'s `native` feature exactly like `ureq`.
...
resolves differently (the page's own origin, e.g. `https://demo.bemtvi.dev/plugin/example/`).
...
browser load  /plugin/example/index.html
     └─► ServiceWorker  fetch handler          (btv-sw.js, scope "/")
...
`btv-sw.js`, served from the site root so its scope covers `/plugin/`:
...
async function routeToEditHost(request) {
  ...
  if (!client) return new Response("bemtvi: no edit-host client", { status: 503 });

  const channel = new MessageChannel();
  const reply = new Promise((resolve) => { channel.port1.onmessage = (e) => resolve(e.data); });
  client.postMessage({ type: "btv-http-request", request: { … } }, [channel.port2]);
...
does not guess: it **probes** every window (`postMessage` a `btv-edithost-probe` with a reply
...
`bemtvi_lua::build_server_request` the native listener calls. Doing it in JS would have been
...
`btv-sw.js` is served from the **root** (`/btv-sw.js`) even though its source lives in `web/`
...
**`opts.timeout` needs no transport at all.** It is enforced in **Lua**: `btv._http_request`
...
1. **Native** — `'httphost'`/`'httpport'` in `bemtvi-core`'s `options.rs` (field + set/get
   ...
   `tests/http_mount.rs`.
2. **Example** — `examples/btv-http-mount/`: a markdown renderer plugin serving the current
   buffer as styled HTML, end-to-end runnable.
3. **Web** — `btv-sw.js`, the window-client relay, the `HostEffects` seam + FFI exports,
   `verify-http-mount.mjs` in headless Chromium. **Done.**

## Tests (`crates/bemtvi-server/tests/http_mount.rs`)
...
- **a failed rebind** — `:set httpport=<taken>` leaves the old listener serving *and* reverts
  the option, so `btv.o.httpport` still equals the live port
