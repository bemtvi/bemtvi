...
// this asserts against ("btv.shada.plugin: this caller attributes to no plugin"). The
...
// A fixture plugin whose single module (a) calls btv.shada.plugin() at load and (b) contains
...
check(
  ...
  out.includes('btv._add_rtp("/bemtvi-plugins/treeish")'),
  "expected an btv._add_rtp(...) line so the plugin's chunks attribute to a namespace",
);
check(
  "compiles the module through load() with a plugin-scoped chunk name",
  out.includes('package.preload["treeish"] = assert(load(') &&
    out.includes('"@/bemtvi-plugins/treeish/lua/treeish/init.lua"'),
  "expected load(<src>, \"@/bemtvi-plugins/treeish/lua/treeish/init.lua\")",
);
check(
