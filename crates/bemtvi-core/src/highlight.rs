//! The highlight-group registry: bemtvi's analogue of neovim's highlight table
//! (`highlight_group.c`).
//!
...
impl Highlights {
    /// A fresh registry: empty until a colorscheme populates it. bemtvi *does*
    /// bundle a default scheme (`:colorscheme bemtvi`, embedded in the server),
    ...
    /// source the server folds into the Lua `btv._hl_defs` mirror that backs
    ...
    pub fn resolve_ns(&self, ns: u32, name: &str) -> Option<Style> {
        ...
        for _ in 0..MAX_LINK_DEPTH {
            let Some(def) = lookup(current) else {
                // Undefined in every table: a small set of btv-internal groups still
...
/// The built-in style for an btv-internal highlight group the renderer must be
...
/// diff groups are defined here today: `BtvSubstituteDelete` (the removed text — a
/// red foreground struck through) and `BtvSubstituteAdd` (the inline replacement —
...
fn builtin_default_style(name: &str) -> Option<Style> {
    ...
    match name {
        "BtvSubstituteDelete" => Some(Style {
            fg: rgb(0xf3, 0x8b, 0xa8), // red
            strikethrough: true,
            ..Style::default()
        }),
        "BtvSubstituteAdd" => Some(Style {
