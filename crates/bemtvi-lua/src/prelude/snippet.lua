-- btv.snippet: the native snippet engine (docs/plans/2026-06-15-btv-snippet-engine.md,
...
btv.snippet._byft = btv.snippet._byft or {}
...
function btv.snippet.setup(opts)
  opts = opts or {}
  if type(opts) ~= "table" then
    error("btv.snippet.setup: expected a table, got " .. type(opts))
  end
  -- Either key may be a string or list of strings; the shared `btv.utils.str_list`
  -- normalizes (nil → {} keeps that key's default).
  local function key_list(spec, name)
    return btv.utils.str_list(spec, "btv.snippet.setup: " .. name)
  end
  btv._snippet_setup(key_list(opts.jump_next, "jump_next"), key_list(opts.jump_prev, "jump_prev"))
end

-- `btv.snippet.add`(`"rust"`, { { trigger = `"fn"`, body = `"fn ${1:name}() {\n\t$0\n}"` }, ... })
...
function btv.snippet.add(ft, list)
  if type(ft) ~= "string" then
    error("btv.snippet.add: filetype (arg 1) must be a string, got " .. type(ft))
  end
  if type(list) ~= "table" then
    error("btv.snippet.add: snippets (arg 2) must be a list of { trigger, body }")
  end
  local triggers, bodies = {}, {}
  for i, s in ipairs(list) do
    if type(s) ~= "table" or type(s.trigger) ~= "string" then
      error("btv.snippet.add: entry " .. i .. " needs a string `trigger`")
    end
    if type(s.body) == "function" then
      error(
        "btv.snippet.add: function snippet bodies are not supported yet (entry '"
          .. s.trigger
          .. "'); use a string body for now"
      )
    end
    if type(s.body) ~= "string" then
      error("btv.snippet.add: entry '" .. s.trigger .. "' needs a string `body`")
    end
    triggers[#triggers + 1] = s.trigger
    bodies[#bodies + 1] = s.body
    btv.snippet._byft[ft] = btv.snippet._byft[ft] or {}
    table.insert(btv.snippet._byft[ft], { trigger = s.trigger, body = s.body })
  end
  btv._snippet_add(ft, triggers, bodies)
end

-- `btv.snippet.expand`(`"for ${1:i} = 1, ${2:n} do\n\t$0\nend"`)
...
function btv.snippet.expand(body)
  if type(body) ~= "string" then
    error("btv.snippet.expand: body must be a string, got " .. type(body))
  end
  btv._snippet_expand(body)
end
