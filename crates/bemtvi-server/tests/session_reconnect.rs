...
//! `btv.session.reconnect(spec)` (Lua) queues a client-directed swap that the server drains
//! into a `btv_session_reconnect` notification pushed OUT to the client. The client (TUI/GUI)
...
/// Poll `incoming` (~3s) for the next `btv_session_reconnect` notification and return its
    ...
    for _ in 0..150 {
        ...
        while let Ok(msg) = incoming.try_recv() {
            if let Incoming::Notification { method, params } = msg {
                if method == "btv_session_reconnect" {
...
async fn reconnect_emits_a_client_notification_with_the_normalized_spawn_spec() {
    ...
    exec_lua(
        &rpc,
        "btv.session.reconnect({\n\
         \x20 transport = { kind = 'spawn', cmd = 'ssh host bemtvi --daemon' },\n\
    ...
    let spec = await_reconnect(&rpc, &mut incoming)
        .await
        .expect("a btv_session_reconnect notification must be emitted");
...
async fn reconnect_carries_a_structured_argv_spawn_spec() {
    ...
    exec_lua(
        &rpc,
        "btv.session.reconnect({\n\
         \x20 transport = { kind = 'spawn', argv = { 'ssh', 'host', 'bemtvi', '--daemon' } },\n\
         })",
    )
    .await;
...
async fn reconnect_defaults_config_source_to_remote_and_keep_buffers_to_false() {
    ...
    exec_lua(
        &rpc,
        "btv.session.reconnect({ transport = { kind = 'quic', addr = 'bemtvi://host:7000/tok?cert=abc' } })",
    )
    .await;
    ...
    assert_eq!(
        ...
        Some("bemtvi://host:7000/tok?cert=abc"),
    );
...
async fn a_malformed_spec_fails_loud_and_emits_nothing() {
    ...
    let ok = exec_lua(
        &rpc,
        "return pcall(function() btv.session.reconnect({ transport = { kind = 'carrier-pigeon' } }) end)",
    )
    .await;
    assert_eq!(
        ok.as_bool(),
    ...
    let ok2 = exec_lua(
        &rpc,
        "return pcall(function() btv.session.reconnect({ transport = { kind = 'spawn' } }) end)",
    )
    .await;
    ...
    assert!(
        ...
        "a malformed spec must not emit a btv_session_reconnect notification",
    );
}
...
async fn config_source_merged_is_reserved_and_fails_loud() {
    ...
    let err = exec_lua(
        ...
        "local ok, err = pcall(function()\n\
         \x20 btv.session.reconnect({\n\
         \x20   transport = { kind = 'quic', addr = 'bemtvi://h:1/t?cert=c' },\n\
