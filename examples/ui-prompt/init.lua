...
-- BOTH are callback-shaped and NON-BLOCKING (ADR 0002 rule 3): the Lua call
-- returns at once and the result arrives on a LATER tick when you submit /
-- cancel. They open over the editor's COMMAND LINE (only one prompt at a time);
-- neovim spells these as the blocking `vim.fn.input` / `vim.fn.confirm`, which
-- the nx model deliberately omits.
...
--    on_confirm(text): the entered string ("" on an empty <CR>), or nil on cancel.
--------------------------------------------------------------------------------
nx.keymap.set("n", "<leader>i", function()
  nx.ui.input({ prompt = "Your name: " }, function(text)
    if text == nil then
...
nx.keymap.set("n", "<leader>r", function()
  nx.ui.input({ prompt = "Rename to: ", default = vim.fn.expand("%:t") }, function(name)
...
--    on_choice(confirmed) gets a BOOLEAN — true on Yes, false on No / cancel.
--    For an arbitrary multi-choice menu use nx.ui.select instead.
--------------------------------------------------------------------------------
nx.keymap.set("n", "<leader>d", function()
  nx.ui.confirm("Delete this line?", function(ok)
    if ok then
...
nx.keymap.set("n", "<leader>q", function()
  nx.ui.confirm("Quit without saving?", { default = false }, function(ok)
    if ok then
      nx.cmd("qa!")
    else
      nx.notify("staying")
