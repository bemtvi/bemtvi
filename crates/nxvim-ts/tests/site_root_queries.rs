...
//! `$XDG_DATA_HOME/nvim/site` (the fallback root [`nxvim_ts::extra_roots`]
//! searches); `NXVIM_DATA_DIR` stays unset so the fallback is consulted. Its own
//! test binary, since those env vars are process-global.

mod fixture;

use fixture::{install_rust_grammar, TempDir};
use nxvim_core::SyntaxEngine;
use nxvim_ts::Engine;
