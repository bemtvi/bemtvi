...
// leg (`sock_*`) — the DAP / framed-protocol transports — against a REAL `bemtvi --daemon
// --listen` over WebTransport. The browser has no local process / TCP, so `btv.process.open`
// and `btv.socket.connect` cross the wire to the daemon, which runs the child / dials the
...
// kill from the browser fires on_exit; (3) btv.socket connects — over the wire — to a TCP echo
...
// (target/debug/bemtvi), and a Chromium for Playwright. Run:  node verify-dap.mjs
...
try {
  ...
  // ── 1. btv.process duplex: a `cat` child on the daemon echoes the browser's stdin ─────────
  await luaResult(
    page,
    `_G.__got, _G.__exit = "", nil
     _G.__p = btv.process.open({
       cmd = "cat",
  ...
  const got = await until(
    page,
    () => window.__bemtvi.execLua("return _G.__got or ''").then((r) => r.result),
    (v) => /ping-over-webtransport/.test(String(v)),
  );
  check("dproc: btv.process stdin round-trips to stdout via a daemon child (duplex over the wire)",
  ...
  const exited = await until(
    page,
    () => window.__bemtvi.execLua("return tostring(_G.__exit)").then((r) => r.result),
  ...
  // ── 2. btv.socket: connect over the wire to the harness's TCP echo server ──────────────────
  await luaResult(
    page,
    `_G.__sdata, _G.__sconn, _G.__sclosed = "", false, nil
     _G.__s = btv.socket.connect({
  ...
  const sconn = await until(
    page,
    () => window.__bemtvi.execLua("return _G.__sconn and 1 or 0").then((r) => r.result),
    (v) => /1$/.test(String(v)),
  );
  check("sock: btv.socket connected to the echo server (over the wire)", /1$/.test(String(sconn)),
    `conn=${JSON.stringify(sconn)}`);
  const sdata = await until(
    page,
    () => window.__bemtvi.execLua("return _G.__sdata or ''").then((r) => r.result),
  ...
  const sclosed = await until(
    page,
    () => window.__bemtvi.execLua("return tostring(_G.__sclosed)").then((r) => r.result),
...
console.log(failures === 0
  ? "\nALL PASS — browser edit-host runs btv.process + btv.socket on a real bemtvi --daemon over WebTransport"
  : `\n${failures} FAILED`);
