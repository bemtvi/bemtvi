//! The parsed form of a `nx.session.reconnect(spec)` request (§B of the remote-connectors
//! plan). The Lua seam (`nx.session.reconnect`) validates + normalizes the spec and the
//! server pushes it to the client verbatim as the single param of a `nx_session_reconnect`
...
pub enum SpawnCommand {
    ...
    Argv(Vec<String>),
    /// A shell command line, run via `sh -c` — mirrors `NXVIM_DAEMON_CMD`, so a one-line
    /// `"ssh host nxvim --daemon"` / `"docker exec … nxvim --daemon"` works verbatim. Only
...
/// A client-directed session swap, parsed from the `nx_session_reconnect` notification.
#[derive(Clone, Debug, PartialEq, Eq)]
...
impl ReconnectSpec {
    ...
    pub fn from_value(value: &Value) -> Result<Self> {
        let transport = get(value, "transport")
            .ok_or_else(|| anyhow!("nx_session_reconnect: missing `transport`"))?;
        let kind = get_str(transport, "kind")
            .ok_or_else(|| anyhow!("nx_session_reconnect: missing `transport.kind`"))?;
        let transport = match kind {
            "spawn" => {
                ...
                let command = if let Some(argv) = get(transport, "argv") {
                    let argv = str_list(argv).ok_or_else(|| {
                        anyhow!("nx_session_reconnect: spawn `argv` must be a list of strings")
                    })?;
                    if argv.is_empty() {
                        bail!("nx_session_reconnect: spawn `argv` must be non-empty");
                    }
                    SpawnCommand::Argv(argv)
                } else if let Some(cmd) = get_str(transport, "cmd").filter(|s| !s.is_empty()) {
                    SpawnCommand::Shell(cmd.to_string())
                } else {
                    bail!("nx_session_reconnect: spawn transport needs `argv` or `cmd`");
                };
            ...
            "quic" => {
                let addr = get_str(transport, "addr")
                    .filter(|s| !s.is_empty())
                    .ok_or_else(|| anyhow!("nx_session_reconnect: quic transport needs `addr`"))?;
                ReconnectTransport::Quic {
                    addr: addr.to_string(),
                }
            }
            other => bail!("nx_session_reconnect: unknown transport.kind {other:?}"),
        };
        let config_source = match get_str(value, "config_source") {
            ...
            Some("merged") => bail!(
                "nx_session_reconnect: config_source \"merged\" is not implemented yet — use \"remote\" or \"local\""
            ),
            Some(other) => bail!("nx_session_reconnect: unknown config_source {other:?}"),
        };
    ...
    /// `nxvim://…` → a QUIC dial; a bare `[user@]host[:port]` → `ssh <target> nxvim --daemon`
    ...
    pub fn fallback_from_url(url: &str) -> Result<Self> {
        let transport = if url.starts_with("nxvim://") {
            ReconnectTransport::Quic {
                addr: url.to_string(),
            }
            ...
            bail!(
                "connect: no provider for {url:?} (only nxvim:// and [user@]host are dialed directly)"
            );
        } else {
...
/// Build the argv for `ssh [-p PORT] -- <target> nxvim --daemon` from a bare
