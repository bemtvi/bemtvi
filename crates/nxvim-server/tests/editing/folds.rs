...
async fn vim_bo_foldmethod_sets_the_fold_source() {
    // The `vim.bo`/`nx.bo` option path reaches the live fold engine (not just a
    ...
    assert_eq!(
        visible_numbers(&map),
        vec![1, 2],
        "nx.bo.foldmethod=indent folds the indented block, got {:?}",
        visible_numbers(&map)
    );
}
...
async fn treesitter_foldexpr_lua_marker() {
    ...
    // The native marker exists under `nx.*` and is aliased as `vim.*` — both
    // resolve to the one function `'foldexpr'` references.
    let same = exec_lua(
        &rpc,
        "return nx.treesitter.foldexpr == vim.treesitter.foldexpr",
    )
    .await;
    assert_eq!(
        same.as_bool(),
        Some(true),
        "vim.treesitter.foldexpr aliases nx.treesitter.foldexpr"
    );
    ...
    let errored = exec_lua(
        &rpc,
        "local ok, err = pcall(nx.treesitter.foldexpr, 1)\n\
...
async fn lsp_foldexpr_lua_marker() {
    let (rpc, _incoming) = start(None).await;
    // The LSP fold marker exists under `nx.*` and is aliased as `vim.*` — both
    // resolve to the one function `'foldexpr'` references for the LSP fold source.
    let same = exec_lua(&rpc, "return nx.lsp.foldexpr == vim.lsp.foldexpr").await;
    assert_eq!(
        same.as_bool(),
        Some(true),
        "vim.lsp.foldexpr aliases nx.lsp.foldexpr"
    );
    ...
    let errored = exec_lua(
        &rpc,
        "local ok, err = pcall(nx.lsp.foldexpr, 1)\n\
         return (not ok) and tostring(err):find('native marker') ~= nil",
    )
    .await;
    assert_eq!(
