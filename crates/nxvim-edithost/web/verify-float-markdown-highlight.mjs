...
//
// This reproduces the hover shape with `nx.view`: a markdown float carrying a ```rust fence,
...
try {
  ...
  await page.evaluate(() => window.__nxvim.feed(":e demo.rs<CR>ggdGifn main() { let z = 99; }<Esc>"));
  await sleep(700);
  ...
  const mounted = await luaResult(page, `
    _G.v = nx.view.create({ filetype = "markdown" })
  ...
  await page.evaluate(() => window.__nxvim.feed("<C-w>w")); // unfocus the float — like a real hover
