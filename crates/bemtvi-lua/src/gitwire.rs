//! The `btv.git` off-tick op wire codec — [`GitJob`] / [`GitValue`] / [`GitError`] ⟷
//! [`rmpv::Value`]. The git twin of [`fswire`](crate::fswire), used by the `git_op`
//! leg (a native-daemon or web session routing `btv.git.*` ops to the daemon, where the
//! files — and the gix engine — live):
//!
//! - the **daemon** decodes the request map into a [`GitJob`] ([`git_job_from_value`]),
//!   runs it through `bemtvi_git::run_git_job`, and encodes the typed result back
//!   ([`git_result_to_value`]);
...
/// the daemon runs `bemtvi_git::run_git_job` there.
