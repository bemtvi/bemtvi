//! `nx.lsp.restart(name)` tears down and respawns a running server, re-starting it
//! from the config in force NOW — so a `nx.lsp.config` change made after the server
...
async fn restart_respawns_the_server_with_the_latest_config() {
    ...
    let start_v = |v: &str| {
        format!(
            "nx.lsp.start({{ name = 'srv', cmd = {{ '{}', '{}' }}, root_dir = '{}' }}, {{ filetype = 'rust' }})",
            script.display(),
            v,
            dir.as_path().display()
        )
...
async fn restart_is_a_noop_when_nothing_is_running() {
    ...
    exec_lua(&rpc, "nx.lsp.restart('ghost')").await;
