-- bemtvi Lua prelude — `btv.promise`, a Promises/A+ surface shaped like the browser's.
--
-- `btv` async is PROMISE-ONLY (ADR 0002): a one-shot async API returns a promise
-- (`btv.run`, `btv.fs`, …) and streaming is an async-iterator over them (`btv.run_stream` +
-- `btv.await_each`). `btv.promise` is the foundation: the exact object model the
-- browser exposes — `btv.promise.new(executor)`, `:next`/`:catch`/`:finally`, and
-- the `btv.promise.all/all_settled/race/any/resolve/reject/try` combinators — plus
-- `btv.async`/`btv.await` coroutine sugar so a chain of awaits reads like
-- straight-line code. (`btv.promise.try` folds a function that may throw
...
-- run as MICROTASKS via `btv.schedule` (prelude/runtime.lua) — deferred to the end
...
--
-- It is an bemtvi-native surface (no `vim.*` twin — neovim core has no Promise), so
-- it lives entirely on `btv`. As the async FOUNDATION every later surface builds on
-- (process / picker / complete / `btv.ui`), it loads early — right after the runtime
-- services it needs: `btv.schedule` (the microtask primitive the reactions run on)
-- and `btv.timer` (the wall-clock deferral `btv.promise.delay` uses), both installed
...
local function report_unhandled(p)
  microtask(function()
    if p._state == REJECTED and not p._handled then
      vim.notify("bemtvi: unhandled promise rejection: " .. tostring(p._value), 4)
    end
  end)
end
...
-- ----- btv.promise: constructors + combinators --------------------------------

local M = {}

-- `btv.promise.new(executor)`: the browser constructor. `executor(resolve, reject)`
-- runs SYNCHRONOUSLY now (as in the browser); a throw inside it rejects the
-- promise. `btv.promise(executor)` is the same thing via `__call` sugar.
function M.new(executor)
  local p = new_pending()
  if executor ~= nil then
    if not is_callable(executor) then
      error("btv.promise.new: executor must be a function", 2)
    end
...
-- `btv.promise.resolve(value)`: a promise already fulfilled with `value` (or, if
...
-- `btv.promise.reject(reason)`: a promise already rejected with `reason`.
...
-- `btv.promise.try(fn, ...)`: run `fn(...)` INSIDE a promise — a synchronous throw
...
-- call site, just `btv.promise.try(fn, ...):next(...):catch(...)`. Mirrors the
...
-- `btv.promise.all(list)`: fulfils with the array of every value once ALL fulfil, in
...
-- `btv.promise.all_settled(list)` [alias `allSettled`]: fulfils once every promise
...
-- `btv.promise.race(list)`: settles the moment the FIRST input settles, adopting its
...
-- `btv.promise.any(list)`: fulfils with the first value to fulfil; rejects only if
...
-- ----- btv-native conveniences (still promise-shaped) -------------------------

-- `btv.promise.delay(ms[, value])`: a promise that fulfils with `value` after `ms`
...
-- `btv.wait_for(predicate[, opts])` -> promise: poll `predicate` BETWEEN ticks until it
...
-- buffer that exists next tick. It yields the tick (`btv.on_next_tick`) between checks, so
-- those mirrors actually refresh, instead of spinning within one convergence the way
-- a bare `btv.schedule` re-arm does. `predicate` is checked once immediately, then once
-- per following tick.
--
...
--
-- REJECTS (so an `btv.await` fails loud and a chain can `:catch`) if the condition
...
function M.wait_for(predicate, opts)
  ...
  return M.new(function(resolve, reject)
    ...
    local function step()
      ...
      if not ok then
        return reject({ message = "btv.wait_for: predicate errored: " .. tostring(val) })
      end
      if val then
        return resolve(val)
      end
      ...
      if n >= tries then
        return reject({
          message = opts.message or ("btv.wait_for: condition not met after " .. tries .. " ticks"),
        })
      end
      if interval then
        btv.timer(step, interval)
      else
        btv.on_next_tick(step)
      end
    end
    step() -- check immediately (tick 0), then poll the next ticks
  end)
end
btv.wait_for = M.wait_for
...
-- all of them as a table when there's more than one) — the shape bemtvi's own
-- callback APIs use (`btv.ui.select`'s `on_choice`, an `on_exit`, …). Use this to turn
-- "pass me a callback" surfaces into await-ables; reach for `btv.promise.new`
...
btv._is_promise = is_promise

btv.promise = M

-- ----- btv.async / btv.await: coroutine sugar over the same promises -----------
--
...
--
-- ```lua
-- local load = btv.async(function(path)
--   local stat = btv.await(fs.stat(path))
--   local data = btv.await(fs.read(path))
...
function btv.async(fn)
  ...
  return function(...)
    ...
    return M.new(function(resolve, reject)
      ...
      -- (ok, value) back so `btv.await` can return the value or raise the reason.
      local function step(...)
...
-- `btv.await(awaitable)`: suspend the enclosing `btv.async` coroutine until `awaitable`
...
-- called outside an `btv.async` coroutine — there is nothing to suspend.
function btv.await(awaitable)
  -- `coroutine.isyieldable()` is false on the main thread and true inside a
  -- coroutine — exactly "is there an btv.async frame to suspend?". (The 5.1
  -- spelling `coroutine.running() == nil` broke in 5.2+, where `running()`
  -- returns the main thread itself rather than nil.)
  if not coroutine.isyieldable() then
    error("btv.await must be called inside an btv.async function", 2)
  end
