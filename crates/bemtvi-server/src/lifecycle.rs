...
impl EditHost {
        ...
        match result {
            ...
            Err(e) => {
                ...
                if self.pending_replica_edits.remove(&buffer).is_some() {
                    ...
                    self.editor.echo(format!(
                        "bemtvi: could not open {path} over the daemon: {e}"
                    ));
                }
            }
        }
    ...
    pub(crate) fn load_replica_bytes(
        ...
        bytes: &[u8],
        existed: bool,
        stat: Option<bemtvi_core::FileStat>,
    ) {
    ...
    /// than re-read by the explorer plugin's `btv.fs` (the `btv.fs` op and the file-open
    /// fetch are separate legs; a daemon may wire only the latter). The result is the
    /// same shape the plugin's local fill produces — a `nomodifiable`, `filetype=btvdir`
    /// buffer named for `dir` — so the plugin's stateless navigation and the decor
    /// provider work identically. Setting the filetype fires `FileType btvdir`, which
    ...
    pub(crate) fn load_dir_listing(
        ...
        let text = bemtvi_core::dir_listing(entries);
        self.editor
            .load_bytes_into(buffer, Some(dir.clone()), text.as_bytes());
        // The fill is a read, not an edit; hold it read-only via the `'modifiable'`
        // option (the explorer has no core buffer-kind), and mark it `btvdir` so its
        // `FileType btvdir` autocmd installs the activation maps and the decor provider
        // colours it.
        self.editor
            .set_buffer_option_bool(buffer, "modifiable", false);
        self.editor
            .set_buffer_option_str(buffer, "filetype", "btvdir");
        ...
        let _ = self.lua.set_buf_snapshot(buffer.0, &dir, "btvdir");
        self.push_buf_mirror();
        self.emit_lifecycle_events();
        self.run_pending();
    }
    ...
    fn fire_buf_read_cmd(&mut self, buffer: BufferId, path: &Path) -> bool {
        ...
        // the path; a directory has none (the handler sets `btvdir` itself).
    ...
    pub(crate) fn drain_pending_opens(&mut self) {
        ...
        // refreshes this, but a drain can also run during startup (a deferred `bemtvi .`
    ...
    /// server-side emission point (design D1) that keeps `bemtvi-core` free of
    ...
    pub(crate) fn emit_lifecycle_events(&mut self) {
        ...
        // reuse across kinds (throwaway → `btvdir` listing, file → directory), which
        ...
        // `btv.bo.filetype`, or a core-created special buffer's `set_filetype`: the
        // explorer's `btvdir`, the quickfix display's `qf`, a view's `btvview`/content
    ...
    /// the boot-sourced plugins are in place, refresh the `btv._view_pending` mirror and run
    /// the Lua dispatch: each owning plugin whose `btv.view.on_restore` is already registered
    ...
    /// restore coordinator (`btv._maybe_collapse_view_restores`). A plugin loaded via
    /// `btv.plugins({ config = … })` registers its handler asynchronously, on a tick *after*
    /// this boot dispatch, so reaping orphans now would collapse its slot before it ever got
    /// to claim it. Instead `btv._run_view_restores()` collapses immediately only when it can
    ...
    pub(crate) fn restore_persisted_views(&mut self) {
        ...
        if let Err(e) = self.lua.exec("btv._run_view_restores()") {
            self.editor
    ...
    pub(crate) fn fire_vim_enter(&mut self) {
        if let Err(e) = self.lua.exec("btv.autocmd.exec('VimEnter', {})") {
            self.editor
    ...
    /// Fire `UIEnter` — a client has attached and `btv.ui.caps()` now describes its
    ...
    pub(crate) fn fire_ui_enter(&mut self) {
        if let Err(e) = self.lua.exec("btv.autocmd.exec('UIEnter', {})") {
            self.editor
    ...
    fn fix_current_dir(&mut self) {
        ...
        // cwd). The focused window's effective dir is the cwd, mirrored into `btv._cwd` so
    ...
    /// `btv._cwd` Lua mirror that `vim.fn.getcwd()` reads. Called on every cwd change —
    ...
    /// Mirrors the phase into `btv.daemon.status()` and fires the `User DaemonStatusChanged`
    ...
    pub fn apply_daemon_phase(&mut self, phase: &str, reconnected: bool) {
        ...
        // injection-safe.
        if let Err(e) = self.lua.exec(&format!("btv._set_daemon_status('{phase}')")) {
    ...
    /// and neovim has entered it by the time a handler runs, so `btv.wo`,
    /// `btv.win.current()` and the cursor reads inside the handler must address that
    ...
    ///
    /// The context is the *mirror* one (`btv._fire_in_win` → `btv.win.call`), not a real
    /// focus change: running a handler must not move the user's cursor. Reads and
    /// explicit-handle writes resolve against `win`; a drain-time mutation (`btv.cmd`,
    /// feedkeys) raises through `btv._call_ctx_lock` rather than silently landing in the
    ...
    /// callback runs before the caller's `run_pending`, so `btv._bufs` / the cursor
    ...
    /// mirror push comes first, exactly as above — `btv._fire_in_win` swaps its window
    ...
    /// autocmd, so `vim.bo.filetype` / `btv._bufs` read the written buffer even when it's
    ...
    /// commits it when `btv._au_gate_done(gate_id)` settles (`drain_au_gate_done`). A
    ...
    /// `btv.promise.all_settled(...):next(…)` called `btv._au_gate_done(gate_id)` once every
    /// handler promise settled, landing the id in `au_gate_done`; here we pop the parked
    /// [`PreWrite`](bemtvi_core::PreWrite) and commit it (writing by id, so no buffer scope
    ...
    /// `btv._au_gate_done(gate_id)` fires. A throwing handler is reported and treated as settled
    ...
    /// Re-seed the Lua snapshot (`btv._cur_buf`) at the editor's *current* buffer after a
    ...
    pub(crate) fn source_plugins(&mut self) {
        ...
        // Skip every dir the `btv.plugins` manager sources itself (via `source_runtime`):
    ...
    /// Every dir the `btv.plugins` manager sources itself — the system tier PLUS every
    ...
    fn manager_owned_plugin_dirs(&self) -> Vec<PathBuf> {
        let value = match self.lua.eval_to_value(
            "return btv.plugins and btv.plugins._manager_owned_dirs and btv.plugins._manager_owned_dirs() or {}",
        ) {
            Ok(v) => v,
