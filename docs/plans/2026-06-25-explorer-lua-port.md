...
drive the Lua explorer. The file explorer is now a pure-`btv.*` plugin.

Make the in-window directory listing (vim's netrw, `filetype=btvdir`) a **pure-Lua
plugin** built on the `btv.*` API, deleting the Rust explorer engine. This follows the
prime dogfood directive (every feature that can reasonably be an `btv.*` Lua plugin IS
...
explorer, so they are built as general `btv.*` / autocmd surfaces, not explorer shims.

## Why it isn't free today

A grep of `bemtvi-lua` confirms the gaps:

1. **No Lua mouse surface at all.** `on_mouse` / `btv.mouse` / `<LeftMouse>` return
...
Everything else is already Lua-able: `btv.fs` reads directories asynchronously and
already works over the daemon/remote (`fs.lua`, `daemon_luafs`); `btv.view` is the
established plugin-owned `nomodifiable` buffer with activation maps; navigation maps
and the new directory-row highlighting (`prelude/explorer.lua`, an `btv.decor`
provider) are already Lua.

## Current Rust surface to be removed at cutover

- `crates/bemtvi-core/src/buffer.rs` — `Buffer::from_dir` / `from_dir_entries`,
  `BufferKind::Directory`, `Buffer::dir()`.
- `crates/bemtvi-core/src/editor/explorer.rs` — the whole file (`enter_dir`,
  `explorer_open_entry`, `explorer_up`, `is_explorer_buffer`,
  `apply_explorer_action`).
- `crates/bemtvi-core/src/editor/buffers.rs:798` `load_dir_into` + the `enqueue_open`
...
- `prelude/explorer.lua` — the `btv.decor` highlight provider (filetype `btvdir`).
- The `FileType btvdir` keymap wiring (`prelude/keymap.lua`) — folds into the plugin.
...
2. on a claimed open, reads entries via `btv.fs` (async; works locally and over the
   daemon) and renders them into an **`btv.view`** buffer (`nomodifiable`, owned);
3. installs buffer-local **`<CR>` / `-` / `<2-LeftMouse>`** maps (Primitive A gives
   the mouse one) for open / parent / open-on-click;
4. highlights rows via the existing `btv.decor` provider.
...
- **Phase 1 — Primitive A (mouse maps). ✅ DONE 2026-06-25.** `<LeftMouse>` /
  ...
  `FileType btvdir` autocmd; the interim `mouse.rs` special-case is gone. Tested in
  ...
  - **Follow-up — `getmousepos()`. ✅ DONE 2026-06-26.** `vim.fn.getmousepos()` (canonical
    `btv.getmousepos()`) returns the last mouse event's position as a dict —
    ...
    so chrome above the window is excluded). The server mirrors it to `btv._mouse_pos` in
...
- **Phase 2 — Primitive B (`BufReadCmd`). ✅ DONE 2026-06-25.** A file open is now
  ...
  *before* the default load: a handler **claims** the read by returning a truthy value
  (`btv._fire_read_cmd` — a per-path decision, so a `pattern = "*"` handler claims
...
- **Phase 3 — the Lua explorer plugin. ✅ DONE 2026-06-25.** `prelude/explorer.lua`
  gained the explorer itself, opt-in via `btv.explorer.enable()` (registered in parallel
  ...
  threaded through `fire_autocmd_cmd` → `btv._fire_read_cmd`) — the live Lua fs surface is
  async, so the handler can't re-stat synchronously. On a claim it reads entries with
  `btv.fs.readdir` (async; local disk or, over a daemon, the wire), renders them the same
  ...
  no `[+]`), marks it `btvdir` (drives the decor + the buffer-local maps), and installs the
  ...
  - **Daemon claim deferred to Phase 4.** `args.isdir` is a *local* stat, so off-tick
    ...
    `btv.fs` *fill* works over the wire — verified by the existing daemon fs suites — but
...
- **Phase 4 — cutover + delete. ✅ DONE 2026-06-25.** The explorer is on by default
  (`btv.explorer.enable()` called at the bottom of `prelude/explorer.lua`), so a `:e dir`
  ...
  `from_dir_entries`, `Editor::load_dir_into`, the `btv._explorer_action` bridge +
  ...
  `load_dir_replica_wasm`. The `FileType btvdir` autocmd (prelude/keymap.lua) now installs
  the plugin's pure-Lua `<CR>`/`-`/`<2-LeftMouse>` maps.
  - **Navigation is stateless.** `btv.explorer._open`/`_up` derive the directory from the
  ...
  - **Two fill paths, one shape.** A **local** `:e dir` is claimed by the `BufReadCmd`
    handler, which reads entries with `btv.fs` and renders them (Lua `render`). A **remote**
    directory (daemon / web) is filled **server-side** from the entries the off-tick fetch
    already read (`EditHost::load_dir_listing` → `bemtvi_core::dir_listing`) — because the
    `btv.fs` op and the file-open fetch are *separate daemon legs* and re-reading via
    `btv.fs` can't be relied on to reach the same remote. Both produce a `nomodifiable`,
    `filetype=btvdir` buffer named for the directory, so the stateless nav + decor are
  ...
  - **Startup.** A local `bemtvi somedir` can't list at construction (no Lua VM yet), so
...
- **Daemon/off-tick parity.** The Lua explorer must fill asynchronously over the
  daemon. `btv.fs` already does remote `read_dir`, but Phase 3 must verify the
  view-fill timing matches the current off-tick `load_dir_into` behavior (the
  cross-tick winid caveat — use `btv.on_next_tick`/`btv.wait_for`, never re-armed
  `btv.schedule`).
- **Buffer identity/name.** The current listing's buffer shows the directory path as
  its name; the `btv.view` must reproduce that (`view_name`) so `:ls` and the
