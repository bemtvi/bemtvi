...
//
// Faithfulness (not a no-op): the picker is opened through the real `btv.picker` API over
...
try {
  ...
  await page.evaluate(() =>
    window.__bemtvi.execLua(`btv.picker.open('webpaths', { exclude = 'target' })`),
  );
  await sleep(400);
  ...
  await page.evaluate(() => window.__bemtvi.feed("<C-g>"));
  await sleep(300);
  ...
  await page.evaluate(() => window.__bemtvi.feed("src"));
  await sleep(500);
