//! Behavior tests for `btv.cmdline_complete` — command-line completion, the unified
//! float-list widget's **fifth orchestration** (`docs/specs/2026-06-14-btv-ui-float-widget.md`,
...
//! `btv.cmdline_complete.setup{}`, the line is driven over the same msgpack-RPC a UI
...
const INIT: &str = "btv.cmdline_complete.setup {}";
...
async fn disabled_without_setup() {
    let dir = temp_dir("cmdcomplete");
    // No btv.cmdline_complete.setup{} — the engine stays off.
...
async fn enabled_by_default_via_server_init() {
    ...
    // `btv.cmdline_complete.setup{}` before init.lua — so `:`+<Tab> completes even
...
async fn docs_disabled_by_setup() {
    ...
    let (rpc, mut incoming) = start(&dir, "btv.cmdline_complete.setup { docs = false }").await;
...
async fn plugin_command_appears_in_wildmenu() {
    ...
    let init = "btv.cmdline_complete.setup {}\n\
        btv.user_command.create('Hello', 'echo hi', { desc = 'Greet the world' })";
    let (rpc, mut incoming) = start(&dir, init).await;
...
async fn plugin_command_usage_heads_the_docs_like_a_builtin() {
    ...
    let init = "btv.cmdline_complete.setup {}\n\
        btv.user_command.create('Greet', 'echo hi', \
...
async fn cmdline_docs_wrap_long_lines_to_the_box_width() {
    ...
    let init = format!(
        "btv.cmdline_complete.setup {{}}\n\
         btv.user_command.create('Wrapcmd', function() end, {{ desc = '{long}' }})"
    );
    let (rpc, mut incoming) = start(&dir, &init).await;
...
async fn cmdline_docs_use_the_whole_editor_width_not_the_focused_window() {
    ...
    let init = format!(
        "btv.cmdline_complete.setup {{}}\n\
         btv.user_command.create('LongCommandName', function() end, {{ desc = '{long_desc}' }})"
    );
    let (rpc, mut incoming) = start(&dir, &init).await;
...
async fn cmdline_docs_keep_a_usable_width_instead_of_a_one_column_sliver() {
    ...
    let init = format!(
        "btv.cmdline_complete.setup {{}}\n\
         btv.user_command.create('AVeryLongUserCommandNameForCmdlineDocs', function() end, \
         {{ desc = '{long_desc}' }})"
    );
    let (rpc, mut incoming) = start(&dir, &init).await;
...
async fn plugin_command_without_desc_shows_synopsis_only() {
    let dir = temp_dir("cmdcomplete");
    let init = "btv.cmdline_complete.setup {}\n\
        btv.user_command.create('Frobnicate', function() end)";
    let (rpc, mut incoming) = start(&dir, init).await;
...
async fn bundled_dock_command_shows_with_its_desc() {
    let dir = temp_dir("cmdcomplete");
    // The prelude registers the :Dock* commands via btv.command{ desc = … }; they show
...
async fn catalog_names(rpc: &Rpc) -> Vec<String> {
    let value = exec_lua(
        rpc,
        "local out = {}\n\
         for _, c in ipairs(btv._cmdline_complete_run('', 0)) do out[#out + 1] = c.label end\n\
         return out",
    )
    .await;
    match value {
...
/// Each name is run via `btv_command` — a *request*, so a following `barrier`
...
async fn catalog_commands_are_recognized() {
    ...
    for name in &names {
        ...
        rpc.request("btv_command", vec![Value::from(name.as_str())])
            .await
            .expect("btv_command");
        barrier(&rpc).await;
...
async fn colorscheme_arg_completes_scheme_names() {
    ...
    // …and the scheme bundled in the binary (injected as `btv._builtin_colorschemes`).
    assert!(items.contains(&"bemtvi".to_string()), "items: {items:?}");
    ...
    let colo = exec_lua(
        &rpc,
        "local out = {}\n\
         for _, c in ipairs(btv._cmdline_complete_run('colo ', 5)) do out[#out + 1] = c.label end\n\
         return out",
    )
    .await;
    ...
    assert!(
        colo_names.contains(&"mytheme".to_string()) && colo_names.contains(&"bemtvi".to_string()),
...
async fn highlight_arg_completes_group_names() {
    ...
    // `nvim_set_hl` write folds into the core registry and pushes `btv._hl_defs` on a
    ...
    let mut ready = false;
    for _ in 0..60 {
        if exec_lua(&rpc, "return btv.hl.exists('MyCoolGroup')")
            .await
            .as_bool()
            == Some(true)
        {
...
async fn setfiletype_arg_completes_filetype_names() {
    ...
    // `btv._filetypes` from `bemtvi_core::known_filetypes` — the extension table).
    ...
    let setf = exec_lua(
        &rpc,
        "local out = {}\n\
         for _, c in ipairs(btv._cmdline_complete_run('setf ', 5)) do out[#out + 1] = c.label end\n\
         return out",
    )
    .await;
...
/// core actually recognizes — both read `bemtvi_core::known_filetypes`, so this proves
/// the bridge (`btv._filetypes`) is wired and non-empty.
#[tokio::test]
async fn completed_filetypes_are_the_core_known_set() {
    ...
    let value = exec_lua(&rpc, "return #(btv._filetypes or {})").await;
...
async fn tsinstall_arg_completes_languages_in_every_argument() {
    ...
    // `:TSInstall <Tab>` offers tree-sitter language names (the filetypes bemtvi can
...
async fn augroup_arg_completes_defined_group_names() {
    ...
    exec_lua(&rpc, "btv.augroup.create('MyPluginGroup')").await;
    let mut ready = false;
    for _ in 0..60 {
        if exec_lua(&rpc, "return btv._augroups['MyPluginGroup'] ~= nil")
            .await
            .as_bool()
            == Some(true)
        {
...
async fn put_arg_completes_registers_that_hold_content() {
    ...
    for _ in 0..60 {
        if exec_lua(
            &rpc,
            "return btv._registers['a'] ~= nil and btv._registers['z'] ~= nil",
        )
        .await
        .as_bool()
            == Some(true)
...
async fn move_arg_completes_addresses_and_current_buffer_marks() {
    ...
    for _ in 0..60 {
        let has = exec_lua(
            &rpc,
            "for _, m in ipairs(btv.mark.list()) do if m.name == 't' then return true end end\n\
             return false",
        )
        .await;
...
/// line on a later tick (`btv._cmdline_set_arg` → `cmdline_replace_arg`), so this
...
/// The picker box's title (`btv.picker.open{ title = … }`), projected on the `menu`.
...
async fn user_command_complete_dir_lists_directories_only() {
    ...
    let init = "btv.cmdline_complete.setup {}\n\
        btv.user_command.create('workspace', function() end, \
...
async fn client_init_lua_registers_virtual_commands_with_completion() {
    ...
    std::fs::create_dir_all(tree.join("sub")).unwrap();
    std::fs::write(tree.join("top.txt"), "TOP\n").unwrap();
    std::fs::write(dir.join("init.lua"), "btv.cmdline_complete.setup {}").unwrap();
    ...
    let init = ServerInit {
        ...
        client_init_lua: Some(
            "btv.user_command.create('connect', function() end, { desc = 'Connect to a daemon' })\n\
             btv.user_command.create('workspace', function() end, \
...
async fn user_command_complete_file_lists_files() {
    ...
    let init = "btv.cmdline_complete.setup {}\n\
        btv.user_command.create('Grepin', function() end, { complete = 'file' })";
    let (rpc, mut incoming) = start(&dir, init).await;
    rpc.request(
        "btv_command",
        vec![Value::from(format!("cd {}", tree.display()))],
...
async fn user_command_complete_fn_sync_lists_inline() {
    ...
    let init = "btv.cmdline_complete.setup {}\n\
        btv.user_command.create('Sub', function() end, { complete = function(_args) \
...
async fn user_command_complete_fn_receives_args_so_far() {
    ...
    let init = "btv.cmdline_complete.setup {}\n\
        btv.user_command.create('Sub', function() end, { complete = function(args) \
...
async fn user_command_complete_fn_async_uses_picker() {
    ...
    // An ASYNC completer (returns a promise — here an `btv.async` function) can't feed the
    // synchronous wildmenu, so it lists in the picker instead.
    let init = "btv.cmdline_complete.setup {}\n\
        btv.user_command.create('ASub', function() end, \
        { complete = btv.async(function(_args) return { 'alpha', 'beta' } end) })";
    let (rpc, mut incoming) = start(&dir, init).await;
...
/// the catalog bridge (`btv._options_catalog`) is wired.
#[tokio::test]
async fn completed_options_are_recognized_by_set() {
    ...
    let value = exec_lua(
        &rpc,
        "local out = {}\n\
         for _, c in ipairs(btv._cmdline_complete_run('set ', 4)) do out[#out + 1] = c.label end\n\
         return out",
    )
    .await;
    ...
    for name in &names {
        rpc.request("btv_command", vec![Value::from(format!("set {name}?"))])
            .await
            .expect("btv_command");
        barrier(&rpc).await;
        let msg = drain_latest_redraw(&mut incoming)
...
async fn clicking_a_wildmenu_candidate_selects_then_accepts_into_the_line() {
    ...
    let (rpc, mut incoming) = start(&dir, INIT).await;
    // Deliberately the *default* 'mouse' = "nvi" (no 'c'): the wildmenu is bemtvi's own
    ...
    bemtvi_test_harness::command(&rpc, "set nonumber norelativenumber").await;
    ...
    bemtvi_test_harness::feed_mouse(&rpc, "left", "press", click_row, click_col);
    let map = poll_menu(&rpc, &mut incoming)
        .await
...
async fn wheeling_over_the_wildmenu_cycles_candidates() {
    ...
    bemtvi_test_harness::feed_mouse(&rpc, "wheel", "down", r, c);
    let map = wait_redraw(&mut incoming, |m| menu_sel_is(m, 0, true)).await;
...
async fn prompt_wildmenu_anchors_past_a_multichar_prompt_label() {
    ...
    // An `btv.ui.input` prompt whose label (`dap> `) is five cells wide. The client
    ...
    exec_lua(
        &rpc,
        "btv.ui.input({ prompt = 'dap> ', complete = function()
