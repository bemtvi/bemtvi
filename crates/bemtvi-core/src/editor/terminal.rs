...
//! `crates/bemtvi-server/src/terminal.rs` and
...
impl Editor {
    ...
    /// entry; the `btv.terminal` Lua control surface (programmatic open + keymaps)
    ...
    pub(crate) fn modifiable(&self) -> bool {
        ...
        // `btv.view`, a live terminal mirror, an image preview — and the quickfix /
