...
async fn get_autocmds_reflects_clear_and_del() {
    ...
    // (An unfiltered or by-event `FileType` query would now also include bemtvi's
    // built-in ftplugin autocmds — `FileType btvdir`/`qf` install the explorer /
...
async fn buf_get_name_and_expand_read_the_snapshot() {
    ...
    let msg = lua_message(
        &rpc,
        &mut incoming,
        "btv._set_cur_buf(4, '/tmp/foo/bar.rs') \
...
async fn extension_detection_covers_installable_treesitter_grammars() {
    // `EXT_FILETYPE` (bemtvi-core) maps a file extension to its tree-sitter language
    ...
    for (name, want) in cases {
        ...
        bemtvi_test_harness::feed(&rpc, &format!(":edit {}<CR>", path.display()));
...
// The Vimscript front-end (btv._ex_autocmd / _ex_augroup / _ex_doautocmd) drives
...
async fn ex_augroup_bang_deletes_the_group_and_its_autocmds() {
    ...
    let id = exec_lua(&rpc, "return btv._augroups.Foo == nil").await;
...
async fn rejecting_async_bufwritepre_still_writes() {
    ...
    let (rpc, mut incoming) = start_with_file_and_config(
        ...
        "vim.api.nvim_create_autocmd('BufWritePre', {\n\
         \x20 callback = function()\n\
         \x20   return btv.promise.delay(20):next(function() error('formatter blew up') end)\n\
         \x20 end })\n",
    )
    .await;
...
async fn non_gating_event_async_handler_runs_in_background() {
    ...
    let (rpc, _incoming) = start_with_config(
        ...
        "_G.ran = false\n\
         ...
         \x20 callback = function()\n\
         \x20   return btv.promise.delay(20):next(function() _G.ran = true end)\n\
         \x20 end })\n",
    )
    .await;
...
async fn non_gating_event_async_rejection_surfaces() {
    ...
    // captures `btv.notify` (the reporter the tracker calls); without the tracker's
    ...
    let (rpc, _incoming) = start_with_config(
        ...
        "_G.notified = {}\n\
         ...
         \x20 callback = function()\n\
         \x20   return btv.promise.delay(20):next(function() error('kaboom') end)\n\
         \x20 end })\n",
    )
    .await;
    exec_lua(
...
async fn wqa_writes_all_with_bufwritepre_then_quits() {
    ...
    feed(&rpc, ":wqa<CR>");
    // Wait for exit (an `bemtvi_exit` notification or the closed channel).
    let quit = {
...
const EXIT_LIFECYCLE_CONFIG: &str = "_G.log = {}\n\
     ...
     \x20 return btv.promise.new(function(resolve) _G.res[name] = resolve end)\n\
     end end\n\
...
/// Drain notifications until the editor exits — an `bemtvi_exit` **or** a closed channel
/// (the quit tears the connection down, and the close can beat the racing `bemtvi_exit`) —
...
async fn set_topline_scrolls_an_inactive_window_and_fires_winscrolled() {
    // `btv.win.set_topline(win, N)` scrolls an explicit (here inactive) window to the
    ...
    exec_lua(&rpc, "_G.n = 0; _G.m = ''; btv.win.set_topline(1, 40)").await;
    ...
    let top = exec_lua(&rpc, "return btv.win.call(1, vim.fn.winsaveview).topline")
        .await
        .as_u64();
    assert_eq!(
        top,
...
async fn set_leftcol_horizontally_scrolls_an_inactive_window() {
    // `btv.win.set_leftcol(win, N)` moves a window's first visible screen column
    ...
    // Vertical split: focus moves to the new window (id 2); window 1 is now inactive.
    redraw_after(&rpc, &mut incoming, "<C-w>v").await;
    exec_lua(&rpc, "btv.win.set_leftcol(1, 7)").await;
    let left = exec_lua(&rpc, "return btv.win.call(1, vim.fn.winsaveview).leftcol")
        .await
        .as_u64();
    assert_eq!(
        left,
...
async fn bufwinenter_fires_every_time_a_window_changes_buffer() {
    ...
    // buffer when there is one (bemtvi's own `:drop`-like behavior), which would move
...
async fn rereading_the_current_buffer_fires_the_enter_sequence() {
    ...
    let (rpc, mut incoming) = start_with_file_and_config(
        ...
        "_G.log = {}\n\
         ...
         \x20 btv.on(e, function() _G.log[#_G.log+1] = e end)\n\
         end\n",
    )
    .await;
...
async fn splitting_onto_the_current_file_displays_without_reading() {
    ...
    //
    // Regression: bemtvi ran the command through the ordinary `:edit` reload, so it also
...
async fn bufwinenter_fires_with_the_displaying_window_current() {
    ...
    let (rpc, mut incoming) = start_with_file_and_config(
        ...
        "_G.seen = {}\n\
         ...
         \x20 _G.seen[#_G.seen+1] = tostring(btv.win.current()) .. '/cc=' ..\n\
         \x20   tostring(btv.wo.colorcolumn) .. '/' ..\n\
    ...
    exec_lua(
        &rpc,
        "btv.wo[2].colorcolumn = '42'; btv.wo[1].colorcolumn = '7'; return 1",
    )
    .await;
    // …then move focus to window 1, leaving 2 in the background.
    redraw_after(&rpc, &mut incoming, "<C-w>w").await;
    assert_eq!(
        exec_lua(&rpc, "return btv.win.current()").await.as_u64(),
        Some(1),
...
async fn a_mutation_bound_to_the_current_window_raises_in_a_background_fire() {
    ...
    // drains — an ex-command, feedkeys — would still land in the focused window. bemtvi
    // cannot retarget those, so it raises, naming the fire: a handler is told its `btv.cmd`
    ...
    // guard would be indistinguishable from banning `btv.cmd` in the handler outright.
    ...
    let (rpc, mut incoming) = start_with_file_and_config(
        ...
        "_G.err = 'never ran'\n\
         ...
         \x20 local ok, e = pcall(function() btv.cmd('normal! gg') end)\n\
    ...
    let err = exec_lua(&rpc, "return _G.err")
        ...
        .to_string();
    assert!(
        err.contains("the BufWinEnter fire for window 2") && err.contains("btv.cmd"),
...
const WINEV_INIT: &str = "_G.ev = {}\n\
     ...
     \x20 btv.autocmd.create(e, { callback = function() _G.ev[#_G.ev+1] = e end })\n\
     end\n";
...
async fn bufreadpost_setting_filetype_fires_filetype_once_in_order() {
    ...
    bemtvi_test_harness::feed(&rpc, &format!(":edit {}<CR>", file.display()));
    let _ = rpc.request("nvim_get_mode", vec![]).await;
...
async fn deferred_open_fires_read_events_in_order_before_bufwinenter() {
    ...
    bemtvi_test_harness::feed(&rpc, &format!(":edit {}<CR>", b.display()));
    let _ = rpc.request("nvim_get_mode", vec![]).await;
    let v = exec_lua(&rpc, "return table.concat(_G.log, ' ')").await;
    assert_eq!(
...
/// Autocmd patterns speak the full `btv.glob` dialect, so `**` crosses path
...
/// the `btv.glob` defaults on two counts: a bare `*` crosses `/` (btv.glob defaults to
...
async fn autocmd_patterns_keep_vims_star_and_basename_rules() {
    ...
    let out = exec_lua(
        ...
        "_G.hits = {}\n\
         ...
         -- a `*` in a rooted pattern still crosses `/` (vim's rule, not btv.glob's default)\n\
...
async fn an_uncompilable_autocmd_pattern_fails_loud_at_registration() {
    ...
    assert!(
        s.contains("btv.autocmd.create") && s.contains("User"),
        "the error must name the call and the event, got {s:?}"
    );
    assert!(
        !s.contains("bemtvi:prelude/autocmd"),
...
async fn hot_path_handler_returning_a_promise_raises_and_names_the_event() {
    ...
    let (rpc, mut incoming) = start_with_file_and_config(
        ...
        "btv.autocmd.create('CursorMoved', {\n\
         \x20 callback = function() return btv.promise.resolve(1) end })\n",
    )
    .await;
...
async fn hot_path_handler_may_start_async_work_it_does_not_return() {
    ...
    let (rpc, mut incoming) = start_with_file_and_config(
        ...
        "_G.ran = false\n\
         btv.autocmd.create('CursorMoved', {\n\
         \x20 callback = function()\n\
         \x20   btv.promise.resolve(1):next(function() _G.ran = true end)\n\
         \x20 end })\n",
    )
    .await;
    let msg = message(&redraw_after(&rpc, &mut incoming, "j").await);
...
async fn non_hot_path_handler_may_return_a_promise() {
    ...
    let (rpc, mut incoming) = start_with_file_and_config(
        ...
        "_G.ran = false\n\
         ...
         \x20 callback = function()\n\
         \x20   return btv.promise.resolve(1):next(function() _G.ran = true end)\n\
         \x20 end })\n",
    )
    .await;
...
async fn autocmd_records_the_file_and_line_it_was_registered_at() {
    ...
    //
    // Both entry points must report the CALLER, not the prelude: `btv.on` forwards to
    // `btv.autocmd.create`, so a naive `debug.getinfo(2)` would blame
    // `bemtvi:prelude/btv` for every `btv.on` subscription in existence.
    let dir = temp_dir("au_site");
    let (rpc, _incoming) = start_with_config(
        &dir,
        "btv.autocmd.create('User', { pattern = 'Direct', callback = function() end })\n\
         btv.on('User', { pattern = 'ViaOn' }, function() end)\n",
    )
    .await;
    let v = exec_lua(
        &rpc,
    ...
    assert!(
        !s.contains("bemtvi:prelude/"),
        "the site is the CALLER's config, not the forwarding prelude module, got {s:?}"
    );
    // init.lua line 1 registers the direct one, line 2 the btv.on one.
    assert!(
    ...
    assert!(
        s.ends_with(":2"),
        "and btv.on reports the caller's line, got {s:?}"
    );
}
...
async fn the_autocmd_listing_names_each_callbacks_registration_site() {
    ...
    let (rpc, _incoming) = start_with_config(
        &dir,
        "btv.autocmd.create('User', { pattern = 'Listed', callback = function() end })\n",
    )
    .await;
    ...
    let listing = exec_lua(&rpc, "return btv._ex_autocmd(false, 'User')").await;
...
/// Wrap `btv.notify` so the warnings the settle protocol emits are capturable, on top
...
const CAPTURE_WARNS: &str = "_G.warns = {}\n\
     local _real_notify = btv.notify\n\
     btv.notify = function(m, l, o) _G.warns[#_G.warns+1] = tostring(m); return _real_notify(m, l, o) end\n";
...
async fn late_subscriber_registered_during_an_async_handler_still_gets_the_event() {
    ...
    let (rpc, _incoming) = start_with_file_and_config(
        ...
        file.to_str().unwrap(),
        "_G.log = {}\n\
         btv.autocmd.create('FileType', { callback = function()\n\
         \x20 _G.log[#_G.log+1] = 'first'\n\
         \x20 return btv.promise.delay(1):next(function()\n\
         \x20   btv.autocmd.create('FileType', { callback = function(a)\n\
...
async fn replay_does_not_refire_handlers_that_already_ran() {
    ...
    let (rpc, _incoming) = start_with_file_and_config(
        ...
        "_G.n = 0\n_G.late = 0\n\
         ...
         \x20 _G.n = _G.n + 1\n\
         \x20 return btv.promise.delay(1):next(function()\n\
         \x20   btv.autocmd.create('FileType', { callback = function() _G.late = _G.late + 1 end })\n\
         \x20 end)\n\
         end })\n",
    )
    .await;
...
async fn a_fires_two_live_replay_paths_still_deliver_at_most_once() {
    ...
    let (rpc, _incoming) = start_with_config(
        ...
        &(CAPTURE_WARNS.to_string()
            + "_G.seen = 0\n\
         btv.autocmd.create('User', { pattern = 'Dup', timeout = 30, callback = function()\n\
         \x20 return btv.promise.delay(200)\n\
         end })\n\
         btv.promise.delay(10):next(function()\n\
         \x20 btv.autocmd.create('User', { pattern = 'Dup', callback = function()\n\
         \x20   return btv.promise.delay(100)\n\
         \x20 end })\n\
         end)\n\
         btv.promise.delay(60):next(function()\n\
         \x20 btv.autocmd.create('User', { pattern = 'Dup', callback = function()\n\
         \x20   _G.seen = _G.seen + 1\n\
         \x20 end })\n\
         end)\n\
         btv.autocmd.exec('User', { pattern = 'Dup' })\n"),
    )
    .await;
    assert!(
...
async fn a_handler_past_its_budget_warns_and_the_replay_happens_anyway() {
    ...
    let cfg = format!(
        "{CAPTURE_WARNS}\
         _G.late = false\n\
         btv.autocmd.create('FileType', {{ timeout = 30, callback = function()\n\
         \x20 btv.promise.delay(1):next(function()\n\
         \x20   btv.autocmd.create('FileType', {{ callback = function() _G.late = true end }})\n\
         \x20 end)\n\
         \x20 return btv.promise.delay(1500)\n\
         end }})\n"
    );
...
async fn a_handler_that_settles_late_warns_with_its_elapsed_time() {
    ...
    let cfg = format!(
        "{CAPTURE_WARNS}\
         btv.autocmd.create('FileType', {{ timeout = 20, callback = function()\n\
         \x20 return btv.promise.delay(300)\n\
         end }})\n"
    );
    let (rpc, _incoming) = start_with_file_and_config(&dir, file.to_str().unwrap(), &cfg).await;
...
async fn a_hung_handler_stays_visible_in_autocmd_pending() {
    ...
    let (rpc, _incoming) = start_with_file_and_config(
        ...
        "btv.autocmd.create('FileType', { timeout = 20, callback = function()\n\
         \x20 return btv.promise.new(function() end)\n\
         end })\n",
    )
    .await;
    assert!(
        poll_true(&rpc, "return #btv.autocmd.pending() > 0").await,
        "the hung handler is listed"
    );
    let p = exec_lua(
        &rpc,
        "local p = btv.autocmd.pending()[1]\n\
...
async fn replay_gives_up_loudly_when_handlers_keep_registering_handlers() {
    ...
    let cfg = format!(
        "{CAPTURE_WARNS}\
         _G.n = 0\n\
         local function arm()\n\
         \x20 btv.autocmd.create('FileType', {{ callback = function()\n\
         \x20   _G.n = _G.n + 1\n\
         \x20   return btv.promise.delay(1):next(arm)\n\
         \x20 end }})\n\
         end\n\
         arm()\n"
    );
...
async fn filetype_waits_for_an_async_bufreadpost_handler() {
    ...
    let (rpc, _incoming) = start_with_file_and_config(
        ...
        file.to_str().unwrap(),
        "_G.fts = {}\n\
         btv.autocmd.create('BufReadPost', { callback = function()\n\
         \x20 return btv.promise.delay(5):next(function() btv.bo[btv.buf.current()].filetype = 'detected' end)\n\
         end })\n\
         btv.autocmd.create('FileType', { callback = function(a) _G.fts[#_G.fts+1] = a.match end })\n",
    )
    .await;
    assert!(
...
async fn bufenter_is_sequenced_after_the_chains_gates() {
    ...
    let (rpc, _incoming) = start_with_file_and_config(
        ...
        "_G.log = {}\n\
         ...
         \x20 return btv.promise.delay(5):next(note('read:done'))\n\
         end })\n\
         btv.autocmd.create('FileType', { callback = note('filetype') })\n\
         btv.autocmd.create('BufEnter', { callback = note('bufenter') })\n",
    )
    .await;
    assert!(
...
async fn bufwinenter_is_sequenced_after_the_chains_gates_too() {
    ...
    let (rpc, _incoming) = start_with_file_and_config(
        ...
        "_G.log = {}\n\
         ...
         btv.autocmd.create('FileType', { callback = note('filetype') })\n\
         btv.autocmd.create('BufEnter', { callback = note('bufenter') })\n\
         btv.autocmd.create('BufWinEnter', { callback = note('bufwinenter') })\n",
    )
    .await;
    assert!(
...
const PARKED_READ: &str = "_G.log = {}\n\
     ...
     \x20 return btv.promise.new(function(resolve) _G.release = resolve end)\n\
     end })\n\
     btv.on('BufWinEnter', function() _G.log[#_G.log+1] = btv.win.current() end)\n";
...
async fn a_chain_with_no_async_handler_completes_within_one_tick() {
    ...
    let (rpc, mut incoming) = start_with_file_and_config(
        ...
        file.to_str().unwrap(),
        "_G.log = {}\n\
         btv.autocmd.create('BufReadPost', { callback = function() _G.log[#_G.log+1] = 'read' end })\n\
         btv.autocmd.create('FileType', { callback = function() _G.log[#_G.log+1] = 'ft' end })\n\
         btv.autocmd.create('BufEnter', { callback = function() _G.log[#_G.log+1] = 'enter' end })\n",
    )
    .await;
...
async fn a_hung_bufreadpost_handler_does_not_wedge_the_chain() {
    ...
    let (rpc, _incoming) = start_with_file_and_config(
        ...
        file.to_str().unwrap(),
        "_G.log = {}\n\
         btv.autocmd.create('BufReadPost', { timeout = 30, callback = function()\n\
         \x20 return btv.promise.new(function() end)\n\
         end })\n\
         btv.autocmd.create('FileType', { callback = function() _G.log[#_G.log+1] = 'ft' end })\n\
         btv.autocmd.create('BufEnter', { callback = function() _G.log[#_G.log+1] = 'enter' end })\n",
...
async fn deleting_a_buffer_mid_chain_does_not_panic_or_orphan_the_gate() {
    ...
    let (rpc, _incoming) = start_with_file_and_config(
        ...
        "_G.done = false\n\
         _G.after = {}\n\
         btv.autocmd.create('BufReadPost', { callback = function(a)\n\
         \x20 _G.wiped = a.buf\n\
         \x20 return btv.promise.delay(5):next(function()\n\
         \x20   btv.cmd('enew')\n\
         \x20   pcall(btv.cmd, 'bwipeout! ' .. a.buf)\n\
         \x20   _G.done = true\n\
         \x20 end)\n\
         end })\n\
         for _, ev in ipairs({ 'FileType', 'BufEnter', 'BufWinEnter' }) do\n\
         \x20 btv.autocmd.create(ev, { callback = function(x)\n\
...
async fn a_rejecting_async_read_handler_surfaces_its_rejection() {
    ...
    // printed nothing at all: `btv._fire_gated` collected the promise without the
    // `track_au_promise` `:catch` that `btv._fire` attaches, and `all_settled`'s own
    ...
    let (rpc, _incoming) = start_with_file_and_config(
        ...
        "_G.notified = {}\n\
         ...
         _G.log = {}\n\
         btv.autocmd.create('BufReadPost', { callback = function()\n\
         \x20 return btv.promise.delay(5):next(function() error('read blew up') end)\n\
         end })\n\
         btv.autocmd.create('FileType', { callback = function() _G.log[#_G.log+1] = 'ft' end })\n",
    )
    .await;
...
async fn re_reading_a_file_mid_chain_abandons_the_previous_chain() {
    ...
    let (rpc, mut incoming) = start_with_file_and_config(
        ...
        "_G.log = {}\n\
         _G.reads = 0\n\
         btv.autocmd.create('BufReadPost', { timeout = 30000, callback = function()\n\
         \x20 _G.reads = _G.reads + 1\n\
         \x20 local n = _G.reads\n\
         \x20 _G.log[#_G.log+1] = 'read' .. n .. ':start'\n\
         \x20 return btv.promise.delay(n == 1 and 1500 or 3000):next(function()\n\
         \x20   _G.log[#_G.log+1] = 'read' .. n .. ':done'\n\
         \x20 end)\n\
         end })\n\
         btv.autocmd.create('FileType', { callback = function() _G.log[#_G.log+1] = 'ft' end })\n\
         btv.autocmd.create('BufEnter', { callback = function() _G.log[#_G.log+1] = 'enter' end })\n\
         btv.autocmd.create('BufWinEnter', { callback = function() _G.log[#_G.log+1] = 'winenter' end })\n",
    )
    .await;
...
#[tokio::test]
async fn btv_on_takes_a_bare_handler_with_no_options_table() {
    // `btv.on(event, fn)` — the two-argument spelling a config reaches for when there
    ...
    // `attempt to index a function value` from inside `bemtvi:prelude/autocmd`, and the
    ...
    let (rpc, _incoming) = start_with_config(
        &dir,
        "btv.on('User', function(ev) _G.saw = tostring(ev.match) end)\n\
         _G.reached_end = true\n",
    )
    .await;
    assert_eq!(
    ...
    exec_lua(&rpc, "btv.autocmd.exec('User', { pattern = 'Bare' })").await;
    assert_eq!(
    ...
    let site = exec_lua(
        &rpc,
        "return tostring(btv.autocmd.get({ event = 'User' })[1].site)",
    )
    .await;
    let site = site.as_str().unwrap_or("<nil>");
    assert!(
        site.ends_with(":1") && !site.contains("bemtvi:prelude/"),
...
async fn autocmd_create_names_a_non_table_opts_instead_of_dying_on_index() {
    ...
    let err = exec_lua(
        &rpc,
        "local ok, err = pcall(btv.autocmd.create, 'User', function() end)\n\
    ...
    assert!(
        err.contains("opts must be a table, got function") && err.contains("btv.on(event, fn)"),
...
async fn ui_enter_fires_at_attach_with_the_client_capabilities_readable() {
    ...
    // seam: it fires on attach, after `btv.ui.caps()` is refreshed from what the
    ...
    let (rpc, _incoming) = bemtvi_test_harness::spawn(bemtvi_test_harness::config_init(
        &dir,
        "_G.caps_at_config = btv.ui.caps().keyboard_protocol\n\
         btv.on('UIEnter', {}, function()\n\
           _G.caps_at_uienter = btv.ui.caps().keyboard_protocol\n\
         end)\n",
    ));
    ...
    assert_eq!(
        exec_lua(&rpc, "return tostring(btv.ui.caps().keyboard_protocol)")
            .await
            .as_str(),
        Some("true"),
...
async fn ui_enter_reports_truecolor_and_osc52_from_the_attach_map() {
    ...
    let (rpc, _incoming) = bemtvi_test_harness::spawn(bemtvi_test_harness::config_init(&dir, ""));
