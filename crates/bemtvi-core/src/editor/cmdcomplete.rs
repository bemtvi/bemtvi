...
//! (`btv.cmdline_complete`). Pressing `<Tab>` while an ex command line (`:`) is open
...
//! owned by the bundled `btv.cmdline_complete` Lua plugin (the curated command catalog
//! merged with `btv.user_command.get()`), so core stays out of "what commands exist":
...
/// `btv.cmdline_complete.setup{}` configuration. Off until a config arrives, so an
...
impl Editor {
    /// Apply an `btv.cmdline_complete.setup{}` config (enable the engine).
    ...
    pub(crate) fn cmdline_complete_trigger(&mut self) {
        // A scripted prompt (`btv.ui.input{ complete = fn }`) routes to its own
    ...
    /// Stamp a [`Editor::prompt_complete_request`] for the open `btv.ui.input` prompt
    ...
    /// The token being completed in an `btv.ui.input` prompt: the trailing run of
