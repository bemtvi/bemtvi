...
//! threshold is read once from `$BEMTVI_LSP_LOG_LEVEL` (`off`/`error`/`warn`/
//! `info`/`debug`/`trace`, default `warn`); a message is written when its
//! severity is at or above the threshold. `$BEMTVI_LSP_LOG_FILE` overrides the
...
impl LspLog {
    ...
    pub(crate) fn from_env() -> LspLog {
        let threshold = std::env::var("BEMTVI_LSP_LOG_LEVEL")
            .ok()
