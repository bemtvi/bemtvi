...
use nxvim_server::{ServerInit, SystemPluginSpec};
...
async fn a_system_plugin_loads_before_init_lua_and_is_require_able() {
    ...
    assert_eq!(
        exec_lua(&rpc, "return nx.plugins._system['myconnector'] ~= nil")
            .await
            .as_bool(),
        Some(true),
    ...
    assert_eq!(
        exec_lua(&rpc, "return nx.plugins._specs['myconnector'] == nil")
            .await
            .as_bool(),
        Some(true),
...
async fn a_system_plugin_is_absent_with_the_default_empty_tier() {
    ...
    assert_eq!(
        exec_lua(&rpc, "return #nx.plugins.list_system()")
            .await
            .as_u64(),
        Some(0),
        "the tier is empty",
