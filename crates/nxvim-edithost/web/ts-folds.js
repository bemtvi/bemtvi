// In-worker tree-sitter FOLDS for the nxvim edit-host (the browser twin of
// nxvim-ts's `Engine::folds`).
//
...
// native does (crates/nxvim-core/src/editor/fold.rs). Fold queries come from
...
// OPFS path where `:TSInstall` caches grammars: /.nxvim/treesitter/<lang>/{parser.wasm,folds.scm}.
...
// trim it — matching nxvim-ts's `Engine::folds` (and neovim's foldexpr).
...
export function createFolder() {
  ...
  (async () => {
    ...
    } catch (e) { console.error('[nxvim] folder: vendor manifest load failed:', e); }
    try {
    ...
    ready = true;
    readyResolve();
  })().catch((e) => console.error('[nxvim] folder: init failed:', e && e.stack ? e.stack : e));
  ...
  async function ensure(reg) {
    ...
    try {
      ...
      console.error('[nxvim] folder: failed to load', reg, e);
      langs.set(reg, null);
    } finally {
      loading.delete(reg);
    }
  ...
  return {
    ...
    folds(coreLang, text) {
      ...
      try {
        ...
        console.error('[nxvim] folder: folds failed for', reg, e);
        return null;
      } finally {
        tree.delete();
      }
