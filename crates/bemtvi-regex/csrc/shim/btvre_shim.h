...
// btvre_*/vim_* calls must be externally serialized. The Rust wrapper holds a
// global mutex.
#pragma once
...
typedef bool (*btvre_mark_lookup_fn)(void *userdata, int name, linenr_T *lnum, colnr_T *col);

/// Allocates a buffer handle for matching. `get_line`/`userdata` provide
/// lines 1..line_count (see btvre_get_line_fn); the buffer starts with vim's
...
/// Returns false (with the error retrievable via btvre_take_last_error) on a
