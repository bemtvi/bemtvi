...
/// An image-preview window's payload (mirrors `bemtvi_core::view::ImageView`): the
...
/// must fetch them out-of-band over the editor RPC (`bemtvi_image_read`) — `path` is a
...
pub struct ImageData {
    ...
    /// bytes over the editor RPC (`bemtvi_image_read [path]`) instead of opening `path`
...
/// `bemtvi_core::view::WindowRegion`). Its `rect`/coords are relative to the
...
/// A window's rect in windows-area cells (mirrors `bemtvi_core::ViewRect`).
#[derive(Default, Clone, Copy)]
pub struct WinRect {
    pub x: u16,
    pub y: u16,
...
pub struct View {
    ...
    /// The floating selectable-list menu (`btv.ui.select`; later the picker), or
...
pub struct MenuData {
    ...
    pub width: u16,
    pub height: u16,
    /// The picker prompt query — `Some` (even when empty) for a `btv.picker`,
    /// `None` for a promptless `btv.ui.select`. Presence tells the client to draw a
    ...
    /// `0` for a promptless `btv.ui.select`.
    ...
    /// than above it (the default). `false` for a promptless `btv.ui.select`.
    ...
    /// `btv.picker` overlay (an `Editor` / `Bottom` placement): the client anchors the
    ...
    /// cursor-anchored completion popup / `btv.ui.select`. Mirrors
...
/// The list-less content float mirrored from the redraw (`btv.ui.float`; LSP hover
