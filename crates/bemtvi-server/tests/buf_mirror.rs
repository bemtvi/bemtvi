...
//!
//!  * **Correctness** — the mirror (`btv.buf.lines`, read out of `btv._bufs`) matches
...
/// The mirror's own view of the current buffer — what `btv.buf.lines` reads out of
/// `btv._bufs`, which is the array a delta splices into.
async fn mirror(rpc: &Rpc) -> Vec<String> {
    match exec_lua(rpc, "return btv.buf.lines(0, 0, -1, false)").await {
        Value::Array(a) => a
            .iter()
...
async fn pin(rpc: &Rpc) {
    exec_lua(rpc, "_G.__pinned = btv._bufs[btv._resolve_bufnr(0)].lines").await;
}
...
async fn spliced(rpc: &Rpc) -> bool {
    let v = exec_lua(
        rpc,
        "return btv._bufs[btv._resolve_bufnr(0)].lines == _G.__pinned",
    )
    .await;
...
async fn a_queued_set_lines_lands_in_the_mirror() {
    // `btv.buf.set_lines` does NOT write through to the mirror — it queues a buffer op
    ...
    push_every_key(&rpc).await;
    pin(&rpc).await;
    exec_lua(&rpc, "btv.buf.set_lines(0, 5, 8, false, { 'a', 'b' })").await;
    assert_mirrors(&rpc, true).await;
    let core = lines(&rpc).await;
