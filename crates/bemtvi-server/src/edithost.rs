...
use bemtvi_core::{BufferId, PendingSave};
use rmpv::Value;
...
use bemtvi_lsp::{ApplyEditOutcome, LspNotify, LspRequest, ReqToken, ServerKey, ServerSpawn};
#[cfg(feature = "native")]
use bemtvi_rpc::Rpc;
#[cfg(feature = "native")]
use std::io;
...
pub trait HostEffects {
    /// Push a notification to the attached client (the `redraw` frame, `bemtvi_exit`,
    ...
    /// [`FileStat`](bemtvi_core::FileStat)) or a loud error returns *inbound* on the save
    ...
    /// `stream` requests incremental stdout (`btv.run_stream`'s streamed stdout): the daemon emits
    ...
    /// Off-tick `btv.fs` op (`btv._fs_op`) over the daemon `luafs_op` leg — the wasm twin of the
    ...
    /// Native-only builds run `btv.fs` ops on the event-loop actor via `loop_command`, so this
    ...
    /// Off-tick `btv.git.*` op (`btv._git_op`) over the daemon `git_op` leg — the wasm twin of
    /// the native `loop_command(LoopCommand::Git)`. Fire-and-forget: the op runs
    /// `bemtvi_git::run_git_job` on the daemon and its typed result returns *inbound* via
    /// [`EditHost::git_op_result`](crate::EditHost::git_op_result), not here. Unlike `btv.fs`
    ...
    /// Off-tick `btv.http.fetch` request (`btv._http_fetch`) — the wasm twin of the native
    ...
    /// not here. Unlike `btv.fs` / processes there is no host gate: the browser always has
    /// `fetch()`, so a serverless session runs HTTP directly (no daemon required). Native-only
    /// builds run `btv.http` on the event-loop actor via `loop_command`, so this is wasm-only.
    /// `local` (`btv.http.fetch_local`) forces the browser's own `fetch()` even when a daemon
    ...
    /// `btv.http.mount` — publish a plugin's subroute, the wasm twin of the native
    ...
    /// Arm a streaming `btv.fs.watch` over the daemon `luafs_watch` leg (Phase 3b) — the wasm twin
    ...
    /// Disarm the streaming `btv.fs.watch` armed under `id` (`:stop()`) — the wasm twin of
    ...
    /// Open a daemon-side **duplex** child (`btv.process.open` — the DAP / framed-protocol
    ...
    /// Open a daemon-side TCP connection (`btv.socket.connect` — a DAP `type="server"` adapter
    ...
    /// [`LspEvent::ApplyEdit`](bemtvi_lsp::LspEvent). Unlike every other inbound LSP
    ...
    /// finished [`InstallReport`](bemtvi_ts::install::InstallReport) (or a loud error)
    ...
    /// first needs the language ([`Editor::take_ts_grammar_requests`](bemtvi_core::Editor::take_ts_grammar_requests)).
...
impl HostEffects for NativeEffects {
            ...
            None => {
                tokio::spawn(async move {
                    let reply = tokio::fs::read(&path)
                        .await
                        .map(Value::Binary)
                        .map_err(|e| Value::from(format!("bemtvi_image_read: {path}: {e}")));
                    rpc.respond(id, reply);
                });
            }
        }
    ...
    fn ts_install(&mut self, lang: String) {
        let dir = bemtvi_ts::data_dir();
        let tx = self.install_tx.clone();
        tokio::task::spawn_blocking(move || {
            let result = bemtvi_ts::install::install(&dir, &lang);
            // The receiver only drops at shutdown; a send error means we're exiting, so
            // there's nothing to report to.
...
/// Project a daemon [`FsRead`] (the off-tick read result for an `bemtvi_image_read`)
...
fn image_bytes(path: &str, result: io::Result<FsRead>) -> Result<Value, Value> {
    match result {
        ...
        Ok(FsRead::New) => Err(Value::from(format!(
            "bemtvi_image_read: {path}: no such file"
        ))),
        Ok(FsRead::Dir { .. }) => Err(Value::from(format!(
            "bemtvi_image_read: {path}: is a directory"
        ))),
        Err(e) => Err(Value::from(format!("bemtvi_image_read: {path}: {e}"))),
    }
}
