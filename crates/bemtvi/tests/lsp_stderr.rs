...
//!
//! Wired like `lsp_config.rs`: the scripted mock (`bemtvi --__lsp-mock`) stands in
//! for the server via `$BEMTVI_LSP_CMD` (process-global env ⇒ `serial_lock`). The
...
/// Retry `btv.lsp.hover()` until the hover float carries `want`, or panic after the
...
async fn server_with_non_utf8_stderr_flood_still_initializes_and_answers() {
    ...
    std::env::set_var("BEMTVI_LSP_LOG_FILE", dir.join("lsp.log"));
