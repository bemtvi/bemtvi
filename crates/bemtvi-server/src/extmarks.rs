...
//!
//! The client ([`bemtvi_tui`]'s `cell_style`) takes the **first** span covering a
...
//! [`bemtvi_core::TS_HL_PRIORITY`] (100); extmarks default to
//! [`bemtvi_core::DEFAULT_PRIORITY`] (4096), so a plugin / semantic-token mark
...
//! [`ExtmarkStore`](bemtvi_core::ExtmarkStore) every frame: the set is small and
//! always reflects the current marks, so there is no cache to stale.

use crate::redraw::StyleTable;
use crate::EditHost;
use bemtvi_core::{unicode, BufferId, HlMode, VirtChunk, VirtTextPos, WinHl};
use rmpv::Value;
...
/// ([`virt_text_fg_only`](bemtvi_core::VirtDecor::virt_text_fg_only)).
struct Placement {
    pos: u64,
    col: u64,
    hl_mode: u64,
...
pub(crate) struct HlMarkIndex<'a> {
    ...
    marks: Vec<(u32, usize, usize, &'a bemtvi_core::Extmark)>,
...
impl<'a> HlMarkIndex<'a> {
    pub(crate) fn build(buf: &'a bemtvi_core::Buffer) -> Self {
        let mut marks: Vec<(u32, usize, usize, &'a bemtvi_core::Extmark)> = buf
            .extmarks
            .iter_all()
            .enumerate()
            .filter_map(|(i, m)| {
    ...
    fn overlapping(&self, lo: usize, hi: usize) -> Vec<(u32, &'a bemtvi_core::Extmark)> {
...
impl EditHost {
    ...
    pub(crate) fn extmark_intervals<'a>(
        ...
        let clip = move |m: &'a bemtvi_core::Extmark, order: u32| -> Option<HlInterval<'a>> {
    ...
    /// genuinely server-sourced overlays — extmark highlights (the `btv.decor` /
    /// `btv.buf.set_extmark` layer) and LSP semantic tokens — which the JS side can't
...
impl EditHost {
    ...
    ///
    /// Read live from the buffer's [`ExtmarkStore`](bemtvi_core::ExtmarkStore), like
    /// the hl spans.
    pub(crate) fn virt_text_for(
        ...
        let mut by_line: HashMap<usize, Vec<&bemtvi_core::Extmark>> = HashMap::new();
        ...
        let rows = segs
            ...
            .map(|(row, seg)| {
                ...
                let anchor_col = |m: &bemtvi_core::Extmark| -> u64 {
    ...
    /// anchor on one line the highest [`priority`](bemtvi_core::Extmark::priority) wins.
    pub(crate) fn line_bg_for(
        ...
        if let Some(st) = self.syntax_states.get(&buffer) {
            for &line in &st.block_bg_lines {
                by_line
                    .entry(line)
                    .or_insert(("@markup.raw.block", bemtvi_core::TS_HL_PRIORITY));
            }
        }
        if by_line.is_empty() {
            return empty();
    ...
    /// [`virt_text_fg_only`](bemtvi_core::VirtDecor::virt_text_fg_only): when set, each
    ...
    /// `sign_hl_group` resolves to a frame style id via the shared `btv.decor`
    ...
    /// build (LSP or `btv.diagnostic.set`) gets the gutter signs too.
    ...
    pub(crate) fn apply_line_fill(
        ...
        use std::collections::HashMap;
        let mut by_line: HashMap<usize, &bemtvi_core::Extmark> = HashMap::new();
        for m in buf.extmarks.iter_all() {
        ...
        for (i, seg) in segs.iter().enumerate() {
            ...
            let from = bemtvi_core::unicode::virtcol(&line, line.len(), tabstop);
            let chunk = VirtChunk {
...
pub(crate) fn sign_width_from_cells(
    cells: &[Option<Value>],
    signcolumn: bemtvi_core::SignColumn,
) -> u16 {
    use bemtvi_core::SignColumn;
