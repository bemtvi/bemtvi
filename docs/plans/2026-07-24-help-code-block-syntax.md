...
- the picker **preview pane** (server tree-sitter, `nxvim-ts`), and
- the **help window** itself (the `nxvim-help` plugin's `nx.view`).
...
plugin's `nxHelpCode` (→ `String`) extmark. Neither runs the injected language.
...
The window is an `nx.view` highlighted by the plugin's own extmarks (core has no
...
1. **New `nx.treesitter.highlight(lang, text)` Lua API** → returns per-line
   ...
   a request/response like the other `nx.treesitter.*` surfaces — carry the
...
3. **`highlight.lua`**: for each `>{lang}` block, extract the code rows' text,
   call `nx.treesitter.highlight(lang, text)`, and place the returned spans as
   ...
   columns). A bare `>` block keeps the flat `nxHelpCode` paint.
