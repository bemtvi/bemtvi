...
**The Lua backend is vendored PUC Lua 5.1 — the only backend.** LuaJIT was dropped; nxvim is PUC-Lua-only, matching the wasm build (which never supported LuaJIT). There is no backend Cargo feature to select — `lua51` is baked into the shared `mlua` dependency in the root `Cargo.toml`. Lint and test on the **default features** as shown above. (`nxvim-server`'s `native` feature is default-on and only turned *off* for the wasm edit-host via `--no-default-features`; it gates the async/transport tree, not the Lua VM.)
...
- `crates/nxvim-test-harness/src/lib.rs` — the shared black-box test harness (a `publish = false` dev-dependency of `nxvim` and `nxvim-server`): server spawn/attach, the RPC-driving conveniences, the redraw accessors, and temp-fs / lock / clipboard helpers every integration suite shares. It links the same single Lua backend (PUC lua51, via the shared `mlua` dep) as the crate under test, so one backend resolves through the dev-dependency cycle.
