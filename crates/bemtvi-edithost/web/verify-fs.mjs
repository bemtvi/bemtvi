...
// drive the full round trips through `window.__bemtvi` and assert the bytes truly move
...
try {
  ...
  // must pop the save picker (mirrors bemtvi-gui), here returning "from-unnamed.txt".
  await page.evaluate(() => window.__bemtvi.feed("ihello unnamed<Esc>"));
  ...
  // The buffer is now bound to that file (bemtvi's `:w <path>` renames it), so [+] clears.
  ...
  await page.evaluate(() => window.__bemtvi.feed("oSECOND-LINE<Esc>"));
  ...
  await page.evaluate(() => window.__bemtvi.feed(":eo"));
  await page.evaluate(() => window.__bemtvi.pressEnter());
  const opened = await until(page, () => window.__bemtvi.lines(), (v) => v === "alpha\nbeta\ngamma");
  ...
  await page.evaluate(() => window.__bemtvi.feed("GoDELTA<Esc>"));
  await page.evaluate(() => window.__bemtvi.feed(":w"));
  await page.evaluate(() => window.__bemtvi.pressEnter());
  ...
  await page.evaluate(() => window.__bemtvi.feed(":wo"));
  await page.evaluate(() => window.__bemtvi.pressEnter());
  ...
  await page.evaluate(() => window.__bemtvi.feed(":e ghost-no-handle.txt"));
  await page.evaluate(() => window.__bemtvi.pressEnter()); // not a dialog verb → feeds <CR>, runs it
  const ghost = await until(page, () => window.__bemtvi.lines(), (v) => v === "");
  check(
