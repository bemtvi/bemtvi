-- btv.plugins.ui — the lazy.nvim-style UI for the native package manager.
--
-- Two floating surfaces, both built on `btv.view.component` (the Vue-shaped reactive
-- component, prelude/component.lua) over the manager's own state — no buffer
-- mutation, no manual tick-dance:
--
--   * the WELCOME offer (`M.ui.welcome`) — the first-run ask. bemtvi ships minimal; on
...
-- btv.view.component / btv.hl / btv.command / btv.timer — all installed above.
...
local HL = {
  BtvPluginsLoaded = { fg = "#a6e3a1" }, -- ● a loaded plugin
  BtvPluginsInstalled = { fg = "#89b4fa" }, -- ○ installed, not yet loaded
  BtvPluginsMissing = { fg = "#f38ba8" }, -- ○ declared but not on disk / a failed op
  BtvPluginsBusy = { fg = "#f9e2af" }, -- the spinner frame while git runs
  BtvPluginsHeader = { fg = "#b4befe", bold = true }, -- a section / title line
  BtvPluginsDim = { fg = "#6c7086" }, -- counts, flags, details, the key hint
}
...
-- (`btv.plugins._source_name`), so the checklist labels match exactly what
-- normalize() would install.
local function spec_label(s)
  if type(s) == "string" then
    return btv.plugins._source_name(s) or s
  end
  if s.name then
    return s.name
  end
  local src = s.src or s.url or s[1] or s.dir
  return src and (btv.plugins._source_name(src) or src) or "?"
end
...
-- `bemtvi/bemtvi-tree` -> `github.com/bemtvi`, `https://git.sr.ht/~u/p` -> `git.sr.ht/~u`,
...
local function open_doc()
  local url = btv.plugins.RECOMMENDED_DOC_URL
  btv.ui
    .open(url)
    :next(function()
      btv.notify("opened " .. url)
    end)
    :catch(function(err)
      btv.notify(
        "btv.plugins: could not open " .. url .. " (" .. tostring(err and err.message or err) .. ")",
        3
      )
    end)
end

local Customize = btv.view.component({
  setup = function(ctx, props)
    ...
    -- rather than via `btv.schedule` is what makes it stick: an earlier same-tick
  ...
  render = function(view)
    ...
    add("Untick anything you don't want — each row is fetched from the", "BtvPluginsDim")
    add("source shown, then declared in your config.", "BtvPluginsDim")
    add("")
    ...
    for _, it in ipairs(view.items) do
      ...
      decor[#decor + 1] = {
        ...
        end_row = line,
        end_col = #box,
        hl_group = it.checked and "BtvPluginsLoaded" or "BtvPluginsDim",
      }
      if desc_col then
        decor[#decor + 1] = {
          line = line,
...
local function mount_customize(recommended, on_done)
  ...
  local cols, rows_avail = btv.o.columns, btv.o.lines
  ...
  for _, raw in ipairs(recommended) do
    ...
    rows = rows + math.max(1, math.ceil(btv.str.displaywidth(text) / inner))
  end
  Customize.mount({
    name = "btv-plugins-customize",
    filetype = "btvpluginscustomize",
    float = {
      width = width,
...
local Offer = btv.view.component({
  setup = function(ctx, props)
  ...
  render = function(view)
    ...
    add("bemtvi ships minimal by design — no bundled plugins.", "BtvPluginsDim")
    add("")
    add("Install the recommended set?", "BtvPluginsHeader")
    ...
    add("")
    add(btv.plugins.RECOMMENDED_DOC_URL, "BtvPluginsDim")
    return { lines = lines, decor = decor }
  end,
})
...
function M.ui.welcome(recommended)
  return btv.promise.new(function(resolve)
    Offer.mount({
      name = "btv-plugins-welcome",
      filetype = "btvpluginswelcome",
      float = {
        width = 74,
...
local Manager = btv.view.component({
  setup = function(ctx, props)
    ...
    local function refresh_status()
      btv
        ...
        :catch(function(e)
          btv.notify("btv.plugins.ui: " .. tostring(e and e.message or e), 4)
        end)
    end
    refresh_status()
    ...
    local function run(p)
      p:catch(function(e)
        btv.notify(tostring(e and e.message or e), 4)
      end)
    end
    ...
    local function restart_popup(n)
      ...
      btv.ui.float({
        ...
        "",
        "Restart bemtvi to finish loading them.",
    ...
    local function under_cursor(verb)
      ...
      if not name then
        btv.notify("btv.plugins: no plugin under the cursor to " .. verb, 3)
      end
      return name
    end
  ...
  render = function(view)
    ...
    if #plugins == 0 then
      add("No plugins declared yet.", "BtvPluginsDim")
      add(
        "Add some with btv.plugins{ … } in your config, then press I to install.",
        "BtvPluginsDim"
      )
    end
    ...
    local function section(title, list, icon, icon_hl)
      ...
      for _, p in ipairs(list) do
        ...
        if suffix ~= "" then
          decor[#decor + 1] =
            { line = line, col = #text, virt_text = { { " — " .. suffix, "BtvPluginsDim" } } }
        end
    ...
    section("Loaded", loaded, "●", "BtvPluginsLoaded")
    section("Not loaded", ready, "○", "BtvPluginsInstalled")
    section("Missing", missing, "○", "BtvPluginsMissing")
...
btv.command("Plugins", function()
  M.ui.open()
end, { desc = "Open the btv.plugins manager UI (lazy-style dashboard)." })
...
btv.command("PluginsWelcome", function()
  if #M._recommended == 0 then
    return btv.notify(
      "btv.plugins: no recommended set registered — call btv.plugins.recommend{...} first",
      3
    )
  end
  M.ui
    ...
    :catch(function(err)
      btv.notify("btv.plugins: " .. tostring(err and err.message or err), 4)
    end)
