...
/// `'foldexpr'` is the canonical tree-sitter one bemtvi computes natively. This is
...
/// Whether `'foldexpr'` is the canonical tree-sitter foldexpr bemtvi computes
/// natively — neovim's `v:lua.vim.treesitter.foldexpr()` and the `btv.*` spellings.
...
/// Whether `'foldexpr'` is the canonical LSP foldexpr marker — `btv.lsp.foldexpr`
