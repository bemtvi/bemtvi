...
//!
//! Phase 5 exposes every Helix verb *by name* through `btv._helix_action` ->
...
//! `btv._helix_action`) and the plugin's default `helix`-bucket maps (via fed keys),
...
async fn registry_enables_and_drives_verbs() {
    ...
    exec_lua(
        &rpc,
        "btv._helix_action('enable_helix'); btv._helix_action('select_all')",
    )
    .await;
...
async fn unknown_action_fails_loud() {
    ...
    exec_lua(&rpc, "btv._helix_action('does_not_exist')").await;
...
/// `btv.helix.enable()` / `disable()` are the public opt-in surface, idempotent both
/// ways — the plugin's wrapper over the registry.
#[tokio::test]
async fn public_enable_disable_toggle() {
    let (rpc, _i) = start(None).await;
    feed(&rpc, "iabc<Esc>0");
    exec_lua(&rpc, "btv.helix.enable(); btv.helix.enable()").await;
    assert_eq!(
        mode(&rpc).await,
        "hn",
        "enable is idempotent → Helix normal"
    );
    exec_lua(&rpc, "btv.helix.disable()").await;
...
async fn bracket_d_navigates_diagnostics_in_helix() {
    ...
    exec_lua(
        ...
        btv.diagnostic.set(btv.ns.create("hxtest"), 0, {
