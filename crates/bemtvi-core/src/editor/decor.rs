//! The viewport-changed signal for `btv.decor` — viewport-scoped decoration
...
//!
//! A `btv.decor` provider is woken **once per visible-range change** (scroll,
...
//! `btv.complete`/`btv.picker` off-tick generation machinery.
//!
//! This module is pure and synchronous (it lives in `bemtvi-core`); the Lua
//! provider dispatch and the publish→extmark lowering live in `bemtvi-server`.
...
/// window showing one buffer, or a single window. The `btv.decor.invalidate` scope.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum DecorScope {
    /// Every visible window (an unscoped `btv.decor.invalidate()`).
    All,
...
impl Editor {
    ...
    pub fn recompute_decor_dirty(&mut self) {
        ...
        for win in self.window_ids() {
            ...
            // An outstanding `btv.decor.invalidate` re-dispatches this window even
    ...
    /// plugin raises through `btv.decor.invalidate` when nothing the viewport detector
    /// watches has moved.
    ///
    ...
    /// the server calls it when a `btv.decor` publish first targets a namespace.
