//! Per-workspace **option overrides** (`btv.wso`): a sparse overlay above the process-
...
//! an override from Lua, then read the effective value back through `btv.o` (the GoMirror,
//! which carries the *effective* options) and the override through `btv.wso`.
...
/// `btv.o.<name>` — the EFFECTIVE value (global base with the workspace overlay applied),
...
async fn effective_bool(rpc: &Rpc, name: &str) -> Option<bool> {
    exec_lua(rpc, &format!("return btv.o.{name}"))
        .await
        .as_bool()
}
...
async fn workspace_override_wins_over_the_global_value() {
    ...
    // Global base off; the workspace overlay forces it on. `btv.o` (effective) reads the
    // override, and `btv.wso` reads the override itself.
    exec_lua(&rpc, "btv.o.ignorecase = false").await;
    exec_lua(&rpc, "btv.wso.ignorecase = true").await;
    assert_eq!(effective_bool(&rpc, "ignorecase").await, Some(true));
    assert_eq!(
        exec_lua(&rpc, "return btv.wso.ignorecase").await.as_bool(),
        Some(true),
        "btv.wso reads the override value"
    );
    ...
    // Clearing the override (`btv.wso.x = nil`) reverts to the global base.
    exec_lua(&rpc, "btv.wso.ignorecase = nil").await;
    assert_eq!(
        effective_bool(&rpc, "ignorecase").await,
        Some(false),
    ...
    assert!(
        exec_lua(&rpc, "return btv.wso.ignorecase").await.is_nil(),
...
async fn override_covers_numeric_and_string_globals() {
    ...
    exec_lua(&rpc, "btv.o.laststatus = 2").await;
    exec_lua(&rpc, "btv.wso.laststatus = 3").await;
    exec_lua(&rpc, "btv.o.switchbuf = 'usetab'").await;
    exec_lua(&rpc, "btv.wso.switchbuf = 'useopen'").await;
    assert_eq!(
        exec_lua(&rpc, "return btv.o.laststatus").await.as_i64(),
        Some(3)
    );
    assert_eq!(
        exec_lua(&rpc, "return btv.o.switchbuf").await.as_str(),
        Some("useopen")
    );
    assert_eq!(
        exec_lua(&rpc, "return btv.wso.switchbuf").await.as_str(),
        Some("useopen")
    );
}
...
async fn a_non_global_option_is_rejected() {
    ...
    // `btv.wso` write raises (the prelude catches it before it reaches the core).
    let (rpc, _incoming) = start().await;
    let err = exec_lua(
        &rpc,
        "local ok, e = pcall(function() btv.wso.number = true end); return e",
    )
    .await;
    assert!(
        err.as_str()
...
async fn overrides_persist_across_a_workspace_session() {
    ...
    // the effective options reflect the overrides (ignorecase defaults OFF, laststatus 2).
    {
        ...
        // And `btv.wso` reports the restored overrides (from the shada, not set this session).
        assert_eq!(
            exec_lua(&rpc, "return btv.wso.ignorecase").await.as_bool(),
            Some(true)
        );
