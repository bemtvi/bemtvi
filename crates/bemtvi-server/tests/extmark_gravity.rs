//! Black-box tests for per-mark extmark **gravity** (`btv.buf.set_extmark`'s
...
async fn span(rpc: &Rpc, id: u64) -> String {
    let lua = format!(
        r#"local ms = btv.buf.extmarks(0, _G.__ns, 0, -1, {{ details = true }})
...
async fn place_marks(rpc: &Rpc) {
    exec_lua(
        rpc,
        r#"_G.__ns = btv.ns.create("gravtest")
           -- id 1: GROWS — start left-gravity, end right-gravity.
           btv.buf.set_extmark(0, _G.__ns, 0, 1, {
