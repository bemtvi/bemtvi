//! The `btv.glob` bridge: Lua-facing wrappers over the canonical glob engine in
//! [`bemtvi_core::glob`]. The parsing, the glob→regex translation and the compiled-regex
...
//! loud Lua error.
//!
//! The documented surface (`btv.glob.match`, `btv.glob.compile`, …) is the thin Lua
//! wrapper in `prelude/glob.lua`; the `btv._glob*` bridges here are what it calls.

use bemtvi_core::glob::{GlobOpts, PathStyle};
...
pub(crate) fn opts_from_table(opts: Option<&Table>) -> mlua::Result<GlobOpts> {
    ...
    if let Some(name) = t.get::<Option<String>>("style")? {
        out.style = PathStyle::from_name(&name).ok_or_else(|| {
            mlua::Error::runtime(format!(
                "btv.glob: unknown path style '{name}' (expected \"unix\" or \"windows\")"
            ))
        })?;
    }
...
/// A compiled single glob, handed to Lua as `btv.glob.compile(...)`. Holds the core's
/// cached [`bemtvi_core::glob::Glob`], so several Lua objects over the same
/// (pattern, opts) share one compiled regex.
pub struct LuaGlob {
    inner: std::rc::Rc<bemtvi_core::glob::Glob>,
}

impl LuaGlob {
    pub fn compile(pattern: &str, opts: Option<&Table>) -> mlua::Result<Self> {
        let opts = opts_from_table(opts)?;
        let inner = bemtvi_core::glob::compile(pattern, &opts)
...
impl UserData for LuaGlob {
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        ...
        methods.add_method("test", |_, this, path: Value| {
            let path = candidate("btv.glob glob:test", "path", &path)?;
...
/// A compiled glob **set**, handed to Lua as `btv.glob.set(...)`. One `RegexSet` pass
...
pub struct LuaGlobSet {
    inner: std::rc::Rc<bemtvi_core::glob::GlobSet>,
}

impl LuaGlobSet {
    pub fn compile(patterns: Vec<String>, opts: Option<&Table>) -> mlua::Result<Self> {
        let opts = opts_from_table(opts)?;
        let inner = bemtvi_core::glob::compile_set(&patterns, &opts)
            .map_err(|e| mlua::Error::runtime(format!("btv.glob: {e}")))?;
        Ok(LuaGlobSet { inner })
    }
}
...
impl UserData for LuaGlobSet {
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        ...
        methods.add_method("test", |_, this, path: Value| {
            let path = candidate("btv.glob globset:test", "path", &path)?;
            Ok(this.inner.is_match(&*path.as_bytes()))
        });
        ...
        methods.add_method("matches", |lua, this, path: Value| {
            let path = candidate("btv.glob globset:matches", "path", &path)?;
