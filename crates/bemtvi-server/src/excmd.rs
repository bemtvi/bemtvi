...
impl EditHost {
    pub(crate) fn run_command(&mut self, cmd: &str) {
        ...
        // the `btv_input` batch does. A `:e` opens a file, and whether it defers for a
    ...
    /// `:sil[ent][!] {cmd}` modifier, and what `btv.cmd(cmd, { silent = … })`
    ...
    pub(crate) fn resolve_command(&mut self, cmd: &str, range: Option<(usize, usize)>) {
        ...
        match name {
            ...
            // suppresses the jump). Async, via the job machinery (`btv._qf_make`).
            ...
            // `btv.lsp.hover{ name = … }`. Without it the request goes to the
            ...
            // server formats (the ex twin of `btv.lsp.format{ name = … }`).
            ...
            // filter / one-shot apply are `btv.lsp.code_action(opts)` options. An
            ...
            // the same `btv._user_commands` store the `nvim_*` API uses), so the
            ...
            // compile into the data dir, see `bemtvi_ts::install`; browser: the JS
            ...
            // `:help [topic]` — the help system ships as the optional `bemtvi-help`
            ...
            _ if matches!(base, "help" | "h") && !self.lua.has_user_command(name, cur_buf) => {
                self.editor.echo(
                    "help: the bemtvi-help plugin is not installed — add it with \
                     :Plugins (bemtvi/bemtvi-help), then use :help {topic}",
                )
            }
            // `:helpt[ags]` — tag generation lives in bemtvi-help as `:BtvHelptags`
            // (bemtvi has no built-in `:helptags`). A plugin that registered
            ...
            {
                self.editor.echo(
                    "helptags: install the bemtvi-help plugin (:Plugins) and use \
                     :BtvHelptags to generate help tags",
                )
            }
    ...
    pub(crate) fn ex_chdir(&mut self, scope: CdScope, arg: &str) {
        ...
        // Keep the `btv._cwd` mirror (`vim.fn.getcwd`) in step with `DirState`.
        self.publish_cwd_mirror();
    ...
    /// the first valid entry. The actual spawn is async (`btv._qf_make`); on a build
            ...
            if !repos.is_empty() {
                self.editor.echo(format!(
                    "TSInstall: installing from a GitHub repo ({}) needs a C compiler and \
                     isn't supported in the browser build — use a native bemtvi",
                    repos.join(", ")
                ));
            }
            if langs.is_empty() {
    ...
    /// data-dir search path (bemtvi's own dir + a borrowed neovim `site/`), each
    ...
    #[cfg(feature = "native")]
    fn ts_install_info(&mut self) {
        let parsers = bemtvi_ts::installed_parsers();
        let mut lines = Vec::new();
        if parsers.is_empty() {
    ...
    pub(crate) fn set_colorscheme(&mut self, name: &str) {
        ...
        // binary, so `:colorscheme bemtvi` works with zero config.
        ...
        // visibly when a truecolor attach defaults in the bundled `bemtvi` (One Dark)
        ...
        for group in &dropped {
            self.editor
                .highlights
                .set_ns(0, group, bemtvi_core::highlight::HlDef::default());
        }
        ...
        // leaves `catppuccin-mocha` — and plugins key off that name (bemtvi-line looks
    ...
    fn ex_source(&mut self, arg: &str) {
        ...
        match path.extension().and_then(|e| e.to_str()) {
            ...
            _ => self.editor.echo(format!(
                "bemtvi: :source of vimscript is not supported yet ({arg}); only .lua files can be sourced"
            )),
        }
    }
    ...
    /// Surface the text a `btv._ex_*` autocmd driver returned: empty is nothing,
...
/// (the server hands the names to Lua as `btv._builtin_colorschemes`) — add a scheme
...
pub(crate) const BUILTIN_COLORSCHEMES: &[(&str, &str)] =
    &[("bemtvi", include_str!("../runtime/colors/bemtvi.lua"))];
