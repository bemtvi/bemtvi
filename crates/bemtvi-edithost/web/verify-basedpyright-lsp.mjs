...
try {
  ...
  await page.evaluate(() => window.__bemtvi.feed(":e /main.py<CR>"));
  const opened = await until(page, () => window.__bemtvi.lines(), (v) => /add\("x", 1\)/.test(String(v)));
  ...
  // 1. The server started locally (it appears in btv.lsp.clients).
  const clients = await until(
    page,
    () => window.__bemtvi.execLua("return tostring(#btv.lsp.clients())").then((r) => r.result),
    (v) => /[1-9]/.test(String(v)),
  );
  check("lsp: basedpyright started in-browser (btv.lsp.clients() lists it)", /[1-9]/.test(String(clients)), `clients=${JSON.stringify(clients)}`);

  // 2. A real type diagnostic lands in btv.diagnostic.get() — proving typeshed loaded (int resolves)
  ...
  check("lsp: basedpyright type-checked the buffer (str→int error in btv.diagnostic.get())",
  ...
  //    (`btv.lsp.request` → the SyncLspClient → the bridge → basedpyright → reply), so it exercises
  ...
  //    through `btv.diagnostic.get()`, but the `:Lsp*` ex-command surface lives in the server's
  ...
  await page.evaluate(() => window.__bemtvi.feed(":LspDiagnostics<CR>"));
  const loclist = await until(
    page,
    () =>
      window.__bemtvi
        .execLua(
          `local ll = btv.getloclist(0) or {}
           local txts = {}
