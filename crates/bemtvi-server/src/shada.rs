...
//! [`ShadaStore`] trait — exactly like [`HostFs`](bemtvi_core::HostFs) /
...
/// `hist_input` table: the per-namespace `btv.ui.input{ history = … }` rings. Key is
...
/// key/value data (`btv.shada.plugin`). Value is a msgpack [`StoredPlugin`]. Keyed
...
/// overlay ([`bemtvi_core::options::WorkspaceOptions`] — the `btv.wso` per-workspace global
...
/// Rewrite the per-namespace `btv.ui.input` history table wholesale: clear it, then
...
struct MergedRaw {
    ...
    hist_ex: HistMerge,
    /// The per-namespace `btv.ui.input` history rings: one [`HistMerge`] per namespace,
    ...
    session: (u64, Option<bemtvi_core::SessionState>),
    /// The per-workspace option overlay (`btv.wso`): newest store's whole value wins,
    /// keyed by `meta.flush_ts` like the session. Empty until a store carries one.
    workspace_options: (u64, bemtvi_core::options::WorkspaceOptions),
}
...
fn read_session(db: &Database) -> Option<bemtvi_core::SessionState> {
...
fn read_workspace_options(db: &Database) -> bemtvi_core::options::WorkspaceOptions {
    let read = || -> Option<bemtvi_core::options::WorkspaceOptions> {
        let rtxn = db.begin_read().ok()?;
    ...
    match (source, state_dir) {
        (crate::ConfigSource::Remote, Some(dir)) => {
            match prepare_remote_shada(host_fs, dir, namespace).await {
                ...
                Err(e) => {
                    eprintln!("bemtvi: remote shada unavailable ({e}); using local shada");
                    (fallback_local, None)
                }
            }
        }
        (crate::ConfigSource::Remote, None) => {
            eprintln!("bemtvi: daemon reported no shada dir; using local shada");
            (fallback_local, None)
        }
