...
- **Synchronous Lua eval already exists** — `LuaRuntime::eval_to_value_pumped`
  (`crates/bemtvi-lua/src/runtime.rs:238`) evaluates an expression and returns an
...
- **The highlight registry is done** — named groups, `nvim_set_hl`, links, and
  `@`-fallback resolution all live in `crates/bemtvi-core/src/highlight.rs`, and
  the redraw path already resolves chrome regions to concrete `Style`s
  (`crates/bemtvi-server/src/redraw.rs:223`, including a `StatusLine` group).
...
- **The status line is the last client-composed chrome.** Today the TUI builds
  ...
  (`crates/bemtvi-tui/src/render.rs:796`) from projected `WindowView` fields
  (`mode_label`, `file_name`, `modified`, `cursor_line`, `cursor_col`). The rest
  of bemtvi's philosophy is "server resolves style + content, client paints"
...
The `%`-format language splits cleanly along bemtvi's purity boundary:

- **Parsing + field expansion + layout are pure** → a new
  `crates/bemtvi-core/src/statusline.rs` module. No Lua, no I/O — exactly the kind
...
`%{}` / `%!` evaluate **Vimscript** in real neovim. bemtvi has no Vimscript, so we
...
tests inside `bemtvi-core` — but every case is **derived from real neovim
...
bemtvi matches that ground truth (capture the nvim output, encode it as the
...
- The built-in default is expressed *as a format string* and rendered through
  ...
  `" %{mode} %f%m%=%l,%c "` (mode is an bemtvi field, since neovim shows mode
...
- `OptionValue::String` variant (`crates/bemtvi-lua/src/ops.rs`); core `Options`
  gains `statusline: String`.
- `resolve_set` / `canonical` (`crates/bemtvi-core/src/options.rs:219,285`): a
...
- `GlobalOptionOp` apply path stores the string; Lua bridge `_set_global_option`
  (`crates/bemtvi-lua/src/install.rs:469`) gains a `String` arm; `vim.opt` /
...
`crates/bemtvi-core/src/statusline.rs`: `parse`, `StatuslineCtx`, `expand`,
...
- Project per-window `status` as a styled-segment array in `redraw`
  (`crates/bemtvi-server/src/redraw.rs` `window_value`), replacing the implicit
  field-based status.
...
- `render_status` (`crates/bemtvi-tui/src/render.rs:796`) paints the projected
  `status` segments (text + style); drop the client-side field composition.
- `crates/bemtvi-tui/src/view.rs` parses the new `status` segment array.
- Keep a reverse-video fallback if the array is absent (older server).
- **Tests** (`crates/bemtvi/tests/screen.rs`): end-to-end paint of a custom
  statusline.
...
(`btv._cur_mode`, via `set_buf_mirror`), and `redraw` refreshes that mirror
...
- `laststatus` option ✅ done — `0` never, `1` only with ≥2 windows, `2` always
  ...
  **Shipped:** `Options.laststatus` (`crates/bemtvi-core/src/options.rs`), wired
...
**Shipped:**
- **Parse/expand/layout** (`crates/bemtvi-core/src/statusline.rs`):
...
- **Mouse** (`crates/bemtvi-core/src/editor/mouse.rs`): `MouseTarget::StatusLine`
...
- **Server** (`redraw.rs` / `effects.rs` / `dispatch.rs`):
  ...
  (`btv._statusline_click` resolves the `v:lua.` reference loud), then
...
- Click regions also work for `btv.statusline` **segment** layouts — a segment/cell
  `on_click = "v:lua.<fn>"` fires through the same dispatch (see
  `docs/plans/2026-06-15-btv-statusline-segments.md`).
- The single **global bar** (`laststatus=3`) is clickable too: `hit_test` resolves
  ...
  clickable; tests in `bemtvi-server/tests/mouse.rs`.)
