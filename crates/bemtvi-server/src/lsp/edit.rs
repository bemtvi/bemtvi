...
use bemtvi_lsp::lsp_types::{Location, Position, Range, TextEdit, Url};
...
impl EditHost {
    ...
    /// the flag alone.
    fn set_endofline(&mut self, id: bemtvi_core::BufferId, endofline: Option<bool>) {
        if let Some(eol) = endofline {
            self.editor.set_buffer_option_bool(id, "endofline", eol);
        }
    }

    /// Apply a `WorkspaceEdit` handed up from Lua (`btv.lsp.apply_workspace_edit` /
    ...
    pub(crate) fn apply_lua_workspace_edit(&mut self, edit: serde_json::Value, encoding: &str) {
        ...
        match bemtvi_lsp::try_normalize_workspace_edit_value(&edit) {
            Ok(changes) => {
    ...
    ///   editor tick, so they are queued on the `btv.fs` `FsJob` seam
    ...
    pub(crate) fn apply_workspace_edit(
        ...
        // bemtvi advertises — without touching another edit's, and a server-initiated
    ...
    fn apply_workspace_changes(
        ...
        // That is all the resource operation asks for and all bemtvi does: the file exists,
    ...
    /// the statusline, `btv.buf.name`), so a refactor that created or moved a file would
    ...
    /// what bemtvi advertises) — the whole point of change annotations: a server can
    ...
    ///
    /// Asking is Lua's job (`btv.lsp._confirm_edit` → `btv.ui.confirm`, a promise chain):
    ...
    /// [`LspOp::WorkspaceEditDecision`](bemtvi_lua::LspOp), which
    ...
    pub(crate) fn on_workspace_edit_decision(
        ...
        // config calling `btv._lsp_edit_decision` by hand, a duplicate from a
    ...
    /// behavior. (bemtvi used to write the contents out too; that deviation is gone.)
    ///
    ...
    /// half (the same `FsJob` seam `btv.fs` rides, so a local session runs it on the
    ...
    fn pump_workspace_fs_queue(&mut self) {
        ...
        while let Some(id) = self.workspace_fs_queue.pop_front() {
            ...
            self.apply_loop_op(bemtvi_lua::LoopOp::Fs {
                id,
                job,
                local: false,
            });
        ...
        self.apply_loop_op(bemtvi_lua::LoopOp::TimerStop {
            id: crate::WORKSPACE_FS_TIMEOUT_TIMER_ID,
        });
    }
    ...
    pub(crate) fn on_workspace_fs_timeout(&mut self) {
        ...
        let result = Err(bemtvi_lua::FsError {
    ...
    /// bemtvi advertises: when a change fails, the ones after it do not run. Returns how
    ...
    pub(crate) fn on_workspace_fs_result(
        &mut self,
        id: u64,
        result: &Result<bemtvi_lua::FsValue, bemtvi_lua::FsError>,
    ) -> bool {
        ...
        let failure = match (&job.op, result) {
            ...
            // W12 conflict over bemtvi's own placeholder:
            ...
            (WorkspaceFsOp::RenameGuard { from, to, to_name }, Ok(value)) => {
                if matches!(value, bemtvi_lua::FsValue::Bool(true)) {
    ...
    ///
    /// `opts` is the caller's `btv.lsp.code_action{ context = { only = … }, apply = … }`:
    ...
    pub(crate) fn show_code_actions_from(
        ...
        self.lsp_code_actions = actions;
        self.editor
            .open_menu(lines, bemtvi_core::MenuPlacement::Cursor, 0);
    ...
    /// ([`LspEvent::ApplyEdit`](bemtvi_lsp::LspEvent)). This is how a refactor
    ...
    pub(crate) fn dispatch_lsp_command(
        &mut self,
        command: bemtvi_lsp::lsp_types::Command,
    ...
    pub(crate) fn resolve_code_action(
        &mut self,
        action: Box<bemtvi_lsp::lsp_types::CodeAction>,
        cb_id: u64,
        origin: Option<ServerKey>,
    ) {
...
/// The [`PositionEncoding`] a Lua caller named (`btv.lsp.apply_workspace_edit` /
/// `btv.lsp.show_document`'s `opts.encoding`). Anything but the two exact alternatives
