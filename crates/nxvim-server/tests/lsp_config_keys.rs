//! Behavior tests for the `nx.lsp` config keys added for the native nvim-lspconfig
...
//!
//! `$NXVIM_LSP_CMD` is deliberately NOT used here: it replaces the whole argv, which
...
use nxvim_test_harness::{attach, command, exec_lua, serial_lock, spawn, temp_dir};
...
async fn enable(rpc: &Rpc, script: &Path, body: &str) {
    exec_lua(
        rpc,
        &format!(
            "nx.lsp.config('rec', {{ cmd = {{ '{}' }}, filetypes = {{ 'rust' }}, {body} }})\n\
             nx.lsp.enable('rec')",
            script.display()
        ),
    )
...
async fn workspace_required_declines_a_buffer_with_no_root() {
    ...
    enable(
        &rpc,
        &script,
        "root_markers = { 'nxvim-no-such-marker.json' }, workspace_required = true",
    )
    .await;
...
async fn without_workspace_required_a_rootless_buffer_still_starts() {
    ...
    enable(
        &rpc,
        &script,
        "root_markers = { 'nxvim-no-such-marker.json' }",
    )
    .await;
...
async fn a_cmd_builder_may_return_a_promise_of_the_argv() {
    ...
    // (`nx.fs.which`), so the builder is async and returns a promise of the argv.
    // Blocking on the lookup is the thing nxvim doesn't do, so this path is the only
    // way those configs can work at all.
    exec_lua(
        &rpc,
        &format!(
            "nx.lsp.config('rec', {{\n\
               cmd = nx.async(function(_, config)\n\
                 local found = nx.await(nx.fs.which('{}'))\n\
...
async fn a_cmd_builder_that_rejects_does_not_start_a_server() {
    ...
    exec_lua(
        ...
        &format!(
            "nx.lsp.config('rec', {{\n\
               cmd = nx.async(function() error('no toolchain here') end),\n\
               filetypes = {{ 'rust' }},\n\
             }})\n\
             nx.lsp.enable('rec')\n\
             _G.unused = '{}'",
...
async fn a_config_name_override_still_resolves_its_lifecycle_hooks() {
    ...
    let key = exec_lua(&rpc, "return nx.lsp._config_key['renamed']").await;
    assert_eq!(
        key.as_str(),
        Some("rec"),
...
async fn an_unknown_config_key_is_reported_rather_than_silently_dropped() {
    ...
    exec_lua(
        &rpc,
        "_G.warnings = {}\n\
         local real = nx.notify\n\
         nx.notify = function(msg, lvl) _G.warnings[#_G.warnings + 1] = msg return real(msg, lvl) end",
    )
    .await;
    ...
    // a config's own `cmd` / `before_init` / `on_attach` may well read a key nx.lsp
    ...
    // A key nxvim knows about but doesn't act on says what will happen instead.
    assert!(
