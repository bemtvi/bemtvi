...
// seeds the session cwd from the daemon's `config_bundle`; a *runtime* `:connect bemtvi://…` re-points
// the fs seam WITHOUT re-fetching the bundle, so the daemon's cwd used to never reach `DirState` and
// a relative `btv.fs` path stayed unrebased (resolving against the stale serverless dir). This proves
// the fix: the Worker fetches the new daemon's cwd (`realpath(".")`) on runtime connect and seeds it,
// so `getcwd()` reports the daemon cwd and a RELATIVE `btv.fs.read_text` resolves against it over the wire.
//
...
// (target/debug/bemtvi), and a Chromium for Playwright. Run:  node verify-connect-cwd.mjs
...
// The current `btv.daemon.status()` phase ("disconnected" contains "connected", so match the quotes).
async function status(page) {
  const r = plainStr(await luaResult(page, "return btv.daemon.status()"));
...
const root = mkdtempSync(join(tmpdir(), "bemtvi-connect-cwd-"));
...
// Run a relative `btv.fs.read_text("marker.txt")` and return its content, or `ERR:<code>` on reject.
async function relRead(page) {
  await luaResult(page, `_G.__r, _G.__e = nil, nil
     btv.fs.read_text("marker.txt"):next(
       function(t) _G.__r = t end,
       function(e) _G.__e = "ERR:" .. e.code end)
     return 1`);
  return plainStr(await until(page,
    () => window.__bemtvi.execLua("return _G.__r or _G.__e or ''").then((r) => r.result),
...
try {
  ...
  check("before `:connect`: a relative btv.fs read hits OPFS (not the daemon) — ENOENT",
  ...
  // Runtime `:connect bemtvi://…` — typed on the command line, confirmed through the UI intercept
  ...
  await page.evaluate(() => window.__bemtvi.pressEnter());
  const phase = await until(page, () => window.__bemtvi.execLua("return btv.daemon.status()").then((r) => r.result),
  ...
  check("after `:connect`: a RELATIVE btv.fs read resolves against the daemon cwd (over the wire)",
...
console.log(failures === 0
  ? "\nALL PASS — a runtime :connect seeds the session cwd from the daemon; a relative btv.fs path then follows it"
  : `\n${failures} FAILED`);
