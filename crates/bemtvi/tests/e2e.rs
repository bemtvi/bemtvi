...
/// spawned `bemtvi` starts with no plugins / `init.lua` — hermetic, independent of
/// the developer's `~/.config/bemtvi`.
...
impl Session {
    ...
    /// used to point `bemtvi` at a throwaway config dir / runtimepath / cache so
    ...
    fn spawn_with_env(args: &[&str], env: &[(&str, &str)], cols: u16, rows: u16) -> Session {
        ...
        // Hermetic by default: unless the test supplies its own `BEMTVI_CONFIG`, point
        // the child at a fresh *empty* config dir so startup never reads the
        // developer's real `~/.config/bemtvi`. Otherwise the test's outcome (and
        ...
        let cfg_dir = (!env.iter().any(|(k, _)| *k == "BEMTVI_CONFIG")).then(|| {
...
impl Drop for Session {
    ...
    /// unwinds) can't leave an orphaned `bemtvi` process and a reader thread
...
fn startup_shows_the_file_contents() {
    let path = bemtvi_test_harness::temp_root()
        .join(format!("bemtvi_e2e_startup_{}.txt", std::process::id()));
...
/// query), so `BEMTVI_OSC52=1` forces the capability on — the same escape hatch a
...
fn a_clipboard_yank_leaves_as_an_osc52_escape() {
    let path = bemtvi_test_harness::temp_root()
        .join(format!("bemtvi_e2e_osc52_{}.txt", std::process::id()));
    std::fs::write(
        &path, "clip me
",
    )
...
/// stack. Prefers `$BEMTVI_CATPPUCCIN`, else the place a user installs it
/// (`~/.config/bemtvi/pack/plugins/start/catppuccin`). `None` (→ the test skips)
/// when it isn't on disk, since we deliberately don't vendor it into the repo.
fn catppuccin_dir() -> Option<PathBuf> {
    let candidate = std::env::var_os("BEMTVI_CATPPUCCIN")
        .map(PathBuf::from)
        .or_else(|| {
            std::env::var_os("HOME")
                .map(|h| PathBuf::from(h).join(".config/bemtvi/pack/plugins/start/catppuccin"))
        })?;
    candidate
...
fn catppuccin_repaints_the_editor_in_truecolor() {
    let Some(catppuccin) = catppuccin_dir() else {
        eprintln!(
            "skipping: no catppuccin checkout found \
             (set $BEMTVI_CATPPUCCIN or install it under ~/.config/bemtvi)"
        );
        return;
    };
    ...
    let base =
        bemtvi_test_harness::temp_root().join(format!("bemtvi_e2e_cat_{}", std::process::id()));
    ...
    let mut s = Session::spawn_with_env(
        &[file.to_str().unwrap()],
        &[
            ("BEMTVI_CONFIG", config.to_str().unwrap()),
            ("BEMTVI_RUNTIMEPATH", catppuccin.to_str().unwrap()),
...
/// bemtvi defaults its bundled `bemtvi` One Dark colorscheme in at startup — the
...
#[test]
#[ignore = "PTY/terminal e2e; needs a real controlling terminal. Run with --ignored. See module header."]
fn truecolor_terminal_defaults_in_the_bemtvi_colorscheme() {
    let base =
        bemtvi_test_harness::temp_root().join(format!("bemtvi_e2e_tc_{}", std::process::id()));
    ...
    // No BEMTVI_CONFIG override ⇒ the harness points at a fresh *empty* config dir,
    ...
    // bemtvi One Dark: Normal foreground #abb2bf, background #282c34.
    ...
    assert!(
        ok,
        "the bundled bemtvi scheme never auto-loaded on a truecolor terminal:\n{}",
        s.screen_text()
    );
...
fn a_server_thread_panic_exits_nonzero() {
    ...
    // exited 0). The debug-only `BEMTVI_PANIC_TEST` hook forces that panic; with
    // it set the process must exit with the panic code 101.
    let mut s = Session::spawn_with_env(&[], &[("BEMTVI_PANIC_TEST", "1")], 80, 24);
    let status = s
        .wait_exit(Duration::from_secs(10))
        .expect("bemtvi should exit promptly after the server thread panics");
...
fn an_edit_host_connect_failure_exits_nonzero() {
    ...
    // The cheapest real trigger: a `bemtvi://…` connect target with a malformed cert
    ...
    let mut s = Session::spawn(&["bemtvi://127.0.0.1:1/tok?cert=zz"], 80, 24);
    let status = s
        .wait_exit(Duration::from_secs(15))
        .expect("bemtvi should exit promptly when the daemon connect fails");
    assert!(
        !status.success(),
...
fn killing_the_editor_leaves_a_usable_terminal() {
    ...
    // in `bemtvi-tui/tests/signal_restore.rs`; this asserts the real binary wires it up
...
fn killing_the_editor_runs_the_exit_sequence() {
    ...
    let mut s = Session::spawn_with_env(&[], &[("BEMTVI_CONFIG", dir.to_str().unwrap())], 80, 24);
    assert!(
        s.wait_until(Duration::from_secs(10), |scr| scr
            .contents()
            .contains("NORMAL")),
    ...
    s.wait_exit(Duration::from_secs(15))
        .expect("bemtvi should exit promptly on SIGTERM");
...
fn a_killed_session_is_still_there_on_the_next_launch() {
    ...
    let path = bemtvi_test_harness::temp_root()
        .join(format!("bemtvi_e2e_shada_{}.txt", std::process::id()));
...
fn daemon_stderr_log_is_private_and_per_pid() {
    ...
    // *per-pid* name — not the old fixed, world-readable `bemtvi-daemon.log`, which let
    ...
    let log = std::env::temp_dir().join(format!("bemtvi-daemon-{pid}.log"));
...
fn startup_writes_no_diagnostics_to_the_terminal() {
    let path = bemtvi_test_harness::temp_root()
        .join(format!("bemtvi_e2e_quiet_{}.txt", std::process::id()));
    std::fs::write(&path, "alpha\nbeta\n").unwrap();
