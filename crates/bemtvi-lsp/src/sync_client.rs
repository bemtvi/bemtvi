...
/// minted exactly as [`RemoteLspTransport`](../../bemtvi_server) mints its spawn id.
#[derive(Debug, Clone)]
pub enum WireOp {
    Spawn {
        id: u64,
    ...
    fn on_server_request(&mut self, key: &ServerKey, req_id: Value, method: &str, msg: &Value) {
        ...
        match method {
            ...
            "workspace/applyEdit" => {
                ...
                // its text edits' `annotationId`s, which decide what bemtvi asks about.
            ...
            // native router does (`btv.lsp._register_capability` arms the file watches),
...
fn distill(kind: ReqKind, result: Result<Value, String>) -> LspReply {
    match kind {
        ...
        // which decide whether bemtvi asks before applying (see
