...
/// separator — vim's `:help :bar` exception list, restricted to the commands bemtvi
...
///
/// That last case is where bemtvi departs from vim, and it matters: bemtvi's regex
...
impl Editor {
    ...
    fn execute_ex_one(&mut self, raw: &str) {
        ...
        // name, so it's matched on the raw remainder ahead of `split_ex`. bemtvi's own
        ...
        match name {
            ...
            // system ships as the optional `bemtvi-help` plugin. They defer to the
    ...
    /// line has its replacement half open — lay a `BtvSubstituteDelete` range over
    /// every match with an inline `BtvSubstituteAdd` `virt_text` holding the
    ...
    /// `BtvSubstituteDelete` range over the matched bytes `[start, end)` (the
    /// removed side, struck through) and — when `repl` is non-empty and single
    /// line — an inline `BtvSubstituteAdd` `virt_text` at `end` holding the
    ...
    fn set_subst_preview_marks(&mut self, start: usize, end: usize, repl: &str) {
        ...
        buf.extmarks.set(
            ...
            start,
            Some(end),
            Some("BtvSubstituteDelete".to_string()),
            DEFAULT_PRIORITY,
            None,
        );
        if !repl.is_empty() && !repl.contains('\n') {
            let decor = VirtDecor {
                virt_text: vec![VirtChunk {
                    text: repl.to_string(),
                    hl_group: Some("BtvSubstituteAdd".to_string()),
                }],
    ...
    pub(crate) fn subst_confirm_key(&mut self, key: Key) {
        ...
        // and required here, because bemtvi re-runs `ensure_visible` every redraw
    ...
    /// `btv.open(path, { where })` — open `path` (a file or directory) in the editing
