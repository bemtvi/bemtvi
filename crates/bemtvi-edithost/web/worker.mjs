...
// bemtvi's core + the PUC Lua 5.4 VM: it loads the emscripten module (`dist/eh.mjs`),
...
//     (`vim.defer_fn` / `btv.timer`) that `evloop.rs` can't provide in-Worker. No Asyncify.
...
// The `btv.fs` reply is re-encoded to msgpack bytes (faithful binary for a `read` result)
...
// fetches the new daemon's cwd and hands it here so relative `btv.fs`/spawn paths rebase against it.
...
// (re-arm watches, re-open LSP, freeze lost terminals) and it mirrors to `btv.daemon.status()`.
...
// Streaming stdout (`btv.run_stream`'s batches): the daemon pushes `proc_stdout` batches; the
...
// Remote `btv.fs` leg (Phase 2 of the off-tick plan): the editor enqueues each high-level
// `btv.fs.*` op off-tick (only with a daemon connected); the Worker forwards each as one
...
// Remote `btv.git.*` leg (`git_op`): the editor enqueues each op off-tick; the Worker runs it
...
// Remote `btv.http.fetch` leg (`http_op`): the editor enqueues each request off-tick; the Worker
...
// `btv.http.mount` (the Service Worker leg): the browser twin of the native listener. The
...
// Streaming `btv.fs.watch` over the daemon (Phase 3b): the editor arms/disarms a recursive watch
...
// (btv.process / btv.socket). Like the LSP leg, the editor enqueues bidirectional ops off-tick
...
import("./ts-indent.js")
  .then(({ createIndenter }) => {
    ...
    globalThis.__bemtviTsIndent = (lang, text, line, sw, ts) => indenter.indent(lang, text, line, sw, ts);
    globalThis.__bemtviTsAvailable = (lang) => indenter.available(lang);
  })
...
import("./ts-folds.js")
  .then(({ createFolder }) => {
    folder = createFolder();
    globalThis.__bemtviTsFolds = (lang, text) => folder.folds(lang, text);
    globalThis.__bemtviTsFoldsAvailable = (lang) => folder.available(lang);
  })
...
import("./ts-textobjects.js")
  .then(({ createTextObjects }) => {
    ...
    globalThis.__bemtviTsTextObjects = (lang, text, capture, byte) => textobjects.textObjectsAt(lang, text, capture, byte);
    globalThis.__bemtviTsTextObjectsAvailable = (lang) => textobjects.available(lang);
  })
...
globalThis.__bemtviTsReload = (lang) => {
...
// shipped the OLD plugin bundle even after a rebuild (a bundled plugin's `btv.shada.plugin()` then
...
async function sourceServerlessConfig() {
  ...
  // (web/demo-seed/, fetched as static assets). A sentinel (/.bemtvi/.demo-seeded) makes this
  ...
  if (BUILD.demoSeed) {
    try {
      const seeded = await opfsRead("/.bemtvi/.demo-seeded"); // kind 1 = absent (not seeded yet)
      if (seeded.kind !== 0) {
        ...
        await opfsWrite("/.bemtvi/.demo-seeded", enc.encode("1"));
      }
    } catch (e) {
  ...
  // `require("bemtvi-line")`-class resolves it. Absent → skipped, exactly like an absent
...
async function bootWithConfig() {
  ...
  try {
    ...
    const cache = await opfsRead("/.bemtvi/treesitter/manifest.json"); // kind 0 = present (raw `bytes`)
...
// =============================================================================
// Serverless `btv.fs` over OPFS (Phase 3 of the off-tick plan). The JS twin of the daemon's
// `run_fs_job`: when no daemon is connected, a high-level `btv.fs.*` op runs against OPFS here
...
// Run one high-level `btv.fs` op against OPFS, returning the `["ok"|"err", …]` reply envelope.
  ...
  try {
    switch (req.op) {
      ...
      case "which":
        return ["ok", fsNil()];
      default:
        return ["err", "EINVAL", `unknown btv.fs op '${req.op}'`];
    }
  } catch (e) {
...
//
// When the page is opened with `?daemon=bemtvi://HOST:PORT/TOKEN?cert=HASH`, the same
// off-tick fs seam that OPFS satisfies serverless-ly is instead fulfilled over a real
// WebTransport connection to a remote `bemtvi --daemon --listen` (rpc.mjs). The editor is
...
// `:connect bemtvi://…` (the browser twin of bemtvi-gui's `:connect`) re-points it after boot —
...
// supervisor (crates/bemtvi-server/src/daemon.rs). Status rides `btv.daemon.status()` like native.
...
// mirrors it to `btv.daemon.status()`, fires `User DaemonStatusChanged`, and — when `reconnected`
...
// an in-flight `btv.run` promise / `btv.process` on_exit / `btv.socket` on_close / `btv.fs.watch`
...
// Runtime `:connect bemtvi://…`: dial a daemon *after* boot and re-point the off-tick fs/watch
// seam from OPFS onto the wire — the browser twin of bemtvi-gui's client-side `:connect`, which
...
async function runtimeConnect(uri) {
  ...
  // `btv.fs` path would resolve against the stale (serverless/previous) dir. `realpath(".")` over the
  // fresh luafs leg returns the daemon's own cwd; hand it to the core (DirState + `btv._cwd` mirror +
  ...
  if (daemon) {
    try {
      ...
      // leave the cwd unseeded — relative btv.fs stays daemon-launch-dir-relative until a fix
    }
  }
...
const liveFsWatches = new Set(); // btv.fs.watch stream ids armed on the daemon (also gates the async park)
const liveLsp = new Set(); // LSP server wire ids spawned on the daemon (also gates the async park)
const liveDprocs = new Set(); // duplex btv.process child ids in flight on the daemon (gates the park)
const liveSocks = new Set(); // btv.socket connection ids in flight on the daemon (gates the park)
...
let pendingConnectUri = null; // a runtime `:connect bemtvi://…` to dial on the next run-loop pass
...
function applyDaemonNotifications() {
  ...
  for (let n; (n = daemonNotifications.shift()); ) {
    ...
    if (method === "__daemon_status") {
      ...
      // params = [phase, reconnected]. Land it on the editor — mirrors `btv.daemon.status()`,
      ...
      // (`btv.run_stream`). Hand the lines to the Lua callback as JSON.
      const id = params[0];
      ...
      // params = [id, kind, [path, …]] — a coalesced btv.fs.watch change batch (Phase 3b).
      const id = params[0];
      ...
      // btv.process child (the DAP transport). Hand it to the Lua receiver.
...
// `has_remote_proc`, so a serverless btv.process fails loud in the core).
...
async function drainProcRequests() {
  ...
  for (const s of reqs.spawn) {
    ...
    // a streaming spawn (`btv.run_stream`) gets `proc_stdout` batches back.
...
// Forward the off-tick `btv.fs` ops the tick enqueued, routed to the daemon when connected (the
...
// `btv.fs` in the promise continuation. Returns whether any op was handled (so the caller
...
// Run one `btv.fs` op against the daemon (the `luafs_op` leg). In daemon mode fs NEVER silently
...
// Forward the off-tick `btv.git.*` ops the tick enqueued, routed to the daemon (the `git_op`
...
// Run one `btv.git` op against the daemon (the `git_op` leg). A dropped link returns a loud
...
// Land one `btv.git` op reply into the tick: re-encode the daemon's `["ok"|"err", …]` envelope
...
// Forward the streaming `btv.fs.watch` arms/disarms the tick enqueued to the daemon (the
...
// daemon (`has_remote_proc`), so a serverless `btv.fs.watch` fails loud in the core and never
...
// Land one `btv.fs` op reply into the tick: re-encode the daemon's `["ok"|"err", …]` envelope to
...
// Forward the off-tick `btv.http.fetch` requests the tick enqueued, routed to the daemon when
// connected (the `http_op` leg — the daemon owns the network, dodging CORS) else the browser's
// own `fetch()` (serverless — no daemon required, unlike `btv.fs`). Each produces the `["ok",
...
async function drainHttpMounts() {
  ...
  for (const m of mounts) {
    ...
    postMessage({ type: "btv-http-mount", id: m.id, name: m.name });
  }
  return true;
}
...
function drainHttpUnmounts() {
  ...
  postMessage({ type: "btv-http-unmount", ids });
  return true;
}
...
function drainHttpServerReplies() {
  ...
  postMessage({ type: "btv-http-reply", replies });
  return true;
}
...
async function drainHttpRequests() {
  ...
  for (;;) {
    ...
    for (const op of ops) {
      ...
      const req = { ...op };
      delete req.id;
      // `local` (`btv.http.fetch_local`) forces the browser `fetch()`, never the daemon.
      const forceLocal = req.local === true;
      delete req.local;
...
// Run one `btv.http.fetch` on the daemon (the `http_op` leg). A dropped link returns a loud error
...
// Run one `btv.http.fetch` with the browser's own `fetch()` (serverless — no daemon). Fetch
...
// Land one `btv.http.fetch` reply into the tick: re-encode the `["ok"|"err", …]` envelope to
...
//   9 = btv-http-request (payload = JSON, a Service Worker request for an btv.http.mount),
...
async function runLoopSAB(ctrl, data) {
  ...
  function drain() {
    ...
    while (rp !== wp) {
      ...
      if (type === 0) {
        ...
        // connect: a runtime `:connect bemtvi://…`. Record the URI; the run loop dials it
        ...
        // btv-http-request: a Service-Worker-intercepted request the window relayed in. It
        ...
        // plugin's `btv.http.mount` promise.
        const r = JSON.parse(utf8(payload));
  ...
  for (;;) {
    ...
    // `vim.defer_fn` / `btv.timer` (feed or exec_lua) computes its deadline from now —
    // not a stale clock that would make it instantly due.
    eh_set_clock(h, nowMs());
    const { acks, results, tsLanded } = drain();
    // A runtime `:connect bemtvi://…` arrived this pass (`<Esc>` already dismissed the command
    ...
    // Forward any off-tick `btv.fs` ops the tick enqueued to the daemon (the `luafs_op` leg).
    ...
    // Forward any off-tick `btv.git.*` ops the tick enqueued to the daemon (the `git_op` leg —
    ...
    // Forward any off-tick `btv.http.fetch` the tick enqueued — to the daemon (`http_op`) when
    ...
    // `btv.http.mount`: hand the window any new publications (it owns the Service Worker
    ...
    // Forward any streaming `btv.fs.watch` arms/disarms the tick enqueued to the daemon (its
...
onmessage = async (ev) => {
  ...
  // Runtime `:connect bemtvi://…` (5c; SAB routes it via a ring frame in `drain()`). Dial the
  ...
  // `btv.http.mount` (5c; SAB routes these as ring type-9 / type-10 frames in `drain()`).
  // Under 5c the loop never parks, so a plain message is delivered fine.
  if (msg.type === "btv_http_request") {
