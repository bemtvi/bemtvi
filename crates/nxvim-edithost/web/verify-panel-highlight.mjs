...
try {
  ...
  await page.evaluate(() => window.__nxvim.feed("ggdGifn main() {<CR>    let x = 42;<CR>}<Esc>"));
  ...
  await page.evaluate(() => window.__nxvim.feed(":echomsg 'panel test message'<CR>"));
  await page.evaluate(() => window.__nxvim.feed(":messages<CR>"));
  await sleep(300);

  // The panel is focused now, but the main code window must KEEP its highlighting.
  const focused = await page.evaluate(() => window.__nxvim.execLua("return nx.bo.buftype or ''").then((r) => r.result));
