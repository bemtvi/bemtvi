// Playwright verifier for `btv.http.fetch` in the SERVERLESS browser edit-host — the leg
...
// wire + codec and is covered natively by crates/bemtvi-server/tests/daemon_http.rs.
//
...
// `btv.http.fetch` settles on a later tick — poll `_G.<key>` (rendered by rmpv as an
...
try {
  ...
  await luaResult(
    page,
    `_G.g = nil
     btv.http.fetch('${ORIGIN}/api/hello'):next(function(r)
  ...
  await luaResult(
    ...
    `_G.j = nil
     btv.async(function()
       local r = btv.await(btv.http.fetch('${ORIGIN}/api/data'))
       local d = r:json()
  ...
  check("serverless res:json() decodes the body", json.includes("btv:3"), `got=${JSON.stringify(json)}`);

  // ── opts.query builds + encodes the query string (lib-backed, same in wasm) ─────────────
  await luaResult(
    page,
    `_G.q = nil
     btv.http.fetch('${ORIGIN}/api/target', { query = { q = 'hi there', n = 2 } })
  ...
  await luaResult(
    page,
    `_G.nf = nil
     btv.http.fetch('${ORIGIN}/api/missing')
  ...
  await luaResult(
    page,
    `_G.e = nil
     btv.http.fetch('${ORIGIN}/api/echo', { method = 'POST', body = { hi = 'there' } })
  ...
  // ── btv.http.fetch_local runs on the browser fetch() (serverless: identical to fetch) ───
  await luaResult(
    page,
    `_G.loc = nil
     btv.http.fetch_local('${ORIGIN}/api/hello'):next(function(r) _G.loc = r:text() end)
     return 1`,
  );
  const loc = await pollGlobal(page, "loc");
  check(
    "serverless btv.http.fetch_local resolves via the browser fetch()",
...
console.log(failures === 0
  ? "\nALL PASS — serverless btv.http.fetch works (browser fetch(): 2xx / json / 404-resolves / POST body / transport-reject)"
  : `\n${failures} FAILED`);
