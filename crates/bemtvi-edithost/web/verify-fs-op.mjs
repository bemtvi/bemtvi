// Playwright verifier for the browser edit-host's `btv.fs` leg (Phase 2 of the off-tick plan)
// against a REAL `bemtvi --daemon --listen` over WebTransport. A browser `btv.fs.*` op has no
...
// promise in the tick. The browser twin of the native off-tick `btv.fs` actor leg.
//
...
// (target/debug/bemtvi), and a Chromium for Playwright. Run:  node verify-fs-op.mjs
...
const root = mkdtempSync(join(tmpdir(), "bemtvi-fsop-"));
...
try {
  ...
  await luaResult(page, `_G.__text, _G.__terr = nil, nil
     btv.fs.read_text("${readFile}"):next(
       function(t) _G.__text = t end,
       function(e) _G.__terr = e.message end)
     return 1`);
  const text = await until(page,
    () => window.__bemtvi.execLua("return _G.__text or ''").then((r) => r.result),
    (v) => /HELLO-FROM-DAEMON-DISK/.test(String(v)));
  check("btv.fs.read_text returns the daemon file's content (over WebTransport, not MEMFS)",
  ...
  await luaResult(page, `_G.__names, _G.__rderr = nil, nil
     btv.fs.readdir("${listDir}"):next(function(entries)
       local out = {}
  ...
  const names = await until(page,
    () => window.__bemtvi.execLua("return _G.__names or ''").then((r) => r.result),
    (v) => /alpha\.txt/.test(String(v)));
  check("btv.fs.readdir lists the daemon dir's entries with kinds (file/directory)",
  ...
  await luaResult(page, `_G.__wrote, _G.__werr = nil, nil
     btv.fs.write("${writeTarget}", "WROTE-FROM-BROWSER"):next(
       function() _G.__wrote = true end,
       function(e) _G.__werr = e.message end)
     return 1`);
  await until(page,
    () => window.__bemtvi.execLua("return _G.__wrote and 1 or (_G.__werr or 0)").then((r) => r.result),
  ...
  check("btv.fs.write created the file ON THE DAEMON (Node reads it back from the daemon's tree)",
    onDisk.includes("WROTE-FROM-BROWSER"), `onDisk=${JSON.stringify(onDisk)}`);

  // ── 4. a missing path REJECTS with err.code == "ENOENT" (the error envelope round-trips) ───
  await luaResult(page, `_G.__code = nil
     btv.fs.read_text("${missing}"):next(
  ...
  const code = await until(page,
    ...
    (v) => !/^nil$/.test(String(v)));
  check("btv.fs read of a missing daemon path rejects with err.code == ENOENT",
  ...
  // low-level `btv._local_fs_op` seam the manager uses routes to the local OPFS store, so a file
  // that exists ONLY on the daemon's disk is INVISIBLE to it — while a session `btv.fs.exists`
  ...
  await luaResult(page, `_G.__sess, _G.__loc, _G.__locerr = nil, nil, nil
     btv.fs.exists("${readFile}"):next(function(v) _G.__sess = v end)
     do
       ...
       btv._local_fs_op({ op = "exists", path = "${readFile}" }, id)
     end
     return 1`);
  ...
  // ── 6. RELATIVE btv.fs path resolves against the session cwd and FOLLOWS a remote `:cd` ──────
  // The session cwd seeds from the daemon's cwd (`root`); a relative `btv.fs` path must be
  ...
  // rebase (the bug), `btv.fs.read_text("nested.txt")` after `:cd deep` ENOENTs / hits the wrong dir.
  ...
  const cwd1 = plainStr(await until(page,
    () => window.__bemtvi.execLua("return vim.fn.getcwd()").then((r) => r.result),
  ...
  await luaResult(page, `_G.__rel, _G.__relerr = nil, nil
     btv.fs.read_text("nested.txt"):next(
       function(t) _G.__rel = t end,
       function(e) _G.__relerr = e.code .. ":" .. e.message end)
     return 1`);
  const rel = await until(page,
    () => window.__bemtvi.execLua("return _G.__rel or _G.__relerr or ''").then((r) => r.result),
    (v) => /NESTED-VIA-RELATIVE-PATH|ENOENT/.test(String(v)));
  check("a RELATIVE btv.fs.read_text after `:cd` resolves against the new cwd (over the wire)",
...
console.log(failures === 0
  ? "\nALL PASS — browser btv.fs runs on a real bemtvi --daemon over WebTransport (read_text, readdir, write-to-daemon, ENOENT, local→OPFS split)"
  : `\n${failures} FAILED`);
