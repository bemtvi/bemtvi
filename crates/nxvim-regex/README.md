...
Vim regular expressions for nxvim — by embedding the real engines.
...
| Path | What it is |
| --- | --- |
| `csrc/nvim/regexp.c` + headers | vendored from `vendor/neovim` @ `70cfeabe23` (v0.12.0-794), with `// NXVIM:`-marked patches |
...
| `csrc/shim/nxre_shim.{h,c}` | the host interface: line provider, cursor/Visual/marks state, error sink, interrupts, allocation |
...
1. Update `vendor/neovim`, re-copy the vendored files, re-apply the
   `// NXVIM:` patches (`git diff` against the previous vendored copy shows
   them all).
...
4. `cargo test -p nxvim-regex`, and for the browser target
   `CARGO_TARGET_WASM32_UNKNOWN_EMSCRIPTEN_RUNNER=node cargo test -p nxvim-regex --target wasm32-unknown-emscripten`.
...
- A "modified" notice belongs in nxvim's `:version`/`--version` output once
