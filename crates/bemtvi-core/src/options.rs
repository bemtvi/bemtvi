...
//! `option.c`. Kept deliberately small for now — only the options bemtvi
...
pub struct Options {
    ...
    /// popup and `btv.ui.select` — or `0` for no maximum. The box is otherwise sized
    ...
    /// Which regex dialect `/` search and `:substitute` speak (bemtvi's
    ...
    /// as its raw bytes (bemtvi's `'imagepreview'`, not a standard vim option). Off
    ...
    /// The interface `btv.http.mount` binds its listener on (bemtvi's `'httphost'`, not
    ...
    ///
    /// Read when the listener binds — the first [`btv.http.mount`]. Writing it while
    /// mounts are live rebinds them onto the new address.
    ///
    /// [`btv.http.mount`]: https://docs.rs/bemtvi-lua
    pub httphost: String,
    /// The port `btv.http.mount` binds its listener on (bemtvi's `'httpport'`, not a
    /// standard vim option). `0` (the default) picks a free ephemeral port, so two
    /// bemtvi instances never collide; set it for a stable, bookmarkable URL.
    ///
    /// Read when the listener binds — the first `btv.http.mount`. Until a plugin
    /// mounts, this is inert: setting it opens nothing.
    pub httpport: u16,
    /// Animate viewport scrolls (`<C-d>`/`<C-u>`/`<C-f>`/`<C-b>`, the wheel, and
    /// off-screen jumps) as a slide instead of a teleport (bemtvi's `'scrollanim'`,
    ...
    pub scrollanim: bool,
    /// The longest a scroll animation may last, in milliseconds (bemtvi's
    ...
    /// the `/` search history, and each `btv.ui.input` namespace ring (neovim's
    ...
    pub history: usize,
    /// Where command-line / search history persists across sessions (bemtvi's
    ...
    /// `vsplit`. bemtvi defaults to `usetab` (vim's default is empty, reusing the
    ...
    /// bemtvi-native, default `true`). When set, `:copen` and a named-list show host
    /// the `filetype=qf` display as a tab in the **bottom dock**, entries jumping into
    /// the main editing layer — the bemtvi way, where several lists sit side by side as
    ...
    /// bemtvi-native, default `true`). When set, deleting the only buffer a tab shows —
    ...
    /// than absolute cells (`'relativesplits'`; bemtvi-native, default `true`). A
    ...
    /// (`'workspacepersistunnamed'`; bemtvi-native, default `true`). Only *modified*
    ...
    /// trees, `btv.view` widgets) are non-ordinary buffers and are never captured this way.
...
/// global one. Empty outside a `--workspace` session (or before any `btv.wso` write).
...
/// [`valid_persisthistory`]; the lenient `btv.o` path skips unknown tokens here.)
...
/// `btv.o` stores any string (lenient policy) and [`parse_persisthistory`] ignores the
/// rest.
...
impl Default for Options {
    fn default() -> Self {
        Options {
            ...
            // bemtvi default: a jump to a buffer already shown in another tab switches
            ...
            // The bemtvi way: quickfix / loclist displays open as bottom-dock tabs.
            qfdock: true,
            // The bemtvi way: `:bd` of a tab's last buffer closes the tab.
            bdclosetab: true,
...
/// with breathing room from its rect edges. bemtvi's own option (vim has no
...
pub struct WindowOptions {
    ...
    /// `'breakindentopt'`: comma-separated tweaks to [`breakindent`]. bemtvi honors the
    ...
    /// drawn in structural spots (vim's `fillchars`). bemtvi honors only the `eob`
    ...
    /// `'padding'` (bemtvi's own; no vim equivalent): a per-side blank margin in
    ...
    /// [`fillchars`](WindowOptions::fillchars)). bemtvi honors **absolute** column
    /// numbers (`"80,120"`); vim's `'textwidth'`-relative `+N`/`-N` entries are
    /// accepted but skipped, since bemtvi models no `'textwidth'`.
...
impl WindowOptions {
    ...
    /// entry (vim's form) is skipped because bemtvi has no `'textwidth'` to anchor it
    ...
    pub fn colorcolumns(&self) -> Vec<usize> {
        let mut cols: Vec<usize> = self
            ...
            .filter_map(|piece| {
                ...
                // `+N`/`-N` (bemtvi has no `'textwidth'` to anchor them) and any
...
/// The recognized `'fillchars'` keys (vim's `fillchars` table). bemtvi honors only
...
impl Default for WindowOptions {
    fn default() -> Self {
        // bemtvi ships with the hybrid number column on: the cursor line shows its
...
/// load-bearing `dock_sizes` array — but the `btv.dock.opt` surface presents it
...
pub struct DockOptions {
    ...
    /// content preserved), and re-appears on the next `btv.dock.toggle`/`focus`/`show`.
...
pub enum FoldMethod {
    ...
    Indent,
    /// Folds defined by `'foldexpr'`. bemtvi evaluates the canonical tree-sitter
...
pub enum FoldMethodErr {
    ...
    Unknown,
    /// A real vim foldmethod bemtvi hasn't implemented yet — fail loud naming it.
    Unimplemented,
}
...
/// the indentation options bemtvi honors today, plus the buffer-local
...
pub struct BufferOptions {
    ...
    /// horizontal motion crosses a tab. bemtvi defaults to **4** (not vim's 8):
    /// it's the lone knob the other indent options follow.
    pub tabstop: usize,
    /// Indent width for shift commands and autoindent (vim's `shiftwidth`). `0`
    /// means "follow [`tabstop`]" (vim's documented sentinel), which is bemtvi's
    ...
    /// [`shiftwidth`]", which is bemtvi's default — so it chains
    ...
    /// (bemtvi's own, no vim equivalent): typing an opener inserts its closer and
    ...
    /// (bemtvi's own, no vim equivalent — VS Code's editor exposes the same
    ...
    /// `nomodifiable`. bemtvi uses it for its built-in read-only **scratch listings**
...
impl Default for BufferOptions {
    fn default() -> Self {
        // bemtvi's modern defaults: a 4-cell tab, with shiftwidth and softtabstop
...
impl OptKind {
    ...
    /// API — what `:set` completion shows and what `btv._options_catalog()` exposes.
...
static OPTIONS: &[OptionInfo] = {
    ...
    &[
        ...
        OptionInfo {
            ...
            kind: Str,
            scope: Window,
            doc: "Per-window content margin, CSS shorthand (bemtvi extension).",
        },
        OptionInfo {
            name: "sidescroll",
            abbrev: Some("ss"),
        ...
        OptionInfo {
            ...
            kind: Str,
            scope: Global,
            doc: "Interface btv.http plugin mounts listen on; 0.0.0.0 exposes them to the LAN.",
        },
        OptionInfo {
            name: "httpport",
            abbrev: None,
            kind: Num,
            scope: Global,
            doc: "Port btv.http plugin mounts listen on; 0 picks a free one.",
        },
        OptionInfo {
            name: "showtabline",
            abbrev: Some("stal"),
        ...
        OptionInfo {
            ...
            kind: Bool,
            scope: Global,
            doc: "Open quickfix/loclist as bottom-dock tabs (bemtvi) vs a split (vim).",
        },
        OptionInfo {
            name: "bdclosetab",
            abbrev: Some("bdct"),
            kind: Bool,
            scope: Global,
            doc: "Closing a tab's last buffer with :bd closes the tab (bemtvi).",
        },
        OptionInfo {
            name: "relativesplits",
            abbrev: None,
