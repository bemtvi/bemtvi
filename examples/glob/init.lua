-- ~~~ nxvim nx.glob: shell-style path patterns, compiled to a cached regex ~~~
--
-- Run it (from the repo root) against the sample buffer:
--
--     NXVIM_CONFIG=examples/glob \
--       cargo run -p nxvim -- examples/glob/sample.txt
--
-- Lua has no glob support and its patterns are not a substitute (no alternation, no
-- `**`, a different escaping model). `nx.glob` is the real thing: `globset` parses
...
-- parse.
--
--     nx.glob.match(pattern, path, opts) -> boolean      one-shot, cache-backed
--     nx.glob.compile(pattern, opts)     -> :test/:regex/:pattern
--     nx.glob.set(patterns, opts)        -> :test/:matches  (one RegexSet pass)
--     nx.glob.any / .filter / .to_regex / .is_glob
--
...
local function paths()
  ...
  for _, line in ipairs(nx.buf.lines(0, 0, -1)) do
    local trimmed = nx.str.trim(line)
...
vim.api.nvim_create_user_command("Glob", function(a)
  ...
  for _, p in ipairs(paths()) do
    table.insert(nx.glob.match(a.args, p) and hit or miss, p)
  end
  vim.notify(("glob %q\n  matched (%d):\n    %s\n  not matched (%d):\n    %s"):format(
    a.args, #hit, table.concat(hit, "\n    "), #miss, table.concat(miss, "\n    ")))
end, { nargs = 1, desc = "Match every path in the buffer against a glob (nx.glob.match)" })
...
vim.api.nvim_create_user_command("GlobRegex", function(a)
  vim.notify(("glob %q translates to:\n  %s\n\nwith literal_separator = false:\n  %s"):format(
    a.args,
    nx.glob.to_regex(a.args),
    nx.glob.to_regex(a.args, { literal_separator = false })))
end, { nargs = 1, desc = "Show the regex a glob compiles to (nx.glob.to_regex)" })
...
path = %q
  ...
  nx.glob.match("*.lua", path)                              --> %s
      `*` does not cross `/`  (shell / gitignore / LSP)

  nx.glob.match("**/*.lua", path)                           --> %s
      `**` crosses separators, and spans zero of them too

  nx.glob.match("*.lua", path, { basename = true })         --> %s
      a separator-less pattern matches the path's last component (vim's rule)

  nx.glob.match("*.lua", path, { literal_separator = false }) --> %s
      the opt-out: `*` crosses `/` again (vim's autocmd rule)]]):format(
    path,
    nx.glob.match("*.lua", path),
    nx.glob.match("**/*.lua", path),
    nx.glob.match("*.lua", path, { basename = true }),
    nx.glob.match("*.lua", path, { literal_separator = false })))
...
Same host, same call, opposite answers — the convention belongs to the PATH.]==]):format(
    nx.glob.match([[src\*.rs]], [[src\main.rs]], win),
    nx.glob.match("src/*.rs", [[src\main.rs]], win),
    nx.glob.match([[src\*.rs]], [[src\a\main.rs]], win),
    nx.glob.match([[C:/**/*.txt]], [[C:\Users\me\a.txt]], win),
    nx.glob.match([[a\*b]], "a*b", uni),
    nx.glob.match([[a\*b]], "axyzb", uni)))
end, { desc = "Windows vs unix path style on the same host (nx.glob style opt)" })
...
local ignore = nx.glob.set({ "**/target/**", "**/.git/**", "**/*.tmp" })
...
end, { desc = "Filter the buffer's paths through a glob set (nx.glob.set)" })
...
vim.api.nvim_create_user_command("GlobBench", function()
  ...
  local hits = 0
  for _ = 1, n do
    if nx.glob.match(pattern, path) then
      hits = hits + 1
    end
  end
...
end, { desc = "50k cached glob matches (nx.glob compile cache)" })
...
vim.api.nvim_create_user_command("GlobIsGlob", function()
  ...
  for _, s in ipairs(paths()) do
    out[#out + 1] = ("  %-34s %s"):format(s, nx.glob.is_glob(s) and "glob" or "plain path")
  end
  vim.notify("is_glob:\n" .. table.concat(out, "\n"))
end, { desc = "Which buffer lines are globs (nx.glob.is_glob)" })

vim.notify(table.concat({
  "nx.glob example loaded. Try:",
