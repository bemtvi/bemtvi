//! Black-box tests for `btv.regex` — the compiled pattern object for matching Lua
...
async fn find_plain_with_mid_char_init_matches_like_string_find() {
    ...
    let out = run(
        &rpc,
        r#"local re = btv.regex("b", { plain = true })
...
async fn match_returns_captures_or_whole() {
    ...
    let out = run(
        &rpc,
        r#"local whole = btv.regex([[\d+]]):match("x 42 y")
           local g = btv.regex([[(\w+)=(\w+)]]):match("k=v")
           local k, v = btv.regex([[(\w+)=(\w+)]]):match("k=v")
           return whole..":"..k..":"..v"#,
    )
    .await;
...
async fn vim_engine_supports_zs() {
    ...
    let out = run(
        &rpc,
        r#"local re = btv.regex([[foo\zsbar]], { engine = "vim" })
           local s = "foobar"
           local a, b = re:find(s)
           return s:sub(a, b)"#,
    )
...
async fn plain_treats_pattern_literally() {
    ...
    let out = run(
        &rpc,
        r#"local re = btv.regex("a.c", { plain = true })
...
async fn ignorecase_option() {
    ...
    let out = run(
        &rpc,
        r#"local re = btv.regex([[foo]], { ignorecase = true })
...
async fn invalid_pattern_raises() {
    ...
    let out = run(
        &rpc,
        r#"local ok, err = pcall(btv.regex, "(unterminated")
           err = tostring(err)
